...
public abstract void setUrlForVideoPressId(String videoPressId, String url,
                                           String posterUrl);
...
// Check whether we need to reload the content of the post from the editor or
// not. This was required since Aztec Visual->HTML can slightly change the
// content of the HTML. See #692 for details.
...
private static final String FEATURED_IMAGE_SUPPORT_KEY =
...
public void onAttach(Activity activity) {
	...
	try {
		...
		throw new ClassCastException(activity.toString() +
		                             " must implement EditorFragmentListener");
	}
}
...
public void onCreate(Bundle savedInstanceState) {
	...
	if (savedInstanceState != null) {
		if (savedInstanceState.containsKey(FEATURED_IMAGE_SUPPORT_KEY)) {
			mFeaturedImageSupported =
				savedInstanceState.getBoolean(FEATURED_IMAGE_SUPPORT_KEY);
		}
		if (savedInstanceState.containsKey(FEATURED_IMAGE_WIDTH_KEY)) {
			mBlogSettingMaxImageWidth =
				savedInstanceState.getString(FEATURED_IMAGE_WIDTH_KEY);
		}
	}
}
...
public static MediaType getEditorMimeType(MediaFile mediaFile) {
	...
	return mediaFile.isVideo() ? MediaType.VIDEO : MediaType.IMAGE;
}
