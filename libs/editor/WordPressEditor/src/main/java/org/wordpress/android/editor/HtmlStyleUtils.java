...
public static final String REGEX_HTML_ATTRIBUTES =
	"(?<==)('|\")(.*?\\1)(?=.*?>)";
...
/**
 * Apply styling rules to {@code content} inside the range from {@code start}
 * to {@code end}.
 *
 ...
 */
public static void styleHtmlForDisplay(@NonNull Spannable content, int start,
                                       int end) {
	if (Build.VERSION.RELEASE.equals("4.1") ||
	    Build.VERSION.RELEASE.equals("4.1.1")) {
		// Avoids crashing bug in Android 4.1 and 4.1.1 triggered when spanned
		// text is line-wrapped AOSP issue:
...
/**
 * Applies styles to {@code content} from {@code start} to {@code end}, based
 * on rule {@code regex}.
 * @param content the Spannable to apply style rules to
 * @param start the index in {@code content} to start styling from
 * @param end the index in {@code content} to style until
 ...
 */
private static void applySpansByRegex(Spannable content, int start, int end,
                                      String regex) {
	if (content == null || start < 0 || end < 0 || start > content.length() ||
	    end > content.length() || start >= end) {
	...
	while (matcher.find()) {
		...
		case REGEX_HTML_TAGS:
			content.setSpan(new ForegroundColorSpan(TAG_COLOR), matchStart,
			                matchEnd, SPANNABLE_FLAGS);
			break;
		case REGEX_HTML_ATTRIBUTES:
			content.setSpan(new ForegroundColorSpan(ATTRIBUTE_COLOR), matchStart,
			                matchEnd, SPANNABLE_FLAGS);
			break;
		case REGEX_HTML_COMMENTS:
			content.setSpan(new ForegroundColorSpan(ATTRIBUTE_COLOR), matchStart,
			                matchEnd, SPANNABLE_FLAGS);
			content.setSpan(new StyleSpan(Typeface.ITALIC), matchStart, matchEnd,
			                SPANNABLE_FLAGS);
			content.setSpan(new RelativeSizeSpan(0.75f), matchStart, matchEnd,
			                SPANNABLE_FLAGS);
			break;
		case REGEX_HTML_ENTITIES:
			content.setSpan(new ForegroundColorSpan(TAG_COLOR), matchStart,
			                matchEnd, SPANNABLE_FLAGS);
			content.setSpan(new StyleSpan(Typeface.BOLD), matchStart, matchEnd,
			                SPANNABLE_FLAGS);
			content.setSpan(new RelativeSizeSpan(0.75f), matchStart, matchEnd,
			                SPANNABLE_FLAGS);
			break;
...
/**
 * Clears all relevant spans in {@code content} from {@code start} to {@code
 * end}. Relevant spans are the subclasses of {@link CharacterStyle} applied
 * by {@link HtmlStyleUtils#applySpansByRegex(Spannable, int, int, String)}.
...
public static void clearSpans(Spannable content, int spanStart, int spanEnd) {
	CharacterStyle[] spans =
		content.getSpans(spanStart, spanEnd, CharacterStyle.class);
