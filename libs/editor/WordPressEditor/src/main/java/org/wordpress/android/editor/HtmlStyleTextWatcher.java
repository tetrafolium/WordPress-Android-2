...
public class HtmlStyleTextWatcher implements TextWatcher {
  private enum Operation { INSERT, DELETE, REPLACE, NONE }
  ...
  public void afterTextChanged(Editable s) {
    ...
    // If the modified text included a tag or entity symbol ("<", ">", "&" or
    // ";"), find its match and restyle
    if (mModifiedText.toString().contains("<")) {
      ...
      // If the modified text didn't include any tag or entity symbols, restyle
      // if the modified text is inside a tag or entity
  ...
  /**
   * For changes made which contain at least one opening symbol (e.g. '<' or
   * '&'), whether added or deleted, returns the range of text which should have
   * its style reapplied.
   ...
   */
  protected SpanRange
  getRespanRangeForChangedOpeningSymbol(Editable content,
    ...
    int firstOpeningTagLoc =
        mOffset + mModifiedText.toString().indexOf(openingSymbol);
    int closingTagLoc;
    if (mLastOperation == Operation.INSERT) {
      // Apply span from the first added opening symbol until the closing symbol
      // in the content matching the last added opening symbol e.g. pasting
      // "<b><" before "/b>" - we want the span to be applied to all of
      // "<b></b>"
      int lastOpeningTagLoc =
          mOffset + mModifiedText.toString().lastIndexOf(openingSymbol);
      closingTagLoc =
          content.toString().indexOf(closingSymbol, lastOpeningTagLoc);
    } else {
      // Apply span until the first closing tag that appears after the deleted
      // text
  ...
  /**
   * For changes made which contain at least one closing symbol (e.g. '>' or
   * ';') and no opening symbols, whether added or deleted, returns the range of
   * text which should have its style reapplied.
   * @param content the content after modification
   * @param closingSymbol the closing symbol recognized (e.g. '>' or ';')
   * @return the range of characters to re-apply spans to
   */
  protected SpanRange
  getRespanRangeForChangedClosingSymbol(Editable content,
    ...
    int firstClosingTagInModLoc =
        mOffset + mModifiedText.toString().indexOf(closingSymbol);
    int firstClosingTagAfterModLoc = content.toString().indexOf(
        closingSymbol, mOffset + mModifiedText.length());

    int openingTagLoc = content.toString().lastIndexOf(
        openingSymbol, firstClosingTagInModLoc - 1);
    if (openingTagLoc >= 0) {
  ...
  /**
   * For changes made which contain no opening or closing symbols, checks
   * whether the changed text is inside a tag, and if so returns the range of
   * text which should have its style reapplied.
   * @param content the content after modification
   * @param openingSymbol the opening symbol of the tag to check for (e.g. '<'
   *     or '&')
   * @return the range of characters to re-apply spans to
   */
  protected SpanRange getRespanRangeForNormalText(Editable content,
                                                  String openingSymbol) {
    String closingSymbol = getMatchingSymbol(openingSymbol);
    ...
    if (openingTagLoc >= 0) {
      int closingTagLoc =
          content.toString().indexOf(closingSymbol, openingTagLoc);
  ...
  /**
   * Clears and re-applies spans to {@code content} within range {@code
   * spanRange} according to rules in
   * {@link HtmlStyleUtils}.
  ...
  protected void updateSpans(Spannable content, SpanRange spanRange) {
    ...
    if (spanStart > content.length() || spanEnd > content.length()) {
      AppLog.d(T.EDITOR,
               "The specified span range was beyond the Spannable's length");
      return;
    } else if (spanStart >= spanEnd) {
      // If the span start is after the end position (probably due to a
      // multi-line deletion), selective re-styling won't work Instead, do a
      // clean re-styling of the whole document
      spanStart = 0;
  ...
  /**
   * Returns the closing/opening symbol corresponding to the given
  ...
  protected static class SpanRange {
    ...
    public int getOpeningTagLoc() { return mOpeningTagLoc; }

    public int getClosingTagLoc() { return mClosingTagLoc; }
  }
}
