...
public class RippleToggleButton extends ToggleButton {
  ...
  public RippleToggleButton(Context context) { this(context, null); }
  ...
  private void init() {
    ...
    int rippleColor =
        getResources().getColor(R.color.format_bar_ripple_animation);
  ...
  public void draw(@NonNull Canvas canvas) {
    ...
    if (mAnimationIsRunning) {
      if (DURATION <= mTimer * FRAME_RATE) {
        ...
        mFillPaint.setAlpha(
            (int)(FILL_INITIAL_OPACITY * (1 - progressFraction)));
        mStrokePaint.setAlpha(
            (int)(STROKE_INITIAL_OPACITY * (1 - progressFraction)));

        canvas.drawCircle(mHalfWidth, mHalfWidth, mHalfWidth * progressFraction,
                          mFillPaint);
        canvas.drawCircle(mHalfWidth, mHalfWidth, mHalfWidth * progressFraction,
                          mStrokePaint);
