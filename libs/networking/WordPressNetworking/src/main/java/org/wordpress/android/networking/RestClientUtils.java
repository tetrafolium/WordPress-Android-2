...
public class RestClientUtils {
  public static final String NOTIFICATION_FIELDS =
      "id,type,unread,body,subject,timestamp,meta";
  ...
  public static void setUserAgent(String userAgent) { sUserAgent = userAgent; }

  public RestClientUtils(
      Context context, RequestQueue queue, Authenticator authenticator,
      RestRequest.OnAuthFailedListener onAuthFailedListener) {
    this(context, queue, authenticator, onAuthFailedListener,
         RestClient.REST_CLIENT_VERSIONS.V1);
  }

  public RestClientUtils(Context context, RequestQueue queue,
  ...
  public Authenticator getAuthenticator() { return mAuthenticator; }

  public RestClient getRestClient() { return mRestClient; }

  public void getCategories(long siteId, Listener listener,
   ...
   * Decrement the unread count for a notification. Key=note_ID, Value=decrement
   * amount.
   *
   * <p/>
  ...
  public void getJetpackSettings(long siteId, Listener listener,
                                 ErrorListener errorListener) {
    String path = String.format(
        Locale.US, "jetpack-blogs/%d/rest-api/?path=/jetpack/v4/settings",
        siteId);
    get(path, listener, errorListener);
  }

  public void getGeneralSettings(long siteId, Listener listener,
                                 ErrorListener errorListener) {
    String path = String.format(Locale.US, "sites/%d/settings", siteId);
    get(path, listener, errorListener);
  }

  public void getJetpackMonitorSettings(long siteId, Listener listener,
                                        ErrorListener errorListener) {
    String path = String.format(Locale.US, "jetpack-blogs/%d", siteId);
    get(path, listener, errorListener);
  }

  public void getJetpackModuleSettings(long siteId, Listener listener,
                                       ErrorListener errorListener) {
    String path = String.format(Locale.US, "sites/%d/jetpack/modules", siteId);
    get(path, listener, errorListener);
  }

  public void setGeneralSiteSettings(long siteId, JSONObject params,
                                     Listener listener,
                                     ErrorListener errorListener) {
    String path = String.format(Locale.US, "sites/%d/settings", siteId);
    post(path, params, null, listener, errorListener);
  ...
  public void setJetpackSettings(long siteId, Map<String, Object> bodyData,
                                 ...
                                 ErrorListener errorListener) {
    String path =
        String.format(Locale.US, "jetpack-blogs/%d/rest-api/", siteId);
    ...
    try {
      ...
      if (errorListener != null) {
        errorListener.onErrorResponse(new VolleyError(
            "Error: Attempted to update Jetpack settings with malformed body data",
            e));
      }
    }
  }
  ...
  public void setJetpackModuleSettings(long siteId, String module,
                                       boolean active, Listener listener,
                                       ErrorListener errorListener) {
    String path =
        String.format(Locale.US, "sites/%d/jetpack/modules/%s", siteId, module);
    HashMap<String, String> params = new HashMap<>();
    params.put("active", String.valueOf(active));
    post(path, params, null, listener, errorListener);
  }

  public void getSitePurchases(long siteId, Listener listener,
                               ErrorListener errorListener) {
    String path = String.format(Locale.US, "sites/%d/purchases", siteId);
    get(path, listener, errorListener);
  }

  public void exportContentAll(long siteId, Listener listener,
                               ErrorListener errorListener) {
    String path = String.format(Locale.US, "sites/%d/exports/start", siteId);
    post(path, listener, errorListener);
  }

  public void getSharingButtons(String siteId, Listener listener,
                                ErrorListener errorListener) {
    String path = String.format("sites/%s/sharing-buttons", siteId);
    get(path, listener, errorListener);
  }

  public void setSharingButtons(String siteId, JSONObject params,
                                Listener listener,
                                ErrorListener errorListener) {
    String path = String.format("sites/%s/sharing-buttons", siteId);
    post(path, params, null, listener, errorListener);
  ...
  public void sendLoginEmail(Map<String, String> params, Listener listener,
                             ErrorListener errorListener) {
   ...
   * Make GET request
   */
  public Request<JSONObject> get(String path, Listener listener,
                                 ErrorListener errorListener) {
   ...
   */
  public Request<JSONObject> get(String path, Map<String, String> params,
    ...
    RestRequest request = mRestClient.makeRequest(
        Method.GET, mRestClient.getAbsoluteURL(realPath, paramsWithLocale),
        null, listener, errorListener);

    if (retryPolicy == null) {
      retryPolicy = new DefaultRetryPolicy(
          REST_TIMEOUT_MS, REST_MAX_RETRIES_GET, REST_BACKOFF_MULT);
    }
    request.setRetryPolicy(retryPolicy);
    AuthenticatorRequest authCheck = new AuthenticatorRequest(
        request, errorListener, mRestClient, mAuthenticator);
    authCheck.send();
    return request;
  }
  ...
  public void post(final String path, Map<String, String> params,
                   RetryPolicy retryPolicy, Listener listener,
                   ErrorListener errorListener) {
    final RestRequest request = mRestClient.makeRequest(
        Method.POST,
        mRestClient.getAbsoluteURL(path, getRestLocaleParams(mContext)), params,
        listener, errorListener);
    if (retryPolicy == null) {
      retryPolicy =
          new DefaultRetryPolicy(REST_TIMEOUT_MS, REST_MAX_RETRIES_POST,
   ...
   */
  public void post(final String path, JSONObject params,
                   RetryPolicy retryPolicy, Listener listener,
                   ErrorListener errorListener) {
    final JsonRestRequest request = mRestClient.makeRequest(
        mRestClient.getAbsoluteURL(path, getRestLocaleParams(mContext)), params,
        listener, errorListener);
    if (retryPolicy == null) {
      retryPolicy =
  ...
  private static String getSanitizedPath(String unsanitizedPath) {
    if (unsanitizedPath != null) {
      ...
      if (qmarkPos >
          -1) { // strip any query string params off this to obtain the path
  ...
  getSanitizedParameters(String unsanitizedPath) {
    ...
    if (uri.getHost() == null) {
      uri =
          Uri.parse("://" + unsanitizedPath); // path may contain a ":" leading
                                              // to Uri.parse to misinterpret
  ...
  /**
   * Returns locale parameter used in REST calls which require the response to
   * be localized
   */
  public static HashMap<String, String> getRestLocaleParams(Context context) {
    HashMap<String, String> params = new HashMap<>();
    String deviceLanguageCode =
        LanguageUtils.getCurrentDeviceLanguageCode(context);
    if (!TextUtils.isEmpty(deviceLanguageCode)) {
      // patch locale if it's any of the deprecated codes as can be read in
      // Locale.java source code:
      deviceLanguageCode =
          LanguageUtils.patchDeviceLanguageCode(deviceLanguageCode);
