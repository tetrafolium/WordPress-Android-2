 ...
 * This allows the request maker to disregard the authentication state when
 * making requests.
 */
public class AuthenticatorRequest {
  ...
  protected AuthenticatorRequest(RestRequest request, ErrorListener listener,
  ...
  public String getSiteId() {
    return extractSiteIdFromUrl(mRestClient.getEndpointURL(),
                                mRequest.getUrl());
  }
   ...
   * @return The site ID
   */
  public static String extractSiteIdFromUrl(String restEndpointUrl,
                                            String url) {
    if (url == null) {
      return null;
    }
    ...
    final String sitePrefix = restEndpointUrl.endsWith("/")
                                  ...
                                  : restEndpointUrl + "/sites/";
    final String batchCallPrefix =
        restEndpointUrl.endsWith("/")
    ...
    } else if (url.startsWith(batchCallPrefix) &&
               !batchCallPrefix.equals(url)) {
  ...
  /**
   * Attempt to send the request, checks to see if we have an access token and
   * if not asks the Authenticator to authenticate the request.
   *
