...
public class AnalyticsTrackerNosara extends Tracker {
  ...
  String getAnonIdPrefKey() { return TRACKS_ANON_ID; }
  ...
  public void track(AnalyticsTracker.Stat stat, Map<String, ?> properties) {
    ...
    if (eventName == null) {
      AppLog.w(AppLog.T.STATS,
               "There is NO match for the event " + stat.name() + "stat");
      return;
    }
    ...
    if (getWordPressComUserName() != null) {
      ...
      // This is just a security checks since the anonID is already available
      // here. refresh metadata is called on login/logout/startup and it
      // loads/generates the anonId when necessary.
      if (getAnonID() == null) {
    ...
    // It seems that we're tracking some events with user = null. Make sure
    // we're catching the error here.
    if (user == null) {
      try {
        throw new AnalyticsException(
            "Trying to track analytics with an null user!");
        // TODO add Crash Logging Exception or track this error in Nosara by
        // using a special test user.
    ...
    // Properties defined by the user have precedence over the default ones
    // pre-defined at "event level"
        ...
        for (String key : predefinedEventProperties.keySet()) {
          try {
            if (propertiesToJSON.has(key)) {
              AppLog.w(
                  AppLog.T.STATS,
                  "The user has defined a property named: '" + key +
                      "' that will override"
                      +
            ...
            AppLog.e(
                AppLog.T.STATS,
                "Error while merging user-defined properties with pre-defined properties",
                e);
          }
        }
      } catch (NullPointerException e) {
        AppLog.e(AppLog.T.STATS,
                 "A property passed to the event " + eventName +
                     " has null key!",
                 e);
      }
    }
    ...
    if (propertiesToJSON.length() > 0) {
      mNosaraClient.track(EVENTS_PREFIX + eventName, propertiesToJSON, user,
                          userType);
  ...
  public void refreshMetadata(AnalyticsMetadata metadata) {
    ...
    try {
      ...
      // See:
      // https://github.com/wordpress-mobile/WordPress-Android/pull/10300#discussion_r309145514
    ...
    // De-anonymize user only when it's WPCOM and we have the username available
    // (might still be waiting for it to
    //  be fetched).
