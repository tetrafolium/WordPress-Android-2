...
/**
 * Add scheme prefix to an URL. This method must be called on all user entered
 * or server fetched URLs to ensure http client will work as expected.
 *
 * @param url url entered by the user or fetched from a server
 * @param addHttps true and the url is not starting with http://, it will make
...
public static String addUrlSchemeIfNeeded(String url, boolean addHttps) {
	...
	// Remove leading double slash (eg. // example.com), needed for some wporg
	// instances configured to switch between http or https
...
public static String normalizeUrl(final String urlString) {
	...
	// this routine is called from some performance-critical code and creating a
	// URI from a string is slow, so skip it when possible - if we know it's not
	// a relative path (and 99.9% of the time it won't be for our purposes) then
	// we can normalize it without java.net.URI.normalize()
	if (urlString.startsWith("http") &&
	    !urlString.contains(
		    "build/intermediates/exploded-aar/org.wordpress/graphview/3.1.1")) {
...
public static String appendUrlParameter(String url, String paramName,
                                        String paramValue) {
...
public static String appendUrlParameters(String url,
...
public static String extractSubDomain(String domain) {
	...
	if (host.length() > 0) {
		...
		if (parts.length >
		    1) { // There should be at least 2 dots for there to be a subdomain.
			return parts[0];
		}
	}
	return "";
