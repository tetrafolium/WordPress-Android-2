...
public class MediaFile {
  ...
  protected int
      mHorizontalAlignment; // 0 = none, 1 = left, 2 = center, 3 = right
  ...
  public MediaFile(String blogId, Map<?, ?> resultMap, boolean isWPCom) {
    ...
    setVideoPressShortCode(
        MapUtils.getMapStr(resultMap, VIDEOPRESS_SHORTCODE_ID));

    // get the file name from the link
    String link = MapUtils.getMapStr(resultMap, "link");
    setFileName(
        new String(link).replaceAll("^.*/([A-Za-z0-9_-]+)\\.\\w+$", "$1"));

    String fileType = new String(link)
                          .replaceAll(".*\\.(\\w+)$", "$1")
                          .toLowerCase(Locale.ROOT);
    String fileMimeType =
        MimeTypeMap.getSingleton().getMimeTypeFromExtension(fileType);
    setMimeType(fileMimeType);

    // make the file urls be https://... so that we can get these images with
    // oauth when the blogs are private assume no https for images in
    // self-hosted blogs
  ...
  public void setMediaId(String id) { mMediaId = id; }

  public boolean isFeatured() { return mFeatured; }

  public void setFeatured(boolean featured) { this.mFeatured = featured; }

  public long getPostID() { return mPostID; }

  public void setPostID(long postID) { this.mPostID = postID; }
  ...
  public void setFileURL(String fileURL) { this.mFileURL = fileURL; }
  ...
  public boolean isVerticalAlignmentOnTop() { return mVerticalAligment; }
  ...
  public String getMimeType() { return StringUtils.notNullStr(mMimeType); }

  public void setMimeType(String type) {
    mMimeType = StringUtils.notNullStr(type);
  }

  public String getVideoPressShortCode() { return mVideoPressShortCode; }

  public void setVideoPressShortCode(String videoPressShortCode) {
    this.mVideoPressShortCode = videoPressShortCode;
  }

  public int getHorizontalAlignment() { return mHorizontalAlignment; }

  public void setHorizontalAlignment(int horizontalAlignment) {
    this.mHorizontalAlignment = horizontalAlignment;
  }

  public boolean isVideo() { return mIsVideo; }

  public void setVideo(boolean isVideo) { this.mIsVideo = isVideo; }

  public boolean isFeaturedInPost() { return mFeaturedInPost; }

  public void setFeaturedInPost(boolean featuredInPost) {
    this.mFeaturedInPost = featuredInPost;
  }

  public String getBlogId() { return mBlogId; }

  public void setBlogId(String blogId) { this.mBlogId = blogId; }

  public void setDateCreatedGMT(long dateCreatedGmt) {
    this.mDateCreatedGmt = dateCreatedGmt;
  }

  public long getDateCreatedGMT() { return mDateCreatedGmt; }

  public void setUploadState(String uploadState) {
    this.mUploadState = uploadState;
  }

  public String getUploadState() { return mUploadState; }

  /**
   * Outputs the Html for an image
   * If a fullSizeUrl exists, a link will be created to it from the
   * resizedPictureUrl
   */
  public String getImageHtmlForUrls(String fullSizeUrl,
                                    String resizedPictureURL,
                                    boolean shouldAddImageWidthCSS) {
    String alignment = "";
    ...
    // Check if we uploaded a featured picture that is not added to the Post
    // content (normal case)
    ...
    String content = String.format(
        Locale.US,
        "<a href=\"%s\"><img title=\"%s\" %s alt=\"image\" src=\"%s\" /></a>",
