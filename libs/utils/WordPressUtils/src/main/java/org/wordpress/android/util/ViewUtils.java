...
public class ViewUtils {
   ...
   * This value will not collide with ID values generated at build time by aapt
   * for R.id. Uses the native implementation if API 17 or above, otherwise uses
  ...
  public static void setButtonBackgroundColor(Context context, View button,
                                              @StyleRes int styleId,
                                              @AttrRes int colorAttribute) {
    TypedArray a =
        context.obtainStyledAttributes(styleId, new int[] {colorAttribute});
  ...
  /**
   * adds an inset circular shadow outline the passed view (Lollipop+ only) -
   * note that the view should have its elevation set prior to calling this
   */
