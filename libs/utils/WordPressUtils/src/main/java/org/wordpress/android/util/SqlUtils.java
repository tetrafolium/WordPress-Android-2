...
public class SqlUtils {
  private SqlUtils() { throw new AssertionError(); }

  /*
   * SQLite doesn't have a boolean datatype, so booleans are stored as 0=false,
   * 1=true
   */
  public static long boolToSql(boolean value) { return (value ? 1 : 0); }

  public static boolean sqlToBool(int value) { return (value != 0); }
  ...
  /*
   * wrapper for DatabaseUtils.longForQuery() which returns 0 if query returns
   * no rows
   */
  ...
  public static int intForQuery(SQLiteDatabase db, String query,
                                String[] selectionArgs) {
    long value = longForQuery(db, query, selectionArgs);
    return (int)value;
  }

  public static boolean boolForQuery(SQLiteDatabase db, String query,
                                     String[] selectionArgs) {
    long value = longForQuery(db, query, selectionArgs);
  ...
  /*
   * wrapper for DatabaseUtils.stringForQuery(), returns "" if query returns no
   * rows
   */
  public static String stringForQuery(SQLiteDatabase db, String query,
                                      String[] selectionArgs) {
    try {
   ...
   * writable database
   */
  public static boolean dropAllTables(SQLiteDatabase db)
   ...
   */
  // Max 512K characters (a UTF-8 char is 4 bytes max, so a 512K characters
  // string is always < 2Mb)
