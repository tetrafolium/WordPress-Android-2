...
public class DateTimeUtils {
  ...
  private static final ThreadLocal<DateFormat> ISO8601_FORMAT =
  ...
  public static String javaDateToTimeSpan(final Date date, Context context,
    ...
    // less than a year old, let `DateUtils.getRelativeTimeSpanString` do the
    // job
    if (daysSince < 365) {
      return DateUtils
          .getRelativeTimeSpanString(passedTime, currentTime,
    ...
    return DateUtils.formatDateTime(context, passedTime,
                                    DateUtils.FORMAT_ABBREV_ALL);
  }

  /**
   * Converts a date to a localized relative time span ("Now", "8 hr. ago",
   * "Yesterday", "3 days ago", "Jul 10, 1940") We're using a call to
   * `DateUtils.getRelativeTimeSpanString` in most cases.
   */
  public static String javaDateToTimeSpan(final Date date, Context context) {
  ...
  /**
   * Given an ISO 8601-formatted date as a String, returns a {@link Date} in
   * UTC.
   */
  public static Date dateUTCFromIso8601(String iso8601date) {
    try {
      iso8601date =
          iso8601date.replace("Z", "+0000").replace("+00:00", "+0000");
      DateFormat formatter = ISO8601_FORMAT.get();
  ...
  public static String iso8601UTCFromDate(Date date) {
    ...
    // Use "+00:00" notation rather than "+0000" to be consistent with the
    // WP.COM API
  ...
  /**
   * Given an ISO 8601-formatted date as a String, returns the corresponding
   * UNIX timestamp.
   */
  ...
  /**
   * Given a UNIX timestamp, returns an ISO 8601-formatted date in UTC as a
   * String.
   */
