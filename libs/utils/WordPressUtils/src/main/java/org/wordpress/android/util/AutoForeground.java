...
public abstract class AutoForeground<StateClass extends ServiceState>
  ...
  public static class ServiceEventConnection {
    ...
    public ServiceEventConnection(Context context,
                                  Class<? extends AutoForeground> clazz,
                                  Object client) {
      ...
      context.bindService(new Intent(context, clazz), mServiceConnection,
  ...
  public boolean isForeground() { return mIsForeground; }
  ...
  @Nullable
  protected static <StateClass>
      StateClass getState(Class<StateClass> stateClass) {
  ...
  private EventBus getEventBus() { return EventBus.getDefault(); }
  ...
  protected void setState(StateClass newState) {
    ...
    if ((currentState == null || !currentState.isInProgress()) &&
  ...
  protected void notifyState(StateClass state) {
    // sticky emit the state. The stickiness serves as a state keeping mechanism
    // for clients to re-read upon connect
    ...
    // ok, no connected clients so, update might need to be delivered to a
    ...
    if (state.isInProgress()) {
      ...
      NotificationManagerCompat.from(this).notify(NOTIFICATION_ID_PROGRESS,
