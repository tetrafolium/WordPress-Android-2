...
public final class GeocoderUtils {
  private GeocoderUtils() { throw new AssertionError(); }
  ...
  public static Geocoder getGeocoder(Context context) {
    ...
    try {
      gcd = new Geocoder(context,
                         LanguageUtils.getCurrentDeviceLanguage(context));
    } catch (NullPointerException cannotIstantiateEx) {
      AppLog.e(AppLog.T.UTILS, "Cannot instantiate Geocoder",
               cannotIstantiateEx);
      return null;
    }
  ...
  public static Address getAddressFromCoords(Context context, double latitude,
                                             double longitude) {
    ...
    try {
      ...
      // may get "Unable to parse response from server" IOException here if
      // Geocoder service is hit too frequently
      AppLog.e(
          AppLog.T.UTILS,
  ...
  public static Address getAddressFromLocationName(Context context,
                                                   String locationName) {
    int maxResults = 1;
    Address address = null;
    List<Address> addresses = null;
