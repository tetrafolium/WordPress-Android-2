...
public class HtmlUtils {
  /**
   * Removes html from the passed string - relies on Html.fromHtml which handles
   * invalid HTML, but it's very slow, so avoid using this where performance is
   * important
  ...
  /**
   * This is much faster than stripHtml() but should only be used when we know
   * the html is valid since the regex will be unpredictable with invalid html
  ...
  /*
   * Same as apache.commons.lang.StringUtils.stripStart() but also removes
   * non-breaking space (160) chars
   */
  ...
  public static String colorResToHtmlColor(Context context, int resId) {
    try {
      return String.format("#%06X",
                           0xFFFFFF & context.getResources().getColor(resId));
  ...
  /**
   * Remove {@code <script>..</script>} blocks from the passed string - added to
   * project after noticing comments on posts that use the "Sociable" plugin (
   * http://wordpress.org/plugins/sociable/ ) may have a script block which
   * contains {@code <!--//-->} followed by a CDATA section followed by {@code
   * <!]]>,} all of which will show up if we don't strip it here.
   * @see <a href="http://wordpress.org/plugins/sociable/">Wordpress Sociable
   *     Plugin</a>
   * @return String without {@code <script>..</script>}, {@code <!--//-->}
   *     blocks followed by a CDATA section
   * followed by {@code <!]]>,}
  ...
  /**
   * An alternative to Html.fromHtml() supporting {@code <ul>}, {@code <ol>},
   * {@code <blockquote>} tags and replacing EmoticonsUtils with Emojis
   * @param source
   * @param imageGetter
   */
  public static SpannableStringBuilder fromHtml(String source,
    ...
    try {
      html = (SpannableStringBuilder)Html.fromHtml(source, imageGetter,
    ...
    for (QuoteSpan span : spans) {
      html.setSpan(new WPQuoteSpan(), html.getSpanStart(span),
                   html.getSpanEnd(span), html.getSpanFlags(span));
      html.setSpan(new ForegroundColorSpan(0xFF666666), html.getSpanStart(span),
                   html.getSpanEnd(span), html.getSpanFlags(span));
