...
/**
 * forked from android.util.TimingLogger to use AppLog instead of Log + new
 * static interface.
 */
public class ProfilingUtils {
  ...
  public static void start(String label) { getInstance().reset(label); }

  public static void split(String splitLabel) {
    getInstance().addSplit(splitLabel);
  }

  public static void dump() { getInstance().dumpToLog(); }

  public static void stop() { getInstance().reset(null); }
  ...
  public ProfilingUtils() { reset("init"); }
  ...
  public void dumpToLog() {
    ...
    for (int i = 1; i < mSplits.size(); i++) {
      ...
      AppLog.d(T.PROFILING,
               mLabel + ": " + (now - prev) + " ms, " + splitLabel);
    }
