...
// by default tell gravatar to respond to non-existent images with a 404 -
// this means it's up to the caller to catch the 404 and provide a suitable
// default image
...
/*
 * gravatars often contain the ?s= parameter which determines their size -
 * detect this and replace it with a new ?s= parameter which requests the
 * avatar at the exact size needed
 */
...
public static String fixGravatarUrl(final String imageUrl, int avatarSz,
	...
	return UrlUtils.removeQuery(imageUrl) + "?s=" + avatarSz +
...
public static String gravatarFromEmail(final String email, int size,
	...
	return "http://gravatar.com/avatar/" +
	       ...
	       "?d=" + defaultImage.toString() + "&size=" + Integer.toString(size);
}
