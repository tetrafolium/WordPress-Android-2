...
public class JSONUtils {
  ...
  public static <U> U queryJSON(JSONObject source, String query,
    ...
    if (nextSeperator == -1 && nextIndexStart == -1) {
      ...
      try {
        ...
        AppLog.e(T.UTILS,
                 "Unable to cast the object to " +
                     defaultObject.getClass().getName(),
                 e);
        return defaultObject;
      } catch (JSONException e) {
        AppLog.e(T.UTILS,
                 "Unable to get the Key from the input object. Key:" + query,
                 e);
        return defaultObject;
      }
    }
   ...
   *
   * Acceptable indexes include negative numbers to reference items from the end
   * of the list as well as "last" and "first" as more explicit references to
   * "0" and "-1"
   */
  public static <U> U queryJSON(JSONArray source, String query,
    ...
    try {
      if (remainingQuery.indexOf(QUERY_ARRAY_INDEX_START) == 0) {
        ...
        AppLog.w(T.UTILS, String.format("Incorrect query for next object %s",
                                        remainingQuery));
        return defaultObject;
      }
  ...
  fromJSONArrayToStringList(JSONArray jsonArray) {
  ...
  fromStringListToJSONArray(ArrayList<String> stringList) {
  ...
  /*
   * replacement for JSONObject.optBoolean() - optBoolean() only checks for
   * "true" and "false", but our API sometimes uses "0" to denote false
   */
  ...
  /*
   * returns the JSONObject child of the passed parent that matches the passed
   * query this is basically an "optJSONObject" that supports nested queries,
   * for example:
   *
   ...
   *   }
   */
  public static JSONObject getJSONChild(final JSONObject jsonParent,
                                        final String query) {
