...
public class StringUtils {
   ...
   * Mirrors {@link org.apache.commons.lang3.StringUtils#compare(String,
   * String)}. Use this version when there is a hint that the Apache lib might
   * not be provided by the system.
   * @param s1 the String to compare from
   * @param s2 the String to compare to
   * @return &lt; 0, 0, &gt; 0, if {@code s1} is respectively less, equal ou
   ...
   * Use this version when there is a hint that the Apache lib might not be
   ...
   * TODO: not thoroughly tested yet, so marked as private - not sure I like the
   * way this replaces two spaces with "&nbsp;"
   */
  ...
  /*
   * returns empty string if passed string is null, otherwise returns passed
   * string
   */
  ...
  /*
   * capitalizes the first letter in the passed string - based on Apache
  ...
  replaceUnicodeSurrogateBlocksWithHTMLEntities(final String inputString) {
    ...
    for (int offset = 0; offset < length;) {
      ...
      if (Character.isHighSurrogate(current) ||
        ...
        if (EmoticonsUtils.WP_SMILIES_CODE_POINT_TO_TEXT.get(codepoint) !=
            null) {
          out.append(
              EmoticonsUtils.WP_SMILIES_CODE_POINT_TO_TEXT.get(codepoint));
        } else {
          final String htmlEscapedChar =
  ...
  /**
   * Used to convert a language code ([lc]_[rc] where lc is language code (en,
   * fr, es, etc...) and rc is region code (zh-CN, zh-HK, zh-TW, etc...) to a
   * displayable string with the languages name.
   *
   * The input string must be between 2 and 6 characters, inclusive. An empty
   * string is returned if that is not the case.
   *
   * If the input string is recognized by {@link Locale} the result of this
   * method is the given
   *
   * @return non-null
   */
  public static String getLanguageString(String languagueCode,
                                         Locale displayLocale) {
    if (languagueCode == null || languagueCode.length() < 2 ||
        languagueCode.length() > 6) {
      return "";
    }

    Locale languageLocale = new Locale(languagueCode.substring(0, 2));
    return languageLocale.getDisplayLanguage(displayLocale) +
        languagueCode.substring(2);
  }
  ...
  /*
   * simple wrapper for Integer.valueOf(string) so caller doesn't need to catch
   * NumberFormatException
   */
  public static int stringToInt(String s) { return stringToInt(s, 0); }
  ...
  public static long stringToLong(String s) { return stringToLong(s, 0L); }
   ...
   * We need this because our translation platform doesn't support Android
   * plurals.
   *
   ...
   * @param other The desired string identifier to get when quantity is not (0
   *     or 1)
   * @param quantity The number used to get the correct string
   */
  public static String getQuantityString(Context context, @StringRes int zero,
                                         @StringRes int one,
