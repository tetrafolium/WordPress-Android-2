...
/**
 * @deprecated Use {@link #getCurrentDeviceLanguage()}. As of API 25, setting
 *     the locale by updating the
 * configuration on the resources object was deprecated, so this method
 * stopped working for newer versions of Android. The current active locale
 * should always be set in {@link Locale#getDefault()}. When manually setting
 * the active locale, the developer should set it in {@link
...
public static String patchDeviceLanguageCode(String deviceLanguageCode) {
	...
	/*
	   <p>Note that Java uses several deprecated two-letter codes. The Hebrew
	   ("he") language
	 * code is rewritten as "iw", Indonesian ("id") as "in", and Yiddish ("yi")
	   as "ji". This
	 * rewriting happens even if you construct your own {@code Locale} object,
	   not just for
