	...
	try {
		if (!file.exists()) {
			AppLog.w(AppLog.T.MEDIA,
			         "Can't access the file. It doesn't exists anymore?");
			return -1L;
		}
...
/**
 * Given the full file path, or the filename with extension (i.e.
 * my-picture.jpg), returns the filename part only (my-picture).
 *
