...
public class DeviceUtils {
  private static final String APP_RUNTIME_ON_CHROME_FLAG =
      "org.chromium.arc.device_management";

  private static DeviceUtils instance;
  private boolean mIsKindleFire = false;

  public boolean isKindleFire() { return mIsKindleFire; }
  ...
  private DeviceUtils() {
    mIsKindleFire =
        android.os.Build.MODEL.equalsIgnoreCase("kindle fire") ? true : false;
  }
   ...
   * {@link PackageManager#FEATURE_CAMERA_FRONT} and {@link
   * PackageManager#FEATURE_CAMERA_ANY}, respectively, once they become
  ...
  public String getDeviceName(Context context) {
    ...
    try {
      ...
      fileStream.close();
      String decodedModel =
          prop.getProperty(undecodedModel.replaceAll(" ", "_"));
      ...
      AppLog.e(
          T.UTILS,
          "Can't read `android_models.properties` file from assets, or it's in the wrong form.",
          e);
      AppLog.d(
          T.UTILS,
  ...
  /**
   * Checks if the current device runtime is ARC which effectively means it is a
   * chromebook.
   *
   * @param context The context.
  ...
  public boolean isChromebook(Context context) {
    return context.getPackageManager().hasSystemFeature(
        APP_RUNTIME_ON_CHROME_FLAG);
  }

  /**
   * Checks if the device has a hardware keyboard - note this will return true
   * for emulators
   */
  ...
  private static long availableSpaceAtFilePath(File path) {
    ...
    if (android.os.Build.VERSION.SDK_INT >=
      ...
      bytesAvailable =
          (long)stat.getBlockSize() * (long)stat.getAvailableBlocks();
    }
