...
public class PhotonUtils {
  private PhotonUtils() { throw new AssertionError(); }
  ...
  /*
   * returns a photon url for the passed image with the resize query set to the
   * passed dimensions - note that the passed quality parameter will only affect
   * JPEGs
   */
  public enum Quality { HIGH, MEDIUM, LOW }

  public static String getPhotonImageUrl(String imageUrl, int width,
                                         int height) {
  ...
  public static String getPhotonImageUrl(String imageUrl, int width, int height,
    ...
    // we have encountered some image urls that incorrectly have a # fragment
    // part, which must be removed before removing the query string
    int fragmentPos = imageUrl.indexOf("#");
    if (fragmentPos > 0) {
      imageUrl = imageUrl.substring(0, fragmentPos);
    }

    // remove existing query string since it may contain params that conflict
    // with the passed ones
    imageUrl = UrlUtils.removeQuery(imageUrl);

    // if this is an "mshots" url, skip photon and return it with a query that
    ...
    // if both width & height are passed use the "resize" param, use only "w" or
    // "h" if just one of them is set
    if (width > 0 && height > 0) {
    ...
    if ((imageUrl.contains(".wp.com")) && (imageUrl.contains("i0.wp.com") || imageUrl.contains("i1.wp.com") ||
          imageUrl.contains("i2.wp.com"))) {
      return imageUrl + query;
    }

    // use wordpress.com as the host if image is on wordpress.com since it
    // supports the same query params and, more importantly, can handle images
    // in private blogs
    ...
    if (UrlUtils.isHttps(imageUrl)) {
      return "https://i0.wp.com/" +
          imageUrl.substring(schemePos + 3, imageUrl.length()) + query;
    } else {
      return "http://i0.wp.com/" +
          imageUrl.substring(schemePos + 3, imageUrl.length()) + query;
    }
  }
}
