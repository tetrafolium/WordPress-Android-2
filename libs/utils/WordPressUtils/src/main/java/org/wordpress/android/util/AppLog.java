...
public class AppLog {
  ...
  private AppLog() { throw new AssertionError(); }

  /**
   * Capture log so it can be displayed by AppLogViewerActivity
   * @param enable A boolean flag to capture log. Default is false, pass true to
   *     enable recording
   */
  ...
  public static void removeListeners() { mListeners.clear(); }
   ...
   *     identifies the class or activity where the
   *           log call occurs.
  ...
  private static String getAppInfoHeaderText(Context context) {
    ...
    ApplicationInfo applicationInfo =
        pkInfo != null ? pkInfo.applicationInfo : null;
    String appName;
  ...
  private static String getDeviceInfoHeaderText(Context context) {
    return "Android device name: " +
        DeviceUtils.getInstance().getDeviceName(context);
  }
  ...
  public static ArrayList<String> toHtmlList(Context context) {
    ...
    // add version & device info - be sure to change HEADER_LINE_COUNT if
  ...
  public static synchronized String toPlainText(Context context) {
    ...
    sb.append(getAppInfoHeaderText(context))
        .append("\n")
        .append(getDeviceInfoHeaderText(context))
        .append("\n\n");
