...
public class FormatUtils {
  /*
   * NumberFormat isn't synchronized, so a separate instance must be created for
   * each thread
   * http://developer.android.com/reference/java/text/NumberFormat.html
   */
  private static final ThreadLocal<NumberFormat> INTEGER_INSTANCE =
  ...
  private static final ThreadLocal<DecimalFormat> DECIMAL_INSTANCE =
  ...
  /*
   * returns the passed integer formatted with thousands-separators based on the
   * current locale
   */
   ...
   * unitStrings is expected to be an array of all possible sizes from byte to
   * TeraByte, in the current locale
   */
  public static final String formatFileSize(long size,
                                            final String[] unitStrings) {
    ...
    return String.format(unitStrings[digitGroups],
                         f.format(size / Math.pow(1024, digitGroups)));
  }

  /*
   * returns the passed double percentage (0 to 1) formatted as an human
   * readable percentage. Ex: 0.25 returns 25%
   */
