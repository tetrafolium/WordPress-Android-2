...
public static SignupMagicLinkFragment
newInstance(String email, boolean isJetpackConnect,
...
public View onCreateView(LayoutInflater inflater, ViewGroup container,
	...
	View layout =
		inflater.inflate(R.layout.signup_magic_link, container, false);
...
protected void sendMagicLinkEmail() {
	if (NetworkUtils.checkConnection(getActivity())) {
		...
		AuthEmailPayload authEmailPayload = new AuthEmailPayload(
			mEmail, true, mIsJetpackConnect ? AuthEmailPayloadFlow.JETPACK : null,
			source);
		mDispatcher.dispatch(
			AuthenticationActionBuilder.newSendAuthEmailAction(authEmailPayload));
	}
}
...
protected void showErrorDialog(String message) {
	DialogInterface.OnClickListener dialogListener =
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			case DialogInterface.BUTTON_POSITIVE:
				sendMagicLinkEmail();
				break;
				// DialogInterface.BUTTON_NEGATIVE is intentionally ignored. Just
				// dismiss dialog.
			}
		}
	};
	...
	AlertDialog dialog =
		...
		.setMessage(message)
		.setNegativeButton(R.string.signup_magic_link_error_button_negative,
		                   dialogListener)
		.setPositiveButton(R.string.signup_magic_link_error_button_positive,
		                   dialogListener)
		.create();
	dialog.show();
}
...
public void onAuthEmailSent(OnAuthEmailSent event) {
	if (mInProgress) {
		...
		if (event.isError()) {
			...
			AppLog.e(T.API, "OnAuthEmailSent error: " + event.error.type + " - " +
			         event.error.message);
