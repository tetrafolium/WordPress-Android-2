...
protected void startFlow() {
	if (!mLoginRequested) {
		...
		AppLog.d(T.MAIN,
		         "GOOGLE LOGIN: startFlow called, but is already in progress");
	}
}
...
public void onActivityResult(int request, int result, Intent data) {
	...
	case REQUEST_LOGIN:
		...
		if (result == RESULT_OK) {
			AppLog.d(
				T.MAIN,
				"GOOGLE LOGIN: Google has returned a sign in result - succcess");
			GoogleSignInResult loginResult =
			...
			if (loginResult.isSuccess()) {
				try {
					...
					AppLog.d(
						T.MAIN,
						"GOOGLE LOGIN: Google has returned a sign in result - NPE");
					AppLog.e(T.NUX, "Cannot get ID token from Google login account.",
					         exception);
				...
				AppLog.d(
					T.MAIN,
					"GOOGLE LOGIN: Google has returned a sign in result - error");
			...
			AppLog.d(
				T.MAIN,
				"GOOGLE LOGIN: Google has returned a sign in result - canceled");
			mAnalyticsListener.trackSocialButtonFailure();
			...
			AppLog.d(
				T.MAIN,
				"GOOGLE LOGIN: Google has returned a sign in result - unknown");
			mAnalyticsListener.trackSocialButtonFailure();
...
public void onAuthenticationChanged(OnAuthenticationChanged event) {
	if (event.isError()) {
		...
		AppLog.e(T.API, "LoginGoogleFragment.onAuthenticationChanged: " +
		...
		AppLog.i(T.NUX, "LoginGoogleFragment.onAuthenticationChanged: " +
		         event.toString());
...
public void onSocialChanged(OnSocialChanged event) {
	// Response returns error for non-existing account and existing account not
	// connected.
	if (event.isError()) {
		AppLog.e(T.API, "LoginGoogleFragment.onSocialChanged: " +
		         event.error.type + " - " + event.error.message);
		...
		case USER_EXISTS:
			AppLog.d(
				T.MAIN,
				"GOOGLE LOGIN: onSocialChanged - wordpress acount exists but not connected");
		...
		case UNKNOWN_USER:
			AppLog.d(
				T.MAIN,
				"GOOGLE LOGIN: onSocialChanged - wordpress acount doesn't exist");
		...
		case SMS_CODE_THROTTLED:
			AppLog.d(T.MAIN,
			         "GOOGLE LOGIN: onSocialChanged - error - sms code throttled");
			showError(getString(R.string.login_error_sms_throttled));
			break;
		// Unknown error.
		case GENERIC_ERROR:
		// Do nothing for now (included to show all error types) and just fall
		// through to 'default'
		default:
