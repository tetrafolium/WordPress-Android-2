...
public static LoginHttpAuthDialogFragment
newInstance(@NonNull final String url) {
...
public Dialog onCreateDialog(Bundle savedInstanceState) {
	AlertDialog.Builder alert = new AlertDialog.Builder(
		new ContextThemeWrapper(getActivity(), R.style.LoginTheme));
	alert.setTitle(R.string.http_authorization_required);

	// noinspection InflateParams
	View httpAuth = getActivity().getLayoutInflater().inflate(
		R.layout.login_alert_http_auth, null);
	alert.setView(httpAuth);

	final EditText usernameEditText =
		(EditText)httpAuth.findViewById(R.id.login_http_username);
	final EditText passwordEditText =
		(EditText)httpAuth.findViewById(R.id.login_http_password);
	...
	alert.setPositiveButton(
		R.string.next, new DialogInterface.OnClickListener() {
