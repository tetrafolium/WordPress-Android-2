...
public class LoginSiteAddressFragment
	extends LoginBaseFormFragment<LoginListener>
	implements TextWatcher, OnEditorCommitListener {
private static final String KEY_REQUESTED_SITE_ADDRESS =
	"KEY_REQUESTED_SITE_ADDRESS";
...
protected void setupContent(ViewGroup rootView) {
	...
	if (BuildConfig.DEBUG) {
		mSiteAddressInput.getEditText().setText(
			BuildConfig.DEBUG_WPCOM_WEBSITE_URL);
	}
...
public void onActivityCreated(@Nullable Bundle savedInstanceState) {
	...
	mLoginSiteAddressValidator.getIsValid().observe(
	...
	mLoginSiteAddressValidator.getErrorMessageResId().observe(
...
protected void discover() {
	...
	String cleanedXmlrpcSuffix =
		UrlUtils.removeXmlrpcSuffix(mRequestedSiteAddress);

	if (mLoginListener.getLoginMode() == LoginMode.WOO_LOGIN_MODE) {
		// TODO: This is temporary code to test out sign in flow milestone 1
		// effectiveness. If we move forward with this flow, we will need to just
		// call the XMLRPC discovery code and handle all the edge cases such as
		// HTTP auth and self-signed SSL.
		mAnalyticsListener.trackConnectedSiteInfoRequested(cleanedXmlrpcSuffix);
		mDispatcher.dispatch(
			SiteActionBuilder.newFetchConnectSiteInfoAction(cleanedXmlrpcSuffix));
	} else {
		mDispatcher.dispatch(
			SiteActionBuilder.newFetchWpcomSiteByUrlAction(cleanedXmlrpcSuffix));
	}
...
public void afterTextChanged(Editable s) {
	mLoginSiteAddressValidator.setAddress(
		EditTextUtils.getText(mSiteAddressInput.getEditText()));
}
...
private void askForHttpAuthCredentials(@NonNull final String url) {
	LoginHttpAuthDialogFragment loginHttpAuthDialogFragment =
		LoginHttpAuthDialogFragment.newInstance(url);
	loginHttpAuthDialogFragment.setTargetFragment(
		this, LoginHttpAuthDialogFragment.DO_HTTP_AUTH);
	loginHttpAuthDialogFragment.show(getFragmentManager(),
	                                 LoginHttpAuthDialogFragment.TAG);
}

public void handleDiscoveryError(DiscoveryError error,
                                 final String failedEndpoint) {
	switch (error) {
	case ERRONEOUS_SSL_CERTIFICATE:
		mLoginListener.handleSslCertificateError(
			mMemorizingTrustManager, new LoginListener.SelfSignedSSLCallback() {
				@Override
...
private void showSiteAddressHelp() {
	new LoginSiteAddressHelpDialogFragment().show(
		getFragmentManager(), LoginSiteAddressHelpDialogFragment.TAG);
}
...
public void onActivityResult(int requestCode, int resultCode, Intent data) {
	...
	if (requestCode == LoginHttpAuthDialogFragment.DO_HTTP_AUTH &&
		...
		String httpUsername =
			data.getStringExtra(LoginHttpAuthDialogFragment.ARG_USERNAME);
		String httpPassword =
			data.getStringExtra(LoginHttpAuthDialogFragment.ARG_PASSWORD);
		mHTTPAuthManager.addHTTPAuthCredentials(httpUsername, httpPassword, url,
		                                        null);
		discover();
	}
}
...
public void onWPComSiteFetched(OnWPComSiteFetched event) {
	...
	if (event.isError()) {
		...
		if (mLoginListener.getLoginMode() == LoginMode.WPCOM_LOGIN_ONLY) {
			...
			mDispatcher.dispatch(
				AuthenticationActionBuilder.newDiscoverEndpointAction(
					mRequestedSiteAddress));
		}
	} else {
		if (event.site.isJetpackInstalled() &&
		    mLoginListener.getLoginMode() != LoginMode.WPCOM_LOGIN_ONLY) {
			// If Jetpack site, treat it as self-hosted and start the discovery
			// process An exception is WPCOM_LOGIN_ONLY mode - in that case we're
			// only interested in adding sites through WordPress.com login, and
			// should proceed along that login path
			mDispatcher.dispatch(
				AuthenticationActionBuilder.newDiscoverEndpointAction(
					mRequestedSiteAddress));
			return;
		...
		mLoginListener.gotWpcomSiteInfo(
			UrlUtils.removeScheme(event.site.getUrl()), event.site.getName(),
...
public void onDiscoverySucceeded(OnDiscoveryResponse event) {
	...
	// hold the URL in a variable to use below otherwise it gets cleared up by
	// endProgress
	...
	if (event.isError()) {
		...
		if (event.error == DiscoveryError.WORDPRESS_COM_SITE) {
			...
			if (mAccountStore.hasAccessToken()) {
				...
				AppLog.e(T.NUX, "User is already logged in WordPress.com: " +
				         currentUsername);

				ArrayList<Integer> oldSitesIDs =
					SiteUtils.getCurrentSiteIds(mSiteStore, true);
	...
	mLoginListener.gotXmlRpcEndpoint(requestedSiteAddress,
	                                 event.xmlRpcEndpoint);
}
...
public void onFetchedConnectSiteInfo(OnConnectSiteInfoChecked event) {
	...
	if (event.isError()) {
		mAnalyticsListener.trackConnectedSiteInfoFailed(
			requestedSiteAddress, event.getClass().getSimpleName(),
			event.error.type.name(), event.error.message);

		AppLog.e(T.API,
		         "onFetchedConnectSiteInfo has error: " + event.error.message);

		showError(R.string.invalid_site_url_message);
	} else {
		// TODO: If we plan to keep this logic we should convert these labels to
		// constants
		...
		properties.put("is_jetpack_active",
		               Boolean.toString(event.info.isJetpackActive));
		properties.put("is_jetpack_connected",
		               Boolean.toString(event.info.isJetpackConnected));
		...
		// Determining if jetpack is actually installed takes additional logic.
		// This final calculated event property will make querying this event more
		// straight-forward:
		boolean hasJetpack = false;
		if (event.info.isWPCom && event.info.hasJetpack) {
			// This is likely an atomic site.
			hasJetpack = true;
		} else if (event.info.hasJetpack && event.info.isJetpackActive &&
		           event.info.isJetpackConnected) {
			hasJetpack = true;
		}
		properties.put("login_calculated_has_jetpack",
		               Boolean.toString(hasJetpack));
		...
		if (!event.info.exists) {
			...
			mLoginListener.gotConnectedSiteInfo(
				event.info.url, event.info.urlAfterRedirects, hasJetpack);
		}
	}
}
}
