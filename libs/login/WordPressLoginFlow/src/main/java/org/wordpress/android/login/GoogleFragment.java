...
public class GoogleFragment extends Fragment
  ...
  private static final String STATE_SHOULD_RESOLVE_ERROR =
      "STATE_SHOULD_RESOLVE_ERROR";
  ...
  /**
   * This flag is used to store the information the finishFlow was called when
   * the fragment wasn't attached to an activity (for example an EventBus event
   * was received during ongoing configuration change).
   */
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      ...
      mShouldResolveError =
          savedInstanceState.getBoolean(STATE_SHOULD_RESOLVE_ERROR, false);
    ...
    // Configure sign-in to request user's ID, basic profile, email address, and
    // ID token. ID and basic profile are included in DEFAULT_SIGN_IN.
    ...
    // Build Google API client with access to sign-in API and options specified
    // above.
    mGoogleApiClient =
  ...
  public void onResume() {
    super.onResume();
    // Show account dialog when Google API onConnected callback returns before
    // fragment is attached.
    if (mGoogleApiClient != null && mGoogleApiClient.isConnected() &&
        !mIsResolvingError && !mShouldResolveError) {
      startFlow();
    }
  }
  ...
  public void onConnected(Bundle bundle) {
    // Indicates account was selected, that account has granted any required
    // permissions, and a connection to Google Play services has been
    // established.
    if (mShouldResolveError) {
      mShouldResolveError = false;

      // if the fragment is not attached to an activity, the process is started
      // in the onResume
      if (isAdded()) {
        startFlow();
      }
  ...
  public void onConnectionFailed(@NonNull ConnectionResult connectionResult) {
    // Could not connect to Google Play Services.  The user needs to select an
    // account, grant permissions or resolve an error in order to sign in. Refer
    // to the documentation for ConnectionResult to see possible error codes.
  ...
  public void onConnectionSuspended(int i) {
    // Connection to Google Play services was lost.  GoogleApiClient will
    // automatically attempt to re-connect.  Any UI elements depending on
    // connection to Google APIs should be hidden or disabled until onConnected
    // is called again.
    Log.w(LoginGoogleFragment.class.getSimpleName(),
  ...
  protected void finishFlow() {
    /* This flag might get lost when the finishFlow is called after the
     fragment's onSaveInstanceState was called - however it's a very rare case,
     since the fragment is retained across config changes.  */
    mFinished = true;
    if (getActivity() != null) {
