...
/**
 * Encapsulates the site address validation, cleaning, and error reporting of
 * {@link LoginSiteAddressFragment}.
 */
class LoginSiteAddressValidator {
  ...
  LoginSiteAddressValidator() { this(new Debouncer()); }
  ...
  void dispose() { mDebouncer.shutdown(); }
  ...
  void setAddress(@NonNull String siteAddress) {
    ...
    // Call debounce regardless if there was an error so that the previous
    // Runnable will be cancelled.
  ...
  siteAddressIsValid(@NonNull String cleanedSiteAddress) {
