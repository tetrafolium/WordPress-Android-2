...
public class LoginWpcomService extends AutoForeground<LoginState> {
  ...
  public enum LoginStep {
    ...
    LoginStep() { this.progressPercent = 0; }

    LoginStep(int progressPercent) { this.progressPercent = progressPercent; }
  }

  public static class LoginState implements AutoForeground.ServiceState {
    private final LoginStep mStep;

    LoginState(@NonNull LoginStep step) { this.mStep = step; }

    LoginStep getStep() { return mStep; }
  ...
  private static class LoginNotification {
    static Notification progress(Context context, int progress) {
      return AutoForegroundNotification.progress(
          context, context.getString(R.string.login_notification_channel_id),
          progress, R.string.notification_login_title_in_progress,
          R.string.notification_logging_in, R.drawable.login_notification_icon,
    ...
    static Notification success(Context context) {
      return AutoForegroundNotification.success(
          ...
          R.string.notification_logged_in, R.drawable.login_notification_icon,
          R.color.login_notification_accent_color);
    }
    ...
    static Notification failure(Context context, @StringRes int content) {
      return AutoForegroundNotification.failure(
          context, context.getString(R.string.login_notification_channel_id),
          R.string.notification_login_title_stopped, content,
  ...
  public static void loginWithEmailAndPassword(Context context, String email,
  ...
  public LoginWpcomService() { super(new LoginState(LoginStep.IDLE)); }
  ...
  public Notification getNotification(LoginState state) {
    ...
    case FAILURE_EMAIL_WRONG_PASSWORD:
      return LoginNotification.failure(
          this, R.string.notification_error_wrong_password);
    case FAILURE_2FA:
    ...
    case FAILURE_USE_WPCOM_USERNAME_INSTEAD_OF_EMAIL:
      return LoginNotification.failure(
          this, R.string.notification_wpcom_username_needed);
    case FAILURE_FETCHING_ACCOUNT:
    case FAILURE_CANNOT_ADD_DUPLICATE_SITE:
    case FAILURE:
      return LoginNotification.failure(this,
  ...
  private void setState(LoginStep phase) { setState(new LoginState(phase)); }
  ...
  public void onCreate() {
    ...
    // TODO: Recover any login attempts that were interrupted by the service
    // being stopped?
  }
  ...
  private void handleAuthError(AuthenticationErrorType error,
    ...
    default:
      ...
      ToastUtils.showToast(this, errorMessage == null
  ...
  public void onSocialChanged(OnSocialChanged event) {
    if (event.isError()) {
      ...
      case USER_ALREADY_ASSOCIATED:
        ...
        break;
        // Ignore other error cases.  The above are the only two we have chosen
        // to log.
      }
  ...
  public void onAccountChanged(OnAccountChanged event) {
    if (event.isError()) {
      AppLog.e(T.API, "onAccountChanged has error: " + event.error.type +
                          " - " + event.error.message);
    ...
    if (event.causeOfChange == AccountAction.FETCH_ACCOUNT) {
      ...
      // The user's account info has been fetched and stored - next, fetch the
      // user's settings
      mDispatcher.dispatch(AccountActionBuilder.newFetchSettingsAction());
    } else if (event.causeOfChange == AccountAction.FETCH_SETTINGS) {
      setState(LoginStep.FETCHING_SITES);
      // The user's account settings have also been fetched and stored - now we
      // can fetch the user's sites
  ...
  public void onSiteChanged(OnSiteChanged event) {
    if (event.isError()) {
      AppLog.e(T.API, "onSiteChanged has error: " + event.error.type + " - " +
      ...
      if (event.rowsAffected == 0) {
        // If there is a duplicate site and not any site has been added, show an
        // error and stop the sign in process
