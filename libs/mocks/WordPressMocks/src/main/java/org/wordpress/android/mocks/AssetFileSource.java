...
/**
 * AssetFileSource provides a the necessary logic for WireMock to load its JSON
 * mappings from Android assets. WireMock has no Android specific behaviour so
 * we must implement asset loading here.
 */
public class AssetFileSource implements FileSource {
  ...
  private boolean isDirectory(String path) {
    try {
      // Empty directories are not loaded from assets, so this works for an
      // existence check list() seems to be relatively expensive so we may wish
      // to change this
  ...
  private void recursivelyAddFilePathsToList(String root,
  ...
  private List<TextFile> toTextFileList(List<String> filePaths) {
    return newArrayList(transform(filePaths, new Function<String, TextFile>() {
      public TextFile apply(String input) { return getTextFile(input); }
    }));
  }
