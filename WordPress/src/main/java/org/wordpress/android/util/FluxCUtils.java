...
public class FluxCUtils {
  public static class FluxCUtilsLoggingException extends Exception {
    public FluxCUtilsLoggingException(String message) { super(message); }
  ...
  /**
   * This method doesn't do much, but insure we're doing the same check in all
   * parts of the app.
   *
   * @return true if the user is signed in a WordPress.com account or if he has
   *     a .org site.
   */
  public static boolean isSignedInWPComOrHasWPOrgSite(AccountStore accountStore,
                                                      SiteStore siteStore) {
  ...
  public static MediaModel mediaModelFromMediaFile(MediaFile file) {
    ...
    mediaModel.setMediaId(
        file.getMediaId() != null ? Long.valueOf(file.getMediaId()) : 0);
    ...
    mediaModel.setVideoPressGuid(ShortcodeUtils.getVideoPressIdFromShortCode(
        file.getVideoPressShortCode()));
    return mediaModel;
  }
  ...
  public static MediaFile mediaFileFromMediaModel(MediaModel media) {
    ...
    mediaFile.setMediaId(
        media.getMediaId() > 0 ? String.valueOf(media.getMediaId()) : null);
    ...
    mediaFile.setVideo(
        org.wordpress.android.fluxc.utils.MediaUtils.isVideoMimeType(
            media.getMimeType()));
    mediaFile.setVideoPressShortCode(
        ShortcodeUtils.getVideoPressShortcodeFromId(media.getVideoPressGuid()));
   ...
   */
  public static MediaModel mediaModelFromLocalUri(
      @NonNull Context context, @NonNull Uri uri, @Nullable String mimeType,
    ...
    if (TextUtils.isEmpty(path)) {
      // For now, we're wrapping up the actual log into an exception to reduce
      // possibility of information not travelling to our Crash Logging Service.
      // For more info: http://bit.ly/2oJHMG7 and http://bit.ly/2oPOtFX
      CrashLoggingUtils.logException(
          new FluxCUtilsLoggingException("The input URI " + uri.toString() +
                                         " can't be read."),
          T.UTILS);
      return null;
    }
    ...
    String filename =
        org.wordpress.android.fluxc.utils.MediaUtils.getFileName(path);
