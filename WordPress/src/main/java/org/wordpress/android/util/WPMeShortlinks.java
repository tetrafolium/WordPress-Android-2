...
/**
 * Enable WP.me-powered shortlinks for Posts, Pages, and Blogs on WordPress.com
 * or Jetpack powered sites. <p/> Shortlinks are a quick way to get short and
 * simple links to your posts, pages, and blogs. They use the wp.me domain so
 * you can have more space to write on social media sites. <p/> See:
 * https://github.com/Automattic/jetpack/blob/master/modules/shortlinks.php
 */
 ...
 * @return String The blog shortlink or null (null is returned if the blog
 *     object is empty, or it's not a
...
public static String getPostShortlink(SiteModel site, PostModel post) {
	...
	if (PostStatus.fromPost(post) == PostStatus.PUBLISHED &&
	    postName.length() > 0 && postName.length() <= 8 &&
	...
	if (TextUtils.isEmpty(type) || TextUtils.isEmpty(id) ||
	    TextUtils.isEmpty(blogShortlink)) {
		return null;
	}
