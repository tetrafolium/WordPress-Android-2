...
/**
 * Simple class for limiting the number of times to show a toast message and
 * only showing it after a feature has been used a few times - originally
 * designed to let users know where they can long press to multi-select
 */
...
public enum SmartToastType {
	...
	// key which stores the number of times the feature associated with the
	// smart toast has been used
	private final UndeletablePrefKey mUsageKey;
	// key which stores the number of times the toast associated with the smart
	// toast type has been shown
	private final UndeletablePrefKey mShownKey;

	SmartToastType(@NonNull UndeletablePrefKey usageKey,
	               @NonNull UndeletablePrefKey shownKey) {
...
public static boolean show(@NonNull Context context,
	...
	int yOffset = context.getResources().getDimensionPixelOffset(
		R.dimen.smart_toast_offset_y);
	Toast toast = Toast.makeText(context, context.getString(stringResId),
	                             Toast.LENGTH_LONG);
...
/*
 * prevent the passed smart toast type from being shown by setting its counter
 * to the max - this should be used to disable long press toasts when the user
 * long presses to multiselect since they already know they can do it
 */
