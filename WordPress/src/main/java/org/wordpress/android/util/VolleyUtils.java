...
/*
 * Returns REST API 'error' string code from the response in the passed
 * VolleyError for example, returns "already_subscribed" from this response:
 * {
...
/*
 * Returns REST API 'message' string field from the response in the passed
 * VolleyError for example, returns "You are already subscribed to the
 * specified topic." from this response:
 * {
 * "error": "already_subscribed",
 * "message": "You are already subscribed to the specified topic."
 * }
...
/*
 * Attempts to return JSON from a volleyError - useful for WP REST API
 * failures, which often contain JSON in the response
 */
public static JSONObject volleyErrorToJSON(VolleyError volleyError) {
	...
	String contentType =
		volleyError.networkResponse.headers.get("Content-Type");
