...
public class WPHtml {
   ...
   * found in the wild.
   */
  public static Spanned fromHtml(String source, Context ctx, PostModel post,
                                 int maxImageWidth) {
  ...
  public static Spanned fromHtml(String source, ImageGetter imageGetter,
                                 TagHandler tagHandler, Context ctx,
                                 PostModel post, int maxImageWidth) {
    ...
    HtmlToSpannedConverter converter = new HtmlToSpannedConverter(
        source, imageGetter, tagHandler, parser, ctx, post, maxImageWidth);
      ...
      for (int j = 0; j < style.length; j++) {
        ...
        if (style[j] instanceof MediaGalleryImageSpan) {
          out.append(getGalleryShortcode((MediaGalleryImageSpan)style[j]));
                   ...
                   ((WPImageSpan)style[j]).getMediaFile().getMediaId() !=
                       null) {
          ...
          out.append("\" android-uri=\"" +
                     ((WPImageSpan)style[j]).getImageSource().toString() +
                     "\"");
          out.append(" />");
        ...
        if (style[j] instanceof ForegroundColorSpan) {
          ...
          String color = Integer.toHexString(
              ((ForegroundColorSpan)style[j]).getForegroundColor() +
              0x01000000);
  ...
  public static String getContent(WPImageSpan imageSpan) {
    ...
    if (isVideo) {
      ...
      String inlineCSS = " ";
      content = content + "<a href=\"" + url + "\"><img" + inlineCSS +
                "title=\"" + title + "\" " + alignmentCSS +
                "alt=\"image\" src=\"" + url + "?w=" + width + "\" /></a>";
