...
public class WPMediaUtils {
  ...
  // Max picture size will be 3000px wide. That's the maximum resolution you can
  // set in the current picker.
  ...
  public static Uri getOptimizedMedia(Activity activity, String path,
                                      boolean isVideo) {
    if (isVideo) {
      return null;
    }
    ...
    int resizeDimension = AppPrefs.getImageOptimizeMaxSize() > 1
                              ? AppPrefs.getImageOptimizeMaxSize()
                              : Integer.MAX_VALUE;
    ...
    String optimizedPath =
        ImageUtils.optimizeImage(activity, path, resizeDimension, quality);
  ...
  public static Uri fixOrientationIssue(Activity activity, String path,
                                        boolean isVideo) {
    if (isVideo) {
      return null;
    }
  ...
  /**
   * Check if we should advertise image optimization feature for the current
   * site. <p> The following condition need to be all true: 1) Image
   * optimization is OFF on the site. 2) Didn't already ask to enable the
   * feature. 3) The user has granted storage access to the app. This is because
   * we don't want to ask so much things to users the first time they try to add
   * a picture to the app.
   *
  ...
  public interface OnAdvertiseImageOptimizationListener { void done(); }

  public static void advertiseImageOptimization(
        ...
        new DialogInterface.OnClickListener() {
          ...
          public void onClick(DialogInterface dialog, int which) {
            if (which == DialogInterface.BUTTON_POSITIVE) {
              if (AppPrefs.isImageOptimize()) {
                // null or image optimization already ON. We should not be here
                // though.
              } else {
    ...
    builder.setTitle(
        org.wordpress.android.R.string.image_optimization_promo_title);
    builder.setMessage(
        org.wordpress.android.R.string.image_optimization_promo_desc);
   ...
   */
  public static @Nullable String getErrorMessage(final Context context,
    ...
    case FS_READ_PERMISSION_DENIED:
      return context.getString(
          R.string.error_media_insufficient_fs_permissions);
    case NOT_FOUND:
    ...
    case REQUEST_TOO_LARGE:
      ...
      if (media.isVideo()) {
        return context.getString(
            R.string.media_error_http_too_large_video_upload);
      } else {
        return context.getString(
            R.string.media_error_http_too_large_photo_upload);
      }
    case SERVER_ERROR:
  ...
  private static void showSDCardRequiredDialog(Context context) {
    ...
    dialogBuilder.setTitle(
        context.getResources().getText(R.string.sdcard_title));
    dialogBuilder.setMessage(
        context.getResources().getText(R.string.sdcard_message));
  ...
  public static void launchVideoLibrary(Activity activity,
    ...
    activity.startActivityForResult(
        prepareVideoLibraryIntent(activity, multiSelect),
  ...
  public static void launchVideoCamera(Activity activity) {
    activity.startActivityForResult(prepareVideoCameraIntent(),
  ...
  public static void launchPictureLibrary(Activity activity,
                                          boolean multiSelect) {
    activity.startActivityForResult(
        preparePictureLibraryIntent(activity.getString(R.string.pick_photo),
                                    multiSelect),
  ...
  public static void launchCamera(Activity activity, String applicationId,
  ...
  private static Intent prepareLaunchCamera(Context context,
  ...
  private static Intent getLaunchCameraIntent(Context context,
                                              String applicationId,
                                              LaunchCameraCallback callback)
      throws IOException {
    ...
    String mediaCapturePath = externalStoragePublicDirectory + File.separator +
    ...
    Uri fileUri;
    try {
      fileUri = FileProvider.getUriForFile(context, applicationId + ".provider",
                                           new File(mediaCapturePath));
    } catch (IllegalArgumentException e) {
      AppLog.e(T.MEDIA, "Cannot access the file planned to store the new media",
               e);
      throw new IOException(
          "Cannot access the file planned to store the new media");
  ...
  private static Intent makePickOrCaptureIntent(Context context,
                                                String applicationId,
                                                LaunchCameraCallback callback) {
    Intent pickPhotoIntent =
        prepareGalleryIntent(context.getString(R.string.capture_or_pick_photo));

    if (DeviceUtils.getInstance().hasCamera(context)) {
      try {
        Intent cameraIntent =
            getLaunchCameraIntent(context, applicationId, callback);
        pickPhotoIntent.putExtra(Intent.EXTRA_INITIAL_INTENTS,
  ...
  public static boolean canDeleteMedia(MediaModel mediaModel) {
    ...
    return state == null || (!state.equalsIgnoreCase("uploading") &&
                             !state.equalsIgnoreCase("deleted"));
  }
  ...
  public static String getVideoPressVideoPosterFromURL(String videoUrl) {
    ...
    if (videoUrl != null) {
      ...
      if (fileTypeLocation > 0) {
        posterUrl =
            videoUrl.substring(0, fileTypeLocation) + "_std.original.jpg";
      }
    }
  ...
  /*
   * passes a newly-created media file to the media scanner service so it's
   * available to the media content provider - use this after capturing or
   * downloading media to ensure that it appears in the stock Gallery app
   */
  public static void scanMediaFile(@NonNull Context context,
                                   @NonNull String localMediaPath) {
    MediaScannerConnection.scanFile(
        context, new String[] {localMediaPath}, null,
  ...
  /*
   * returns true if the current user has permission to upload new media to the
   * passed site
   */
  ...
  /*
   * returns the minimum distance for a fling which determines whether to
   * disable loading thumbnails in the media grid or photo picker - used to
   * conserve memory usage during a reasonably-sized fling
   */
  public static int
  getFlingDistanceToDisableThumbLoading(@NonNull Context context) {
    return ViewConfiguration.get(context).getScaledMaximumFlingVelocity() / 2;
  }

  public interface MediaFetchDoNext { void doNext(Uri uri); }

  /**
   * Downloads the {@code mediaUri} and returns the {@link Uri} for the
   * downloaded file <p> If the {@code mediaUri} is already in the the local
   * store, no download will be done and the given
   * {@code mediaUri} will be returned instead. This may return null if the
   * download fails. <p> The current thread is blocked until the download is
   * finished.
   *
   * @return A local {@link Uri} or null if the download failed
   */
  public static @Nullable Uri fetchMedia(@NonNull Context context,
  ...
  /**
   * Downloads the given {@code mediaUri} and calls {@code listener} if
   * successful <p> If the download fails, a {@link android.widget.Toast} will
   * be shown.
   *
   * @return A {@link Boolean} indicating whether the download was successful
   */
  public static boolean fetchMediaAndDoNext(Context context, Uri mediaUri,
                                            MediaFetchDoNext listener) {
