 ...
 * selected language is properly saved and resources appropriately updated for
 * the android version.
 */
...
/**
 * Pattern to split a language string (to parse the language and region
 * values).
 */
...
/**
 * Change the active locale to the language provided. Save the updated
 * language settings to sharedPreferences.
 *
 * @param context  The current context
...
/**
 * If the user has selected a language other than the device default, return
 * that language code, else just return the device default language code.
 *
...
public static String getLanguage(Context context) {
	...
	return prefs.getString(LANGUAGE_KEY,
	                       LanguageUtils.getCurrentDeviceLanguageCode());
}

/**
 * Convert the device language code (codes defined by ISO 639-1) to a Language
 * ID. Language IDs, used only by WordPress, are integer values that map to a
 * language code. http://bit.ly/2H7gksN
 **/
public static @NonNull String getLanguageWordPressId(Context context) {
	...
	Map<String, String> languageCodeToID =
		LocaleManager.generateLanguageMap(context);
	String langID = null;
 ...
 * Use commit() instead of apply() to ensure the language preference is saved
 * instantly as the app may be restarted immediately.
 *
 * @param context  The current context
...
private static Context updateResources(Context context, String language) {
	...
	// NOTE: Earlier versions of Android require both of these to be set,
	// otherwise RTL may not be implemented properly.
...
/**
 * Method gets around a bug in the java.util.Formatter for API 7.x as detailed
 * here [https://bugs.openjdk.java.net/browse/JDK-8167567]. Any strings that
 * contain locale-specific grouping separators should use: <code>
...
public static Locale getSafeLocale(@Nullable Context context) {
	...
	if (context == null) {
		...
		baseLocale = Build.VERSION.SDK_INT >= 24 ? config.getLocales().get(0)
		                               : config.locale;
	}
 ...
 *
 * @param languageCode The language code (example "en" or "es-US"). If null or
 *     empty will return
...
public static Map<String, String> generateLanguageMap(Context context) {
	String[] languageIds = context.getResources().getStringArray(
		org.wordpress.android.R.array.lang_ids);
	String[] languageCodes = context.getResources().getStringArray(
		org.wordpress.android.R.array.language_codes);
...
/**
 * Generates display strings for given language codes. Used as entries in
 * language preference.
 */
@Nullable
public static Pair<String[], String[]>
createSortedLanguageDisplayStrings(CharSequence[] languageCodes,
                                   Locale locale) {
	...
	for (int i = 0; i < languageCodes.length; ++i) {
		// "__" is used to sort the language code with the display string so both
		// arrays are sorted at the same time
		entryStrings.add(i, StringUtils.capitalize(getLanguageString(
								   languageCodes[i].toString(), locale)) +
		                 "__" + languageCodes[i]);
	}
	...
	for (int i = 0; i < entryStrings.size(); ++i) {
		// now, we can split the sorted array to extract the display string and
		// the language code
...
/**
 * Generates detail display strings in the currently selected locale. Used as
 * detail text in language preference dialog.
 */
@Nullable
public static String[] createLanguageDetailDisplayStrings(
	CharSequence[] languageCodes) {
	if (languageCodes == null || languageCodes.length < 1) {
		return null;
	}
	...
	for (int i = 0; i < languageCodes.length; ++i) {
		detailStrings[i] = StringUtils.capitalize(
			getLanguageString(languageCodes[i].toString(),
			                  languageLocale(languageCodes[i].toString())));
	}
 ...
 */
public static String getLanguageString(String languageCode,
                                       Locale displayLocale) {
	if (languageCode == null || languageCode.length() < 2 ||
	    languageCode.length() > 6) {
		return "";
	}

	Locale languageLocale = languageLocale(languageCode);
	String displayLanguage = StringUtils.capitalize(
		languageLocale.getDisplayLanguage(displayLocale));
