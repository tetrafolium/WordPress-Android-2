...
public class AnalyticsUtils {
   ...
   */
  public static void refreshMetadata(AccountStore accountStore,
                                     SiteStore siteStore) {
    AnalyticsMetadata metadata = new AnalyticsMetadata();

    metadata.setUserConnected(
        FluxCUtils.isSignedInWPComOrHasWPOrgSite(accountStore, siteStore));
  ...
  /**
   * @return true if the siteStore has sites accessed via the WPCom Rest API
   *     that are not WPCom sites. This only
   * counts Jetpack sites connected via WPCom Rest API. If there are Jetpack
   * sites in the site store and they're all accessed via XMLRPC, this method
   * returns false.
   */
  static boolean isJetpackUser(SiteStore siteStore) {
    return siteStore.getSitesAccessedViaWPComRestCount() -
        siteStore.getWPComSitesCount() >
        0;
  }
  ...
  public static int getWordCount(String content) {
    String text =
        Html.fromHtml(content.replaceAll("<img[^>]*>", "")).toString();
  ...
  public static void trackWithSiteDetails(AnalyticsTracker.Stat stat,
    ...
    if (site == null || !SiteUtils.isAccessedViaWPComRest(site)) {
      AppLog.w(AppLog.T.STATS,
               "The passed blog obj is null or it's not a wpcom or Jetpack."
  ...
  public enum QuickActionTrackPropertyValue {
    ...
    APPROVE {
      public String toString() { return "approve"; }
    },
    REPLY_TO {
      public String toString() { return "reply-to"; }
    }
  }
  ...
  public static void trackQuickActionTouched(QuickActionTrackPropertyValue type,
    ...
    AnalyticsTracker.track(
        AnalyticsTracker.Stat.NOTIFICATION_QUICK_ACTIONS_QUICKACTION_TOUCHED,
        properties);
    AnalyticsTracker.flush();
  }

  /**
   * Bump Analytics for comment reply, and add blog and comment details into
   * properties.
   *
   ...
   */
  public static void trackCommentReplyWithDetails(boolean isQuickReply,
                                                  SiteModel site,
                                                  CommentModel comment) {
    AnalyticsTracker.Stat stat =
        isQuickReply
            ? AnalyticsTracker.Stat.NOTIFICATION_QUICK_ACTIONS_REPLIED_TO
  ...
  public static void trackWithReaderPostDetails(AnalyticsTracker.Stat stat,
    ...
    // wpcom/jetpack posts should pass: feed_id, feed_item_id, blog_id, post_id,
    // is_jetpack RSS pass should pass: feed_id, feed_item_id, is_jetpack
    Map<String, Object> properties = new HashMap<>();
    ...
    // record a railcar interact event if the post has a railcar and this can be
    // tracked as an interaction
  ...
  public static void trackWithReaderPostDetails(AnalyticsTracker.Stat stat,
                                                long blogId, long postId) {
    trackWithReaderPostDetails(
        stat, ReaderPostTable.getBlogPost(blogId, postId, true));
  }

  public static void trackWithBlogPostDetails(AnalyticsTracker.Stat stat,
                                              long blogId, long postId) {
    Map<String, Object> properties = new HashMap<>();
    properties.put(BLOG_ID_KEY, blogId);
  ...
  public static void trackWithFeedPostDetails(AnalyticsTracker.Stat stat,
   ...
   */
  public static void trackWithDeepLinkData(AnalyticsTracker.Stat stat,
  ...
  /**
   * Track when app launched via deep-linking but then fell back to external
   * browser
   *
   * @param stat The Stat to bump
   * @param interceptedUri The fallback URI the app was started with
   */
  public static void trackWithInterceptedUri(AnalyticsTracker.Stat stat,
   ...
   *
   * @param stat The Stat to bump
   * @param interceptorClassname The name of the class that handles the
   *     intercept by default
   */
  public static void trackWithDefaultInterceptor(AnalyticsTracker.Stat stat,
  ...
  public static void trackRailcarRender(String railcarJson) {
    ...
    AnalyticsTracker.track(TRAIN_TRACKS_RENDER,
                           railcarJsonToProperties(railcarJson));
  }
   ...
   * @param railcarJson The JSON string of the railcar
   */
  private static void trackRailcarInteraction(AnalyticsTracker.Stat stat,
                                              String railcarJson) {
    if (TextUtils.isEmpty(railcarJson)) {
      return;
    }
   ...
   * @return True if the passed stat event can be recorded as a railcar
   *     interaction
   */
  private static boolean
  canTrackRailcarInteraction(AnalyticsTracker.Stat stat) {
    return stat == READER_ARTICLE_LIKED || stat == READER_ARTICLE_OPENED ||
  ...
  /*
   * Converts the JSON string of a railcar to a properties list using the
   * existing json key names
   */
  private static Map<String, Object>
  railcarJsonToProperties(@NonNull String railcarJson) {
    Map<String, Object> properties = new HashMap<>();
    try {
  ...
  public static Map<String, Object> getMediaProperties(Context context,
                                                       boolean isVideo,
                                                       Uri mediaURI,
                                                       String path) {
    Map<String, Object> properties = new HashMap<>();
    if (context == null) {
      AppLog.e(AppLog.T.MEDIA,
               "In order to track media properties Context cannot be null.");
      return properties;
    }
    if (mediaURI == null && TextUtils.isEmpty(path)) {
      AppLog.e(
          AppLog.T.MEDIA,
          "In order to track media properties mediaURI and path cannot be both null!!");
      return properties;
    }
    ...
    try {
      ...
      AppLog.e(AppLog.T.MEDIA,
               "Can't access the media file. Properties are not being tracked.",
               e);
      return properties;
    }
  ...
  public static void trackAnalyticsSignIn(AccountStore accountStore,
                                          SiteStore siteStore,
                                          boolean isWpcomLogin) {
   ...
   * @param email
   */
  public static void trackAnalyticsAccountCreated(String username,
                                                  String email) {
  ...
  public static void
  trackAnalyticsPostListToggleLayout(PostListViewLayoutType viewLayoutType) {
    Map<String, String> properties = new HashMap<>();
    properties.put("post_list_view_layout_type", viewLayoutType.toString());
    AnalyticsTracker.track(AnalyticsTracker.Stat.POST_LIST_VIEW_LAYOUT_TOGGLED,
                           properties);
  }
  ...
  public static void trackNewsCardShown(String origin, int version) {
    AnalyticsTracker.track(NEWS_CARD_SHOWN,
                           createNewsCardProperties(origin, version));
  }
  ...
  public static void trackNewsCardDismissed(String origin, int version) {
    AnalyticsTracker.track(NEWS_CARD_DIMISSED,
                           createNewsCardProperties(origin, version));
  }
   ...
   * Don't use this method directly, use injectable NewsTracker instead.
   */
  public static void trackNewsCardExtendedInfoRequested(String origin,
                                                        int version) {
    AnalyticsTracker.track(NEWS_CARD_EXTENDED_INFO_REQUESTED,
                           createNewsCardProperties(origin, version));
  }

  private static Map<String, String> createNewsCardProperties(String origin,
                                                              int version) {
    Map<String, String> properties = new HashMap<>();
