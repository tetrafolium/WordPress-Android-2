...
public class InstallationReferrerServiceLogic {
  ...
  public InstallationReferrerServiceLogic(
      Context context, ServiceCompletionListener completionListener) {
  ...
  public void performTask(Bundle extras, Object companion) {
    ...
    // if a referrer string has been passed already (coming from
    // com.android.vending.INSTALL_REFERRER receiver), just send it to tracks
    if (extras != null && extras.containsKey(ARG_REFERRER)) {
      Map<String, Object> properties = new HashMap<>();
      properties.put("install_referrer", extras.getString(ARG_REFERRER));
      AnalyticsTracker.track(
          AnalyticsTracker.Stat.INSTALLATION_REFERRER_OBTAINED, properties);
    ...
    // if not, try to obtain it from Play Store app connection through the
    // Install Referrer API Library if possible
    mReferrerClient = InstallReferrerClient.newBuilder(mContext).build();
    final InstallReferrerStateListener installReferrerStateListener =
          ...
          public void onInstallReferrerSetupFinished(int responseCode) {
            ...
            case InstallReferrerResponse.OK:
              ...
              try {
                ...
                // Citing here:
                //  Caution: The install referrer information will be available
                //  for 90 days and won't change unless the application is
                //  reinstalled. To avoid unnecessary API calls in your app, you
                //  should invoke the API only once during the first execution
                //  after install.
                // read more:
                ...
                Map<String, Object> properties = new HashMap<>();
                properties.put("install_referrer",
                ...
                AnalyticsTracker.track(
                    AnalyticsTracker.Stat.INSTALLATION_REFERRER_OBTAINED,
                    properties);
                mReferrerClient.endConnection();
              } catch (RemoteException e) {
                e.printStackTrace();
                AppLog.i(T.UTILS,
                         "installation referrer: RemoteException occurred");
              }
              break;
            case InstallReferrerResponse.FEATURE_NOT_SUPPORTED:
              // API not available on the current Play Store app
              // just log but DO NOT mark
              // AppPrefs.setInstallationReferrerObtained(true);, because the
              // user could update to a version of Google PLay that supports it
              // and we can obtain it also from the old
            ...
            case InstallReferrerResponse.SERVICE_UNAVAILABLE:
              ...
              // just log but DO NOT mark
              // AppPrefs.setInstallationReferrerObtained(true);, we can obtain
              // it also from the old com.android.vending.INSTALL_REFERRER
              // intent if this is retried but the error persists
    ...
    try {
      ...
      // just bail if we were not able to connect to the installation referrer
      // service
      stopService();
    }
  }
