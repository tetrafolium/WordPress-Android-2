...
/**
 * WebViewClient that adds the ability of restrict URL loading (navigation) to a
 * list of allowed URLs. Generally used to disable links and navigation in admin
 * pages.
 */
public class URLFilteredWebViewClient extends ErrorManagedWebViewClient {
  ...
  public URLFilteredWebViewClient(Collection<String> urls,
    ...
    if (urls == null || urls.size() == 0) {
      AppLog.w(
          AppLog.T.UTILS,
          "No valid URLs passed to URLFilteredWebViewClient! HTTP Links in the"
              +
  ...
  private boolean isAllURLsAllowed() { return mAllowedURLs.size() == 0; }
  ...
  public boolean shouldOverrideUrlLoading(WebView view, String url) {
    ...
    if (isAllURLsAllowed() ||
        mAllowedURLs.contains(url)
        // If a url is allowed without the trailing `/`, it should be allowed
        // with it as well
        || mAllowedURLs.contains(StringUtils.removeTrailingSlash(url))) {
      boolean isComingFromLoginUrl =
          view.getUrl().endsWith(WP_LOGIN_URL_SUFFIX) ||
          view.getUrl().endsWith(REMOTE_LOGIN_URL_SUFFIX);
      ...
      boolean newUrlIsOnTheSameHost =
          ...
          currentUri.getHost().equals(incomingUri.getHost());

      boolean openInExternalBrowser = !isRemoteLoginUrl && !isLoginUrl &&
                                      !isComingFromLoginUrl &&
                                      !newUrlIsOnTheSameHost;
      ...
      int linksDisabledMessageResId =
          org.wordpress.android.R.string.preview_screen_links_disabled;
      Toast
          .makeText(ctx, ctx.getText(linksDisabledMessageResId),
                    Toast.LENGTH_SHORT)
          .show();
    }
    return true;
