...
public class WPPermissionUtils {
  // permission request codes - note these are reported to analytics so they
  ...
  /**
   * called by the onRequestPermissionsResult() of various activities and
   * fragments - tracks the permission results, remembers that the permissions
   * have been asked for, and optionally shows a dialog enabling the user to
   * edit permissions if any are always denied
   *
  ...
  public static boolean setPermissionListAsked(@NonNull Activity activity,
    ...
    for (int i = 0; i < permissions.length; i++) {
      ...
      if (key != null) {
        ...
        trackPermissionResult(requestCode, permissions[i], grantResults[i],
                              isFirstTime);
   ...
   */
  private static boolean isPermissionAsked(@NonNull Context context,
    ...
    // if the key exists, we've already stored whether this permission has been
    // asked for
    if (AppPrefs.keyExists(key)) {
      return AppPrefs.getBoolean(key, false);
    }

    // otherwise, check whether permission has already been granted - if so we
    // know it has been asked
  ...
  /*
   * returns true if the passed permission has been denied AND the user checked
   * "never show again" in the native permission dialog
   */
  public static boolean isPermissionAlwaysDenied(@NonNull Activity activity,
                                                 @NonNull String permission) {
    // shouldShowRequestPermissionRationale returns false if the permission has
    // been permanently denied, but it also returns false if the app has never
    // requested that permission - so we check it only if we know we've asked
    // for this permission
  ...
  private static void trackPermissionResult(int requestCode,
                                            @NonNull String permission,
                                            int result, boolean isFirstTime) {
    ...
    if (result == PackageManager.PERMISSION_GRANTED) {
      AnalyticsTracker.track(AnalyticsTracker.Stat.APP_PERMISSION_GRANTED,
                             props);
    } else if (result == PackageManager.PERMISSION_DENIED) {
      AnalyticsTracker.track(AnalyticsTracker.Stat.APP_PERMISSION_DENIED,
                             props);
    }
  }

  /*
   * key in shared preferences which stores a boolean telling whether the app
   * has already asked for the passed permission
   */
  private static AppPrefs.PrefKey
  getPermissionAskedKey(@NonNull String permission) {
    switch (permission) {
  ...
  /*
   * returns the name to display for a permission, ex:
   * "permission.WRITE_EXTERNAL_STORAGE" > "Storage"
   */
  public static String getPermissionName(@NonNull Context context,
                                         @NonNull String permission) {
    switch (permission) {
    case android.Manifest.permission.WRITE_EXTERNAL_STORAGE:
    case android.Manifest.permission.READ_EXTERNAL_STORAGE:
  ...
  /*
   * called when the app detects that the user has permanently denied a
   * permission, shows a dialog alerting them to this fact and enabling them to
   * visit the app settings to edit permissions
   */
  private static void
  showPermissionAlwaysDeniedDialog(@NonNull final Activity activity,
                                   @NonNull String permission) {
    String message =
        String.format(activity.getString(R.string.permissions_denied_message),
    ...
    AlertDialog.Builder builder =
        new AlertDialog
            ...
            .setPositiveButton(R.string.button_edit_permissions,
  ...
  /*
   * open the device's settings page for this app so the user can edit
   * permissions
   */
