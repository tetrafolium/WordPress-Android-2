...
public static void addToolbarToDialog(final android.app.Fragment context,
                                      final Dialog dialog, String title) {
	if (!context.isAdded() || dialog == null) {
		return;
	}

	View dialogContainerView =
		DialogExtensionsKt.getPreferenceDialogContainerView(dialog);

	if (dialogContainerView == null) {
		AppLog.e(T.SETTINGS,
		         "Preference Dialog View was null when adding Toolbar");
		return;
	}
	...
	Toolbar toolbar =
		(Toolbar)LayoutInflater.from(context.getActivity())
	...
	toolbar.setNavigationIcon(
		org.wordpress.android.R.drawable.ic_arrow_left_white_24dp);
...
/**
 * Checks for a {@link Toolbar} at the first child element of a given {@link
 * Dialog} and removes it if it exists. <p> Originally added to prevent a
 * crash that occurs with nested PreferenceScreens that added a toolbar via
 * {@link WPActivityUtils#addToolbarToDialog(android.app.Fragment, Dialog,
 * String)}. The crash can be reproduced by turning 'Don't keep activities' on
...
// Needs to be replaced with android.support.v4.app.Fragment
// See https://developer.android.com/reference/android/app/Fragment
public static void removeToolbarFromDialog(final android.app.Fragment context,
                                           final Dialog dialog) {
	...
	if (dialogContainerView == null) {
		AppLog.e(T.SETTINGS,
		         "Preference Dialog View was null when removing Toolbar");
		return;
	}
...
public static void setStatusBarColor(Window window, int color) {
	...
	window.setStatusBarColor(
		window.getContext().getResources().getColor(color));
}
...
public static boolean isEmailClientAvailable(Context context) {
	...
	List<ResolveInfo> emailApps = packageManager.queryIntentActivities(
