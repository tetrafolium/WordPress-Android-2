...
public class SiteUtils {
  ...
  /**
   * Migrate the old app-wide editor preference value to per-site setting. wpcom
   * sites will make a network call and store the value on the backend.
   * selfHosted sites just store the value in the local DB in FluxC <p>
   * Strategy: Check if there is the old app-wide preference still available
   * (v12.9 and before used it).
   * -- 12.9 ON -> turn all sites ON in 13.0
   * -- 12.9 OPTED OUT (were auto-opted in but turned it OFF) -> turn all sites
   * OFF in 13.0
   *
   */
  public static void
  migrateAppWideMobileEditorPreferenceToRemote(final Context context,
                                               final Dispatcher dispatcher) {
    if (!AppPrefs.isDefaultAppWideEditorPreferenceSet()) {
      return;
    }
    final boolean oldAppWidePreferenceValue =
    ...
    if (oldAppWidePreferenceValue) {
      dispatcher.dispatch(
          SiteActionBuilder.newDesignateMobileEditorForAllSitesAction(
              new DesignateMobileEditorForAllSitesPayload(
  ...
  public static boolean enableBlockEditorOnSiteCreation(Dispatcher dispatcher,
                                                        SiteStore siteStore,
                                                        int siteLocalSiteID) {
    ...
    if (newSiteModel != null) {
      ...
      AnalyticsUtils.trackWithSiteDetails(
          Stat.EDITOR_GUTENBERG_ENABLED, newSiteModel,
  ...
  public static void enableBlockEditor(Dispatcher dispatcher,
                                       SiteModel siteModel) {
  ...
  public static void disableBlockEditor(Dispatcher dispatcher,
                                        SiteModel siteModel) {
    dispatcher.dispatch(SiteActionBuilder.newDesignateMobileEditorAction(
        new DesignateMobileEditorPayload(siteModel,
                                         SiteUtils.AZTEC_EDITOR_NAME)));
  }
  ...
  public static String getSiteIconUrl(SiteModel site, int size) {
    return PhotonUtils.getPhotonImageUrl(site.getIconUrl(), size, size,
   ...
   * @return
   */
  public static boolean checkMinimalJetpackVersion(SiteModel site,
                                                   String limitVersion) {
    String jetpackVersion = site.getJetpackVersion();
    if (site.isUsingWpComRestApi() && site.isJetpackConnected() &&
        !TextUtils.isEmpty(jetpackVersion)) {
      try {
  ...
  public static boolean isNonAtomicBusinessPlanSite(@Nullable SiteModel site) {
    return site != null && !site.isAutomatedTransfer() &&
