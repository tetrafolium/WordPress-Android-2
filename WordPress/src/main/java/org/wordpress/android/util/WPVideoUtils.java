 ...
 * Audio track is encoded with Advanced Audio Coding (AAC). Not resampled.
...
public class WPVideoUtils {
  ...
  /**
   * This method return the media composer object that is in charge of video
   * optimization.
   *
   ...
   * @return The media composer that is in charge of video transcoding, ready to
   *     be started,
   * or null in case the video cannot be transcoded.
   */
  public static MediaComposer getVideoOptimizationComposer(
      @NonNull Context ctx, @NonNull String inputFile, @NonNull String outFile,
      @NonNull org.m4m.IProgressListener listener, int width, int bitrate) {
    ...
    try {
      ...
      AppLog.e(AppLog.T.MEDIA, "Cannot access the input file at " + inputFile,
               e);
      return null;
    }
    ...
    if (videoFormat.getVideoFrameSize().width() < width) {
      AppLog.w(AppLog.T.MEDIA, "Input file width is lower than than " + width +
                                   ". Keeping the original file");
      return null;
    }
    if (videoFormat.getVideoFrameSize().height() == 0) {
      AppLog.w(AppLog.T.MEDIA,
               "Input file height is unknown. Can't calculate the correct "
    ...
    try {
      ...
      AppLog.e(AppLog.T.MEDIA,
               "Cannot access/write the output file at " + outFile, e);
      return null;
    }
  ...
  private static void
  configureVideoEncoderWithDefaults(MediaComposer mediaComposer, int width,
                                    int height, int bitrate) {
    VideoFormatAndroid videoFormat =
        new VideoFormatAndroid(VIDEO_MIME_TYPE, width, height);
  ...
  private static void configureAudioEncoder(org.m4m.MediaComposer mediaComposer,
                                            AudioFormat audioFormat) {
    /**
     * TODO: Audio resampling is unsupported by current m4m release
     * Output sample rate and channel count are the same as for input.
     */
    AudioFormatAndroid aFormat = new AudioFormatAndroid(
        AUDIO_MIME_TYPE, audioFormat.getAudioSampleRateInHz(),
