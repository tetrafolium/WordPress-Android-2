...
public static void startService(Context context, String referrer) {
	...
	if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
		...
		// available since API 21, but since we are only using it elsewhere on API
		// >= 26 to not break old behavior for pre-existing services, let's stick
		// to that version as well here.
		ComponentName componentName =
			new ComponentName(context, InstallationReferrerJobService.class);
		...
		JobInfo jobInfo =
			new JobInfo.Builder(JOB_INSTALL_REFERRER_SERVICE_ID, componentName)
		...
		if (resultCode == JobScheduler.RESULT_SUCCESS) {
			AppLog.i(AppLog.T.UTILS,
			         "installation referrer job service > job scheduled");
		} else {
			AppLog.e(
				AppLog.T.UTILS,
				"installation referrer job service > job could not be scheduled");
		}
	}
}
}
