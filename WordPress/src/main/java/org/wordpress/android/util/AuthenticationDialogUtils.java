...
public class AuthenticationDialogUtils {
  public static void showAuthErrorView(AppCompatActivity activity,
                                       SiteStore siteStore, SiteModel site) {
    showAuthErrorView(activity, siteStore,
                      AuthErrorDialogFragment.DEFAULT_RESOURCE_ID,
                      AuthErrorDialogFragment.DEFAULT_RESOURCE_ID, site);
  }

  public static void showAuthErrorView(AppCompatActivity activity,
                                       SiteStore siteStore, int titleResId,
    ...
    // TODO: check isJetpackConnected - there is probably something to do here
    // for Jetpack sites connected to a wpcom account different than the main
    // account in the app. We could at least show a different message like:
    // "This configuration is not supported in the app, blablabla".
    ...
    if (site.isWPCom()) {
      if (siteStore.hasSiteAccessedViaXMLRPC()) {
        ...
        // only WPCOM sites are available so, need to ask the user to log in
        // again
