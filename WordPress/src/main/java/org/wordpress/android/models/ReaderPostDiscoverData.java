...
/**
 * additional data for "discover" posts in the reader - these are posts chosen
 * by Editorial which highlight other posts or sites - the reader shows an
 * attribution line for these posts, and when tapped they open the original post
 * - the like and comment counts come from the original post
 */
public class ReaderPostDiscoverData {
  public enum DiscoverType { EDITOR_PICK, SITE_PICK, OTHER }
  ...
  public ReaderPostDiscoverData(@NonNull JSONObject json) {
    ...
    // walk the post formats array until we find one we know we should handle
    // differently
    // - image-pick, quote-pick, and standard-pick all display as editors picks
    // - site-pick displays as a site pick
    // - collection + feature can be ignored because those display the same as
    // normal posts
    ...
    if (jsonPostFormats != null) {
      for (int i = 0; i < jsonPostFormats.length(); i++) {
        String slug =
            JSONUtils.getString(jsonPostFormats.optJSONObject(i), "slug");
        if (slug.equals("site-pick")) {
          mDiscoverType = DiscoverType.SITE_PICK;
          break;
        } else if (slug.equals("standard-pick") || slug.equals("image-pick") ||
                   slug.equals("quote-pick")) {
  ...
  public long getBlogId() { return mBlogId; }

  public long getPostId() { return mPostId; }

  private String getAuthorName() { return StringUtils.notNullStr(mAuthorName); }

  private String getAuthorUrl() { return StringUtils.notNullStr(mAuthorUrl); }
  ...
  public String getAvatarUrl() { return StringUtils.notNullStr(mAvatarUrl); }

  public String getPermaLink() { return StringUtils.notNullStr(mPermaLink); }

  public boolean hasBlogUrl() { return !TextUtils.isEmpty(mBlogUrl); }

  public boolean hasBlogName() { return !TextUtils.isEmpty(mBlogName); }

  private boolean hasAuthorName() { return !TextUtils.isEmpty(mAuthorName); }
  ...
  public DiscoverType getDiscoverType() { return mDiscoverType; }
    ...
    if (mAttributionHtml == null) {
      ...
      case EDITOR_PICK:
        if (hasBlogName() && hasAuthorName()) {
          ...
          html = String.format(
              context.getString(
                  R.string.reader_discover_attribution_author_and_blog),
              author, blog);
        } else if (hasBlogName()) {
          // "Originally posted on [BlogName]"
          html = String.format(
              context.getString(R.string.reader_discover_attribution_blog),
              blog);
        } else if (hasAuthorName()) {
          // "Originally posted by [AuthorName]"
          html = String.format(
              context.getString(R.string.reader_discover_attribution_author),
              author);
        } else {
          return null;
        }
      ...
      case SITE_PICK:
        if (mBlogId != 0 && hasBlogName()) {
          ...
          html = String.format(
              context.getString(R.string.reader_discover_visit_blog), blog);
        } else {
          return null;
        }
        break;
