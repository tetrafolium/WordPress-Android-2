...
public class ReaderBlog {
  ...
  public static ReaderBlog fromJson(JSONObject json) {
    ...
    JSONObject jsonNotification =
        JSONUtils.getJSONChild(json, "delivery_methods/notification");
    if (jsonNotification != null) {
      blog.isNotificationsEnabled =
          JSONUtils.getBool(jsonNotification, "send_posts");
    }

    // if meta/data/site exists then JSON is for a read/following/mine?meta=site
    // subscription, if meta/data/feed exists then JSON is for a
    // read/following/mine?meta=feed subscription, otherwise JSON the response
    // for a single site/$siteId or read/feed/$feedId
    ...
    if (jsonSite != null) {
      ...
      // read/following/mine doesn't include is_following for feeds, so assume
      // to be true
    ...
    // blogId will be empty for feeds, so set it to the feedId (consistent with
    // /read/ endpoints)
  ...
  public String getFeedUrl() { return StringUtils.notNullStr(mFeedUrl); }
  ...
  public boolean hasFeedUrl() { return !TextUtils.isEmpty(mFeedUrl); }

  public boolean hasImageUrl() { return !TextUtils.isEmpty(mImageUrl); }

  public boolean hasName() { return !TextUtils.isEmpty(mName); }

  public boolean hasDescription() { return !TextUtils.isEmpty(mDescription); }

  public boolean isExternal() { return (feedId != 0); }
  ...
  public String getMshotsUrl(int width) {
    return "http://s.wordpress.com/mshots/v1/" + UrlUtils.urlEncode(getUrl()) +
        String.format(Locale.US, "?w=%d", width);
  }

  public boolean isSameAs(ReaderBlog blogInfo) {
    return blogInfo != null && this.blogId == blogInfo.blogId &&
