...
public class ReaderPostList extends ArrayList<ReaderPost> {
   ...
   * We might want to use original isSameList when bookmarked flag will be
   * implemented on server side and Post model updated.
   */
  public boolean isSameListWithBookmark(ReaderPostList posts) {
    ...
    for (ReaderPost post : posts) {
      ...
      if (!post.isSamePost(postInsideList) ||
          post.isBookmarked != postInsideList.isBookmarked) {
        return false;
      }
    }
