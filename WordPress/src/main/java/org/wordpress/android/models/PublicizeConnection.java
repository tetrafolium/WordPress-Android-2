...
public class PublicizeConnection {
  ...
  // `user_id` is the ID of the user that the connection belongs to, it will be
  // `0` when `shared` is `true`. only connections belonging to the current user
  // and shared connections should be available to a user
  public int userId;
  ...
  public String getService() { return StringUtils.notNullStr(mService); }

  public void setService(String service) {
    this.mService = StringUtils.notNullStr(service);
  }

  public String getLabel() { return StringUtils.notNullStr(mLabel); }
  ...
  public String getExternalId() { return StringUtils.notNullStr(mExternalId); }

  public void setExternalId(String id) {
    this.mExternalId = StringUtils.notNullStr(id);
  }

  public String getRefreshUrl() { return StringUtils.notNullStr(mRefreshUrl); }

  public void setRefreshUrl(String url) {
    this.mRefreshUrl = StringUtils.notNullStr(url);
  }

  public boolean hasRefreshUrl() { return !TextUtils.isEmpty(mRefreshUrl); }
  ...
  public String getStatus() { return StringUtils.notNullStr(mStatus); }
  ...
  public ConnectStatus getStatusEnum() {
    if (getStatus().equalsIgnoreCase(ConnectStatus.OK.toString())) {
      return ConnectStatus.OK;
                   ...
                   ConnectStatus.MUST_DISCONNECT.toString())) {
  ...
  public boolean isSameAs(PublicizeConnection other) {
    return other != null && other.connectionId == this.connectionId &&
        ...
        other.getExternalProfilePictureUrl().equals(
            this.getExternalProfilePictureUrl()) &&
        other.getRefreshUrl().equals(this.getRefreshUrl()) &&
        other.getService().equals(this.getService());
  }

  public void setSites(long[] sites) { mSites = sites; }

  public long[] getSites() { return mSites; }
  ...
  /*
   * passed JSON is a single connection from the response to
   sites/%d/publicize-connections
     {"ID":12783250,
      "site_ID":52451176,
      "user_ID":5399133,
  ...
  public static PublicizeConnection fromJson(JSONObject json) {
    ...
    connection.keyringConnectionUserId =
        json.optInt("keyring_connection_user_ID");
    ...
    connection.mExternalProfilePictureUrl =
        json.optString("external_profile_picture");
  ...
  public static void
  updateConnectionfromExternalJson(PublicizeConnection connection,
                                   JSONObject json) {
    if (connection == null) {
      return;
    }
  ...
  private static long[] getSitesArrayFromJson(JSONArray jsonArray)
