...
public class Note {
  ...
  private Boolean isType(String type) { return getType().equals(type); }

  public Boolean isCommentType() {
    synchronized (mSyncLock) {
      return (isAutomattcherType() &&
              JSONUtils.queryJSON(mNoteJSON, "meta.ids.comment", -1) != -1) ||
          isType(NOTE_COMMENT_TYPE);
    }
  }

  public Boolean isAutomattcherType() { return isType(NOTE_MATCHER_TYPE); }

  public Boolean isNewPostType() { return isType(NOTE_NEW_POST_TYPE); }

  public Boolean isFollowType() { return isType(NOTE_FOLLOW_TYPE); }

  public Boolean isLikeType() {
    return isPostLikeType() || isCommentLikeType();
  }

  public Boolean isPostLikeType() { return isType(NOTE_LIKE_TYPE); }

  public Boolean isCommentLikeType() { return isType(NOTE_COMMENT_LIKE_TYPE); }

  public Boolean isReblogType() { return isType(NOTE_REBLOG_TYPE); }
  ...
  public boolean canModerate() {
    ...
    return enabledActions != null &&
        (enabledActions.contains(EnabledActions.ACTION_APPROVE) ||
         enabledActions.contains(EnabledActions.ACTION_UNAPPROVE));
  }

  public boolean canMarkAsSpam() {
    EnumSet<EnabledActions> enabledActions = getEnabledActions();
    return (enabledActions != null &&
            enabledActions.contains(EnabledActions.ACTION_SPAM));
  }

  public boolean canReply() {
    EnumSet<EnabledActions> enabledActions = getEnabledActions();
    return (enabledActions != null &&
            enabledActions.contains(EnabledActions.ACTION_REPLY));
  }

  public boolean canTrash() { return canModerate(); }

  public boolean canLike() {
    EnumSet<EnabledActions> enabledActions = getEnabledActions();
    return (enabledActions != null &&
            enabledActions.contains(EnabledActions.ACTION_LIKE));
  }
  ...
  public void setLocalStatus(String localStatus) { mLocalStatus = localStatus; }
  ...
  public Spannable
  getFormattedSubject(NotificationsUtilsWrapper notificationsUtilsWrapper) {
    return notificationsUtilsWrapper.getSpannableContentForRanges(getSubject());
  }

  public String getTitle() { return queryJSON("title", ""); }

  public String getIconURL() { return queryJSON("icon", ""); }
    ...
    synchronized (mSyncLock) {
      ...
      if (subjectArray != null) {
        String commentSubject =
            JSONUtils.queryJSON(subjectArray, "subject[1].text", "");
        ...
        if (commentSubject != null &&
          ...
          commentSubject =
              commentSubject.substring(0, MAX_COMMENT_PREVIEW_LENGTH - 1);
        }
    ...
    if (subjectRanges != null) {
      for (int i = 0; i < subjectRanges.length(); i++) {
        try {
          ...
          if (rangeItem.has("type") &&
              rangeItem.optString("type").equals("noticon")) {
  ...
  private Boolean isRead() { return queryJSON("read", 0) == 1; }
  ...
  public String getTimestampString() { return queryJSON("timestamp", ""); }
  ...
  public String getNoticonCharacter() { return queryJSON("noticon", ""); }
    ...
    if (mActions == null) {
      ...
      for (int i = 0; i < bodyArray.length(); i++) {
        try {
          ...
          if (bodyItem.has("type") &&
              bodyItem.optString("type").equals("comment") &&
              commentId ==
                  JSONUtils.queryJSON(bodyItem, "meta.ids.comment", 0)) {
            mActions =
                JSONUtils.queryJSON(bodyItem, "actions", new JSONObject());
            break;
          }
        } catch (JSONException e) {
          break;
  ...
  /*
   * returns the actions allowed on this note, assumes it's a comment
   * notification
   */
  public EnumSet<EnabledActions> getEnabledActions() {
    ...
    if (jsonActions.has(ACTION_KEY_APPROVE) &&
        jsonActions.optBoolean(ACTION_KEY_APPROVE, false)) {
  ...
  public int getSiteId() { return queryJSON("meta.ids.site", 0); }

  public int getPostId() { return queryJSON("meta.ids.post", 0); }

  public long getCommentId() { return queryJSON("meta.ids.comment", 0); }
  ...
  private String getCommentText() { return queryJSON("body[last].text", ""); }
  ...
  public boolean hasLikedComment() {
    JSONObject jsonActions = getCommentActions();
    return !(jsonActions == null || jsonActions.length() == 0) &&
        jsonActions.optBoolean(ACTION_KEY_LIKE);
  }

  public String getUrl() { return queryJSON("url", ""); }

  public JSONArray getHeader() {
    synchronized (mSyncLock) { return mNoteJSON.optJSONArray("header"); }
  }

  // this method is used to compare two Notes: as it's potentially a very
  // processing intensive operation, we're only comparing the note id,
  // timestamp, and raw JSON length, which is accurate enough for the purpose of
  ...
  public static synchronized Note
  buildFromBase64EncodedData(String noteId, String base64FullNoteData) {
    Note note = null;
    ...
    byte[] b64DecodedPayload =
        Base64.decode(base64FullNoteData, Base64.DEFAULT);
    ...
    try {
      ...
      AppLog.e(AppLog.T.NOTIFS,
               "Can't decompress the PN BlockListPayload. It could be > 4K", e);
    }
    ...
    try {
      ...
      AppLog.e(AppLog.T.NOTIFS,
               "Notification data contains non UTF8 characters.", e);
    }
    ...
    if (out != null) {
      try {
        ...
        AppLog.e(AppLog.T.NOTIFS,
                 "Can't parse the Note JSON received in the PN", e);
      }
    }
