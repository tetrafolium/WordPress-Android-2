...
public class ReaderBlogList extends ArrayList<ReaderBlog> {
  ...
  public static ReaderBlogList fromJson(JSONObject json) {
    ...
    if (jsonBlogs != null) {
      for (int i = 0; i < jsonBlogs.length(); i++) {
        ...
        // make sure blog is valid before adding to the list - this can happen
        // if user added a URL that isn't a feed or a blog since as of
        // 29-May-2014 the API will let you follow any URL regardless if it's
        // valid
  ...
  /*
   * returns true if the passed blog list has the same blogs that are in this
   * list - differs from isSameList() in that isSameList() checks for *any*
   * changes (subscription count, etc.) whereas this only checks if the passed
   * list has any blogs that are not in this list, or this list has any blogs
