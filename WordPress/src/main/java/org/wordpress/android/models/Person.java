...
public class Person {
  ...
  @Nullable
  public static Person userFromJSON(JSONObject json, int localTableBlogId)
    ...
    try {
      ...
      person.setDisplayName(
          StringEscapeUtils.unescapeHtml4(json.optString("name")));
      person.setAvatarUrl(json.optString("avatar_URL"));
      person.mPersonType = PersonType.USER;
      // We don't support multiple roles, so the first role is picked just as
      // it's in Calypso
      ...
      AppLog.e(AppLog.T.PEOPLE,
               "The ID parsed from the JSON couldn't be converted to long: " +
                   e);
    }
  ...
  @Nullable
  public static Person followerFromJSON(JSONObject json, int localTableBlogId,
    ...
    // Response parameters are in:
    // https://developer.wordpress.com/docs/api/1.1/get/sites/%24site/stats/followers/
    try {
      long personID = Long.parseLong(json.getString("ID"));
      Person person = new Person(personID, localTableBlogId);
      person.setDisplayName(
          StringEscapeUtils.unescapeHtml4(json.optString("label")));
      person.setUsername(json.optString("login"));
      person.setAvatarUrl(json.optString("avatar"));
      person.setSubscribed(json.optString("date_subscribed"));
      person.mPersonType =
          isEmailFollower ? PersonType.EMAIL_FOLLOWER : PersonType.FOLLOWER;
  ...
  @Nullable
  public static Person viewerFromJSON(JSONObject json, int localTableBlogId)
      throws JSONException {
    if (json == null) {
      return null;
    }
  ...
  public int getLocalTableBlogId() { return mLocalTableBlogId; }

  public String getUsername() { return StringUtils.notNullStr(mUsername); }
  ...
  public String getSubscribed() { return StringUtils.notNullStr(mSubscribed); }
  ...
  public PersonType getPersonType() { return mPersonType; }

  public void setPersonType(PersonType personType) { mPersonType = personType; }
}
