...
public class RoleUtils {
  public static String getDisplayName(String userRole,
                                      List<RoleModel> siteUserRoles) {
  ...
  public static List<RoleModel>
  getInviteRoles(SiteStore siteStore, SiteModel siteModel, Fragment fragment) {
    // Setup invite roles
    List<RoleModel> inviteRoles = siteStore.getUserRoles(siteModel);
    // The API doesn't return the follower/viewer role, so we need to manually
    // add it for invites
    RoleModel viewerOrFollowerRole = new RoleModel();
    // the remote expects "follower" as the role parameter even if the role is
    // "viewer"
    viewerOrFollowerRole.setName("follower");
    int displayNameRes =
        siteModel.isPrivate() ? R.string.role_viewer : R.string.role_follower;
