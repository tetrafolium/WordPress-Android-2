...
public class ReaderTag implements Serializable, FilterCriteria {
  ...
  public static final String DISCOVER_PATH = String.format(
      Locale.US, "read/sites/%d/posts", ReaderConstants.DISCOVER_SITE_ID);
  ...
  private String
      mTagDisplayName;      // tag for display, usually the same as the slug
  ...
  public ReaderTag(String slug, String displayName, String title,
                   String endpoint, ReaderTagType tagType) {
    // we need a slug since it's used to uniquely ID the tag (including setting
    // it as the primary key in the tag table)
  ...
  public String getEndpoint() { return StringUtils.notNullStr(mEndpoint); }

  private void setEndpoint(String endpoint) {
    this.mEndpoint = StringUtils.notNullStr(endpoint);
  }

  public String getTagTitle() { return StringUtils.notNullStr(mTagTitle); }

  private void setTagTitle(String title) {
    this.mTagTitle = StringUtils.notNullStr(title);
  }

  private boolean hasTagTitle() { return !TextUtils.isEmpty(mTagTitle); }
  ...
  /*
   * returns the tag name for use in the application log - if this is a default
   * tag it returns the full tag name, otherwise it abbreviates the tag name
   * since exposing followed tags in the log could be considered a privacy issue
   */
  ...
  public static boolean isSameTag(ReaderTag tag1, ReaderTag tag2) {
    return tag1 != null && tag2 != null && tag1.tagType == tag2.tagType &&
        tag1.getTagSlug().equalsIgnoreCase(tag2.getTagSlug());
  }

  public boolean isPostsILike() {
    return tagType == ReaderTagType.DEFAULT &&
        getEndpoint().endsWith("/read/liked");
  }

  public boolean isFollowedSites() {
    return tagType == ReaderTagType.DEFAULT &&
        getEndpoint().endsWith(FOLLOWING_PATH);
  }

  public boolean isBookmarked() { return tagType == ReaderTagType.BOOKMARKED; }

  public boolean isDiscover() {
    return tagType == ReaderTagType.DEFAULT &&
        getEndpoint().endsWith(DISCOVER_PATH);
  }
  ...
  /*
   * returns true if the tag display name contains only alpha-numeric characters
   * or hyphens
   */
  ...
  public boolean equals(Object object) {
    if (object instanceof ReaderTag) {
      ...
      return (tag.tagType == this.tagType &&
              tag.getLabel().equals(this.getLabel()));
    } else {
      return false;
    }
  }
