...
public class ReaderPost {
  ...
  private String
      mPrimaryTag; // most popular tag on this post based on usage in blog
  private String mSecondaryTag; // second most popular tag on this post based on
                                // usage in blog
  ...
  public static ReaderPost fromJson(JSONObject json) {
    ...
    if (json.has("pseudo_ID")) {
      ...
      post.mPseudoId =
          JSONUtils.getString(json, "global_ID"); // sites/ endpoint
    }

    // remove HTML from the excerpt
    post.mExcerpt =
        HtmlUtils.fastStripHtml(JSONUtils.getString(json, "excerpt")).trim();
    ...
    if (post.hasTitle() && post.mTitle.contains("<") &&
        post.mTitle.contains(">")) {
    ...
    if (jsonSite != null) {
      ...
      // TODO: as of 29-Sept-2014, this is broken - endpoint returns false when
      // it should be true
    ...
    // if there's no featured image, check if featured media has been set to an
    // image
    ...
    // if the post doesn't have a featured image but it contains an IMG tag,
    // check whether we can find a suitable image from the content
    if (!post.hasFeaturedImage() && post.hasImages()) {
      post.mFeaturedImage =
          new ReaderImageScanner(post.mText, post.isPrivate)
              .getLargestImage(ReaderConstants.MIN_FEATURED_IMAGE_WIDTH);
    }

    // if there's no featured image or featured video and the post contains an
    // iframe, scan the content for a suitable featured video
    if (!post.hasFeaturedImage() && !post.hasFeaturedVideo() &&
        post.getText().contains("<iframe")) {
      post.setFeaturedVideo(
          new ReaderIframeScanner(post.getText()).getFirstUsableVideo());
    }
    ...
    // set the card type last since it depends on information contained in the
    // post - note that this is stored in the post table rather than calculated
    // on-the-fly
  ...
  public boolean hasImages() { return hasText() && mText.contains("<img "); }
   ...
   * ],
   */
  private static void assignXpostIdsFromJson(ReaderPost post,
                                             JSONArray jsonMetadata) {
  ...
  /*
   * assigns author-related info to the passed post from the passed JSON
   * "author" object
   */
  private static void assignAuthorFromJson(ReaderPost post,
    ...
    post.mAuthorFirstName =
        JSONUtils.getStringDecoded(jsonAuthor, "first_name");
    post.authorId = jsonAuthor.optLong("ID");

    // v1.2 endpoint contains a "has_avatar" boolean which tells us whether the
    // author has a valid avatar - if this field exists and is set to false,
    // skip setting the avatar URL
    boolean hasAvatar;
  ...
  /*
   * assigns primary/secondary tags to the passed post from the passed JSON
   * "tags" object
   */
  private static void assignTagsFromJson(ReaderPost post, JSONObject jsonTags) {
    ...
    // most popular tag & second most popular tag, based on usage count on this
    // blog
    ...
    while (it.hasNext()) {
      ...
      String thisTagName =
          UrlUtils.urlDecode(JSONUtils.getString(jsonThisTag, "slug"));

      // if the number of posts on this blog that use this tag is higher than
      // previous, set this as the most popular tag, and set the second most
      // popular tag to the current most popular tag
  ...
  public String getText() { return StringUtils.notNullStr(mText); }

  public void setText(String text) {
    this.mText = StringUtils.notNullStr(text);
  }

  public String getExcerpt() { return StringUtils.notNullStr(mExcerpt); }
  ...
  public String getFormat() { return StringUtils.notNullStr(mFormat); }
  ...
  public String getUrl() { return StringUtils.notNullStr(mUrl); }

  public void setUrl(String url) { this.mUrl = StringUtils.notNullStr(url); }

  public String getShortUrl() { return StringUtils.notNullStr(mShortUrl); }

  public void setShortUrl(String shortUrl) {
    this.mShortUrl = StringUtils.notNullStr(shortUrl);
  }

  public boolean hasShortUrl() { return !TextUtils.isEmpty(mShortUrl); }
  ...
  public String getBlogUrl() { return StringUtils.notNullStr(mBlogUrl); }
  ...
  public String getPostAvatar() { return StringUtils.notNullStr(mPostAvatar); }

  public void setPostAvatar(String postAvatar) {
    this.mPostAvatar = StringUtils.notNullStr(postAvatar);
  }

  public String getPseudoId() { return StringUtils.notNullStr(mPseudoId); }
  ...
  public String getDateTagged() { return StringUtils.notNullStr(mDateTagged); }

  public void setDateTagged(String dateStr) {
    this.mDateTagged = StringUtils.notNullStr(dateStr);
  }

  public String getPrimaryTag() { return StringUtils.notNullStr(mPrimaryTag); }

  public void setPrimaryTag(String tagName) {
    this.mPrimaryTag = StringUtils.notNullStr(tagName);
  }

  public boolean hasPrimaryTag() { return !TextUtils.isEmpty(mPrimaryTag); }
  ...
  public boolean hasSecondaryTag() { return !TextUtils.isEmpty(mSecondaryTag); }

  /*
   * attachments are stored as the actual JSON to avoid having a separate table
   * for them, may need to revisit this if/when attachments become more
   * important
   */
  ...
  public boolean isDiscoverPost() { return !TextUtils.isEmpty(mDiscoverJson); }
  ...
  public ReaderPostDiscoverData getDiscoverData() {
    if (mDiscoverData == null && !TextUtils.isEmpty(mDiscoverJson)) {
      try {
        mDiscoverData =
            new ReaderPostDiscoverData(new JSONObject(mDiscoverJson));
  ...
  public boolean hasText() { return !TextUtils.isEmpty(mText); }

  public boolean hasUrl() { return !TextUtils.isEmpty(mUrl); }

  public boolean hasExcerpt() { return !TextUtils.isEmpty(mExcerpt); }
  ...
  public boolean hasPostAvatar() { return !TextUtils.isEmpty(mPostAvatar); }
  ...
  public boolean hasTitle() { return !TextUtils.isEmpty(mTitle); }

  public boolean hasBlogUrl() { return !TextUtils.isEmpty(mBlogUrl); }

  public boolean hasBlogImageUrl() { return !TextUtils.isEmpty(mBlogImageUrl); }

  /*
   * we should show the excerpt rather than full content for Jetpack posts that
   * specify to show only the excerpt
   */
   ...
   */
  public boolean isWP() { return !isExternal; }

  /*
   * returns true if this is a cross-post
   */
  public boolean isXpost() { return xpostBlogId != 0 && xpostPostId != 0; }

  /*
   * returns true if the passed post appears to be the same as this one - used
   * when posts are retrieved to determine which ones are new/changed/unchanged
   */
  public boolean isSamePost(ReaderPost post) {
    return post != null && post.blogId == this.blogId &&
        post.postId == this.postId && post.feedId == this.feedId &&
        post.feedItemId == this.feedItemId && post.numLikes == this.numLikes &&
  ...
  public boolean hasIds(ReaderBlogIdPostId ids) {
    return ids != null && ids.getBlogId() == this.blogId &&
        ids.getPostId() == this.postId;
  }

  /*
   * liking is enabled for all wp.com and jp posts with the exception of
   * discover posts
   */
  ...
  public boolean hasRailcar() { return !TextUtils.isEmpty(mRailcarJson); }
  ...
  /****
   * the following are transient variables - not stored in the db or returned in
   *the json - whose sole purpose is to cache commonly-used values for the post
   *that speeds up using them inside adapters
   ****/

  /*
   * returns the featured image url as a photon url set to the passed
   * width/height
   */
    ...
    if (mFeaturedImageForDisplay == null) {
      if (!hasFeaturedImage()) {
        ...
        mFeaturedImageForDisplay = ReaderUtils.getResizedImageUrl(
            mFeaturedImage, width, height, isPrivate);
      }
    }
  ...
  /*
   * converts iso8601 pubDate to a java date for display - this is the date that
   * appears on posts
   */
  ...
  /*
   * used when a unique numeric id is required by an adapter (when
   * hasStableIds() = true)
   */
