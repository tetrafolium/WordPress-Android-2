...
// Maps to notification settings returned from the /me/notifications/settings
// endpoint on wp.com
public class NotificationsSettings {
  ...
  // The main notification settings channels (displayed at root of
  // NoticationsSettingsFragment)
  public enum Channel { OTHER, BLOGS, WPCOM }
  ...
  public NotificationsSettings(JSONObject json) { updateJson(json); }

  // Parses the json response from /me/notifications/settings endpoint and
  ...
  public void updateJson(JSONObject json) {
    ...
    JSONArray siteSettingsArray =
        JSONUtils.queryJSON(json, KEY_BLOGS, new JSONArray());
    for (int i = 0; i < siteSettingsArray.length(); i++) {
      try {
        ...
        AppLog.e(AppLog.T.NOTIFS,
                 "Could not parse blog JSON in notification settings");
      }
    }
  }

  // Updates a specific notification setting after a user makes a change
  public void updateSettingForChannelAndType(Channel channel, Type type,
                                             String settingName,
                                             boolean newValue, long blogId) {
    ...
    try {
      ...
      case BLOGS:
        ...
        if (blogJson != null) {
          JSONObject blogSetting =
              JSONUtils.queryJSON(blogJson, typeName, new JSONObject());
        ...
        break;
      case OTHER:
        JSONObject otherSetting =
            JSONUtils.queryJSON(getOtherSettings(), typeName, new JSONObject());
  ...
  public JSONObject getOtherSettings() { return mOtherSettings; }

  public LongSparseArray<JSONObject> getBlogSettings() { return mBlogSettings; }

  public JSONObject getWPComSettings() { return mWPComSettings; }
}
