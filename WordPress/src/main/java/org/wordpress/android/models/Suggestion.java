...
public class Suggestion {
  ...
  public Suggestion(long siteID, String userLogin, String displayName,
                    String imageUrl, String taxonomy) {
    this.siteID = siteID;
  ...
  public static Suggestion fromJSON(JSONObject json, long siteID) {
    ...
    // the api currently doesn't return a taxonomy field but we want to be ready
    // for when it does
    return new Suggestion(siteID, userLogin, displayName, imageUrl,
                          MENTION_TAXONOMY);
  }
  ...
  public static List<Suggestion> suggestionListFromJSON(JSONArray jsonArray,
    ...
    ArrayList<Suggestion> suggestions =
        new ArrayList<Suggestion>(jsonArray.length());

    for (int i = 0; i < jsonArray.length(); i++) {
      Suggestion suggestion =
          Suggestion.fromJSON(jsonArray.optJSONObject(i), siteID);
  ...
  public String getUserLogin() { return StringUtils.notNullStr(mUserLogin); }
  ...
  public String getTaxonomy() { return StringUtils.notNullStr(mTaxonomy); }
}
