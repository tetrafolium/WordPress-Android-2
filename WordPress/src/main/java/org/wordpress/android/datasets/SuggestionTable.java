...
public class SuggestionTable {
  ...
  public static void
  insertSuggestionsForSite(final long siteId,
                           final List<Suggestion> suggestions) {
    // we want to delete the current suggestions, so that removed users will not
  ...
  public static void addSuggestion(final Suggestion suggestion) {
    ...
    getWritableDb().insertWithOnConflict(SUGGESTIONS_TABLE, null, values,
  ...
  public static List<Suggestion> getSuggestionsForSite(long siteId) {
    ...
    Cursor c =
        getReadableDb().rawQuery("SELECT * FROM " + SUGGESTIONS_TABLE +
                                     " WHERE site_id=? ORDER BY user_login ASC",
                                 args);
  ...
  public static int deleteSuggestionsForSite(long siteId) {
    return getWritableDb().delete(SUGGESTIONS_TABLE, "site_id=?",
  ...
  private static Suggestion getSuggestionFromCursor(Cursor c) {
    ...
    return new Suggestion(siteId, userLogin, displayName, imageUrl, taxonomy);
  }

  public static void insertTagsForSite(final long siteId,
                                       final List<Tag> tags) {
  ...
  public static void addTag(final Tag tag) {
    ...
    getWritableDb().insertWithOnConflict(TAXONOMY_TABLE, null, values,
                                         SQLiteDatabase.CONFLICT_REPLACE);
  }
  ...
  public static List<Tag> getTagsForSite(long siteId) {
    ...
    String[] args = {Long.toString(siteId)};
    Cursor c = getReadableDb().rawQuery("SELECT * FROM " + TAXONOMY_TABLE +
                                            " WHERE site_id=? ORDER BY tag ASC",
                                        args);
  ...
  public static int deleteTagsForSite(long siteId) {
    return getWritableDb().delete(TAXONOMY_TABLE, "site_id=?",
                                  new String[] {Long.toString(siteId)});
  }
