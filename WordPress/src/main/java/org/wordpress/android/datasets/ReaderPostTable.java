...
/**
 * tbl_posts contains all reader posts - the primary key is pseudo_id + tag_name
 * + tag_type, which allows the same post to appear in multiple streams (ex: it
 * can exist in followed sites, liked posts, and tag streams). note that posts
 * in a specific blog or feed are stored here with an empty tag_name.
 */
public class ReaderPostTable {
  private static final String COLUMN_NAMES = "post_id,"             // 1
                                             + "blog_id,"           // 2
                                             + "feed_id,"           // 3
                                             + "feed_item_id,"      // 4
                                             + "pseudo_id,"         // 5
  ...
  private static final String COLUMN_NAMES_NO_TEXT = "post_id,"             // 1
                                                     + "blog_id,"           // 2
                                                     + "feed_id,"           // 3
                                                     + "feed_item_id,"      // 4
                                                     + "author_id,"         // 5
  ...
  protected static void createTables(SQLiteDatabase db) {
    ...
    db.execSQL(
        "CREATE INDEX idx_posts_post_id_blog_id ON tbl_posts(post_id, blog_id)");
    db.execSQL(
        "CREATE INDEX idx_posts_date_published ON tbl_posts(date_published)");
  ...
  /*
   * purge table of unattached/older posts - no need to wrap this in a
   * transaction since it's only called from ReaderDatabase.purge() which
   * already creates a transaction
   */
  protected static int purge(SQLiteDatabase db) {
    // delete posts attached to tags that no longer exist
    int numDeleted = db.delete(
        "tbl_posts", "tag_name NOT IN (SELECT DISTINCT tag_name FROM tbl_tags)",
        null);
  ...
  /**
   * When the user unbookmarks a post, we keep the row in the database, but we
   * just change the is_bookmarked flag to false, so we can show "undo" items in
   * the saved posts list. This method purges database from such rows.
   */
  public static int purgeUnbookmarkedPostsWithBookmarkTag() {
    ...
    for (ReaderTag tag : tags) {
      if (tag.isBookmarked()) {
        ...
        String[] args = {tag.getTagSlug(),
                         Integer.toString(tag.tagType.toInt())};
        numDeleted += ReaderDatabase.getWritableDb().delete(
            "tbl_posts", "tag_name=? AND tag_type=? AND is_bookmarked=0", args);
      }
    }
    return numDeleted;
  }
   ...
   */
  private static final int MAX_POSTS_PER_TAG =
  ...
  private static int purgePostsForTag(SQLiteDatabase db, ReaderTag tag) {
    ...
    String[] args = {tagName, tagType, tagName, tagType,
                     Integer.toString(MAX_POSTS_PER_TAG)};
    String where =
        "tag_name=? AND tag_type=? AND pseudo_id NOT IN (SELECT DISTINCT pseudo_id FROM tbl_posts WHERE "
        + "tag_name=? AND tag_type=? ORDER BY " + getSortColumnForTag(tag) +
        " DESC LIMIT ?)";
    int numDeleted = db.delete("tbl_posts", where, args);
    AppLog.d(AppLog.T.READER,
             String.format(Locale.ENGLISH,
                           "reader post table > purged %d posts in tag %s",
                           numDeleted, tag.getTagNameForLog()));
    return numDeleted;
  }
  ...
  public static int getNumPostsWithTag(ReaderTag tag) {
    ...
    return SqlUtils.intForQuery(
        ReaderDatabase.getReadableDb(),
        "SELECT count(*) FROM tbl_posts WHERE tag_name=? AND tag_type=?", args);
  }

  public static void updatePost(@NonNull ReaderPost post) {
    // we need to update a few important fields across all instances of this
    // post - this is necessary because a post can exist multiple times in the
    ...
    ReaderDatabase.getWritableDb().update("tbl_posts", values, "pseudo_id=?",
                                          new String[] {post.getPseudoId()});
  ...
  public static ReaderPost getBlogPost(long blogId, long postId,
                                       boolean excludeTextColumn) {
    return getPost("blog_id=? AND post_id=?",
                   new String[] {Long.toString(blogId), Long.toString(postId)},
                   excludeTextColumn);
  }

  public static ReaderPost getBlogPost(String blogSlug, String postSlug,
                                       boolean excludeTextColumn) {
    return getPost("blog_url LIKE ? AND url LIKE ?",
                   new String[] {"%//" + blogSlug, "%/" + postSlug + "/"},
                   excludeTextColumn);
  }

  public static ReaderPost getFeedPost(long feedId, long feedItemId,
                                       boolean excludeTextColumn) {
    return getPost(
        "feed_id=? AND feed_item_id=?",
        new String[] {Long.toString(feedId), Long.toString(feedItemId)},
        excludeTextColumn);
  }

  private static ReaderPost getPost(String where, String[] args,
                                    boolean excludeTextColumn) {
    String columns = (excludeTextColumn ? COLUMN_NAMES_NO_TEXT : "*");
    String sql =
        "SELECT " + columns + " FROM tbl_posts WHERE " + where + " LIMIT 1";
  ...
  public static String getPostTitle(long blogId, long postId) {
    ...
    return SqlUtils.stringForQuery(
        ReaderDatabase.getReadableDb(),
        "SELECT title FROM tbl_posts WHERE blog_id=? AND post_id=?", args);
  }
  ...
  public static String getPostBlogName(long blogId, long postId) {
    ...
    return SqlUtils.stringForQuery(
        ReaderDatabase.getReadableDb(),
        "SELECT blog_name FROM tbl_posts WHERE blog_id=? AND post_id=?", args);
  }
  ...
  public static String getPostText(long blogId, long postId) {
    ...
    return SqlUtils.stringForQuery(
        ReaderDatabase.getReadableDb(),
        "SELECT text FROM tbl_posts WHERE blog_id=? AND post_id=?", args);
  }

  public static boolean postExists(long blogId, long postId) {
    String[] args = {Long.toString(blogId), Long.toString(postId)};
    return SqlUtils.boolForQuery(
        ReaderDatabase.getReadableDb(),
        "SELECT 1 FROM tbl_posts WHERE blog_id=? AND post_id=?", args);
  }

  private static boolean postExistsForReaderTag(long blogId, long postId,
                                                ReaderTag readerTag) {
    String[] args = {Long.toString(blogId), Long.toString(postId),
  ...
  /*
   * returns whether any of the passed posts are new or changed - used after
   * posts are retrieved
   */
  public static ReaderActions.UpdateResult comparePosts(ReaderPostList posts) {
    ...
    return (hasChanges ? ReaderActions.UpdateResult.CHANGED
                       : ReaderActions.UpdateResult.UNCHANGED);
  }

  /*
   * returns true if any posts in the passed list exist in this list for the
   * given tag
   */
  ...
  /*
   * returns the #comments known to exist for this post (ie: #comments the
   * server says this post has), which may differ from
   * ReaderCommentTable.getNumCommentsForPost (which returns # local comments
   * for this post)
   */
  ...
  public static void setNumCommentsForPost(long blogId, long postId,
                                           int numComments) {
    ContentValues values = new ContentValues();
  ...
  /*
   * returns the #likes known to exist for this post (ie: #likes the server says
   * this post has), which may differ from ReaderPostTable.getNumLikesForPost
   * (which returns # local likes for this post)
   */
  public static int getNumLikesForPost(long blogId, long postId) {
    String[] args = {Long.toString(blogId), Long.toString(postId)};
    return SqlUtils.intForQuery(
        ReaderDatabase.getReadableDb(),
        "SELECT num_likes FROM tbl_posts WHERE blog_id=? AND post_id=?", args);
  }
  ...
  public static boolean isPostLikedByCurrentUser(long blogId, long postId) {
    ...
    return SqlUtils.boolForQuery(
        ReaderDatabase.getReadableDb(),
        "SELECT is_liked FROM tbl_posts WHERE blog_id=? AND post_id=?", args);
  }

  /*
   * updates both the like count for a post and whether it's liked by the
   * current user
   */
  public static void setLikesForPost(ReaderPost post, int numLikes,
  ...
  public static void setBookmarkFlag(long blogId, long postId,
                                     boolean bookmark) {
    ContentValues values = new ContentValues();
  ...
  private static void update(long blogId, long postId, ContentValues values) {
    String[] args = {Long.toString(blogId), Long.toString(postId)};
    ReaderDatabase.getWritableDb().update("tbl_posts", values,
                                          "blog_id=? AND post_id=?", args);
  }
  ...
  public static boolean isPostFollowed(ReaderPost post) {
    ...
    String[] args =
        new String[] {Long.toString(post.blogId), Long.toString(post.postId)};
    return SqlUtils.boolForQuery(
        ReaderDatabase.getReadableDb(),
  ...
  public static int deletePostsWithTag(final ReaderTag tag) {
    ...
    return ReaderDatabase.getWritableDb().delete(
        "tbl_posts", "tag_name=? AND tag_type=?", args);
  }

  public static int removeTagsFromPost(long blogId, long postId,
                                       final ReaderTagType tagType) {
    if (tagType == null) {
      return 0;
    }

    String[] args = {Integer.toString(tagType.toInt()), Long.toString(blogId),
                     Long.toString(postId)};
    return ReaderDatabase.getWritableDb().delete(
        "tbl_posts", "tag_type=? AND blog_id=? AND post_id=?", args);
  }

  public static int deletePostsInBlog(long blogId) {
    String[] args = {Long.toString(blogId)};
    return ReaderDatabase.getWritableDb().delete("tbl_posts", "blog_id = ?",
                                                 args);
  }
  ...
  /*
   * ensure that posts in blogs that are no longer followed don't have their
   * followed status set to true
   */
  public static void updateFollowedStatus() {
    ...
    try {
      ...
      if (count > 0) {
        AppLog.d(AppLog.T.READER,
                 String.format(Locale.ENGLISH,
                               "reader post table > marked %d posts unfollowed",
                               count));
      }
    } finally {
      statement.close();
  ...
  public static String getOldestPubDateInBlog(long blogId) {
    ...
    return SqlUtils.stringForQuery(ReaderDatabase.getReadableDb(), sql,
                                   new String[] {Long.toString(blogId)});
  }
  ...
  public static void removeGapMarkerForTag(final ReaderTag tag) {
    ...
    String[] args = {tag.getTagSlug(), Integer.toString(tag.tagType.toInt())};
    String sql =
        "UPDATE tbl_posts SET has_gap_marker=0 WHERE has_gap_marker!=0 AND tag_name=? AND tag_type=?";
    ReaderDatabase.getWritableDb().execSQL(sql, args);
  }

  /*
   * returns the blogId/postId of the post with the passed tag that has a gap
   * marker, or null if none exists
   */
  public static ReaderBlogIdPostId getGapMarkerIdsForTag(final ReaderTag tag) {
    ...
    String[] args = {tag.getTagSlug(), Integer.toString(tag.tagType.toInt())};
    String sql =
        "SELECT blog_id, post_id FROM tbl_posts WHERE has_gap_marker!=0 AND tag_name=? AND tag_type=?";
  ...
  public static void setGapMarkerForTag(long blogId, long postId,
                                        ReaderTag tag) {
    if (tag == null) {
      return;
    }
    ...
    String[] args = {Long.toString(blogId), Long.toString(postId),
                     tag.getTagSlug(), Integer.toString(tag.tagType.toInt())};
    String sql =
  ...
  public static String getGapMarkerDateForTag(ReaderTag tag) {
    ...
    String dateColumn = getSortColumnForTag(tag);
    String[] args = {Long.toString(ids.getBlogId()),
                     Long.toString(ids.getPostId())};
    String sql = "SELECT " + dateColumn +
                 " FROM tbl_posts WHERE blog_id=? AND post_id=?";
    return SqlUtils.stringForQuery(ReaderDatabase.getReadableDb(), sql, args);
  }

  /*
   * the column posts are sorted by depends on the type of tag stream being
   * displayed:
   *
  ...
  /*
   * delete posts with the passed tag that come before the one with the gap
   * marker for this tag - note this may leave some stray posts in tbl_posts,
   * but these will be cleaned up by the next purge
   */
  public static void deletePostsBeforeGapMarkerForTag(ReaderTag tag) {
    ...
    int numDeleted =
        ReaderDatabase.getWritableDb().delete("tbl_posts", where, args);
    if (numDeleted > 0) {
      AppLog.d(AppLog.T.READER,
               "removed " + numDeleted + " posts older than gap marker");
    }
  }

  public static void setFollowStatusForPostsInBlog(long blogId,
                                                   boolean isFollowed) {
    setFollowStatusForPosts(blogId, 0, isFollowed);
  }

  public static void setFollowStatusForPostsInFeed(long feedId,
                                                   boolean isFollowed) {
    setFollowStatusForPosts(0, feedId, isFollowed);
  }

  private static void setFollowStatusForPosts(long blogId, long feedId,
                                              boolean isFollowed) {
    ...
    try {
      if (blogId != 0) {
        String sql = "UPDATE tbl_posts SET is_followed=" +
                     SqlUtils.boolToSql(isFollowed) + " WHERE blog_id=?";
        db.execSQL(sql, new String[] {Long.toString(blogId)});
      } else {
        String sql = "UPDATE tbl_posts SET is_followed=" +
                     SqlUtils.boolToSql(isFollowed) + " WHERE feed_id=?";
        db.execSQL(sql, new String[] {Long.toString(feedId)});
      }

      // if blog/feed is no longer followed, remove its posts tagged with
      // "Followed Sites"
      if (!isFollowed) {
        if (blogId != 0) {
          db.delete("tbl_posts", "blog_id=? AND tag_name=?",
                    new String[] {Long.toString(blogId),
                                  ReaderTag.TAG_TITLE_FOLLOWED_SITES});
        } else {
  ...
  private static String maxText(final ReaderPost post) {
    ...
    // if the post has an excerpt (which should always be the case), store it as
    // the full text with a link to the full article
    if (post.hasExcerpt()) {
      AppLog.w(AppLog.T.READER,
               "reader post table > max text exceeded, storing excerpt");
      return "<p>" + post.getExcerpt() + "</p>" +
          String.format("<p style='text-align:center'><a href='%s'>%s</a></p>",
                        post.getUrl(),
                        WordPress.getContext().getString(
                            R.string.reader_label_view_original));
    } else {
      AppLog.w(AppLog.T.READER,
               "reader post table > max text exceeded, storing truncated text");
      return post.getText().substring(0, MAX_TEXT_LEN);
    }
  }

  public static void addOrUpdatePosts(final ReaderTag tag,
                                      ReaderPostList posts) {
    if (posts == null || posts.size() == 0) {
      return;
    }
    ...
    SQLiteStatement stmtPosts = db.compileStatement(
        "INSERT OR REPLACE INTO tbl_posts (" + COLUMN_NAMES +
    ...
    try {
      ...
      for (ReaderPost post : posts) {
        ...
        boolean hasGapMarker = postWithGapMarker != null &&
                               postWithGapMarker.getPostId() == post.postId &&
        ...
        stmtPosts.bindLong(28,
                           SqlUtils.boolToSql(post.isFollowedByCurrentUser));
  ...
  public static ReaderPostList getPostsWithTag(ReaderTag tag, int maxPosts,
    ...
    String columns = (excludeTextColumn ? COLUMN_NAMES_NO_TEXT : "*");
    String sql =
        "SELECT " + columns + " FROM tbl_posts WHERE tag_name=? AND tag_type=?";

    if (tag.tagType == ReaderTagType.DEFAULT) {
      // skip posts that are no longer liked if this is "Posts I Like", skip
      // posts that are no longer followed if this is "Followed Sites"
      if (tag.isPostsILike()) {
        sql += " AND is_liked != 0";
      } else if (tag.isFollowedSites()) {
  ...
  public static ReaderPostList getPostsInBlog(long blogId, int maxPosts,
    ...
    String sql =
        "SELECT " + columns +
        " FROM tbl_posts WHERE blog_id=? AND tag_name='' ORDER BY date_published DESC";

    if (maxPosts > 0) {
      sql += " LIMIT " + Integer.toString(maxPosts);
    }

    Cursor cursor = ReaderDatabase.getReadableDb().rawQuery(
        sql, new String[] {Long.toString(blogId)});
    try {
      return getPostListFromCursor(cursor);
    } finally {
      SqlUtils.closeCursor(cursor);
  ...
  public static ReaderPostList getPostsInFeed(long feedId, int maxPosts,
    ...
    String sql =
        "SELECT " + columns +
        " FROM tbl_posts WHERE feed_id=? AND tag_name='' ORDER BY date_published DESC";
    ...
    Cursor cursor = ReaderDatabase.getReadableDb().rawQuery(
        sql, new String[] {Long.toString(feedId)});
    try {
      return getPostListFromCursor(cursor);
    } finally {
      SqlUtils.closeCursor(cursor);
   ...
   */
  public static ReaderBlogIdPostIdList getBlogIdPostIdsWithTag(ReaderTag tag,
                                                               int maxPosts) {
    if (tag == null) {
    ...
    String sql =
        "SELECT blog_id, post_id FROM tbl_posts WHERE tag_name=? AND tag_type=?";
  ...
  private static ReaderBlogIdPostIdList
  getBlogIdPostIdsWithTagType(ReaderTagType tagType, int maxPosts) {
    if (tagType == null) {
      return new ReaderBlogIdPostIdList();
    }
  ...
  private static ReaderBlogIdPostIdList
  getBlogIdPostIds(@NonNull String sql, @NonNull String[] args) {
    ...
    try {
      if (cursor != null && cursor.moveToFirst()) {
        do {
          idList.add(
              new ReaderBlogIdPostId(cursor.getLong(0), cursor.getLong(1)));
   ...
   */
  public static ReaderBlogIdPostIdList getBlogIdPostIdsInBlog(long blogId,
                                                              int maxPosts) {
    String sql =
        "SELECT post_id FROM tbl_posts WHERE blog_id=? AND tag_name='' ORDER BY date_published DESC";
  ...
  private static ReaderPost getPostFromCursor(Cursor c) {
    ...
    post.isLikedByCurrentUser =
        SqlUtils.sqlToBool(c.getInt(c.getColumnIndex("is_liked")));
    post.isFollowedByCurrentUser =
        SqlUtils.sqlToBool(c.getInt(c.getColumnIndex("is_followed")));
    post.isCommentsOpen =
        SqlUtils.sqlToBool(c.getInt(c.getColumnIndex("is_comments_open")));
    post.isExternal =
        SqlUtils.sqlToBool(c.getInt(c.getColumnIndex("is_external")));
    post.isPrivate =
        SqlUtils.sqlToBool(c.getInt(c.getColumnIndex("is_private")));
    post.isVideoPress =
        SqlUtils.sqlToBool(c.getInt(c.getColumnIndex("is_videopress")));
    post.isJetpack =
        SqlUtils.sqlToBool(c.getInt(c.getColumnIndex("is_jetpack")));
    post.isBookmarked =
        SqlUtils.sqlToBool(c.getInt(c.getColumnIndex("is_bookmarked")));
    ...
    post.setCardType(
        ReaderCardType.fromString(c.getString(c.getColumnIndex("card_type"))));

    post.useExcerpt =
        SqlUtils.sqlToBool(c.getInt(c.getColumnIndex("use_excerpt")));
  ...
  /**
   * Currently "is_bookmarked" field is not supported by the server, therefore
   * posts from the server have always is_bookmarked set to false. This method
   * is a workaround which makes sure, that the field is always up to date and
   * synced across all instances(rows) of each post.
   */
  private static void updateIsBookmarkedField(final ReaderPostList posts) {
    ...
    for (ReaderPost post : posts) {
      for (ReaderBlogIdPostId bookmarkedPostId : bookmarkedPosts) {
        if (post.blogId == bookmarkedPostId.getBlogId() &&
            post.postId == bookmarkedPostId.getPostId()) {
