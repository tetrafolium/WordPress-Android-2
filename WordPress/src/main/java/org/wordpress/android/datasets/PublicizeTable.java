...
public static PublicizeService getService(String serviceId) {
	...
	Cursor c = getReadableDb().rawQuery(
		"SELECT * FROM " + SERVICES_TABLE + " WHERE id=?", args);
	try {
		if (c.moveToFirst()) {
...
public static PublicizeServiceList getServiceList() {
	...
	Cursor c = getReadableDb().rawQuery(
		"SELECT * FROM " + SERVICES_TABLE + " ORDER BY label", null);
	try {
		while (c.moveToNext()) {
...
public static void setServiceList(final PublicizeServiceList serviceList) {
	...
	try {
		...
		stmt =
			db.compileStatement("INSERT INTO " + SERVICES_TABLE + " (id," // 1
			                    + " label,"                 // 2
			                    + " description,"           // 3
			                    + " genericon,"             // 4
			                    + " icon_url,"              // 5
		...
		for (PublicizeService service : serviceList) {
			...
			stmt.bindLong(
				8, SqlUtils.boolToSql(service.isMultiExternalUserIdSupported()));
...
private static PublicizeService getServiceFromCursor(Cursor c) {
	...
	service.setIsJetpackSupported(
		getBooleanFromCursor(c, "is_jetpack_supported"));
	service.setIsMultiExternalUserIdSupported(
		getBooleanFromCursor(c, "is_multi_user_id_supported"));
	service.setIsExternalUsersOnly(
		getBooleanFromCursor(c, "is_external_users_only"));
...
public static boolean onlyExternalConnections(String serviceId) {
	...
	String sql =
		"SELECT is_external_users_only FROM " + SERVICES_TABLE + " WHERE id=?";
	String[] args = {serviceId};
...
public static PublicizeConnection getConnection(int connectionId) {
	...
	Cursor c = getReadableDb().rawQuery(
		"SELECT * FROM " + CONNECTIONS_TABLE + " WHERE id=?", args);
	try {
		if (c.moveToFirst()) {
...
public static void addOrUpdateConnection(PublicizeConnection connection) {
	...
	values.put("keyring_connection_user_id",
	           connection.keyringConnectionUserId);
	...
	values.put("external_profile_picture",
	...
	getReadableDb().insertWithOnConflict(CONNECTIONS_TABLE, null, values,
...
public static PublicizeConnectionList getConnectionsForSite(long siteId) {
	...
	Cursor c = getReadableDb().rawQuery(
		"SELECT * FROM " + CONNECTIONS_TABLE + " WHERE site_id=?", args);
	try {
		while (c.moveToNext()) {
...
public static void
setConnectionsForSite(long siteId, PublicizeConnectionList connectionList) {
	SQLiteStatement stmt = null;
	SQLiteDatabase db = getWritableDb();
	db.beginTransaction();
	try {
		db.delete(CONNECTIONS_TABLE, "site_id=?",
		          new String[] {Long.toString(siteId)});

		stmt = db.compileStatement(
			"INSERT INTO " + CONNECTIONS_TABLE + " (id," // 1
			+ " site_id,"                  // 2
			+ " user_id,"                  // 3
...
private static PublicizeConnection getConnectionFromCursor(Cursor c) {
	...
	connection.keyringConnectionId =
		c.getInt(c.getColumnIndex("keyring_connection_id"));
	connection.keyringConnectionUserId =
		c.getInt(c.getColumnIndex("keyring_connection_user_id"));

	connection.isShared =
		SqlUtils.sqlToBool(c.getInt(c.getColumnIndex("is_shared")));
	...
	connection.setExternalDisplayName(
		c.getString(c.getColumnIndex("external_display")));
	connection.setExternalProfilePictureUrl(
		c.getString(c.getColumnIndex("external_profile_picture")));
