...
public final class SiteSettingsTable {
  ...
  public static Cursor getCategory(long id) {
    ...
    String sqlCommand =
        sqlSelectAllCategories() +
        sqlWhere(CategoryModel.ID_COLUMN_NAME, Long.toString(id)) + ";";
  ...
  public static Cursor getSettings(long id) {
    ...
    String whereClause =
        sqlWhere(SiteSettingsModel.ID_COLUMN_NAME, Long.toString(id));
  ...
  public static void saveCategory(CategoryModel category) {
    ...
    category.isInLocalTable = WordPress.wpDB.getDatabase().insertWithOnConflict(
                                  CATEGORIES_TABLE_NAME, null, values,
  ...
  public static void saveSettings(SiteSettingsModel settings) {
    ...
    settings.isInLocalTable =
        WordPress.wpDB.getDatabase().insertWithOnConflict(
            SiteSettingsModel.SETTINGS_TABLE_NAME, null, values,
            SQLiteDatabase.CONFLICT_REPLACE) != -1;
  ...
  public static boolean migrateMediaOptimizeSettings(SQLiteDatabase db) {
    ...
    try {
      String sqlCommand =
          "SELECT * FROM " + SiteSettingsModel.SETTINGS_TABLE_NAME + ";";
      ...
      // we're safe to read all the settings now since all the columns must be
      // there
