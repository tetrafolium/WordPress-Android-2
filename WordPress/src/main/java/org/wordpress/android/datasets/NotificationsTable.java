...
public class NotificationsTable {
  ...
  private static SQLiteDatabase getDb() { return WordPress.wpDB.getDatabase(); }
  ...
  public static ArrayList<Note> getLatestNotes(int limit) {
    Cursor cursor = getDb().query(
        NOTIFICATIONS_TABLE, new String[] {"note_id", "raw_note_data"}, null,
        null, null, null, "timestamp DESC", "" + limit);
    ...
    while (cursor.moveToNext()) {
      ...
      try {
        ...
        AppLog.e(AppLog.T.DB, "Can't parse notification with noteId:" + noteId +
                                  ", exception:" + e);
      }
    }
    cursor.close();
  ...
  private static boolean putNote(Note note, boolean checkBeforeInsert) {
    ...
    if (checkBeforeInsert && isNoteAvailable(note.getId())) {
      ...
      result = getDb().insertWithOnConflict(NOTIFICATIONS_TABLE, null, values,
                                            SQLiteDatabase.CONFLICT_REPLACE);
      if (result == -1) {
        AppLog.e(
            AppLog.T.DB,
            "An error occurred while saving the note into the DB - note_id:" +
                note.getId());
      }
      return result != -1;
    }
  ...
  public static void saveNotes(List<Note> notes, boolean clearBeforeSaving) {
    ...
    try {
      ...
      for (Note note : notes) {
        // No need to check if the row already exists if we've just dropped the
        // table.
  ...
  private static boolean isNoteAvailable(String noteID) {
    ...
    return SqlUtils.boolForQuery(
        getDb(), "SELECT 1 FROM " + NOTIFICATIONS_TABLE + " WHERE note_id=?1",
        args);
  }
  ...
  public static Note getNoteById(String noteID) {
    ...
    Cursor cursor =
        getDb().query(NOTIFICATIONS_TABLE, new String[] {"raw_note_data"},
                      "note_id=" + noteID, null, null, null, null);
    try {
      if (cursor.moveToFirst()) {
        JSONObject jsonNote = new JSONObject(cursor.getString(0));
        return new Note(noteID, jsonNote);
      } else {
        AppLog.v(AppLog.T.DB,
                 "No Note found in the DB with this id: " + noteID);
        return null;
      }
    } catch (JSONException e) {
  ...
  public static boolean deleteNoteById(String noteID) {
    if (TextUtils.isEmpty(noteID)) {
      AppLog.e(AppLog.T.DB,
               "Asking to delete a note with null Id. Really?" + noteID);
      return false;
    }
    getDb().beginTransaction();
    try {
