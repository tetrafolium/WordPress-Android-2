...
protected static int purge(SQLiteDatabase db) {
	...
	numDeleted += db.delete(
		"tbl_comment_likes",
		"comment_id NOT IN (SELECT DISTINCT comment_id FROM tbl_comments)",
		null);
	return numDeleted;
}
...
public static ReaderUserIdList getLikesForPost(ReaderPost post) {
	...
	Cursor c = ReaderDatabase.getReadableDb().rawQuery(
		"SELECT user_id FROM tbl_post_likes WHERE blog_id=? AND post_id=?",
		args);
	try {
		if (c.moveToFirst()) {
			do {
...
public static int getNumLikesForPost(ReaderPost post) {
	...
	return SqlUtils.intForQuery(
		...
		"SELECT count(*) FROM tbl_post_likes WHERE blog_id=? AND post_id=?",
		args);
}
...
public static void setCurrentUserLikesPost(ReaderPost post, boolean isLiked,
	...
	if (isLiked) {
		...
		ReaderDatabase.getWritableDb().delete(
			"tbl_post_likes", "blog_id=? AND post_id=? AND user_id=?", args);
	}
}

public static void setLikesForPost(ReaderPost post,
                                   ReaderUserIdList userIds) {
	if (post == null) {
		return;
	}
	...
	SQLiteStatement stmt = db.compileStatement(
		"INSERT INTO tbl_post_likes (blog_id, post_id, user_id) VALUES (?1,?2,?3)");
	try {
...
public static int getNumLikesForComment(ReaderComment comment) {
	...
	return SqlUtils.intForQuery(
		ReaderDatabase.getReadableDb(),
		"SELECT count(*) FROM tbl_comment_likes WHERE blog_id=? AND comment_id=?",
		args);
}

public static void setCurrentUserLikesComment(ReaderComment comment,
                                              boolean isLiked,
                                              long wpComUserId) {
	if (comment == null) {
		return;
	...
	if (isLiked) {
		...
		String[] args = {Long.toString(comment.blogId),
			         ...
			         Long.toString(wpComUserId)};
		ReaderDatabase.getWritableDb().delete(
			"tbl_comment_likes", "blog_id=? AND comment_id=? AND user_id=?",
			args);
	}
}

public static void setLikesForComment(ReaderComment comment,
                                      ReaderUserIdList userIds) {
	if (comment == null) {
		return;
	}
