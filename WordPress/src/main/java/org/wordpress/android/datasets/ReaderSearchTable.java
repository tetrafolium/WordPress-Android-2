...
protected static void createTables(SQLiteDatabase db) {
	db.execSQL("CREATE TABLE tbl_search_suggestions ("
	           ...
	           + " date_used TEXT)");
	db.execSQL(
		"CREATE UNIQUE INDEX idx_search_suggestions_query ON tbl_search_suggestions(query_string)");
}
...
public static void deleteQueryString(@NonNull String query) {
	...
	ReaderDatabase.getWritableDb().delete("tbl_search_suggestions",
	                                      "query_string=?", args);
}

public static void deleteAllQueries() {
	SqlUtils.deleteAllRowsInTable(ReaderDatabase.getWritableDb(),
	                              "tbl_search_suggestions");
}
