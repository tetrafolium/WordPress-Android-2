...
/**
 * tbl_blog_info contains information about blogs viewed in the reader, and
 * blogs the user is following. Note that this table is populated from two
 * endpoints: <p>
 * 1. sites/{$siteId}
 * 2. read/following/mine?meta=site,feed
 * <p>
 * The first endpoint is called when the user views blog preview, the second is
 * called to get the full list of blogs the user is following
 */
...
public static ReaderBlog getBlogInfo(long blogId) {
	...
	Cursor cursor = ReaderDatabase.getReadableDb().rawQuery(
		"SELECT * FROM tbl_blog_info WHERE blog_id=?", args);
	try {
...
public static ReaderBlog getFeedInfo(long feedId) {
	...
	Cursor cursor = ReaderDatabase.getReadableDb().rawQuery(
		"SELECT * FROM tbl_blog_info WHERE feed_id=?", args);
	try {
		if (!cursor.moveToFirst()) {
			return null;
		}
...
public static long getFeedIdFromUrl(String url) {
	...
	return SqlUtils.longForQuery(
		ReaderDatabase.getReadableDb(),
		"SELECT feed_id FROM tbl_blog_info WHERE feed_url=?", args);
}
...
private static ReaderBlog getBlogInfoFromCursor(Cursor c) {
	...
	blogInfo.isFollowing =
		SqlUtils.sqlToBool(c.getInt(c.getColumnIndex("is_following")));
	blogInfo.isNotificationsEnabled = SqlUtils.sqlToBool(
		c.getInt(c.getColumnIndex("is_notifications_enabled")));
...
public static void addOrUpdateBlog(ReaderBlog blogInfo) {
	...
	String sql =
		"INSERT OR REPLACE INTO tbl_blog_info"
		+
...
public static ReaderUrlList getFollowedBlogUrls() {
	Cursor c = ReaderDatabase.getReadableDb().rawQuery(
		"SELECT DISTINCT blog_url FROM tbl_blog_info WHERE is_following!=0",
		null);
	try {
...
/*
 * sets the follow state for passed blog without creating a record for it if
 * it doesn't exist
 */
...
public static boolean isFollowedBlogUrl(String blogUrl) {
	...
	String sql =
		"SELECT 1 FROM tbl_blog_info WHERE is_following!=0 AND blog_url=?";
...
public static boolean isFollowedBlog(long blogId) {
	String sql =
		"SELECT 1 FROM tbl_blog_info WHERE is_following!=0 AND blog_id=?";
	String[] args = {Long.toString(blogId)};
	return SqlUtils.boolForQuery(ReaderDatabase.getReadableDb(), sql, args);
}
...
public static boolean isFollowedFeedUrl(String feedUrl) {
	...
	String sql =
		"SELECT 1 FROM tbl_blog_info WHERE is_following!=0 AND feed_url=?";
...
public static boolean isFollowedFeed(long feedId) {
	String sql =
		"SELECT 1 FROM tbl_blog_info WHERE is_following!=0 AND feed_id=?";
	String[] args = {Long.toString(feedId)};
	return SqlUtils.boolForQuery(ReaderDatabase.getReadableDb(), sql, args);
}
...
public static boolean isNotificationsEnabled(long blogId) {
	String sql =
		"SELECT 1 FROM tbl_blog_info WHERE is_notifications_enabled!=0 AND blog_id=?";
	String[] args = {Long.toString(blogId)};
	return SqlUtils.boolForQuery(ReaderDatabase.getReadableDb(), sql, args);
}

public static void setNotificationsEnabledByBlogId(long blogId,
                                                   boolean isEnabled) {
	ReaderDatabase.getWritableDb().execSQL(
		"UPDATE tbl_blog_info SET is_notifications_enabled=" +
		SqlUtils.boolToSql(isEnabled) + " WHERE blog_id=?",
		new String[] {Long.toString(blogId)});
}
...
public static String getBlogName(long blogId) {
	...
	return SqlUtils.stringForQuery(
		ReaderDatabase.getReadableDb(),
		"SELECT name FROM tbl_blog_info WHERE blog_id=?", args);
}
...
public static String getBlogUrl(long blogId) {
	...
	return SqlUtils.stringForQuery(
		ReaderDatabase.getReadableDb(),
		"SELECT blog_url FROM tbl_blog_info WHERE blog_id=?", args);
}
...
public static String getFeedName(long feedId) {
	...
	return SqlUtils.stringForQuery(
		ReaderDatabase.getReadableDb(),
		"SELECT name FROM tbl_blog_info WHERE feed_id=?", args);
}
...
/*
 * determine whether the passed blog info should be updated based on when it
 * was last updated
 */
...
private static String getBlogInfoLastUpdated(ReaderBlog blogInfo) {
	...
	if (blogInfo.blogId != 0) {
		...
		return SqlUtils.stringForQuery(
			ReaderDatabase.getReadableDb(),
			"SELECT date_updated FROM tbl_blog_info WHERE blog_id=?", args);
	} else {
		String[] args = {Long.toString(blogInfo.feedId)};
		return SqlUtils.stringForQuery(
			ReaderDatabase.getReadableDb(),
			"SELECT date_updated FROM tbl_blog_info WHERE feed_id=?", args);
	}
}
