...
private static final String COLUMN_NAMES = " user_id,"          // 1
                                           + " blog_id,"        // 2
                                           + " user_name,"      // 3
                                           + " display_name,"   // 4
                                           + " url,"            // 5
...
public static void addOrUpdateUsers(ReaderUserList users) {
	...
	SQLiteStatement stmt =
		db.compileStatement("INSERT OR REPLACE INTO tbl_users (" +
		                    COLUMN_NAMES + ") VALUES (?1,?2,?3,?4,?5,?6,?7)");
	try {
...
/*
 * returns avatar urls for the passed user ids - used by post detail to show
 * avatars for liking users
 */
public static ArrayList<String> getAvatarUrls(ReaderUserIdList userIds,
                                              int max, int avatarSz,
                                              long wpComUserId) {
	...
	StringBuilder sb = new StringBuilder(
		"SELECT user_id, avatar_url FROM tbl_users WHERE user_id IN (");

	// make sure current user's avatar is returned if the passed list contains
	// them - this is important since it may not otherwise be returned when a
	// "max" is passed, and we want the current user to appear first in post
	// detail when they like a post
...
private static ReaderUser getUser(long userId) {
	...
	Cursor c = ReaderDatabase.getReadableDb().rawQuery(
		"SELECT * FROM tbl_users WHERE user_id=?", args);
	try {
		if (!c.moveToFirst()) {
			return null;
		}
...
private static String getAvatarForUser(long userId) {
	...
	return SqlUtils.stringForQuery(
		...
		"SELECT avatar_url FROM tbl_users WHERE user_id=?", args);
}

public static ReaderUserList getUsersWhoLikePost(long blogId, long postId,
                                                 int max) {
...
public static ReaderUserList getUsersWhoLikeComment(long blogId,
                                                    long commentId, int max) {
