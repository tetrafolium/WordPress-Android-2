...
public class ReaderCommentTable {
  private static final String COLUMN_NAMES = " blog_id,"
                                             + " post_id,"
                                             + " comment_id,"
                                             + " parent_id,"
                                             + " author_name,"
  ...
  public static boolean isEmpty() { return (getNumComments() == 0); }

  private static int getNumComments() {
    long count =
        SqlUtils.getRowCount(ReaderDatabase.getReadableDb(), "tbl_comments");
    return (int)count;
  }
  ...
  public static int getLastPageNumberForPost(long blogId, long postId) {
    ...
    return SqlUtils.intForQuery(
        ...
        "SELECT MAX(page_number) FROM tbl_comments WHERE blog_id=? AND post_id=?",
        args);
  }
   ...
   */
  public static int getPageNumberForComment(long blogId, long postId,
                                            long commentId) {
    String[] args = {Long.toString(blogId), Long.toString(postId),
  ...
  public static void purgeCommentsForPost(long blogId, long postId) {
    String[] args = {Long.toString(blogId), Long.toString(postId)};
    ReaderDatabase.getWritableDb().delete("tbl_comments",
                                          "blog_id=? AND post_id=?", args);
  }

  /*
   * returns the #comments stored locally for this post, which may differ from
   * ReaderPostTable.getNumCommentsOnPost (which is the #comments the server
   * says exist for this post)
   */
  ...
  private static int getNumCommentsForPost(long blogId, long postId) {
    ...
    return SqlUtils.intForQuery(
        ReaderDatabase.getReadableDb(),
        "SELECT count(*) FROM tbl_comments WHERE blog_id=? AND post_id=?",
        args);
  }
  ...
  public static ReaderCommentList getCommentsForPost(ReaderPost post) {
    ...
    Cursor c = ReaderDatabase.getReadableDb().rawQuery(
        "SELECT * FROM tbl_comments WHERE blog_id=? AND post_id=? ORDER BY timestamp",
        args);
    try {
  ...
  public static void addOrUpdateComments(ReaderCommentList comments) {
    ...
    SQLiteStatement stmt = db.compileStatement(
        "INSERT OR REPLACE INTO tbl_comments (" + COLUMN_NAMES + ") "
  ...
  public static ReaderComment getComment(long blogId, long postId,
                                         long commentId) {
    String[] args = new String[] {Long.toString(blogId), Long.toString(postId),
                                  Long.toString(commentId)};
    Cursor c = ReaderDatabase.getReadableDb().rawQuery(
        "SELECT * FROM tbl_comments WHERE blog_id=? AND post_id=? AND comment_id=? LIMIT 1",
        args);
    try {
      if (!c.moveToFirst()) {
        return null;
  ...
  public static void deleteComment(ReaderPost post, long commentId) {
    ...
    String[] args = {Long.toString(post.blogId), Long.toString(post.postId),
                     Long.toString(commentId)};
    ReaderDatabase.getWritableDb().delete(
        "tbl_comments", "blog_id=? AND post_id=? AND comment_id=?", args);
  }
  ...
  public static boolean hasNewComments(ReaderCommentList comments) {
    ...
    int numExisting = SqlUtils.intForQuery(ReaderDatabase.getReadableDb(),
                                           sb.toString(), args);
   ...
   */
  public static int getNumLikesForComment(long blogId, long postId,
                                          long commentId) {
    String[] args = {Long.toString(blogId), Long.toString(postId),
                     Long.toString(commentId)};
    return SqlUtils.intForQuery(
  ...
  /*
   * updates both the like count for a comment and whether it's liked by the
   * current user
   */
  public static void setLikesForComment(ReaderComment comment, int numLikes,
    ...
    ReaderDatabase.getWritableDb().update(
        "tbl_comments", values, "blog_id=? AND post_id=? AND comment_id=?",
        args);
  }
  ...
  public static boolean isCommentLikedByCurrentUser(ReaderComment comment) {
    ...
    return isCommentLikedByCurrentUser(comment.blogId, comment.postId,
                                       comment.commentId);
  }

  public static boolean isCommentLikedByCurrentUser(long blogId, long postId,
                                                    long commentId) {
    String[] args = {Long.toString(blogId), Long.toString(postId),
                     Long.toString(commentId)};
  ...
  public static boolean commentExists(long blogId, long postId,
                                      long commentId) {
    String[] args = {Long.toString(blogId), Long.toString(postId),
                     Long.toString(commentId)};
    ...
    return SqlUtils.boolForQuery(
        ReaderDatabase.getReadableDb(),
        "SELECT 1 FROM tbl_comments WHERE blog_id=? AND post_id=? AND comment_id=?",
        args);
  }
