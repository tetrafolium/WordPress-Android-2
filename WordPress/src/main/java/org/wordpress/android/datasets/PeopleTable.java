...
private static void save(String table, Person person,
...
public static void saveUsers(List<Person> peopleList, int localTableBlogId,
                             boolean isFreshList) {
	savePeople(TEAM_TABLE, peopleList, localTableBlogId, isFreshList);
}

public static void saveFollowers(List<Person> peopleList,
                                 int localTableBlogId, boolean isFreshList) {
	savePeople(FOLLOWERS_TABLE, peopleList, localTableBlogId, isFreshList);
}

public static void saveEmailFollowers(List<Person> peopleList,
                                      int localTableBlogId,
                                      boolean isFreshList) {
	savePeople(EMAIL_FOLLOWERS_TABLE, peopleList, localTableBlogId,
	           isFreshList);
}

public static void saveViewers(List<Person> peopleList, int localTableBlogId,
                               boolean isFreshList) {
	savePeople(VIEWERS_TABLE, peopleList, localTableBlogId, isFreshList);
}

private static void savePeople(String table, List<Person> peopleList,
                               int localTableBlogId, boolean isFreshList) {
	getWritableDb().beginTransaction();
	try {
		// We have a fresh list, remove the previous list of people in case it was
		// deleted on remote
		if (isFreshList) {
...
/**
 * In order to avoid syncing issues, this method will be called when People
 * page is created. We only keep the first page of users, so we don't show an
 * empty screen. When fresh data is received, it'll replace the existing page.
 *
	...
	try {
		for (String table : tables) {
			...
			if (size > fetchLimit) {
				...
				String inQuery = SQLiteQueryBuilder.buildQueryString(
					false, table, columns, where, null, null, orderBy, limit);

				String[] args = new String[] {Integer.toString(localTableBlogId)};
				getWritableDb().delete(
					table, "local_blog_id=?1 AND person_id IN (" + inQuery + ")",
					args);
			}
		}
		getWritableDb().setTransactionSuccessful();
...
private static int getPeopleCountForLocalBlogId(String table,
                                                int localTableBlogId) {
	String[] args = new String[] {Integer.toString(localTableBlogId)};
...
private static void deletePerson(String table, long personID,
                                 int localTableBlogId) {
	String[] args = new String[] {Long.toString(personID),
...
private static List<Person> getPeople(String table, int localTableBlogId) {
	...
	Cursor c = getReadableDb().rawQuery(
		"SELECT * FROM " + table + " WHERE local_blog_id=?" + orderBy, args);
 ...
 */
private static Person getPerson(String table, long personId,
                                int localTableBlogId) {
	String[] args = {Long.toString(personId),
		         Integer.toString(localTableBlogId)};
	Cursor c = getReadableDb().rawQuery(
		"SELECT * FROM " + table + " WHERE person_id=? AND local_blog_id=?",
		args);
	try {
		if (!c.moveToFirst()) {
			return null;
...
private static Person getPersonFromCursor(Cursor c, String table,
                                          int localTableBlogId) {
...
// order is disabled for followers & viewers for now since the API is not
// supporting it
