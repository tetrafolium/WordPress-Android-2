...
public static boolean isEmpty() {
	return (SqlUtils.getRowCount(ReaderDatabase.getReadableDb(), "tbl_tags") ==
	        0);
}
...
public static boolean tagExists(ReaderTag tag) {
	...
	return SqlUtils.boolForQuery(
		...
		"SELECT 1 FROM tbl_tags WHERE tag_slug=?1 AND tag_type=?2", args);
}

/*
 * returns true if the passed tag exists and it has the passed type
 */
private static boolean tagExistsOfType(String tagSlug,
...
public static ReaderTag getTag(String tagSlug, ReaderTagType tagType) {
	...
	Cursor c = ReaderDatabase.getReadableDb().rawQuery(
		"SELECT * FROM tbl_tags WHERE tag_slug=? AND tag_type=? LIMIT 1", args);
	try {
		if (!c.moveToFirst()) {
			return null;
		}
...
public static String getEndpointForTag(ReaderTag tag) {
	...
	return SqlUtils.stringForQuery(
		ReaderDatabase.getReadableDb(),
		"SELECT endpoint FROM tbl_tags WHERE tag_slug=? AND tag_type=?", args);
}
...
private static ReaderTagList getTagsOfType(ReaderTagType tagType) {
	...
	Cursor c = ReaderDatabase.getReadableDb().rawQuery(
		"SELECT * FROM tbl_tags WHERE tag_type=? ORDER BY tag_slug", args);
	try {
...
static ReaderTagList getAllTags() {
	Cursor c = ReaderDatabase.getReadableDb().rawQuery(
		"SELECT * FROM tbl_tags ORDER BY tag_slug", null);
	try {
		ReaderTagList tagList = new ReaderTagList();
		if (c.moveToFirst()) {
			do {
...
public static ReaderTag getFirstTag() {
	Cursor c = ReaderDatabase.getReadableDb().rawQuery(
		"SELECT * FROM tbl_tags ORDER BY tag_slug LIMIT 1", null);
	try {
		if (c.moveToFirst()) {
			return getTagFromCursor(c);
		}
...
public static void deleteTag(ReaderTag tag) {
	...
	String[] args = {tag.getTagSlug(), Integer.toString(tag.tagType.toInt())};
	ReaderDatabase.getWritableDb().delete("tbl_tags",
	                                      "tag_slug=? AND tag_type=?", args);
}
...
public static void setTagLastUpdated(ReaderTag tag) {
	...
	String sql =
		"UPDATE tbl_tags SET date_updated=?1 WHERE tag_slug=?2 AND tag_type=?3";
...
/*
 * determine whether the passed tag should be auto-updated based on when it
 * was last updated
 */
...
/**
 * recommended tags - stored in a separate table from default/subscribed tags,
 * but have the same column names
 **/
public static ReaderTagList getRecommendedTags(boolean excludeSubscribed) {
	...
	if (excludeSubscribed) {
		...
		c = ReaderDatabase.getReadableDb().rawQuery(
			"SELECT * FROM tbl_tags_recommended ORDER BY tag_slug", null);
	}
	try {
		ReaderTagList tagList = new ReaderTagList();
		if (c.moveToFirst()) {
...
public static void setRecommendedTags(ReaderTagList tagList) {
	...
	SQLiteStatement stmt = db.compileStatement(
		"INSERT INTO tbl_tags_recommended (tag_slug, tag_display_name, "
