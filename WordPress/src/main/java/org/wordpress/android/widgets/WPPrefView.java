...
/**
 * Single preference view which enables building preference screens without
 * relying on Android's built-in preference tools - enables us to be in the
 * charge of the UI and customizing it as we wish without fighting
 * PreferenceFragment, etc.
 */
...
public class WPPrefView extends LinearLayout
  ...
  public static class PrefListItem implements Serializable {
    ...
    private final String
        mItemValue;             // value for this item (can be same as name)
    private boolean mIsChecked; // whether this item is checked

    public PrefListItem(@NonNull String itemName, @NonNull String itemValue,
                        boolean isChecked) {
      mItemName = itemName;
    ...
    public @NonNull String getItemValue() { return mItemValue; }
  }
  ...
  /*
   * Wrapper that will allow us to preserve type of PrefListItems when
   * serializing it
   */
  public static class PrefListItemsWrapper implements Serializable {
    private PrefListItems mList;

    public PrefListItems getList() { return mList; }

    PrefListItemsWrapper(PrefListItems mList) { this.mList = mList; }
  }
  ...
  private void initView(Context context, AttributeSet attrs) {
    ...
    if (attrs != null) {
      TypedArray a = context.getTheme().obtainStyledAttributes(
          attrs, R.styleable.wpPrefView, 0, 0);
      try {
        ...
        String dialogSubtitle =
            a.getString(R.styleable.wpPrefView_wpTextDialogSubtitle);
        boolean showDivider =
            a.getBoolean(R.styleable.wpPrefView_wpShowDivider, true);
  ...
  public Parcelable onSaveInstanceState() {
    ...
    bundle.putSerializable(KEY_LIST_ITEMS,
                           new PrefListItemsWrapper(mListItems));
  ...
  public void onRestoreInstanceState(Parcelable state) {
    if (state instanceof Bundle) {
      ...
      PrefListItemsWrapper listWrapper =
          (PrefListItemsWrapper)bundle.getSerializable(KEY_LIST_ITEMS);
      if (listWrapper != null) {
  ...
  /*
   * blue heading text that should appear above the preference when it's the
   * first in a group
   */
   ...
   */
  public String getTextEntry() { return mTextEntry; }
  ...
  public void setChecked(boolean checked) { mToggleSwitch.setChecked(checked); }

  /*
   * the list of data when the PrefType is CHECKLIST or RADIOLIST
   */
  public @NonNull PrefListItems getListItems() { return mListItems; }
  ...
  /*
   * user clicked the view when the PrefType is TEXT - shows a dialog enabling
  ...
  private Dialog getTextDialog() {
    ...
    AlertDialog.Builder builder =
        new AlertDialog
            .Builder(new ContextThemeWrapper(getContext(),
  ...
  /*
   * user clicked the view when the PrefType is CHECKLIST - shows a multi-select
   * dialog enabling the user to modify the list
   */
  ...
  /*
   * user clicked the view when the PrefType is RADIOLIST - shows a
   * single-select dialog enabling the user to choose a different item
   */
  ...
  public static class WPPrefDialogFragment extends DialogFragment {
    ...
    public static WPPrefDialogFragment
    newInstance(@NonNull WPPrefView prefView) {
    ...
    private @Nullable WPPrefView getPrefView() {
      if (getActivity() != null) {
