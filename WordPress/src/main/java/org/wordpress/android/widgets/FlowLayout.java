/*
 * Sample FlowLayout wrote by Romain Guy:
 * http://www.parleys.com/play/514892280364bc17fc56c0e2/chapter38/about Fixed
 * and tweaked since
 */
...
protected void onLayout(boolean changed, int l, int t, int r, int b) {
	...
	for (int i = 0; i < count; i++) {
		...
		if (ViewCompat.getLayoutDirection(this) ==
			...
			child.layout(child.getMeasuredWidth() - lp.x - child.getMeasuredWidth(),
			             lp.y, r - lp.x, lp.y + child.getMeasuredHeight());
		} else {
...
public LayoutParams(Context context, AttributeSet attrs) {
	...
	try {
		horizontalSpacing = a.getDimensionPixelSize(
			R.styleable.FlowLayout_Layout_layout_horizontalSpacing, -1);
	} finally {
		a.recycle();
	}
}
