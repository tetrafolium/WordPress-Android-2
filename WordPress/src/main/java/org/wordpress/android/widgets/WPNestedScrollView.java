...
/**
 * Code copied from android.support.v4.widget.NestedScrollView with one
 * modification in the AccessibilityDelegate .performAccessibilityAction(..)
 * method. There is a bug in the Support Library which breaks scroll in the
 * Accessibility Switch Control mode.
 * Steps to reproduce the bug
 * 1. Go to Stats
 * 2. Scroll to the bottom using the Switch Control
 * 3. Try to scroll to the top -> clicking on the scrollView doesn't do
 * anything. <p> WARNING - Do not modify this file, so we can remove it, when
 * the bug in the support library is fixed.
...
public class WPNestedScrollView extends NestedScrollView {
  ...
  public WPNestedScrollView(Context context) { this(context, null); }
