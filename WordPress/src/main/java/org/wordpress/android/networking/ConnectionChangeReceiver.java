...
/*
 * global network connection change receiver - declared in the manifest to
 * monitor android.net.conn.CONNECTIVITY_CHANGE
 */
public class ConnectionChangeReceiver extends BroadcastReceiver {
  ...
  /*
   * note that onReceive occurs when anything about the connection has changed,
   * not just when the connection has been lost or restated, so it can happen
   * quite often when the user is on the move. for this reason we only fire the
   * event the first time onReceive is called, and afterwards only when we know
   * connection availability has changed
   */
  @Override
  ...
  public static EventBus getEventBus() { return EventBus.getDefault(); }
}
