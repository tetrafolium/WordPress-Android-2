...
/*
 * This method checks that a bundle for a given JobService matches perfectly
 * (all extras and all of its values match) to check if it is already
 * scheduled or not.
 * TODO IMPORTANT: note that this particular method checks for int[] and
 * compares within the array, as that's a case we needed to implement. In the
 * future, if you need to compare other kind of arrays, you'll need to
 * implement each case as you see fit.
 */

public static boolean isJobServiceWithSameParamsPending(
	...
	for (JobInfo jobInfo : jobs) {
		...
		if (jobInfo.getService().getClassName().compareTo(
			    componentName.getClassName()) == 0) {
			...
			if (extras != null) {
				...
				if ((exceptKey == null && (keySetOne.size() == keySetTwo.size())) ||
				    ((exceptKey != null &&
				      (keySetOne.size() - 1 == keySetTwo.size())) &&
