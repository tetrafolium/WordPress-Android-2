...
public class AppPrefs {
  ...
  private static final int MAX_RECENTLY_PICKED_SITES_TO_SAVE =
      MAX_RECENTLY_PICKED_SITES_TO_SHOW * 2;
  ...
  /**
   * Application related preferences. When the user disconnects, these
  ...
  public enum DeletablePrefKey implements PrefKey {
    ...
    // Store the number of times Stats are loaded without errors. It's used to
    ...
    // Used to flag the account created stat needs to be bumped after account
  ...
  /**
   * These preferences won't be deleted when the user disconnects. They should
   * be used for device specifics or user independent prefs.
   */
  public enum UndeletablePrefKey implements PrefKey {
    ...
    // When we need to show the snackbar indicating how notifications can be
    // navigated through
    ...
    // permission keys - set once a specific permission has been asked,
    ...
    // used to indicate that we already obtained and tracked the installation
    // referrer
    IS_INSTALLATION_REFERRER_OBTAINED,

    // used to indicate that user dont want to see the Gutenberg warning dialog
    // anymore
    IS_GUTENBERG_WARNING_DIALOG_DISABLED,

    // used to indicate that user dont want to see the Gutenberg informative
    // dialog anymore
  ...
  private static String getString(PrefKey key) { return getString(key, ""); }
  ...
  private static long getLong(PrefKey key) { return getLong(key, 0); }
  ...
  public static ReaderTag getReaderTag() {
    ...
    return ReaderUtils.getTagFromTagName(tagName,
  ...
  /**
   * title of the last active page in ReaderSubsActivity - this is stored rather
   * than the index of the page so we can re-order pages without affecting this
   * value
   */
  ...
  public static CommentStatusCriteria getCommentsStatusFilter() {
    ...
    CommentStatusCriteria[] commentStatusValues =
        CommentStatusCriteria.values();
  ...
  public static void
  setCommentsStatusFilter(CommentStatusCriteria commentStatus) {
    if (commentStatus != null) {
      setInt(DeletablePrefKey.COMMENTS_STATUS_TYPE_INDEX,
             commentStatus.ordinal());
    } else {
      prefs()
          .edit()
  ...
  public static void setPeopleListFilter(PeopleListFilter peopleListFilter) {
    if (peopleListFilter != null) {
      setInt(DeletablePrefKey.PEOPLE_LIST_FILTER_INDEX,
             peopleListFilter.ordinal());
    } else {
      prefs()
          .edit()
  ...
  /**
   * name of the last shown activity - used at startup to restore the previously
   * selected activity, also used by analytics tracker
   */
  public static String getLastActivityStr() {
    return getString(DeletablePrefKey.LAST_ACTIVITY_STR,
  ...
  public static void setAztecEditorEnabled(boolean isEnabled) {
    ...
    AnalyticsTracker.track(isEnabled ? Stat.EDITOR_AZTEC_TOGGLED_ON
                                     : Stat.EDITOR_AZTEC_TOGGLED_OFF);
  }

  public static boolean isAztecEditorEnabled() {
    // hardcode Aztec enabled to "true". It's Aztec and Gutenberg that we're
    // going to expose to the user now.
    return true;
  }
  ...
  public static boolean shouldShowBookmarksSavedLocallyDialog() {
    return getBoolean(UndeletablePrefKey.BOOKMARKS_SAVED_LOCALLY_DIALOG_SHOWN,
                      true);
  }
  ...
  // Store the number of times Stats are loaded successfully before showing the
  // Promo Dialog
  ...
  public static boolean isNotificationsSwipeToNavigateShown() {
    return getBoolean(UndeletablePrefKey.SWIPE_TO_NAVIGATE_NOTIFICATIONS,
                      false);
  }

  public static void
  setNotificationsSwipeToNavigateShown(boolean alreadyShown) {
    setBoolean(UndeletablePrefKey.SWIPE_TO_NAVIGATE_NOTIFICATIONS,
               alreadyShown);
  }
  ...
  public static void setPendingDraftsLastNotificationDate(PostModel post,
                                                          long timestamp) {
    String key =
        DeletablePrefKey.PENDING_DRAFTS_NOTIFICATION_LAST_NOTIFICATION_DATES
            .name() +
  ...
  public static int getImageOptimizeMaxSize() {
    ...
    return resizeWidth == 0 ? WPMediaUtils.OPTIMIZE_IMAGE_MAX_SIZE
                            : resizeWidth;
  }
  ...
  public static boolean isGutenbergEditorEnabled() {
    // hardcode Gutenberg enabled to "true". It's Aztec and Gutenberg that we're
    // going to expose to the user now.
    return true;
  }

  /**
   * @deprecated  As of release 13.0, replaced by SiteSettings mobile editor
   *     value
   */
  @Deprecated
  ...
  public static boolean isDefaultAppWideEditorPreferenceSet() {
    ...
    return !"".equals(
        getString(DeletablePrefKey.GUTENBERG_DEFAULT_FOR_NEW_POSTS));
  }
  ...
  shouldShowGutenbergInfoPopupForTheNewPosts(String siteURL) {
    ...
    Set<String> urls;
    try {
      urls = prefs().getStringSet(
          DeletablePrefKey.SHOULD_AUTO_ENABLE_GUTENBERG_FOR_THE_NEW_POSTS
              .name(),
          null);
  ...
  public static void setShowGutenbergInfoPopupForTheNewPosts(String siteURL,
                                                             boolean show) {
    if (TextUtils.isEmpty(siteURL)) {
      return;
    }
  ...
  public static boolean isGutenbergInfoPopupDisplayed(String siteURL) {
    ...
    try {
      urls = prefs().getStringSet(
          DeletablePrefKey.GUTENBERG_OPT_IN_DIALOG_SHOWN.name(), null);
    } catch (ClassCastException exp) {
      // no operation - This should not happen.
      return false;
    }
  ...
  public static void setGutenbergInfoPopupDisplayed(String siteURL) {
    ...
    SharedPreferences.Editor editor = prefs().edit();
    editor.putStringSet(DeletablePrefKey.GUTENBERG_OPT_IN_DIALOG_SHOWN.name(),
                        newUrls);
    editor.apply();
  }
  ...
  public static int getVideoOptimizeWidth() {
    ...
    return resizeWidth == 0 ? WPMediaUtils.OPTIMIZE_VIDEO_MAX_WIDTH
                            : resizeWidth;
  }
  ...
  public static int getVideoOptimizeQuality() {
    ...
    return quality > 1 ? quality
                       : WPMediaUtils.OPTIMIZE_VIDEO_ENCODER_BITRATE_KB;
  }
  ...
  private static ArrayList<Integer> getRecentlyPickedSiteIds(int limit) {
    String idsAsString =
        getString(DeletablePrefKey.RECENTLY_PICKED_SITE_IDS, "");
  ...
  public static void addRecentlyPickedSiteId(Integer localId) {
    ...
    ArrayList<Integer> currentIds =
        getRecentlyPickedSiteIds(MAX_RECENTLY_PICKED_SITES_TO_SAVE);
  ...
  public static boolean hasQuickStartMigrationDialogShown() {
    return getBoolean(UndeletablePrefKey.HAS_QUICK_START_MIGRATION_SHOWN,
                      false);
  }
  ...
  public static void setInstallationReferrerObtained(Boolean isObtained) {
    setBoolean(UndeletablePrefKey.IS_INSTALLATION_REFERRER_OBTAINED,
               isObtained);
  }

  public static boolean isInstallationReferrerObtained() {
    return getBoolean(UndeletablePrefKey.IS_INSTALLATION_REFERRER_OBTAINED,
                      false);
  }
  ...
  public static AuthorFilterSelection getAuthorFilterSelection() {
    long id = getLong(DeletablePrefKey.POST_LIST_AUTHOR_FILTER,
                      AuthorFilterSelection.getDefaultValue().getId());
    return AuthorFilterSelection.fromId(id);
  }

  public static void
  setAuthorFilterSelection(@NonNull AuthorFilterSelection selection) {
  ...
  public static void
  setPostsListViewLayoutType(@NonNull PostListViewLayoutType type) {
    setLong(DeletablePrefKey.POST_LIST_VIEW_LAYOUT_TYPE, type.getId());
  }

  public static void setStatsWidgetSelectedSiteId(long siteId,
                                                  int appWidgetId) {
  ...
  public static void setStatsWidgetColorModeId(int colorModeId,
                                               int appWidgetId) {
    prefs()
        .edit()
        .putInt(getColorModeIdWidgetKey(appWidgetId), colorModeId)
        .apply();
  }
  ...
  public static void setStatsWidgetDatatTypeId(int dataTypeId,
                                               int appWidgetId) {
    prefs()
        .edit()
        .putInt(getDatatTypeIdWidgetKey(appWidgetId), dataTypeId)
        .apply();
  }
  ...
  @NonNull
  private static String getDatatTypeIdWidgetKey(int appWidgetId) {
