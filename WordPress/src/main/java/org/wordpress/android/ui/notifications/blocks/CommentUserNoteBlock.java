...
// A user block with slightly different formatting for display in a comment
// detail
public class CommentUserNoteBlock extends UserNoteBlock {
  ...
  public CommentUserNoteBlock(
      Context context, FormattableContent noteObject,
      FormattableContent commentTextBlock, long timestamp,
      OnNoteBlockTextClickListener onNoteBlockTextClickListener,
      OnGravatarClickedListener onGravatarClickedListener,
      ImageManager imageManager,
      NotificationsUtilsWrapper notificationsUtilsWrapper) {
    super(context, noteObject, onNoteBlockTextClickListener,
          onGravatarClickedListener, imageManager, notificationsUtilsWrapper);

    mCommentData = commentTextBlock;
    mTimestamp = timestamp;

    if (context != null) {
      setAvatarSize(context.getResources().getDimensionPixelSize(
  ...
  public View configureView(View view) {
    final CommentUserNoteBlockHolder noteBlockHolder =
        (CommentUserNoteBlockHolder)view.getTag();

    noteBlockHolder.mNameTextView.setText(
        Html.fromHtml("<strong>" + getNoteText().toString() + "</strong>"));
    noteBlockHolder.mAgoTextView.setText(DateTimeUtils.timeSpanFromTimestamp(
        getTimestamp(), WordPress.getContext()));
    if (!TextUtils.isEmpty(getMetaHomeTitle()) ||
        !TextUtils.isEmpty(getMetaSiteUrl())) {
      ...
      if (!TextUtils.isEmpty(getMetaHomeTitle())) {
        ...
        noteBlockHolder.mSiteTextView.setText(
            getMetaSiteUrl().replace("http://", "").replace("https://", ""));
      }
    } else {
    ...
    noteBlockHolder.mSiteTextView.setImportantForAccessibility(
        View.IMPORTANT_FOR_ACCESSIBILITY_NO);

    String imageUrl = "";
    if (hasImageMediaItem()) {
      imageUrl = GravatarUtils.fixGravatarUrl(getNoteMediaItem().getUrl(),
                                              getAvatarSize());
      noteBlockHolder.mAvatarImageView.setContentDescription(
          view.getContext().getString(R.string.profile_picture,
                                      getNoteText().toString()));
      if (!TextUtils.isEmpty(getUserUrl())) {
        ...
        noteBlockHolder.mAvatarImageView.setOnTouchListener(
            mOnGravatarTouchListener);
      } else {
    ...
    mImageManager.loadIntoCircle(noteBlockHolder.mAvatarImageView,
    ...
    NoteBlockClickableSpan[] spans =
        spannable.getSpans(0, spannable.length(), NoteBlockClickableSpan.class);
    ...
    ViewCompat.setPaddingRelative(view, paddingStart, paddingTop, paddingEnd,
                                  paddingBottom);
  ...
  private Spannable
  getCommentTextOfNotification(CommentUserNoteBlockHolder noteBlockHolder) {
    SpannableStringBuilder builder =
        mNotificationsUtilsWrapper.getSpannableContentForRanges(
            mCommentData, noteBlockHolder.mCommentTextView,
            getOnNoteBlockTextClickListener(), false);
  ...
  private Spannable removeNewLineInList(SpannableStringBuilder builder) {
    ...
    while (content.contains(DOUBLE_EMPTY_LINE)) {
      ...
      builder.replace(doubleSpaceIndex,
  ...
  private long getTimestamp() { return mTimestamp; }

  private boolean hasCommentNestingLevel() {
    return mCommentData.getNestLevel() != null &&
        mCommentData.getNestLevel() > 0;
  }
      ...
      mSiteTextView.setOnClickListener(new View.OnClickListener() {
        ...
        public void onClick(View v) {
          if (getOnNoteBlockTextClickListener() != null) {
            getOnNoteBlockTextClickListener().showSitePreview(getMetaSiteId(),
                                                              getMetaSiteUrl());
          }
        }
      });
  ...
  public void configureResources(Context context) {
    ...
    mIndentedLeftPadding = context.getResources().getDimensionPixelSize(
                               R.dimen.margin_extra_large) *
                           2;
  }

  private final OnCommentStatusChangeListener mOnCommentChangedListener =
      new OnCommentStatusChangeListener() {
        @Override
