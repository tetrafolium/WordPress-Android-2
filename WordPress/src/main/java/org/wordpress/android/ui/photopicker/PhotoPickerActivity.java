...
// the enum name of the source will be returned as a string in
// EXTRA_MEDIA_SOURCE
...
// note that the local post id isn't required (default value is
// EMPTY_LOCAL_POST_ID)
...
protected void onCreate(@Nullable Bundle savedInstanceState) {
	...
	if (savedInstanceState == null) {
		mBrowserType = (MediaBrowserType)getIntent().getSerializableExtra(
			PhotoPickerFragment.ARG_BROWSER_TYPE);
		mSite = (SiteModel)getIntent().getSerializableExtra(WordPress.SITE);
		mLocalPostId =
			getIntent().getIntExtra(LOCAL_POST_ID, EMPTY_LOCAL_POST_ID);
	} else {
		mBrowserType = (MediaBrowserType)savedInstanceState.getSerializable(
			PhotoPickerFragment.ARG_BROWSER_TYPE);
		mSite = (SiteModel)savedInstanceState.getSerializable(WordPress.SITE);
		mLocalPostId =
			savedInstanceState.getInt(LOCAL_POST_ID, EMPTY_LOCAL_POST_ID);
	}
...
private PhotoPickerFragment getPickerFragment() {
	Fragment fragment =
		getSupportFragmentManager().findFragmentByTag(PICKER_FRAGMENT_TAG);
	if (fragment != null) {
...
protected void onSaveInstanceState(Bundle outState) {
	...
	outState.putSerializable(PhotoPickerFragment.ARG_BROWSER_TYPE,
	                         mBrowserType);
...
protected void onActivityResult(int requestCode, int resultCode,
	...
	case RequestCodes.TAKE_PHOTO:
		try {
			...
			doMediaUriSelected(capturedImageUri,
			                   PhotoPickerMediaSource.ANDROID_CAMERA);
		} catch (RuntimeException e) {
			AppLog.e(AppLog.T.MEDIA, e);
		}
		break;
	// user selected from WP media library, extract the media ID and pass to
	// caller
...
private void launchStockMediaPicker() {
	if (mSite != null) {
		ActivityLauncher.showStockMediaPickerForResult(
			this, mSite, RequestCodes.STOCK_MEDIA_PICKER_SINGLE_SELECT);
	} else {
		ToastUtils.showToast(this, R.string.blog_not_found);
	}
}

private void doMediaUriSelected(@NonNull Uri mediaUri,
                                @NonNull PhotoPickerMediaSource source) {
	// if user chose a featured image, we need to upload it and return the
	...
	if (mBrowserType == MediaBrowserType.FEATURED_IMAGE_PICKER) {
		...
		WPMediaUtils.fetchMediaAndDoNext(
			this, mediaUri, new WPMediaUtils.MediaFetchDoNext() {
				@Override
				public void doNext(Uri uri) {
				        mFeaturedImageHelper.queueFeaturedImageForUpload(
						PhotoPickerActivity.this, mLocalPostId, mSite, uri, mimeType);
				        Intent intent = new Intent().putExtra(EXTRA_MEDIA_QUEUED, true);
...
@Override
public void
onPhotoPickerIconClicked(@NonNull PhotoPickerFragment.PhotoPickerIcon icon) {
	switch (icon) {
