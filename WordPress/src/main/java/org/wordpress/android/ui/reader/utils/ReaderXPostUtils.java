...
// note that these strings don't need to be localized due to the intended
// audience
private static final String UNKNOWN_SITE = "(unknown)";
private static final String FMT_SITE_XPOST =
	"%1$s cross-posted from %2$s to %3$s";
private static final String FMT_COMMENT_XPOST =
	"%1$s commented on %2$s, cross-posted to %3$s";

/*
 * returns the title to display for this xpost, which is simply the post's
 * title without the "X-post: " prefix
 */
...
public static Spanned getXPostSubtitleHtml(@NonNull ReaderPost post) {
	...
	String name = post.hasAuthorFirstName() ? post.getAuthorFirstName()
	                                    : post.getAuthorName();
	String subtitle =
		String.format(isCommentXPost ? FMT_COMMENT_XPOST : FMT_SITE_XPOST,
		              "<strong>" + name + "</strong>", getFromSiteName(post),
		              getToSiteName(post));
