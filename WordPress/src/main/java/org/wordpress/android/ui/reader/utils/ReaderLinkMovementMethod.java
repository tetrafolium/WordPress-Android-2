...
/*
 * custom LinkMovementMethod which shows photo viewer when an image span is
 * tapped
 */
public class ReaderLinkMovementMethod extends LinkMovementMethod {
  ...
  /*
   * note that separate instances are returned depending on whether we're
   * showing content from a private blog
   */
  ...
  /*
   * override MovementMethod.getInstance() to ensure our getInstance(false) is
   * used
   */
  ...
  public boolean onTouchEvent(@NonNull TextView textView,
    ...
    if (event.getAction() == MotionEvent.ACTION_UP) {
      ...
      if (images != null && images.length > 0) {
        ...
        if (mIsPrivate) {
          options.add(
              ReaderActivityLauncher.PhotoViewerOption.IS_PRIVATE_IMAGE);
        }
        String imageUrl = StringUtils.notNullStr(images[0].getSource());
        ReaderActivityLauncher.showReaderPhotoViewer(
            textView.getContext(), imageUrl, null, textView, options,
