...
/**
 * classes in this package serve as a middleman between local data and server
 * data - used by reader activities/fragments/adapters that wish to perform
 * actions on posts, blogs & topics, or wish to get the latest data from the
 * server. <p> methods in this package which change state (like, follow, etc.)
 * are generally optimistic and work like this: <p>
 * 1. caller asks method to send a network request which changes state
 * 2. method changes state in local data and returns to caller *before* network
 * request completes
 * 3. caller can access local state change without waiting for the network
 * request
 * 4. if the network request fails, the method restores the previous state of
 * the local data
 * 5. if caller passes a listener, it can be alerted to the actual
 * success/failure of the request <p> note that all methods MUST be called from
 * the UI thread in order to guarantee that listeners are alerted on the UI
 * thread
 */
