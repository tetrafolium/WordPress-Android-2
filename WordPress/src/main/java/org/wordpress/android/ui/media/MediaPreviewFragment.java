...
public class MediaPreviewFragment
    extends Fragment implements MediaController.MediaPlayerControl {
  ...
  public interface OnMediaTappedListener { void onMediaTapped(); }
   ...
   */
  public static MediaPreviewFragment newInstance(@Nullable SiteModel site,
  ...
  public View onCreateView(@NonNull LayoutInflater inflater,
    ...
    View view =
        inflater.inflate(R.layout.media_preview_fragment, container, false);
  ...
  private void loadImage(String mediaUri) {
    ...
    if (mSite == null || SiteUtils.isPhotonCapable(mSite)) {
      int maxWidth =
          Math.max(DisplayUtils.getDisplayPixelWidth(getActivity()),
    ...
    mImageManager.loadWithResultListener(
        mImageView, ImageType.IMAGE, mediaUri, ScaleType.CENTER, null,
        new RequestListener<Drawable>() {
          @Override
          public void onResourceReady(@NotNull Drawable resource) {
            if (isAdded()) {
              // assign the photo attacher to enable pinch/zoom - must come
              // before setImageBitmap for it to be correctly resized upon
              // loading
