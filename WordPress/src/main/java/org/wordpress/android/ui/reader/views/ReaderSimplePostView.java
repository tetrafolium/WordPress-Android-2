...
public void showPost(ReaderSimplePost simplePost, ViewGroup parent,
                     boolean isGlobal,
                     final OnSimplePostClickListener listener) {
	mSimplePost = simplePost;
	int avatarSize = DisplayUtils.dpToPx(
		getContext(),
		getResources().getDimensionPixelSize(R.dimen.avatar_sz_extra_small));

	LayoutInflater inflater = LayoutInflater.from(getContext());

	View postView =
		inflater.inflate(R.layout.reader_simple_post_view, parent, false);
	...
	View siteHeader =
		postView.findViewById(R.id.layout_simple_post_site_header);
	...
	if (isGlobal) {
		...
		if (mSimplePost.hasAuthorAvatarUrl()) {
			...
			String avatarUrl = GravatarUtils.fixGravatarUrl(
				mSimplePost.getAuthorAvatarUrl(), avatarSize);
			ImageManager.getInstance().loadIntoCircle(imgAvatar, ImageType.AVATAR,
			                                          avatarUrl);
		} else {
		...
		ReaderFollowButton btnFollow =
			siteHeader.findViewById(R.id.simple_post_follow_button);
	...
	postView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
			        if (listener != null) {
			                listener.onSimplePostClick(view, mSimplePost.getSiteId(),
...
private void toggleFollowStatus(final ReaderFollowButton btnFollow) {
	...
	ReaderBlogActions.followBlogById(mSimplePost.getSiteId(), isAskingToFollow,
	                                 listener);
...
private void showFeaturedImage(final View postView) {
	...
	// post must have an excerpt in order to show featured image (not enough
	// space otherwise)
	if (!mSimplePost.hasFeaturedImageUrl() || !mSimplePost.hasExcerpt()) {
		imgFeatured.setVisibility(View.GONE);
		return;
	}
	// featured image has height set to MATCH_PARENT so wait for parent view's
	// layout to complete before loading image so we can set the image height
	// correctly, then tell the imageView to crop the downloaded image to fit
	// the exact width/height of the view
	postView.getViewTreeObserver().addOnGlobalLayoutListener(
		new ViewTreeObserver.OnGlobalLayoutListener() {
			@Override
			public void onGlobalLayout() {
			        postView.getViewTreeObserver().removeOnGlobalLayoutListener(this);
			        int cropWidth = getContext().getResources().getDimensionPixelSize(
			        ...
			        ImageManager.getInstance().load(imgFeatured, ImageType.PHOTO,
			                                        photonUrl, ScaleType.CENTER_CROP);
			}
		});
