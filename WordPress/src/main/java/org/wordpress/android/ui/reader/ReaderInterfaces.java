...
public class ReaderInterfaces {
  private ReaderInterfaces() { throw new AssertionError(); }
  ...
  /*
   * called from post detail fragment so toolbar can animate in/out when
   * scrolling
   */
  ...
  /*
   * called when user taps the dropdown arrow next to a post to show the popup
   * menu
   */
   ...
   */
  public interface DataLoadedListener { void onDataLoaded(boolean isEmpty); }
  ...
  public interface OnPostBookmarkedListener {
    void onBookmarkedStateChanged(boolean isBookmarked, long blogId,
                                  long postId, boolean isCachingActionRequired);
  }
}
