...
/**
 * ViewHolder for a RecyclerView header used on screens shown after a user logs
 * in.
 */
...
public void updateLoggedInAsHeading(Context context,
                                    ImageManager imageManager,
                                    boolean isAfterLogin,
                                    AccountModel defaultAccount) {
	updateLoggedInAsHeading(context, imageManager, isAfterLogin,
	                        constructGravatarUrl(context, defaultAccount),
	                        defaultAccount.getUserName(),
	                        defaultAccount.getDisplayName());
}

public void updateLoggedInAsHeading(Context context,
                                    ImageManager imageManager,
                                    boolean isAfterLogin, String avatarUrl,
                                    String username, String displayName) {
	if (isAfterLogin) {
		mLoggedInAsHeading.setVisibility(View.VISIBLE);
		mUserDetailsCard.setVisibility(View.VISIBLE);
		imageManager.loadIntoCircle(mAvatarImageView,
		                            ImageType.AVATAR_WITHOUT_BACKGROUND,
		                            StringUtils.notNullStr(avatarUrl));
		mUsernameTextView.setText(
			context.getString(R.string.login_username_at, username));
...
private String constructGravatarUrl(Context context, AccountModel account) {
	int avatarSz =
		context.getResources().getDimensionPixelSize(R.dimen.avatar_sz_large);
