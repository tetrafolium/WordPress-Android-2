...
public class DeepLinkingIntentReceiverActivity extends AppCompatActivity {
  ...
  private static final String HOST_API_WORDPRESS_COM =
  ...
  private boolean shouldOpenEditor(@NonNull Uri uri) {
    ...
    return StringUtils.equals(uri.getHost(), HOST_WORDPRESS_COM) &&
        (!uri.getPathSegments().isEmpty() &&
         StringUtils.equals(uri.getPathSegments().get(0), POST_PATH));
  }
  ...
  private boolean shouldHandleTrackingUrl(@NonNull Uri uri) {
    ...
    return StringUtils.equals(uri.getHost(), HOST_API_WORDPRESS_COM) &&
        (!uri.getPathSegments().isEmpty() &&
         StringUtils.equals(uri.getPathSegments().get(0),
                            MOBILE_TRACKING_PATH));
  }
  ...
  private void handleOpenEditor(@NonNull Uri uri) {
    String urlPathSegment =
        uri.getLastPathSegment() == null ? "" : uri.getLastPathSegment();
    openEditorForSite(urlPathSegment);
  }

  private void openEditorForSite(@NonNull String targetHost) {
    List<SiteModel> matchedSites =
        mSiteStore.getSitesByNameOrUrlMatching(targetHost);
    ...
    if (site != null && host != null && StringUtils.equals(host, targetHost)) {
      // if we found the site with the matching url, open the editor for this
      // site.
  ...
  private void handleViewPost(@NonNull Uri uri) {
    ...
    // if user is signed in wpcom show the post right away - otherwise show
    // welcome activity and then show the post once the user has signed in
  ...
  private boolean isFromAppBanner(String host) {
    return (host != null && (host.equals(DEEP_LINK_HOST_NOTIFICATIONS) ||
    ...
    if (!TextUtils.isEmpty(mBlogId) && !TextUtils.isEmpty(mPostId)) {
      try {
        ...
        AnalyticsUtils.trackWithBlogPostDetails(
            AnalyticsTracker.Stat.READER_VIEWPOST_INTERCEPTED, blogId, postId);

        ReaderActivityLauncher.showReaderPostDetail(
            this, false, blogId, postId, null, 0, false, mInterceptedUri);
