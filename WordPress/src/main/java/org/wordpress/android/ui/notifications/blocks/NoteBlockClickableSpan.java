...
public class NoteBlockClickableSpan extends ClickableSpan {
  ...
  public NoteBlockClickableSpan(FormattableRange range, boolean shouldLink,
                                boolean isFooter) {
  ...
  // We need to use theme-styled colors in NoteBlockClickableSpan but current
  // Notifications architecture makes it difficult to get right type of context
  // to this span to style the colors. We are doing it in this method instead.
  ...
  public void setColors(@ColorInt int textColor, @ColorInt int backgroundColor,
                        @ColorInt int linkColor, @ColorInt int lightTextColor) {
  ...
  private boolean shouldLinkRangeType() {
    return mShouldLink && mRangeType != FormattableRangeType.BLOCKQUOTE &&
        mRangeType != FormattableRangeType.MATCH &&
        (mRangeType != FormattableRangeType.UNKNOWN ||
  ...
  public FormattableRangeType getRangeType() { return mRangeType; }

  public FormattableRange getFormattableRange() { return mFormattableRange; }

  public List<Integer> getIndices() { return mIndices; }
  ...
  public void setPressed(boolean isPressed) { this.mPressed = isPressed; }
