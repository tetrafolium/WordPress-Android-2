...
public class MediaBrowserActivity extends AppCompatActivity
    implements MediaGridListener, OnQueryTextListener, OnActionExpandListener,
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if (savedInstanceState == null) {
      ...
      mBrowserType =
          (MediaBrowserType)getIntent().getSerializableExtra(ARG_BROWSER_TYPE);
    } else {
      mSite = (SiteModel)savedInstanceState.getSerializable(WordPress.SITE);
      mBrowserType = (MediaBrowserType)savedInstanceState.getSerializable(
          ARG_BROWSER_TYPE);
      mMediaCapturePath =
          savedInstanceState.getString(BUNDLE_MEDIA_CAPTURE_PATH);
    ...
    if (mBrowserType == null) {
      ...
      AppLog.w(
          AppLog.T.MEDIA,
          "MediaBrowserType is null. Defaulting to MediaBrowserType.BROWSER mode.");
    ...
    mMediaGridFragment =
        (MediaGridFragment)fm.findFragmentByTag(MediaGridFragment.TAG);
    if (mMediaGridFragment == null) {
      mMediaGridFragment =
          MediaGridFragment.newInstance(mSite, mBrowserType, filter);
      getSupportFragmentManager()
          .beginTransaction()
          .replace(R.id.media_browser_container, mMediaGridFragment,
                   MediaGridFragment.TAG)
  ...
  private void formatQuotaDiskSpace() {
    ...
    final String[] units =
        new String[] {getString(R.string.file_size_in_bytes),
    ...
    if (mSite.getPlanId() == PlansConstants.BUSINESS_PLAN_ID) {
      ...
      quota = String.format(
          getString(R.string.site_settings_quota_space_unlimited), space);
    } else {
      String space = FormatUtils.formatFileSize(mSite.getSpaceAllowed(), units);
      quota = String.format(getString(R.string.site_settings_quota_space_value),
                            percentage, space);
    }

    mQuotaText.setText(getString(R.string.media_space_used, quota));
    mQuotaText.setTextColor(getResources().getColor(
        mSite.getSpacePercentUsed() > 90 ? R.color.error_50 : R.color.neutral));
  }
   ...
   */
  private boolean shouldShowTabs() { return mBrowserType.canFilter(); }
  ...
  private void setupTabs() {
    if (shouldShowTabs()) {
      ...
      mTabLayout.addTab(
          mTabLayout.newTab().setText(R.string.media_all)); // FILTER_ALL
      mTabLayout.addTab(
          mTabLayout.newTab().setText(R.string.media_images)); // FILTER_IMAGES
      mTabLayout.addTab(mTabLayout.newTab().setText(
          R.string.media_documents)); // FILTER_DOCUMENTS
      mTabLayout.addTab(
          mTabLayout.newTab().setText(R.string.media_videos)); // FILTER_VIDEOS
      mTabLayout.addTab(
          mTabLayout.newTab().setText(R.string.media_audio)); // FILTER_AUDIO
      ...
      // tabMode is set to scrollable in layout, set to fixed if there's enough
      // space to show them all
      mTabLayout.getViewTreeObserver().addOnGlobalLayoutListener(
          new ViewTreeObserver.OnGlobalLayoutListener() {
            @Override
            public void onGlobalLayout() {
              mTabLayout.getViewTreeObserver().removeOnGlobalLayoutListener(
                  this);

              if (mTabLayout.getChildCount() > 0) {
                int tabLayoutWidth = 0;
                LinearLayout tabFirstChild =
                    (LinearLayout)mTabLayout.getChildAt(0);
                for (int i = 0; i < mTabLayout.getTabCount(); i++) {
                  LinearLayout tabView =
                      (LinearLayout)(tabFirstChild.getChildAt(i));
                  tabLayoutWidth += (tabView.getMeasuredWidth() +
                                     ViewCompat.getPaddingStart(tabView) +
                                     ViewCompat.getPaddingEnd(tabView));
                }

                int displayWidth = DisplayUtils.getDisplayPixelWidth(
  ...
  private void setFilter(@NonNull MediaFilter filter) {
    ...
    if (shouldShowTabs() && mTabLayout != null &&
        mTabLayout.getSelectedTabPosition() != position) {
      mTabLayout.setScrollPosition(position, 0f, true);
    }

    if (mMediaGridFragment != null &&
        (mMediaGridFragment.getFilter() != filter ||
         mMediaGridFragment.isEmpty())) {
  ...
  private void getMediaFromDeviceAndTrack(Uri imageUri, int requestCode) {
    ...
    WPMediaUtils.fetchMediaAndDoNext(
        this, imageUri, new WPMediaUtils.MediaFetchDoNext() {
          @Override
    ...
    trackAddMediaFromDeviceEvents(
        false, requestCode == RequestCodes.VIDEO_LIBRARY, imageUri);
  }
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    case RequestCodes.GIPHY_PICKER:
      if (resultCode == RESULT_OK &&
        ...
        int[] mediaLocalIds = data.getIntArrayExtra(
   ...
   */
  private void trackAddMediaFromDeviceEvents(boolean isNewMedia,
                                             boolean isVideo, Uri uri) {
    if (uri == null) {
      AppLog.e(AppLog.T.MEDIA,
               "Cannot track new media event if mediaURI is null!!");
      return;
    }
    ...
    if (isVideo) {
      AnalyticsTracker.track(AnalyticsTracker.Stat.MEDIA_LIBRARY_ADDED_VIDEO,
                             properties);
    } else {
      AnalyticsTracker.track(AnalyticsTracker.Stat.MEDIA_LIBRARY_ADDED_PHOTO,
                             properties);
    }
  }
  ...
  public void onRequestPermissionsResult(int requestCode,
    ...
    if (allGranted &&
        requestCode ==
            WPPermissionUtils.MEDIA_BROWSER_PERMISSION_REQUEST_CODE) {
  ...
  public boolean onCreateOptionsMenu(Menu menu) {
    ...
    if (!TextUtils.isEmpty(mQuery) && mMediaGridFragment != null &&
        mMediaGridFragment.isVisible()) {
      String tempQuery = mQuery; // temporary hold onto query
      MenuItemCompat.expandActionView(
          mSearchMenuItem); // this will reset mQuery
      onQueryTextSubmit(tempQuery);
      mSearchView.setQuery(mQuery, true);
    }

    // hide "add media" if the user doesn't have upload permission or this is a
    // multiselect picker
  ...
  public void onMediaItemSelected(int localMediaId, boolean isLongClick) {
    ...
    if (media == null) {
      AppLog.w(AppLog.T.MEDIA,
               "Media browser > unable to load localMediaId = " + localMediaId);
      ToastUtils.showToast(this, R.string.error_media_load);
      return;
    }

    // do nothing for failed uploads
    if (MediaUploadState.fromString(media.getUploadState()) ==
        MediaUploadState.FAILED) {
      return;
    }

    // show detail view when tapped if we're browsing media, when used as a
    // picker show detail when long tapped (to mimic native photo picker)
  ...
  private void showMediaToastError(@StringRes int message,
                                   @Nullable String messageDetail) {
    if (isFinishing()) {
      return;
    }
  ...
  public void onMediaChanged(OnMediaChanged event) {
    ...
    if (event.isError()) {
      AppLog.w(AppLog.T.MEDIA,
               "Received onMediaChanged error: " + event.error.type + " - " +
                   event.error.message);
    ...
    case DELETE_MEDIA:
      if (event.mediaList != null) {
        // If the media was deleted, remove it from multi select if it was
        // selected
  ...
  // TODO: in a future PR this and startMediaDeleteService() can be simplified
  // since multiselect delete was dropped
  private void deleteMedia(final ArrayList<Integer> ids) {
    ...
    for (int currentId : ids) {
      ...
      if (UploadService.isPendingOrInProgressMediaUpload(mediaModel)) {
        CancelMediaPayload payload =
            new CancelMediaPayload(mSite, mediaModel, false);
    ...
    if (processedItemCount != ids.size()) {
      ToastUtils.showToast(this, R.string.cannot_delete_multi_media_items,
  ...
  private void uploadList(List<Uri> uriList) {
    ...
    for (Uri uri : uriList) {
      if (uri != null) {
        WPMediaUtils.fetchMediaAndDoNext(
            this, uri, new WPMediaUtils.MediaFetchDoNext() {
              @Override
  ...
  private void doBindDeleteService(Intent intent) {
    mDeleteServiceBound =
        bindService(intent, mDeleteConnection,
  ...
  public void showAddMediaPopup() {
    ...
    if (mBrowserType.isBrowser()) {
      popup.getMenu()
          .add(R.string.photo_picker_giphy)
  ...
  private void doAddMediaItemClicked(@NonNull AddMenuItem item) {
    ...
    if (item != AddMenuItem.ITEM_CHOOSE_STOCK_MEDIA) {
      String[] permissions;
      if (item == AddMenuItem.ITEM_CAPTURE_PHOTO ||
          item == AddMenuItem.ITEM_CAPTURE_VIDEO) {
  ...
  private Uri getOptimizedPictureIfNecessary(Uri originalUri) {
    ...
    if (optimizedMedia != null) {
      ...
      if (!mSite.isWPCom()) {
        ...
        Uri rotatedMedia =
            WPMediaUtils.fixOrientationIssue(this, filePath, false);
  ...
  private void
  addMediaToUploadService(@NonNull ArrayList<MediaModel> mediaModels) {
    // Start the upload service if it's not started and fill the media queue
    if (!NetworkUtils.isNetworkAvailable(this)) {
      AppLog.v(AppLog.T.MEDIA,
               "Unable to start UploadService, internet connection required.");
  ...
  private void handleSharedMedia() {
    ...
    // clear the intent's action, so that in case the user rotates, we don't
  ...
  private void startMediaDeleteService(ArrayList<MediaModel> mediaToDelete) {
    if (!NetworkUtils.isNetworkAvailable(this)) {
      AppLog.v(
          AppLog.T.MEDIA,
          "Unable to start MediaDeleteService, internet connection required.");
      return;
    }
  ...
  public void onEventMainThread(UploadService.UploadErrorEvent event) {
    ...
    if (event.mediaModelList != null && !event.mediaModelList.isEmpty()) {
      UploadUtils.onMediaUploadedSnackbarHandler(
          this, findViewById(R.id.tab_layout), true, event.mediaModelList,
          mSite, event.errorMessage);
  ...
  public void onEventMainThread(UploadService.UploadMediaSuccessEvent event) {
    ...
    if (event.mediaModelList != null && !event.mediaModelList.isEmpty()) {
      UploadUtils.onMediaUploadedSnackbarHandler(
          this, findViewById(R.id.tab_layout), false, event.mediaModelList,
