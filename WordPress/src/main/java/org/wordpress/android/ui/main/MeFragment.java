...
public class MeFragment extends Fragment
    implements MainToolbarFragment, WPMainActivity.OnScrollToTopListener {
  ...
  // setUserVisibleHint is not available so we need to manually handle the
  // UserVisibleHint state
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container,
    ...
    final ViewGroup rootView =
        (ViewGroup)inflater.inflate(R.layout.me_fragment, container, false);
    ...
    mLoginLogoutTextView =
        rootView.findViewById(R.id.me_login_logout_text_view);
    ...
    rootView.findViewById(R.id.change_photo)
        .setOnClickListener(showPickerListener);
    ...
    rootView.findViewById(R.id.row_app_settings)
    ...
    rootView.findViewById(R.id.row_support)
        .setOnClickListener(new View.OnClickListener() {
          @Override
          public void onClick(View v) {
            ActivityLauncher.viewHelpAndSupport(
                getActivity(), Origin.ME_SCREEN_HELP, getSelectedSite(), null);
          }
        });

    rootView.findViewById(R.id.row_logout)
        .setOnClickListener(new View.OnClickListener() {
          @Override
          public void onClick(View v) {
  ...
  private void refreshAccountDetails() {
    ...
    if (mAccountStore.hasAccessToken()) {
      ...
      mUsernameTextView.setText(
          getString(R.string.at_username, defaultAccount.getUserName()));
  ...
  private void loadAvatar(final String avatarUrl, String injectFilePath) {
    ...
    if (newAvatarUploaded) {
      // invalidate the specific gravatar entry from the bitmap cache. It will
      // be updated via the injected request cache.
    ...
    if (bitmap != null) {
      ...
      mImageManager.loadIntoCircle(
          mAvatarImageView, ImageType.AVATAR_WITHOUT_BACKGROUND,
          newAvatarUploaded ? injectFilePath : avatarUrl,
          new RequestListener<Drawable>() {
            @Override
            public void onLoadFailed(@Nullable Exception e) {
              final String appLogMessage =
                  "onLoadFailed while loading Gravatar image!";
              if (e == null) {
              ...
              // For some reason, the Activity can be null so, guard for it. See
              // #8590.
              if (getActivity() != null) {
                ToastUtils.showToast(getActivity(),
                                     R.string.error_refreshing_gravatar,
  ...
  private void signOutWordPressComWithConfirmation() {
    // if there are local changes we need to let the user know they'll be lost
    // if they logout, otherwise we use a simpler (less scary!) confirmation
    String message;
  ...
  private void signOutWordPressCom() {
    // note that signing out sends a CoreEvents.UserSignedOutWordPressCom
    // EventBus event, which will cause the main activity to recreate this
    // fragment
    (new SignOutWordPressComAsync(getActivity()))
        .executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
  }

  private void showDisconnectDialog(Context context) {
    mDisconnectProgressDialog = ProgressDialog.show(
        context, null, context.getText(R.string.signing_out), false);
  }
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    // If the fragment is not attached to the activity, we can't start the crop
    // activity or upload the cropped image.
    if (!isAdded()) {
      return;
    }
  ...
  private void showPhotoPickerForGravatar() {
    ActivityLauncher.showPhotoPickerForResult(
        getActivity(), MediaBrowserType.GRAVATAR_IMAGE_PICKER, null, null);
  }
        ...
        new GravatarApi.GravatarUploadListener() {
          ...
          public void onSuccess() {
            EventBus.getDefault().post(
                new GravatarUploadFinished(filePath, true));
          }

          @Override
          public void onError() {
            EventBus.getDefault().post(
                new GravatarUploadFinished(filePath, false));
          }
        });
  }
  ...
  public void onEventMainThread(GravatarUploadFinished event) {
    ...
    if (event.success) {
      ...
      ToastUtils.showToast(getActivity(), R.string.error_updating_gravatar,
                           ToastUtils.Duration.SHORT);
    }
  }
  ...
  private class SignOutWordPressComAsync extends AsyncTask<Void, Void, Void> {
    ...
    protected void onPostExecute(Void aVoid) {
      ...
      if (mDisconnectProgressDialog != null &&
          mDisconnectProgressDialog.isShowing()) {
        mDisconnectProgressDialog.dismiss();
      }
      mDisconnectProgressDialog = null;
    }
