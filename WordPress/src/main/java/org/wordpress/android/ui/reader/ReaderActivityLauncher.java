...
public class ReaderActivityLauncher {
  /*
   * show a single reader post in the detail view - simply calls
   * showReaderPostPager with a single post
   */
  public static void showReaderPostDetail(Context context, long blogId,
                                          long postId) {
    showReaderPostDetail(context, false, blogId, postId, null, 0, false, null);
  }

  public static void showReaderPostDetail(Context context, boolean isFeed,
                                          long blogId, long postId,
                                          DirectOperation directOperation,
                                          int commentId, boolean isRelatedPost,
  ...
  /*
   * show pager view of posts with a specific tag - passed blogId/postId is the
   * post to select after the pager is populated
   */
  public static void showReaderPostPagerForTag(Context context, ReaderTag tag,
   ...
   */
  public static void showReaderPostPagerForBlog(Context context, long blogId,
                                                long postId) {
    Intent intent = new Intent(context, ReaderPostPagerActivity.class);
    intent.putExtra(ReaderConstants.ARG_POST_LIST_TYPE,
   ...
   */
  public static void showReaderBlogOrFeedPreview(Context context, long siteId,
                                                 long feedId) {
   ...
   */
  public static void showReaderComments(Context context, long blogId,
                                        long postId) {
  ...
  public static void showReaderComments(Context context, long blogId,
    ...
    showReaderComments(context, blogId, postId, DirectOperation.COMMENT_JUMP,
                       commentId, null);
  }

  /**
   * Show comments for passed Ids and directly perform an action on a specifc
   * comment
   *
   * @param context context to use to start the activity
   * @param blogId blog id
   * @param postId post id
   * @param directOperation operation to perform on the specific comment. Can be
   *     null for no operation.
   * @param commentId specific comment id to perform an action on
   * @param interceptedUri URI to fall back into (i.e. to be able to open in
   *     external browser)
   */
  public static void showReaderComments(Context context, long blogId,
                                        long postId,
                                        DirectOperation directOperation,
                                        long commentId, String interceptedUri) {
   ...
   */
  public static void showReaderLikingUsers(Context context, long blogId,
                                           long postId) {
  ...
  /*
   * show the passed imageUrl in the fullscreen photo activity - optional
   * content is the content of the post the image is in, used by the activity to
   * show all images in the post
   */
  public enum PhotoViewerOption { IS_PRIVATE_IMAGE, IS_GALLERY_IMAGE }

  public static void showReaderPhotoViewer(
      Context context, String imageUrl, String content, View sourceView,
      EnumSet<PhotoViewerOption> imageOptions, int startX, int startY) {
    ...
    boolean isPrivate =
        imageOptions != null &&
        imageOptions.contains(PhotoViewerOption.IS_PRIVATE_IMAGE);
    boolean isGallery =
        imageOptions != null &&
        imageOptions.contains(PhotoViewerOption.IS_GALLERY_IMAGE);
    ...
    if (context instanceof Activity && sourceView != null) {
      ...
      ActivityOptionsCompat options =
          ActivityOptionsCompat.makeScaleUpAnimation(sourceView, startX, startY,
                                                     0, 0);
  ...
  public static void
  showReaderPhotoViewer(Context context, String imageUrl,
