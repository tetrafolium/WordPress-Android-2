...
/**
 * Interface for WordPress (.com and .org) Site Settings. The {@link
 * SiteSettingsModel} class is used to store the following settings: <p>
 * - Title
 * - Tagline
 * - Address
 * - Privacy
 ...
 * <p>
 * This class is marked abstract. This is due to the fact that .org
 * (self-hosted) and .com sites expose different API's to query and edit their
 * respective settings (even though the options offered by each is roughly the
 * same). To get an instance of this interface class use the
 * {@link SiteSettingsInterface#getInterface(Context, SiteModel,
 * SiteSettingsListener)} method.
 */
...
public abstract class SiteSettingsInterface {
  ...
  /**
   * Key for the Standard post format. Used as default if post format is not
   * set/known.
   */
  static final String STANDARD_POST_FORMAT_KEY = "standard";

  /**
   * Standard sharing button style value. Used as default value if button style
   * is unknown.
   */
  private static final String STANDARD_SHARING_BUTTON_STYLE = "icon-text";

  /**
   * Standard post format value. Used as default display value if post format is
   * unknown.
   */
   ...
   */
  @Nullable
  public static SiteSettingsInterface
  getInterface(Context host, SiteModel site, SiteSettingsListener listener) {
  ...
  /**
   * {@link SiteSettingsInterface} implementations should use this method to
   * start a background task to load settings data from a remote source.
   */
  ...
  protected SiteSettingsInterface(Context host, SiteModel site,
  ...
  public void saveSettings() { SiteSettingsTable.saveSettings(mSettings); }
  ...
  public int getPrivacy() { return mSettings.privacy; }
    ...
    if (mContext != null) {
      ...
      case -1:
        return mContext.getString(
            R.string.site_settings_privacy_private_summary);
      case 0:
        return mContext.getString(
            R.string.site_settings_privacy_hidden_summary);
      case 1:
        return mContext.getString(
            R.string.site_settings_privacy_public_summary);
      }
    }
    return "";
  }
  ...
  public @NonNull Map<String, String> getFormats() {
    ...
    String[] postFormatKeys =
        mContext.getResources().getStringArray(R.array.post_format_keys);
    ...
    for (int i = 0;
         i < postFormatKeys.length && i < postFormatDisplayNames.length; ++i) {
    ...
    for (PostFormatModel postFormat : postFormats) {
      mSettings.postFormats.put(postFormat.getSlug(),
                                postFormat.getDisplayName());
    }
    return mSettings.postFormats;
  }
  ...
  public int getDefaultCategory() { return mSettings.defaultCategory; }
  ...
  public @NonNull String getDefaultPostFormat() {
    if (TextUtils.isEmpty(mSettings.defaultPostFormat) ||
        !getFormats().containsKey(mSettings.defaultPostFormat)) {
  ...
  public boolean getShowRelatedPosts() { return mSettings.showRelatedPosts; }
  ...
  public @NonNull String getRelatedPostsDescription() {
    ...
    String desc =
        mContext.getString(getShowRelatedPosts() ? R.string.on : R.string.off);
    return StringUtils.capitalize(desc);
  }

  public boolean getAllowComments() { return mSettings.allowComments; }

  public boolean getSendPingbacks() { return mSettings.sendPingbacks; }

  public boolean getReceivePingbacks() { return mSettings.receivePingbacks; }

  public boolean getShouldCloseAfter() { return mSettings.shouldCloseAfter; }

  public int getCloseAfter() { return mSettings.closeCommentAfter; }
  ...
  public @NonNull String getCloseAfterDescription() {
    return getCloseAfterDescriptionForPeriod(
        getCloseAfterPeriodForDescription());
  }
  ...
  public @NonNull String getCloseAfterDescriptionForPeriod(int period) {
    ...
    return StringUtils.getQuantityString(mContext, R.string.never,
                                         R.string.days_quantity_one,
                                         R.string.days_quantity_other, period);
  }

  public int getCommentSorting() { return mSettings.sortCommentsBy; }
  ...
  public int getThreadingLevels() { return mSettings.threadingLevels; }
  ...
  public @NonNull String getThreadingDescriptionForLevel(int level) {
    ...
    return String.format(
        mContext.getString(R.string.site_settings_threading_summary), level);
  }
  ...
  public int getPagingCount() { return mSettings.commentsPerPage; }
  ...
  public int getMultipleLinks() { return mSettings.maxLinks; }
  ...
  public String getSharingLabel() { return mSettings.sharingLabel; }

  public @NonNull String getSharingButtonStyle(Context context) {
    if (TextUtils.isEmpty(mSettings.sharingButtonStyle)) {
      mSettings.sharingButtonStyle = context.getResources().getStringArray(
          R.array.sharing_button_style_array)[0];
    }
  ...
  public @NonNull String getSharingButtonStyleDisplayText(Context context) {
    ...
    String[] styleArray = context.getResources().getStringArray(
        R.array.sharing_button_style_array);
    String[] styleDisplayArray = context.getResources().getStringArray(
  ...
  public boolean getAllowLikeButton() { return mSettings.allowLikeButton; }

  public boolean getAllowCommentLikes() {
    // We have different settings for comment likes for wpcom and Jetpack sites
    return mSite.isJetpackConnected() ? mJpSettings.commentLikes
                                      : mSettings.allowCommentLikes;
  }
  ...
  public @NonNull String getKeysDescription(int count) {
    ...
    return StringUtils.getQuantityString(
        mContext, R.string.site_settings_list_editor_no_items_text,
        R.string.site_settings_list_editor_summary_one,
        R.string.site_settings_list_editor_summary_other, count);
  }

  public String getStartOfWeek() { return mSettings.startOfWeek; }

  public void setStartOfWeek(String startOfWeek) {
    mSettings.startOfWeek = startOfWeek;
  }

  public String getDateFormat() { return mSettings.dateFormat; }

  public void setDateFormat(String dateFormat) {
    mSettings.dateFormat = dateFormat;
  }

  public String getTimeFormat() { return mSettings.timeFormat; }

  public void setTimeFormat(String timeFormat) {
    mSettings.timeFormat = timeFormat;
  }

  public String getTimezone() { return mSettings.timezone; }

  public void setTimezone(String timezone) { mSettings.timezone = timezone; }

  public int getPostsPerPage() { return mSettings.postsPerPage; }

  public void setPostsPerPage(int postsPerPage) {
    mSettings.postsPerPage = postsPerPage;
  }

  public boolean getAmpSupported() { return mSettings.ampSupported; }

  public void setAmpSupported(boolean supported) {
    mSettings.ampSupported = supported;
  }

  public boolean getAmpEnabled() { return mSettings.ampEnabled; }

  public void setAmpEnabled(boolean enabled) { mSettings.ampEnabled = enabled; }
  ...
  public boolean isJetpackMonitorEnabled() { return mJpSettings.monitorActive; }
  ...
  public void
  enableJetpackMonitorEmailNotifications(boolean emailNotifications) {
  ...
  public boolean isJetpackSsoEnabled() { return mJpSettings.ssoActive; }
  ...
  boolean isLazyLoadImagesEnabled() { return mJpSettings.lazyLoadImages; }

  void enableAdFreeHosting(boolean enabled) {
    mJpSettings.adFreeVideoHosting = enabled;
  }

  boolean isAdFreeHostingEnabled() { return mJpSettings.adFreeVideoHosting; }

  void enableImprovedSearch(boolean enabled) {
    mJpSettings.improvedSearch = enabled;
  }

  boolean isImprovedSearchEnabled() { return mJpSettings.improvedSearch; }

  public boolean isSharingModuleEnabled() { return mJpSettings.sharingEnabled; }

  public void setTitle(String title) { mSettings.title = title; }

  public void setTagline(String tagline) { mSettings.tagline = tagline; }

  public void setAddress(String address) { mSettings.address = address; }

  public void setQuotaDiskSpace(String quotaDiskSpace) {
    mSettings.quotaDiskSpace = quotaDiskSpace;
  }

  public void setPrivacy(int privacy) { mSettings.privacy = privacy; }
  ...
  public void setLanguageId(int languageId) {
    ...
    if (mSettings.languageId != languageId) {
      ...
      mSettings.language =
          languageIdToLanguageCode(Integer.toString(languageId));
    }
  }
  ...
  public void setUsername(String username) { mSettings.username = username; }

  public void setPassword(String password) { mSettings.password = password; }
  ...
  public void setPagingCount(int count) { mSettings.commentsPerPage = count; }
  ...
  public void setMultipleLinks(int count) { mSettings.maxLinks = count; }
  ...
  public void setSharingButtonStyle(String sharingButtonStyle) {
    if (TextUtils.isEmpty(sharingButtonStyle)) {
      ...
      mSettings.sharingButtonStyle =
          sharingButtonStyle.toLowerCase(Locale.ROOT);
    }
  }
   ...
   *
   * @param format if null or empty default format is set to {@link
   *     SiteSettingsInterface#STANDARD_POST_FORMAT_KEY}
   */
  ...
  /**
   * Checks if the provided list of post format IDs is the same (order
   * dependent) as the current list of Post Formats in the local settings
   * object.
   *
   * @param ids an array of post format IDs
   * @return true unless the provided IDs are different from the current IDs or
  ...
  public boolean isSameFormatList(CharSequence[] ids) {
    ...
    if (mSettings.postFormats == null ||
        ids.length != mSettings.postFormats.size()) {
      return false;
    }

    String[] keys = mSettings.postFormats.keySet().toArray(
        new String[mSettings.postFormats.size()]);
  ...
  /**
   * Checks if the provided list of category IDs is the same (order dependent)
   * as the current list of Categories in the local settings object.
   *
  ...
  public boolean isSameCategoryList(CharSequence[] ids) {
    ...
    if (mSettings.categories == null ||
        ids.length != mSettings.categories.length) {
      return false;
    }
  ...
  /**
   * Needed so that subclasses can be created before initializing. The final
   * member variables are null until object has been created so XML-RPC
  ...
  public SiteSettingsInterface init(boolean fetchRemote) {
    ...
    if (fetchRemote) {
      ...
      if (!AppPrefs.isDefaultAppWideEditorPreferenceSet()) {
        // Check if the migration from app-wide to per-site setting has already
        // happened - v12.9->13.0 before fetching site editors from the remote
        mDispatcher.dispatch(
            SiteActionBuilder.newFetchSiteEditorsAction(mSite));
      }
    }
  ...
  protected void credentialsVerified(boolean valid) {
    ...
    mRemoteSettings.hasVerifiedCredentials = mSettings.hasVerifiedCredentials =
        valid;
  }

  /**
   * Language IDs, used only by WordPress, are integer values that map to a
   * language code. http://bit.ly/2H7gksN <p> Language codes are unique
   * two-letter identifiers defined by ISO 639-1. Region dialects can be defined
   * by appending a -** where ** is the region code (en-GB -> English, Great
   * Britain). https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes
   */
  ...
  /**
   * Need to defer loading the cached settings to a thread so it completes after
   * initialization.
   */
  private void loadCachedSettings() {
    ...
    if (localSettings != null && localSettings.getCount() > 0) {
      ...
      SparseArrayCompat<CategoryModel> cachedModels =
          SiteSettingsTable.getAllCategories();
      mSettings.deserializeOptionsDatabaseCursor(localSettings, cachedModels);
      mSettings.language =
          languageIdToLanguageCode(Integer.toString(mSettings.languageId));
    ...
    if (mSite.hasDiskSpaceQuotaInformation()) {
      String percentage =
          FormatUtils.formatPercentage(mSite.getSpacePercentUsed() / 100);
      final String[] units =
          new String[] {mContext.getString(R.string.file_size_in_bytes),
      ...
      if (mSite.getPlanId() == PlansConstants.BUSINESS_PLAN_ID) {
        String usedSpace =
            FormatUtils.formatFileSize(mSite.getSpaceUsed(), units);
        quotaAvailableSentence = String.format(
            mContext.getString(R.string.site_settings_quota_space_unlimited),
            usedSpace);
      } else {
        String spaceAllowed =
            FormatUtils.formatFileSize(mSite.getSpaceAllowed(), units);
        quotaAvailableSentence = String.format(
            mContext.getString(R.string.site_settings_quota_space_value),
  ...
  /**
   * Notifies listener that settings have been updated with the latest remote
   * data.
   */
  ...
  /**
   * Notifies listener that settings have been saved or an error occurred while
   * saving.
   */
