...
public class ReaderPostServiceStarter {
  ...
  public enum UpdateAction {
    ...
    REQUEST_OLDER, // request posts older than the oldest existing one for this
                   // tag/blog/feed
    REQUEST_OLDER_THAN_GAP // request posts older than the one with the gap
                           // marker for this tag
   ...
   */
  public static void startServiceForTag(Context context, ReaderTag tag,
                                        UpdateAction action) {
   ...
   */
  public static void startServiceForBlog(Context context, long blogId,
                                         UpdateAction action) {
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
      Intent intent = new Intent(context, ReaderPostService.class);
   ...
   */
  public static void startServiceForFeed(Context context, long feedId,
                                         UpdateAction action) {
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
      Intent intent = new Intent(context, ReaderPostService.class);
  ...
  private static void doScheduleJobWithBundle(Context context,
                                              PersistableBundle extras,
                                              int jobId) {
    // schedule the JobService here for API >= 26. The JobScheduler is available
    // since API 21, but it's preferable to use it only since enforcement in API
    // 26 to not break any old behavior
    ComponentName componentName =
        new ComponentName(context, ReaderPostJobService.class);

    JobInfo jobInfo =
        new JobInfo.Builder(jobId, componentName)
    ...
    if (resultCode == JobScheduler.RESULT_SUCCESS) {
      ...
      AppLog.e(AppLog.T.READER,
               "reader post service > job could not be scheduled");
    }
  }

  private static void putReaderTagExtras(PersistableBundle extras,
                                         ReaderTag tag) {
