...
public class LearnMorePreference
    extends Preference implements View.OnClickListener {
  ...
  public LearnMorePreference(Context context, AttributeSet attrs) {
    ...
    TypedArray array =
        context.obtainStyledAttributes(attrs, R.styleable.LearnMorePreference);
    for (int i = 0; i < array.getIndexCount(); ++i) {
      int index = array.getIndex(i);
      if (index == R.styleable.LearnMorePreference_url) {
        mUrl = array.getString(index);
      } else if (index ==
                 R.styleable.LearnMorePreference_useCustomJsFormatting) {
  ...
  public void onClick(View v) {
    ...
    if (mOpenInDialog) {
      ...
      if (browserIntent.resolveActivity(v.getContext().getPackageManager()) !=
          null) {
  ...
  private static class SavedState extends BaseSavedState {
    SavedState(Parcel source) { super(source); }
  ...
  private class LearnMoreClient extends WebViewClient {
    ...
    public boolean shouldOverrideUrlLoading(WebView webView, String url) {
      ...
      return !mUrl.equals(url) && !SUPPORT_CONTENT_JS.equals(url) &&
          !CONTENT_PADDING_JS.equals(url);
    }
