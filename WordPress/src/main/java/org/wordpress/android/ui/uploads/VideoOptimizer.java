...
public class VideoOptimizer implements org.m4m.IProgressListener {
  ...
  public VideoOptimizer(@NonNull MediaModel media,
  ...
  private Context getContext() { return WordPress.getContext(); }
  ...
  public void start() {
    ...
    MediaComposer mediaComposer = WPVideoUtils.getVideoOptimizationComposer(
        getContext(), mInputPath, mOutputPath, this,
    ...
    if (mediaComposer == null) {
      ...
      AnalyticsTracker.track(MEDIA_VIDEO_CANT_OPTIMIZE,
                             AnalyticsUtils.getMediaProperties(
                                 getContext(), true, null, mInputPath));
      mListener.onVideoOptimizationCompleted(mMedia);
      return;
    }
  ...
  private void trackVideoProcessingEvents(boolean isError,
                                          Exception exception) {
    ...
    if (mOutputPath != null) {
      Map<String, Object> outputVideoProperties =
          AnalyticsUtils.getMediaProperties(getContext(), true, null,
                                            mOutputPath);
      putAllWithPrefix("output_video_", outputVideoProperties, properties);
      String savedMegabytes = String.valueOf(
          (FileUtils.length(mInputPath) - FileUtils.length(mOutputPath)) /
          (1024 * 1024));
    ...
    if (isError) {
      ...
      // Track to crash reporting service where it's easier to keep track of
      // errors
      CrashLoggingUtils.logException(exception, AppLog.T.MEDIA);
    }

    AnalyticsTracker.Stat currentStatToTrack =
        isError ? MEDIA_VIDEO_OPTIMIZE_ERROR : MEDIA_VIDEO_OPTIMIZED;
    AnalyticsTracker.track(currentStatToTrack, properties);
  }

  private void putAllWithPrefix(String prefix, Map<String, Object> inputMap,
  ...
  public void onMediaProgress(float progress) {
    // this event fires quite often so we only call the listener when progress
    // increases by 1% or more
    if (mLastProgress == 0 || (progress - mLastProgress > 0.01F)) {
      AppLog.d(AppLog.T.MEDIA, "VideoOptimizer > " + mMedia.getId() +
                                   " - progress: " + progress);
  ...
  public void onMediaDone() {
    ...
    String strSavingsKb =
        new DecimalFormat("0.00").format(savingsKb).concat("KB");

    // make sure the resulting file is smaller than the original
    if (savings <= 0) {
      AppLog.w(AppLog.T.MEDIA,
               "VideoOptimizer > no savings, optimized file is " +
                   strSavingsKb + " larger");
  ...
  public void onMediaStop() {
    // This seems to be called called in 2 cases. Do not use to check if we've
    // manually stopped the composer.
