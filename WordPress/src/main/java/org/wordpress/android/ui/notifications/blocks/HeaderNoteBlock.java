...
public class HeaderNoteBlock extends NoteBlock {
  ...
  private final UserNoteBlock
      .OnGravatarClickedListener mGravatarClickedListener;
  private Boolean mIsComment;
  private int mAvatarSize;

  private ImageType mImageType;

  public HeaderNoteBlock(
      Context context, List<FormattableContent> headerArray,
      ImageType imageType,
    ...
    super(new FormattableContent(), imageManager, notificationsUtilsWrapper,
  ...
  public int getLayoutResourceId() { return R.layout.note_block_header; }

  @SuppressLint("ClickableViewAccessibility")
  // fixed by setting a click listener to avatarImageView
  @Override
  public View configureView(View view) {
    final NoteHeaderBlockHolder noteBlockHolder =
        (NoteHeaderBlockHolder)view.getTag();
    ...
    if (mImageType == ImageType.AVATAR_WITH_BACKGROUND) {
      mImageManager.loadIntoCircle(noteBlockHolder.mAvatarImageView, mImageType,
                                   getAvatarUrl());
    } else {
      mImageManager.load(noteBlockHolder.mAvatarImageView, mImageType,
                         getAvatarUrl());
    }

    final long siteId =
        FormattableContentUtilsKt.getRangeSiteIdOrZero(getHeader(0), 0);
    final long userId =
        FormattableContentUtilsKt.getRangeIdOrZero(getHeader(0), 0);
    ...
    if (!TextUtils.isEmpty(getUserUrl()) && siteId > 0 && userId > 0) {
      ...
      if (siteId == userId) {
        noteBlockHolder.mAvatarImageView.setImportantForAccessibility(
  ...
  private String getAvatarUrl() {
    return GravatarUtils.fixGravatarUrl(
        FormattableContentUtilsKt.getMediaUrlOrEmpty(getHeader(0), 0),
        mAvatarSize);
  }
  ...
  public void setIsComment(Boolean isComment) { mIsComment = isComment; }
        ...
        public boolean onTouch(View v, MotionEvent event) {
          ...
          if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (event.getActionMasked() == MotionEvent.ACTION_UP &&
                mGravatarClickedListener != null) {
              // Fire the listener, which will load the site preview for the
              // user's site In the future we can use this to load a 'profile
              // view' (currently in R&D)
              v.performClick();
            }
          }
