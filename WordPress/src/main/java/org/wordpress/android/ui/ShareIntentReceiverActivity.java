...
/**
 * An activity to handle share intents, since there are multiple actions
 * possible. If the user is not logged in, redirects the user to the LoginFlow.
 * When the user is logged in, displays ShareIntentReceiverFragment. The
 * fragment lets the user choose which blog to share to. Moreover it lists what
 * actions the user can perform and redirects the user to the activity, along
 * with the content passed in the intent.
 */
public class ShareIntentReceiverActivity
	extends AppCompatActivity implements ShareIntentFragmentListener {
private static final String SHARE_LAST_USED_BLOG_ID_KEY =
	"wp-settings-share-last-used-text-blogid";
private static final String KEY_SELECTED_SITE_LOCAL_ID =
	"KEY_SELECTED_SITE_LOCAL_ID";
...
protected void onResume() {
	super.onResume();
	// keep setBackground in the onResume, otherwise the transition between
	// activities is visible to the user when sharing text with an account with
	// one visible site
...
private void refreshContent() {
	if (FluxCUtils.isSignedInWPComOrHasWPOrgSite(mAccountStore, mSiteStore)) {
		...
		if (visibleSites.size() == 0) {
			ToastUtils.showToast(this, R.string.cant_share_no_visible_blog,
			                     ToastUtils.Duration.LONG);
			finish();
		} else if (visibleSites.size() == 1 && isSharingText()) {
			// if text/plain and only one blog, then don't show the fragment, share
			// it directly to a new post
			share(ShareAction.SHARE_TO_POST, visibleSites.get(0).getId());
		} else {
			// display a fragment with list of sites and list of actions the user
			// can perform
...
private void initShareFragment(boolean afterLogin) {
	ShareIntentReceiverFragment shareIntentReceiverFragment =
		ShareIntentReceiverFragment.newInstance(
			!isSharingText(), loadLastUsedBlogLocalId(), afterLogin);
	getSupportFragmentManager()
	.beginTransaction()
	.replace(R.id.fragment_container, shareIntentReceiverFragment,
	         ShareIntentReceiverFragment.TAG)
	.commit();
}
...
public void onRequestPermissionsResult(int requestCode,
	...
	if (allGranted &&
	    requestCode == WPPermissionUtils.SHARE_MEDIA_PERMISSION_REQUEST_CODE) {
		// permissions granted
		share(ShareAction.valueOf(mShareActionName), mClickedSiteLocalId);
	} else {
		Toast
		.makeText(this, R.string.share_media_permission_required,
		          Toast.LENGTH_LONG)
		.show();
	}
}
...
private void startActivityAndFinish(@NonNull Intent intent,
                                    int mSelectedSiteLocalId) {
	String action = getIntent().getAction();
	intent.setAction(action);
	intent.setType(getIntent().getType());

	intent.putExtra(WordPress.SITE,
...
private void bumpAnalytics(ShareAction shareAction, int selectedSiteLocalId) {
	...
	AnalyticsUtils.trackWithSiteDetails(
		AnalyticsTracker.Stat.SHARE_TO_WP_SUCCEEDED, selectedSite,
		analyticsProperties);

	if (doesContainMediaAndWasSharedToMediaLibrary(shareAction,
	                                               numberOfMediaShared)) {
...
private void trackMediaAddedToMediaLibrary(SiteModel selectedSite) {
	...
	for (Uri uri : mediaUrls) {
		if (uri != null) {
			...
			AnalyticsTracker.Stat mediaTypeTrack =
				isVideo ? AnalyticsTracker.Stat.MEDIA_LIBRARY_ADDED_VIDEO
		    : AnalyticsTracker.Stat.MEDIA_LIBRARY_ADDED_PHOTO;
			AnalyticsUtils.trackWithSiteDetails(mediaTypeTrack, selectedSite,
			                                    properties);
		}
	}
}
...
private boolean
doesContainMediaAndWasSharedToMediaLibrary(ShareAction shareAction,
                                           int numberOfMediaShared) {
	return shareAction != null &&
	       shareAction.analyticsName.equals(
		ShareAction.SHARE_TO_MEDIA_LIBRARY.analyticsName) &&
