...
public class ParentCategorySpinnerAdapter
  ...
  public CategoryNode getItem(int position) { return mObjects.get(position); }

  public long getItemId(int position) { return position; }

  ParentCategorySpinnerAdapter(Context context, int resource,
  ...
  public View getDropDownView(int position, View convertView,
    ...
    textView.setText(
        StringEscapeUtils.unescapeHtml4(getItem(position).getName()));
    CategoryNode node = getItem(position);
    if (node != null) {
      textView.setText(StringEscapeUtils.unescapeHtml4(node.getName()));
      ViewCompat.setPaddingRelative(
          textView,
          DisplayUtils.dpToPx(getContext(), 16) * (node.getLevel() + 1), 0,
