...
public static final String WPCOM_PUSH_DEVICE_NOTIFICATION_SETTINGS =
	"wp_pref_notification_settings";
public static final String WPCOM_PUSH_DEVICE_UUID =
	"wp_pref_notifications_uuid";
public static final String WPCOM_PUSH_DEVICE_TOKEN =
	"wp_pref_notifications_token";

public static final String WPCOM_PUSH_DEVICE_SERVER_ID =
	"wp_pref_notifications_server_id";
public static final String PUSH_AUTH_ENDPOINT =
	"me/two-step/push-authentication";

private static final String CHECK_OP_NO_THROW = "checkOpNoThrow";
private static final String OP_POST_NOTIFICATION = "OP_POST_NOTIFICATION";

private static final String WPCOM_SETTINGS_ENDPOINT =
...
public static void
getPushNotificationSettings(Context context, RestRequest.Listener listener,
	...
	WordPress.getRestClientUtilsV1_1().get(settingsEndpoint, listener,
	                                       errorListener);
}

public static void registerDeviceForPushNotifications(final Context ctx,
                                                      String token) {
	SharedPreferences settings =
	...
	contentStruct.put("version_code",
	                  String.valueOf(PackageUtils.getVersionCode(ctx)));
	...
	RestRequest.Listener listener = new RestRequest.Listener() {
		...
		public void onResponse(JSONObject jsonObject) {
			...
			try {
				...
				AppLog.e(T.NOTIFS, "Server response is NOT ok, registration skipped.",
				         e1);
			}
		}
	};
	...
	WordPress.getRestClientUtils().post("/devices/new", contentStruct, null,
...
public static void unregisterDevicePushNotifications(final Context ctx) {
	...
	WordPress.getRestClientUtils().post("/devices/" + deviceID + "/delete",
	                                    listener, errorListener);
}

static FormattableContent
mapJsonToFormattableContent(FormattableContentMapper mapper,
...
static SpannableStringBuilder getSpannableContentForRanges(
	FormattableContentMapper formattableContentMapper, JSONObject blockObject,
	TextView textView,
...
static SpannableStringBuilder getSpannableContentForRanges(
	...
	return getSpannableContentForRanges(formattableContent, textView, isFooter,
	                                    clickListener);
}
...
static SpannableStringBuilder getSpannableContentForRanges(
	FormattableContent formattableContent, TextView textView,
	final Function1<FormattableRange, Unit> clickHandler, boolean isFooter) {
	Function1<NoteBlockClickableSpan, Unit> clickListener =
...
getSpannableContentForRanges(FormattableContent formattableContent,
                             TextView textView, boolean isFooter,
	...
	addImageSpansForBlockMedia(textView, formattableContent,
	...
	if (rangesArray != null) {
		for (FormattableRange range : rangesArray) {
			...
			if (indices != null && indices.size() == 2 &&
			    indices.get(0) <= spannableStringBuilder.length() &&
			    indices.get(1) <= spannableStringBuilder.length()) {
				spannableStringBuilder.setSpan(clickableSpan, indices.get(0),
				                               indices.get(1),
				...
				if (clickableSpan.getSpanStyle() != Typeface.NORMAL) {
					...
					spannableStringBuilder.setSpan(styleSpan, indices.get(0),
					                               indices.get(1),
					                               Spanned.SPAN_INCLUSIVE_INCLUSIVE);
				}
 ...
 */
private static void
addImageSpansForBlockMedia(TextView textView, FormattableContent subject,
                           SpannableStringBuilder spannableStringBuilder) {
	...
	// Note: notifications_max_image_size seems to be the max size an ImageSpan
	// can handle, otherwise it would load blank white
	WPCustomImageGetter imageGetter = new WPCustomImageGetter(
		textView, context.getResources().getDimensionPixelSize(
			R.dimen.notifications_max_image_size));
	...
	for (FormattableMedia mediaObject : mediaArray) {
		...
		final Drawable remoteDrawable =
			imageGetter.getDrawable(StringUtils.notNullStr(mediaObject.getUrl()));
		ImageSpan noteImageSpan = new ImageSpan(
			remoteDrawable, StringUtils.notNullStr(mediaObject.getUrl()));
		int startIndex = -1;
		int endIndex = -1;
		List<Integer> indices = (mediaObject.getIndices() != null &&
		                         mediaObject.getIndices().size() == 2)
		                  ? mediaObject.getIndices()
		                  : null;
		if (indices != null) {
		...
		if (startIndex >= 0) {
			...
			if (!spannableHasCharacterAtIndex(spannableStringBuilder, '\n',
			                                  previousCharIndex) ||
			    spannableStringBuilder
			    .getSpans(startIndex, startIndex, ImageSpan.class)
			    .length > 0) {
				imagePlaceholder = "\n ";
			}
			...
			if (!spannableHasCharacterAtIndex(spannableStringBuilder, '\n',
			                                  startIndex) &&
			    !spannableHasCharacterAtIndex(spannableStringBuilder, '\r',
			                                  startIndex)) {
				imagePlaceholder += "\n";
			}
			...
			// Add the image span
			spannableStringBuilder.setSpan(noteImageSpan, spanIndex, spanIndex + 1,
			                               Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

			// Add an AlignmentSpan to center the image
			spannableStringBuilder.setSpan(
				new AlignmentSpan.Standard(Layout.Alignment.ALIGN_CENTER),
				spanIndex, spanIndex + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
...
spannableHasCharacterAtIndex(Spannable spannable, char character, int index) {
	return spannable != null && index < spannable.length() &&
	       spannable.charAt(index) == character;
}

public static boolean
validate2FAuthorizationTokenFromIntentExtras(Intent intent,
                                             TwoFactorAuthCallback callback) {
	// Check for push authorization request
	if (intent != null &&
	    intent.hasExtra(NotificationsUtils.ARG_PUSH_AUTH_TOKEN)) {
		Bundle extras = intent.getExtras();
		String token =
			extras.getString(NotificationsUtils.ARG_PUSH_AUTH_TOKEN, "");
		String title =
			extras.getString(NotificationsUtils.ARG_PUSH_AUTH_TITLE, "");
		String message =
			extras.getString(NotificationsUtils.ARG_PUSH_AUTH_MESSAGE, "");
		long expires =
			extras.getLong(NotificationsUtils.ARG_PUSH_AUTH_EXPIRES, 0);
...
public static void showPushAuthAlert(Context context, final String token,
	...
	builder.setPositiveButton(
		R.string.mnu_comment_approve, new DialogInterface.OnClickListener() {
			@Override
	...
	builder.setNegativeButton(
		R.string.ignore, new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
			        AnalyticsTracker.track(
					AnalyticsTracker.Stat.PUSH_AUTHENTICATION_IGNORED);
			}
		});
...
public static void sendTwoFactorAuthToken(String token) {
	// ping the push auth endpoint with the token, wp.com will take care of the
	// rest!
	...
	WordPress.getRestClientUtilsV1_1().post(
		...
		new RestRequest.ErrorListener() {
			...
			public void onErrorResponse(VolleyError error) {
			        AnalyticsTracker.track(
					AnalyticsTracker.Stat.PUSH_AUTHENTICATION_FAILED);
			}
		});
...
/**
 * Checks if global notifications toggle is enabled in the Android app
 * settings
 */
...
public static Note buildNoteObjectFromBundle(Bundle data) {
	if (data == null) {
		AppLog.e(T.NOTIFS, "Bundle is null! Cannot read '" +
		         GCMMessageService.PUSH_ARG_NOTE_ID + "'.");
		return null;
	}
	...
	String base64FullData =
		data.getString(GCMMessageService.PUSH_ARG_NOTE_FULL_DATA);
	note = Note.buildFromBase64EncodedData(noteId, base64FullData);
	if (note == null) {
		// At this point we don't have the note :(
		AppLog.w(
			T.NOTIFS,
			"Cannot build the Note object by using info available in the PN payload. Please see "
			+
...
public static int findNoteInNoteArray(List<Note> notes,
                                      String noteIdToSearchFor) {
