...
/**
 * topmost view in post detail - shows blavatar + avatar, author name, blog
...
public class ReaderPostDetailHeaderView extends LinearLayout {
  ...
  public void setPost(@NonNull ReaderPost post, boolean isSignedInWPCom) {
    ...
    if (hasBlogName && hasAuthorName) {
      ...
      avatarFrame.setContentDescription(
    ...
    if (mEnableBlogPreview) {
      ...
      int color = ContextExtensionsKt.getColorFromAttribute(getContext(),
                                                            R.attr.wpColorText);
  ...
  private void showBlavatarAndAvatar(String blavatarUrl, String avatarUrl) {
    ...
    int frameSize = getResources().getDimensionPixelSize(
        R.dimen.reader_detail_header_avatar_frame);
    ...
    /*
     * - if there's a blavatar and an avatar, show both of them overlaid using
     * default sizing
     * - if there's only a blavatar, show it the full size of the parent frame
     * and hide the avatar
     * - if there's only an avatar, show it the full size of the parent frame
     * and hide the blavatar
    ...
    avatarFrame.setVisibility(hasAvatar || hasBlavatar ? View.VISIBLE
                                                       : View.GONE);
  ...
  private void toggleFollowStatus(final View followButton) {
    ...
    if (mFollowListener != null) {
      if (isAskingToFollow) {
        mFollowListener.onFollowTapped(followButton, mPost.getBlogName(),
                                       mPost.blogId);
      } else {
    ...
    boolean result;
    if (mPost.isExternal) {
      result = ReaderBlogActions.followFeedById(mPost.feedId, isAskingToFollow,
                                                listener);
    } else {
      result = ReaderBlogActions.followBlogById(mPost.blogId, isAskingToFollow,
                                                listener);
    }
