...
public class ReaderCommentAdapter
  ...
  public interface RequestReplyListener { void onRequestReply(long commentId); }
  ...
  class CommentHolder extends RecyclerView.ViewHolder {
    ...
    CommentHolder(View view) {
      ...
      mTxtText.setMovementMethod(
          ReaderLinkMovementMethod.getInstance(mIsPrivatePost));
    }
  }
  ...
  public ReaderCommentAdapter(Context context, ReaderPost post) {
    ...
    mIndentPerLevel = context.getResources().getDimensionPixelSize(
        R.dimen.reader_comment_indent_per_level);
    mAvatarSz = context.getResources().getDimensionPixelSize(
    ...
    int contentPadding = context.getResources().getDimensionPixelSize(
        R.dimen.reader_card_content_padding);
    int mediumMargin =
        context.getResources().getDimensionPixelSize(R.dimen.margin_medium);
    mContentWidth = displayWidth - (cardMargin * 2) - (contentPadding * 2) -
                    (mediumMargin * 2);

    mColorAuthor = ContextCompat.getColor(context, R.color.primary_40);
    mColorNotAuthor =
        ContextExtensionsKt.getColorFromAttribute(context, R.attr.wpColorText);
  ...
  public void refreshComments() {
    if (mIsTaskRunning) {
      AppLog.w(T.READER,
               "reader comment adapter > Load comments task already running");
    }
  ...
  public boolean isEmpty() { return mComments.size() == 0; }
  ...
  public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        headerHolder.mHeaderView.setOnClickListener(new View.OnClickListener() {
          ...
          public void onClick(View view) {
            ReaderActivityLauncher.showReaderPostDetail(
                view.getContext(), mPost.blogId, mPost.postId);
          }
        });
      }
      return;
    ...
    if (mShowProgressForHighlightedComment &&
        mHighlightCommentId == comment.commentId) {
      dtPublished = new Date();
    } else {
      dtPublished = DateTimeUtils.dateFromIso8601(comment.getPublished());
    }
    commentHolder.mTxtDate.setText(
        DateTimeUtils.javaDateToTimeSpan(dtPublished, WordPress.getContext()));

    String avatarUrl =
        GravatarUtils.fixGravatarUrl(comment.getAuthorAvatar(), mAvatarSz);
    mImageManager.loadIntoCircle(commentHolder.mImgAvatar, ImageType.AVATAR,
                                 avatarUrl);
    ...
    // show indentation spacer for comments with parents and indent it based on
    // comment level
    int indentWidth;
    if (comment.parentId != 0 && comment.level > 0) {
      ...
      RelativeLayout.LayoutParams params =
          (RelativeLayout.LayoutParams)
              commentHolder.mSpacerIndent.getLayoutParams();
    ...
    CommentUtils.displayHtmlComment(commentHolder.mTxtText, comment.getText(),
                                    maxImageWidth);
    ...
    if (mHighlightCommentId != 0 && mHighlightCommentId == comment.commentId) {
      ...
      commentHolder.mProgress.setVisibility(
          mShowProgressForHighlightedComment ? View.VISIBLE : View.GONE);
    } else {
    ...
    if (!mAccountStore.hasAccessToken()) {
      ...
      if (mAnimateLikeCommentId != 0 &&
          mAnimateLikeCommentId == comment.commentId) {
        // simulate tapping on the "Like" button. Add a delay to help the user
        // notice it.
        commentHolder.mCountLikes.postDelayed(new Runnable() {
          @Override
          public void run() {
            ReaderAnim.animateLikeButton(
                commentHolder.mCountLikes.getImageView(), true);
          }
        }, 400);
    ...
    // if we're nearing the end of the comments and we know more exist on the
    // server, fire request to load more
    if (mMoreCommentsExist && mDataRequestedListener != null &&
        (position >= getItemCount() - NUM_HEADERS)) {
  ...
  /*
   * refresh the post from the database - used to reflect changes to comment
   * counts, etc.
   */
  public void refreshPost() {
    if (mPost != null) {
      ReaderPost post =
          ReaderPostTable.getBlogPost(mPost.blogId, mPost.postId, true);
      setPost(post);
    }
  }
  ...
  private void showLikeStatus(final CommentHolder holder, int position) {
    ...
    if (mPost.canLikePost()) {
      ...
      holder.mCountLikes.setContentDescription(ReaderUtils.getLongLikeLabelText(
          holder.mCountLikes.getContext(), comment.numLikes,
  ...
  private void toggleLike(Context context, CommentHolder holder, int position) {
    ...
    ReaderAnim.animateLikeButton(holder.mCountLikes.getImageView(),
                                 isAskingToLike);

    if (!ReaderCommentActions.performLikeAction(
            comment, isAskingToLike, mAccountStore.getAccount().getUserId())) {
      ToastUtils.showToast(context, R.string.reader_toast_err_generic);
      return;
    }

    ReaderComment updatedComment = ReaderCommentTable.getComment(
        comment.blogId, comment.postId, comment.commentId);
    ...
    AnalyticsUtils.trackWithReaderPostDetails(
        isAskingToLike ? AnalyticsTracker.Stat.READER_ARTICLE_COMMENT_LIKED
  ...
  public boolean refreshComment(long commentId) {
    ...
    ReaderComment updatedComment = ReaderCommentTable.getComment(
        comment.blogId, comment.postId, comment.commentId);
    if (updatedComment != null) {
      // copy the comment level over since loading from the DB always has it as
      // 0
  ...
  public void addComment(ReaderComment comment) {
    ...
    // if the comment doesn't have a parent we can just add it to the list of
    // existing comments - but if it does have a parent, we need to reload the
    // list so that it appears under its parent and is correctly indented
  ...
  /*
   * called from post detail when submitted a comment fails - this removes the
   * "fake" comment that was inserted while the API call was still being
   * processed
   */
  ...
  /*
   * sets the passed comment as highlighted with a different background color
   * and an optional progress bar (used when posting new comments) - note that
   * we don't call notifyDataSetChanged() here since in most cases it's
   * unnecessary, so we leave it up to the caller to do that
   */
  ...
  /*
   * returns the position of the passed comment in the adapter, taking the
   * header into account
   */
  ...
  /*
   * sets the passed comment as the one to perform a "Like" on when the list
   * comment list has completed loading
   */
  ...
  private class LoadCommentsTask extends AsyncTask<Void, Void, Boolean> {
    ...
    protected Boolean doInBackground(Void... params) {
      ...
      // determine whether more comments can be downloaded by comparing the
      // number of comments the post says it has with the number of comments
      // actually stored locally for this post
    ...
    protected void onPostExecute(Boolean result) {
      ...
      if (result) {
        // assign the comments with children sorted under their parents and
