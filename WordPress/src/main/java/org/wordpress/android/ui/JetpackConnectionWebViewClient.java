...
class JetpackConnectionWebViewClient extends ErrorManagedWebViewClient {
  ...
  static final String JETPACK_CONNECTION_DEEPLINK =
      "wordpress://jetpack-connection";
  private static final Uri JETPACK_DEEPLINK_URI =
      Uri.parse(JETPACK_CONNECTION_DEEPLINK);

  private final @NonNull JetpackConnectionWebViewClientListener
      mJetpackConnectionListener;
  private final String mSiteUrl;
  private String mRedirectPage;

  JetpackConnectionWebViewClient(
      @NonNull JetpackConnectionWebViewClientListener jetpackConnectionListener,
      @NonNull ErrorManagedWebViewClientListener baseListener, String siteUrl) {
    super(baseListener);
  ...
  public boolean shouldOverrideUrlLoading(WebView view, String stringUrl) {
    try {
      ...
      if (loadedHost.equals(currentSiteHost) && loadedPath != null &&
        ...
        return true;
      } else if (loadedHost.equals(currentSiteHost) && loadedPath != null &&
                 loadedPath.contains(ADMIN_PATH) && mRedirectPage != null) {
        view.loadUrl(mRedirectPage);
        mRedirectPage = null;
        return true;
      } else if (loadedHost.equals(WORDPRESS_COM_HOST) && loadedPath != null &&
                 (loadedPath.equals(WPCOM_LOG_IN_PATH_1) ||
                  loadedPath.equals(WPCOM_LOG_IN_PATH_2)) &&
                 stringUrl.contains(REDIRECT_PARAMETER)) {
        extractRedirect(stringUrl);
      ...
      AppLog.e(AppLog.T.API,
               "Unexpected URL encoding in Jetpack connection flow.", e);
    }
    return false;
  }

  private void extractRedirect(String stringUrl)
      throws UnsupportedEncodingException {
    int from =
        stringUrl.indexOf(REDIRECT_PARAMETER) + REDIRECT_PARAMETER.length();
    ...
    String decodedUrl =
        URLDecoder.decode(redirectUrl, WPWebViewActivity.ENCODING_UTF8);
  ...
  String getRedirectPage() { return mRedirectPage; }

  void setRedirectPage(String redirectPage) { mRedirectPage = redirectPage; }
}
