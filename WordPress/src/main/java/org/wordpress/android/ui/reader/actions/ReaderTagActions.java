...
public class ReaderTagActions {
  private ReaderTagActions() { throw new AssertionError(); }

  public static boolean
  deleteTag(final ReaderTag tag,
    ...
    RestRequest.ErrorListener errorListener = new RestRequest.ErrorListener() {
      ...
      public void onErrorResponse(VolleyError volleyError) {
        // treat it as a success if the error says the user isn't following the
        // deleted tag
  ...
  addTag(final ReaderTag tag,
    ...
    ReaderTag newTag =
        new ReaderTag(tag.getTagSlug(), tag.getTagDisplayName(),
                      tag.getTagTitle(), endpoint, ReaderTagType.FOLLOWED);
    ...
    RestRequest.ErrorListener errorListener = new RestRequest.ErrorListener() {
      @Override
      public void onErrorResponse(VolleyError volleyError) {
        // treat is as a success if we're adding a tag and the error says the
        // user is already following it
        String error = VolleyUtils.errStringFromVolleyError(volleyError);
  ...
  /*
   * return the user's followed tags from the response to
      ...
      "tags": [
          {
              ...
              "title": "Fitness",
              "URL":
     "https://public-api.wordpress.com/rest/v1.1/read/tags/fitness/posts"
          },
          ...
      }
   */
  private static ReaderTagList parseFollowedTags(JSONObject jsonObject) {
    ...
    JSONArray jsonTags =
        jsonObject.optJSONArray(ReaderConstants.JSON_TAG_TAGS_ARRAY);
    ...
    for (int i = 0; i < jsonTags.length(); i++) {
      ...
      String tagTitle = JSONUtils.getStringDecoded(
          jsonThisTag, ReaderConstants.JSON_TAG_TITLE);
      String tagDisplayName = JSONUtils.getStringDecoded(
          jsonThisTag, ReaderConstants.JSON_TAG_DISPLAY_NAME);
      String tagSlug = JSONUtils.getStringDecoded(
          jsonThisTag, ReaderConstants.JSON_TAG_SLUG);
      String endpoint =
          JSONUtils.getString(jsonThisTag, ReaderConstants.JSON_TAG_URL);
      tags.add(new ReaderTag(tagSlug, tagDisplayName, tagTitle, endpoint,
