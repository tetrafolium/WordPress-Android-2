...
public static PersonDetailFragment newInstance(long currentUserId,
                                               long personId,
...
public void onCreate(Bundle savedInstanceState) {
	...
	if (savedInstanceState == null) {
		...
		mPersonType =
			(Person.PersonType)getArguments().getSerializable(ARG_PERSON_TYPE);
	} else {
		...
		mPersonType = (Person.PersonType)savedInstanceState.getSerializable(
			ARG_PERSON_TYPE);
	}
...
public View onCreateView(LayoutInflater inflater, ViewGroup container,
	...
	final ViewGroup rootView = (ViewGroup)inflater.inflate(
		R.layout.person_detail_fragment, container, false);
	...
	mSubscribedDateContainer =
		rootView.findViewById(R.id.subscribed_date_container);
	mSubscribedDateTitleView =
		rootView.findViewById(R.id.subscribed_date_title);
...
public void refreshPersonDetails() {
	...
	if (person != null) {
		int avatarSz =
			getResources().getDimensionPixelSize(R.dimen.people_avatar_sz);
		String avatarUrl =
			GravatarUtils.fixGravatarUrl(person.getAvatarUrl(), avatarSz);

		mImageManager.loadIntoCircle(mAvatarImageView,
		...
		if (mPersonType == Person.PersonType.USER ||
		    mPersonType == Person.PersonType.VIEWER) {
		...
		// Adds extra padding to display name for email followers to make it
		// vertically centered
		...
		AppLog.w(AppLog.T.PEOPLE,
		         "Person returned null from DB for personID: " + mPersonId +
...
// Checks current user's capabilities to decide whether she can change the
// role or not
private void setupRoleContainerForCapability() {
	SiteModel site = mSiteStore.getSiteByLocalId(mLocalTableBlogId);
	boolean isCurrentUser = mCurrentUserId == mPersonId;
	boolean canChangeRole =
		(site != null) && !isCurrentUser && site.getHasCapabilityPromoteUsers();
	if (canChangeRole) {
...
private void showRoleChangeDialog() {
	...
	RoleChangeDialogFragment dialog = RoleChangeDialogFragment.newInstance(
		person.getPersonID(), mSiteStore.getSiteByLocalId(mLocalTableBlogId),
		person.getRole());
