...
public class PostUploadHandler implements UploadHandler<PostModel> {
  ...
  void unregister() { mDispatcher.unregister(this); }
  ...
  public void upload(@NonNull PostModel post) {
    synchronized (sQueuedPostsList) {
      // first check whether there was an old version of this Post still
      // enqueued waiting for being uploaded
      for (PostModel queuedPost : sQueuedPostsList) {
        if (queuedPost.getId() == post.getId()) {
          // we found an older version, so let's remove it and replace it with
          // the newest copy
  ...
  void registerPostForAnalyticsTracking(@NonNull PostModel post) {
    synchronized (sFirstPublishPosts) { sFirstPublishPosts.add(post.getId()); }
  }
  ...
  static boolean isPostUploading(PostModel post) {
    return post != null && sCurrentUploadingPost != null &&
    ...
    synchronized (sQueuedPostsList) {
      if (mCurrentTask == null) { // make sure nothing is running
        ...
        if (sQueuedPostsList.size() > 0) {
          ...
          mCurrentTask.executeOnExecutor(AsyncTask.SERIAL_EXECUTOR,
                                         sCurrentUploadingPost);
        } else {
  ...
  private class UploadPostTask
      extends AsyncTask<PostModel, Boolean, UploadPostTaskResult> {
    ...
    protected void onPostExecute(UploadPostTaskResult result) {
      ...
      case ERROR:
        mPostUploadNotifier
            .incrementUploadedPostCountFromForegroundNotification(mPost);
        mPostUploadNotifier.updateNotificationErrorForPost(mPost, mSite,
                                                           mErrorMessage, 0);
        finishUpload();
        break;
      case NOTHING_TO_UPLOAD:
        // we need to force increment the uploaded count as we know the post was
        // enqueued twice. If we didn't force incremented it, the
        // `PostUploadNotifier.isPostAlreadyInPostCount()` would return true and
        // we'd end up with a dangling upload notification.
        mPostUploadNotifier
            .incrementUploadedPostCountFromForegroundNotification(mPost, true);
        finishUpload();
        break;
    ...
    protected UploadPostTaskResult doInBackground(PostModel... posts) {
      ...
      case UPLOAD:
        AppLog.d(T.POSTS,
                 "PostUploadHandler - UPLOAD. Post: " + mPost.getTitle());
        mDispatcher.dispatch(PostActionBuilder.newPushPostAction(payload));
        break;
      case UPLOAD_AS_DRAFT:
        mPost.setStatus(PostStatus.DRAFT.toString());
        AppLog.d(T.POSTS, "PostUploadHandler - UPLOAD_AS_DRAFT. Post: " +
                              mPost.getTitle());
        mDispatcher.dispatch(PostActionBuilder.newPushPostAction(payload));
        break;
      case REMOTE_AUTO_SAVE:
        AppLog.d(T.POSTS, "PostUploadHandler - REMOTE_AUTO_SAVE. Post: " +
                              mPost.getTitle());
        mDispatcher.dispatch(
            PostActionBuilder.newRemoteAutoSavePostAction(payload));
        break;
      case DO_NOTHING:
        AppLog.d(T.POSTS,
                 "PostUploadHandler - DO_NOTHING. Post: " + mPost.getTitle());
        // A single post might be enqueued twice for upload. It might cause some
        // side-effects when the post is a local draft. The first upload request
        // pushes the post to the server and sets `isLocalDraft` to `false`. The
        // second request would have invoked `Remote_auto_save` on a post which
        // didn't contain any local changes - they were uploaded during the
        // first upload request. This branch takes care of this situations and
        // simply ignores the second request.
    ...
    private void prepareUploadAnalytics(String postContent) {
      // Other methods (like 'uploadNextPost') synchronize over
      // `sQueuedPostsList` before setting
      // `sCurrentUploadingPostAnalyticsProperties` to null. Make sure racing
      // conditions are avoid here by synchronizing over sQueuedPostsList. See
      // https://github.com/wordpress-mobile/WordPress-Android/issues/7990
      synchronized (sQueuedPostsList) {
        // Calculate the words count
        sCurrentUploadingPostAnalyticsProperties = new HashMap<>();
        sCurrentUploadingPostAnalyticsProperties.put(
            "word_count", AnalyticsUtils.getWordCount(mPost.getContent()));
        ...
        if (siteLocalId != -1) {
          ...
          if (selectedSite != null) {
            sCurrentUploadingPostAnalyticsProperties.put(
                "editor_source",
                // making sure to reuse the same logic for both showing
                // Gutenberg and tracking. Note that mIsNewPost is not available
                // as a flag-logic per se outside of EditPostActivity, but the
                // check will pass anyway as long as Gutenberg is enabled and
                // the PostModel contains Gutenberg blocks. As a proxy to
                // mIsNewPost, we're using postModel.isLocalDraft(). The choice
                // is loosely made knowing the other check ("contains blocks")
                // is in place.
        ...
        if (!mHasImage) {
          // Check if there is a img tag in the post. Media added in any editor
          // other than legacy.
          String imageTagsPattern =
              "<img[^>]+src\\s*=\\s*[\"]([^\"]+)[\"][^>]*>";
        ...
        if (!mHasVideo) {
          // Check if there is a video tag in the post. Media added in any
          // editor other than legacy.
    ...
    /**
     * Finds media in post content, uploads them, and returns the HTML to insert
     * in the post
     */
        ...
        if (m.find()) {
          ...
          if (!imageUri.equals("")) {
            MediaModel mediaModel =
                mMediaStore.getMediaForPostWithPath(mPost, imageUri);
            if (mediaModel == null) {
              mIsMediaError = true;
              continue;
            }
            ...
            if (mediaFile != null) {
              mPostUploadNotifier.addMediaInfoToForegroundNotification(
                  mediaModel);
    ...
    private String uploadImage(MediaFile mediaFile) {
      AppLog.i(T.POSTS,
               "PostUploadHandler > UploadImage: " + mediaFile.getFilePath());
      ...
      if (imageUri.toString().contains("content:")) {
        ...
        Cursor cur = mContext.getContentResolver().query(imageUri, projection,
                                                         null, null, null);
    ...
    private String uploadVideo(MediaFile mediaFile) {
      ...
      try {
        ...
        mErrorMessage =
            mContext.getResources().getString(R.string.file_error_create);
        return null;
      }
      ...
      if (videoUri.toString().contains("content:")) {
        ...
        Cursor cur = mContext.getContentResolver().query(videoUri, projection,
                                                         null, null, null);
      ...
      if (videoFile == null) {
        mErrorMessage =
            mContext.getResources().getString(R.string.error_media_upload);
        return null;
      }
      ...
      UploadMediaPayload payload = new UploadMediaPayload(
          mSite, FluxCUtils.mediaModelFromMediaFile(mediaFile),
      ...
      try {
        ...
        AppLog.e(
            T.POSTS,
            "PostUploadHandler > CountDownLatch await interrupted for media file: " +
                mediaFile.getId() + " - " + e);
        mIsMediaError = true;
      }

      MediaModel finishedMedia =
          mMediaStore.getMediaWithLocalId(mediaFile.getId());

      if (finishedMedia == null || finishedMedia.getUploadState() == null ||
          !finishedMedia.getUploadState().equals(
      ...
      if (!TextUtils.isEmpty(finishedMedia.getVideoPressGuid())) {
        ...
        return String.format(
            ...
            xRes, yRes, finishedMedia.getUrl(), mimeType,
            finishedMedia.getUrl());
      }
    }
    ...
    private String uploadImageFile(MediaFile mediaFile, SiteModel site) {
      CountDownLatch countDownLatch = new CountDownLatch(1);
      UploadMediaPayload payload = new UploadMediaPayload(
          site, FluxCUtils.mediaModelFromMediaFile(mediaFile),
          AppPrefs.isStripImageLocation());
      mDispatcher.dispatch(MediaActionBuilder.newUploadMediaAction(payload));
  ...
  /**
   * Has priority 9 on OnPostUploaded events, which ensures that
   * PostUploadHandler is the first to receive and process OnPostUploaded
  ...
  public void onPostUploaded(OnPostUploaded event) {
    ...
    if (event.isError()) {
      AppLog.w(T.POSTS, "PostUploadHandler > Post upload failed. " +
                            event.error.type + ": " + event.error.message);
      Context context = WordPress.getContext();
      String errorMessage = mUiHelpers.getTextOfUiString(
          context,
          UploadUtils.getErrorMessageResIdFromPostError(
              PostStatus.fromPost(event.post), event.post.isPage(), event.error,
              mUploadActionUseCase.isEligibleForAutoUpload(site, event.post)));
      String notificationMessage =
          UploadUtils.getErrorMessage(context, event.post, errorMessage, false);
      mPostUploadNotifier.removePostInfoFromForegroundNotification(
          event.post, mMediaStore.getMediaForPost(event.post));
      ...
      mPostUploadNotifier.updateNotificationErrorForPost(
          event.post, site, notificationMessage, 0);
      ...
      boolean isFirstTimePublish =
          sFirstPublishPosts.remove(event.post.getId());
      mPostUploadNotifier.updateNotificationSuccessForPost(event.post, site,
                                                           isFirstTimePublish);
      if (isFirstTimePublish) {
        if (sCurrentUploadingPostAnalyticsProperties != null) {
          sCurrentUploadingPostAnalyticsProperties.put(
              "post_id", event.post.getRemotePostId());
        } else {
          sCurrentUploadingPostAnalyticsProperties = new HashMap<>();
        }
        PostUtils.addPostTypeToAnalyticsProperties(
      ...
      synchronized (sQueuedPostsList) {
        for (PostModel post : sQueuedPostsList) {
          if (post.getId() == event.post.getId()) {
            // Check if a new version of the post we've just uploaded is in the
            // queue and update its state
