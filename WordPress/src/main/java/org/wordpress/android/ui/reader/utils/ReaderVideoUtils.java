...
public class ReaderVideoUtils {
  private ReaderVideoUtils() { throw new AssertionError(); }

  /*
   * determine whether we can show a thumbnail image for the passed video -
   * currently we support YouTube, Vimeo & standard images
   */
  public static boolean canShowVideoThumbnail(String videoUrl) {
    return isVimeoLink(videoUrl) || isYouTubeVideoLink(videoUrl) ||
        MediaUtils.isValidImage(videoUrl);
  }

  public static void
  retrieveVideoThumbnailUrl(final String videoUrl,
                            final VideoThumbnailUrlListener listener) {
    // if this is a YouTube video we can determine the thumbnail url from the
    // passed url, otherwise check if we've already cached the thumbnail url for
    // this video
    String thumbnailUrl;
    ...
    if (!TextUtils.isEmpty(thumbnailUrl)) {
      ...
      ReaderVideoUtils.requestVimeoThumbnail(
          videoUrl, new FetchVideoThumbnailListener() {
            @Override
  ...
  /*
   * returns the url to get the full-size (480x360) thumbnail url for the passed
   * video see http://www.reelseo.com/youtube-thumbnail-image/ for other sizes
   */
  public static String getYouTubeThumbnailUrl(final String videoUrl) {
    ...
    // note that this *must* use https rather than http - ex:
    // https://img.youtube.com/vi/ClbE019cLNI/0.jpg
  ...
  private static String getYouTubeVideoId(final String link) {
    ...
    try {
      ...
      if (host.equals("youtube.com") || host.equals("www.youtube.com")) {
        // if link contains "watch" in the path, then the id is in the "v="
        // query param
        if (link.contains("watch")) {
          return uri.getQueryParameter("v");
        }
        // if the link contains "embed" in the path, then the id is the last
        // path segment ex:
        // https://www.youtube.com/embed/fw3w68YrKwc?version=3&#038;rel=1&#038;
      ...
      // youtu.be urls have the videoId as the path - ex:
      // http://youtu.be/pEnXclbO9jg
   ...
   */
  public static void
  requestVimeoThumbnail(final String videoUrl,
                        final FetchVideoThumbnailListener thumbListener) {
    ...
    JsonArrayRequest request =
        new JsonArrayRequest(url, listener, errorListener);
