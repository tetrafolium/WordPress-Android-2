...
public static void startService(Context context, long blogId, long postId,
...
public static void startServiceForComment(Context context, long blogId,
...
public int onStartCommand(Intent intent, int flags, int startId) {
	...
	if (requestNextPage) {
		int prevPage =
			ReaderCommentTable.getLastPageNumberForPost(blogId, postId);
	...
	updateCommentsForPost(
		blogId, postId, mCurrentPage, new UpdateResultListener() {
			@Override
			public void onUpdateResult(UpdateResult result) {
			        if (commentId > 0) {
			                if (ReaderCommentTable.commentExists(blogId, postId, commentId) ||
			                    !result.isNewOrChanged()) {
			                        EventBus.getDefault().post(
							new ReaderEvents.UpdateCommentsEnded(result));
			                        stopSelf();
					} else {
...
private static void updateCommentsForPost(
	final long blogId, final long postId, final int pageNumber,
	final ReaderActions.UpdateResultListener resultListener) {
	String path =
		"sites/" + blogId + "/posts/" + postId + "/replies/"
		+ "?number=" +
		Integer.toString(ReaderConstants.READER_MAX_COMMENTS_TO_REQUEST) +
		"&meta=likes"
		+ "&force=wpcom"
		+ "&hierarchical=true"
		+ "&order=ASC"
	...
	RestRequest.Listener listener = new RestRequest.Listener() {
		...
		public void onResponse(JSONObject jsonObject) {
			handleUpdateCommentsResponse(jsonObject, blogId, postId, pageNumber,
			                             resultListener);
		}
	};
	...
	WordPress.getRestClientUtilsV1_1().get(path, null, null, listener,
	                                       errorListener);
}

private static void handleUpdateCommentsResponse(
	final JSONObject jsonObject, final long blogId, final long postId,
	...
	new Thread() {
		...
		public void run() {
			...
			try {
				// purge existing comments if this was a request for the first page of
				// comments
				if (pageNumber == 1) {
				...
				if (jsonCommentList != null) {
					for (int i = 0; i < jsonCommentList.length(); i++) {
						...
						ReaderComment comment =
							ReaderComment.fromJson(jsonComment, blogId);
						comment.pageNumber = pageNumber;
						serverComments.add(comment);

						// extract and save likes for this comment
						JSONObject jsonLikes =
							JSONUtils.getJSONChild(jsonComment, "meta/data/likes");
						if (jsonLikes != null) {
							ReaderUserList likingUsers =
								ReaderUserList.fromJsonLikes(jsonLikes);
							ReaderUserTable.addOrUpdateUsers(likingUsers);
							ReaderLikeTable.setLikesForComment(comment,
							                                   likingUsers.getUserIds());
						}
					}
				}
				...
				// save to db regardless of whether any are new so changes to likes
				// are stored
