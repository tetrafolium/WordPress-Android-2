...
public class SelectCategoriesActivity extends AppCompatActivity {
  public static final String KEY_SELECTED_CATEGORY_IDS =
  ...
  private LongSparseArray<Integer> mCategoryRemoteIdsToListPositions =
      new LongSparseArray<>();
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    mListScrollPositionManager =
        new ListScrollPositionManager(mListView, false);
  ...
  private void populateCategoryList() {
    CategoryNode categoryTree = CategoryNode.createCategoryTreeFromList(
        mTaxonomyStore.getCategoriesForSite(mSite));
    mCategoryLevels =
        CategoryNode.getSortedListOfCategoriesFromRoot(categoryTree);
    for (int i = 0; i < mCategoryLevels.size(); i++) {
      mCategoryRemoteIdsToListPositions.put(
          mCategoryLevels.get(i).getCategoryId(), i);
    }

    CategoryArrayAdapter categoryAdapter = new CategoryArrayAdapter(
        this, R.layout.categories_row, mCategoryLevels);
      ...
      for (Long selectedCategory : mSelectedCategories) {
        if (mCategoryRemoteIdsToListPositions.get(selectedCategory) != null) {
          mListView.setItemChecked(
              mCategoryRemoteIdsToListPositions.get(selectedCategory), true);
        }
      }
    }
  ...
  private void updateSelectedCategoryList() {
    ...
    for (int i = 0; i < selectedItems.size(); i++) {
      ...
      long categoryRemoteId =
          mCategoryLevels.get(selectedItems.keyAt(i)).getCategoryId();
  ...
  private void saveAndFinish() {
    ...
    updateSelectedCategoryList();
    bundle.putSerializable(KEY_SELECTED_CATEGORY_IDS,
                           new ArrayList<>(mSelectedCategories));
      ...
      if (event.isError()) {
        if (!isFinishing()) {
          ToastUtils.showToast(SelectCategoriesActivity.this,
                               R.string.category_refresh_error, Duration.LONG);
        }
      } else {
        populateCategoryList();
      }
  ...
  public void onTermUploaded(OnTermUploaded event) {
    ...
    if (event.isError()) {
      if (!isFinishing()) {
        ToastUtils.showToast(SelectCategoriesActivity.this,
                             R.string.adding_cat_failed, Duration.LONG);
      }
    } else {
      ...
      if (!isFinishing()) {
        ToastUtils.showToast(SelectCategoriesActivity.this,
                             R.string.adding_cat_success, Duration.SHORT);
      }
    }
  }
}
