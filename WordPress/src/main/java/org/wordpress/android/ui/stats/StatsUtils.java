...
public static String getCurrentDate() {
	SimpleDateFormat sdf = new SimpleDateFormat(
		StatsConstants.STATS_INPUT_DATE_FORMAT, Locale.ROOT);
...
public static void openPostInReaderOrInAppWebview(
	Context ctx, final long remoteBlogID, final String remoteItemID,
	@Nullable final String itemType, final String itemURL) {
	...
	if (itemType == null) {
		...
		AppLog.d(AppLog.T.UTILS,
		         "Type of the item is null. Opening it in the in-app browser: " +
		         itemURL);
		WPWebViewActivity.openURL(ctx, itemURL);
	} else if (itemType.equals(StatsConstants.ITEM_TYPE_POST)) {
		// If the post/page has ID == 0 is the home page, and we need to load the
		// blog preview, otherwise 404 is returned if we try to show the post in
		// the reader
		if (itemID == 0) {
...
public static String countryCodeToEmoji(String countryCode) {
	...
	return new String(Character.toChars(char1)) +
	       new String(Character.toChars(char2));
}
}
