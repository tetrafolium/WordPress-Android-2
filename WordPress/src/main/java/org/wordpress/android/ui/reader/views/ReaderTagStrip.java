...
/**
 * used by the detail view to display the primary and secondary tags from a
 * reader post
 */
public class ReaderTagStrip extends LinearLayout {
  ...
  private void addTag(@NonNull final String tagName) {
    String tagDisplayName = mNumTags > 0
                                ? ", " + ReaderUtils.makeHashTag(tagName)
                                : ReaderUtils.makeHashTag(tagName);

    // inflate a new textView to show this tag
    LayoutInflater inflater = LayoutInflater.from(getContext());
    TextView txtTag = (TextView)inflater.inflate(
        R.layout.reader_tag_strip_label, mView, false);
