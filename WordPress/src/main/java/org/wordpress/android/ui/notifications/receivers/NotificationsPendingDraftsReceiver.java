...
public class NotificationsPendingDraftsReceiver extends BroadcastReceiver {
  ...
  private static final long MAX_DAYS_TO_SHOW_DAYS_IN_MESSAGE =
      40; // just over a month
  ...
  public void onReceive(Context context, Intent intent) {
    ...
    // for the case of being spanned after device restarts, get the latest
    // drafts and check the lastUpdated
    String action = intent.getAction();
    if (action != null &&
        action.equals("android.intent.action.BOOT_COMPLETED")) {
      AppLog.i(AppLog.T.NOTIFS,
               "entering Pending Drafts Receiver from BOOT_COMPLETED");
      ...
      if (site != null) {
        ...
        for (PostModel post : draftPosts) {
          // reschedule next notifications for each local draft post we have, as
          // we have just been rebooted
          PendingDraftsNotificationsUtils.scheduleNextNotifications(context,
                                                                    post);
        }
      }
    } else {
    ...
    if (postId != 0) {
      ...
      if (post != null) {
        ...
        long dateLastUpdated =
            DateTimeUtils.timestampFromIso8601(post.getDateLocallyChanged());
        ...
        if (daysInDraft < MAX_DAYS_TO_SHOW_DAYS_IN_MESSAGE) {
          String formattedString =
              context.getString(R.string.pending_draft_one_generic);
          ...
          if (dateLastUpdated < oneMonthAgo) {
            formattedString =
                context.getString(R.string.pending_draft_one_month);
            ...
            if (result == 0) {
              formattedString =
                  context.getString(R.string.pending_draft_one_week_1);
            } else {
              formattedString =
                  context.getString(R.string.pending_draft_one_week_2);
            }
            ...
            if (result == 0) {
              formattedString =
                  context.getString(R.string.pending_draft_one_day_1);
            } else {
              formattedString =
                  context.getString(R.string.pending_draft_one_day_2);
            }
          }

          buildSinglePendingDraftNotification(context, post.getTitle(),
                                              formattedString, postId, isPage);
        } else {
          // if it's been more than MAX_DAYS_TO_SHOW_DAYS_IN_MESSAGE days, or if
          // we don't know (i.e. value for lastUpdated is zero) then just show a
          // generic message
          buildSinglePendingDraftNotificationGeneric(context, post.getTitle(),
                                                     postId, isPage);
        }
      }
    }
  ...
  private void buildSinglePendingDraftNotification(Context context,
                                                   String postTitle,
                                                   String formattedMessage,
                                                   int postId, boolean isPage) {
    buildNotificationWithIntent(
        context, getResultIntentForOnePost(context, postId, isPage),
        String.format(formattedMessage, getPostTitle(context, postTitle)),
        postId, isPage);
  }

  private void buildSinglePendingDraftNotificationGeneric(Context context,
                                                          String postTitle,
                                                          int postId,
                                                          boolean isPage) {
    buildNotificationWithIntent(
  ...
  private PendingIntent getResultIntentForOnePost(Context context, int postId,
                                                  boolean isPage) {
  ...
  private void buildNotificationWithIntent(Context context,
                                           PendingIntent intent, String message,
                                           int postId, boolean isPage) {
    NotificationCompat.Builder builder = NativeNotificationsUtils.getBuilder(
        context, context.getString(R.string.notification_channel_important_id));
    ...
    NativeNotificationsUtils.showMessageToUserWithBuilder(
        builder, message, false,
        PendingDraftsNotificationsUtils.makePendingDraftNotificationId(postId),
        context);
  }

  private void
  addOpenDraftActionForNotification(Context context,
    ...
    PendingIntent pendingIntent = PendingIntent.getActivity(
        context,
        // need to add + 2 so the request code is different, otherwise they
        // overlap
        BASE_REQUEST_CODE + 1 +
            PendingDraftsNotificationsUtils.makePendingDraftNotificationId(
                postId),
        openDraftIntent,
        PendingIntent.FLAG_CANCEL_CURRENT | PendingIntent.FLAG_UPDATE_CURRENT);
    builder.addAction(R.drawable.ic_pencil_white_24dp,
                      context.getText(R.string.edit), pendingIntent);
  }

  private void
  addIgnoreActionForNotification(Context context,
                                 NotificationCompat.Builder builder, int postId,
                                 boolean isPage) {
    // Call processing service when user taps on IGNORE - we should remember
    // this decision for this post
    Intent ignoreIntent =
    ...
    builder.addAction(R.drawable.ic_close_white_24dp,
                      context.getText(R.string.ignore), ignorePendingIntent);
  }

  private void
  addDismissActionForNotification(Context context,
                                  NotificationCompat.Builder builder,
                                  int postId, boolean isPage) {
    // Call processing service when notification is dismissed
    Intent notificationDeletedIntent =
        new Intent(context, NotificationsProcessingService.class);
    ...
    PendingIntent dismissPendingIntent = PendingIntent.getService(
        context,
        // need to add + 3 so the request code is different, otherwise they
        // overlap
        BASE_REQUEST_CODE + 3 +
            PendingDraftsNotificationsUtils.makePendingDraftNotificationId(
                postId),
