...
public class WPMainActivity extends AppCompatActivity
  ...
  public static final String ARG_CONTINUE_JETPACK_CONNECT =
  ...
  public static final String ARG_IS_MAGIC_LINK_LOGIN =
      "ARG_IS_MAGIC_LINK_LOGIN";
  public static final String ARG_IS_MAGIC_LINK_SIGNUP =
      "ARG_IS_MAGIC_LINK_SIGNUP";
  public static final String ARG_JETPACK_CONNECT_SOURCE =
  ...
  public static final String ARG_SHOW_ZENDESK_NOTIFICATIONS =
      "show_zendesk_notifications";

  // Track the first `onResume` event for the current session so we can use it
  ...
  /*
   * fragments implement this if their contents can be scrolled, called when
   * user requests to scroll to the top
   */
  public interface OnScrollToTopListener { void onScrollToTop(); }

  /*
   * fragments implement this and return true if the fragment handles the back
   * button and doesn't want the activity to handle it as well
   */
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    mConnectionBar.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        // slide out the bar on click, then re-check connection after a brief
        // delay
    ...
    mIsMagicLinkLogin =
        getIntent().getBooleanExtra(ARG_IS_MAGIC_LINK_LOGIN, false);
    mIsMagicLinkSignup =
        getIntent().getBooleanExtra(ARG_IS_MAGIC_LINK_SIGNUP, false);
    mJetpackConnectSource =
        (JetpackConnectionSource)getIntent().getSerializableExtra(
            ARG_JETPACK_CONNECT_SOURCE);
    ...
    if (savedInstanceState == null) {
      ...
      if (FluxCUtils.isSignedInWPComOrHasWPOrgSite(mAccountStore, mSiteStore)) {
        ...
        boolean openedFromShortcut =
            (getIntent() != null &&
             getIntent().getStringExtra(
                 ShortcutsNavigator.ACTION_OPEN_SHORTCUT) != null);
        boolean openRequestedPage =
            (getIntent() != null && getIntent().hasExtra(ARG_OPEN_PAGE));
        boolean isQuickStartRequestedFromPush =
            (getIntent() != null &&
             getIntent().getBooleanExtra(MySiteFragment.ARG_QUICK_START_TASK,
                                         false));
        boolean openZendeskTicketsFromPush =
            (getIntent() != null && getIntent().getBooleanExtra(
                                        ARG_SHOW_ZENDESK_NOTIFICATIONS, false));
        ...
        if (openZendeskTicketsFromPush) {
          ...
          if (getIntent().hasExtra(
                  NotificationsPendingDraftsReceiver.POST_ID_EXTRA)) {
            launchWithPostId(
                getIntent().getIntExtra(
                    NotificationsPendingDraftsReceiver.POST_ID_EXTRA, 0),
                getIntent().getBooleanExtra(
                    NotificationsPendingDraftsReceiver.IS_PAGE_EXTRA, false));
          } else {
            launchWithNoteId();
          }
        } else if (openedFromShortcut) {
          initSelectedSite();
          mShortcutsNavigator.showTargetScreen(
              getIntent().getStringExtra(
          ...
          // when app is opened from Quick Start reminder switch to MySite
          // fragment
          ...
          if (mIsMagicLinkLogin) {
            if (mAccountStore.hasAccessToken()) {
              ToastUtils.showToast(this,
                                   R.string.login_already_logged_in_wpcom);
            } else {
              authTokenToSet = getAuthToken();
            }
          }
          // Continue Jetpack connect flow if coming from login/signup magic
          // link.
          if (getIntent() != null && getIntent().getExtras() != null &&
              getIntent().getExtras().getBoolean(ARG_CONTINUE_JETPACK_CONNECT,
                                                 false)) {
            JetpackConnectionWebViewActivity.startJetpackConnectionFlow(
                this, NOTIFICATIONS,
                (SiteModel)getIntent().getSerializableExtra(SITE),
    ...
    // ensure the deep linking activity is enabled. It may have been disabled
    // elsewhere and failed to get re-enabled
    WPActivityUtils.enableComponent(this, ReaderPostPagerActivity.class);

    // monitor whether we're not the default app
    trackDefaultApp();

    // We need to register the dispatcher here otherwise it won't trigger if for
    // example Site Picker is present
    mDispatcher.register(this);
    EventBus.getDefault().register(this);

    if (authTokenToSet != null) {
      // Save Token to the AccountStore. This will trigger a
      // onAuthenticationChanged.
      UpdateTokenPayload payload = new UpdateTokenPayload(authTokenToSet);
      mDispatcher.dispatch(
          AccountActionBuilder.newUpdateAccessTokenAction(payload));
      ...
      ActivityLauncher.showLoginEpilogue(
          this, getIntent().getBooleanExtra(ARG_DO_LOGIN_UPDATE, false),
          getIntent().getIntegerArrayListExtra(ARG_OLD_SITES_IDS));
    } else if (getIntent().getBooleanExtra(ARG_SHOW_SIGNUP_EPILOGUE, false) &&
               savedInstanceState == null) {
      canShowAppRatingPrompt = false;
      ActivityLauncher.showSignupEpilogue(
          this,
          getIntent().getStringExtra(
          ...
          getIntent().getStringExtra(
              SignupEpilogueActivity.EXTRA_SIGNUP_PHOTO_URL),
          getIntent().getStringExtra(
              SignupEpilogueActivity.EXTRA_SIGNUP_USERNAME),
          false);
    }
    ...
    if (isGooglePlayServicesAvailable(this)) {
      ...
      GCMRegistrationIntentService.enqueueWork(
          this, new Intent(this, GCMRegistrationIntentService.class));
    }
  ...
  public boolean isGooglePlayServicesAvailable(Activity activity) {
    ...
    case ConnectionResult.SUCCESS:
      return true;
    // Play Services unavailable, show an error dialog is the Play Services Lib
    // needs an update
  ...
  private @Nullable String getAuthToken() {
    ...
    return uri != null ? uri.getQueryParameter(LoginActivity.TOKEN_PARAMETER)
                       : null;
  }
  ...
  private void handleOpenPageIntent(Intent intent) {
    ...
    if (!TextUtils.isEmpty(pagePosition)) {
      ...
      AppLog.e(
          T.MAIN,
          "WPMainActivity.handleOpenIntent called with an invalid argument.");
    }
  }

  private void registeNewsItemObserver() {
    mNewsManager.notificationBadgeVisibility().observe(
  ...
  private void launchZendeskMyTickets() {
    ...
    // leave the Main activity showing the ME page, so when the user comes back
    // from Help&Support the app is in the right section.
    mBottomNav.setCurrentSelectedPage(PageType.ME);
  ...
  /*
   * called when app is launched from a push notification, switches to the
   * notification page and opens the desired note detail
   */
  private void launchWithNoteId() {
    ...
    if (getIntent().hasExtra(NotificationsUtils.ARG_PUSH_AUTH_TOKEN)) {
      ...
      NotificationsUtils.validate2FAuthorizationTokenFromIntentExtras(
          getIntent(), new NotificationsUtils.TwoFactorAuthCallback() {
            @Override
            public void onTokenValid(String token, String title,
                                     String message) {
              // we do this here instead of using the service in the background
              // so we make sure the user opens the app by using an activity
              // (and thus unlocks the screen if locked, for security).
              String actionType = getIntent().getStringExtra(
                  NotificationsProcessingService.ARG_ACTION_TYPE);
              if (NotificationsProcessingService.ARG_ACTION_AUTH_APPROVE.equals(
                      actionType)) {
                // ping the push auth endpoint with the token, wp.com will take
                // care of the rest!
                NotificationsUtils.sendTwoFactorAuthToken(token);
              } else {
                NotificationsUtils.showPushAuthAlert(WPMainActivity.this, token,
                                                     title, message);
              }
            }
            ...
            public void onTokenInvalid() {
              ...
              ToastUtils.showToast(WPMainActivity.this,
                                   R.string.push_auth_expired,
                                   ToastUtils.Duration.LONG);
              AnalyticsTracker.track(
                  AnalyticsTracker.Stat.PUSH_AUTHENTICATION_EXPIRED);
            }
          });
    }
    ...
    // it could be that a notification has been tapped but has been removed by
    // the time we reach here. It's ok to compare to <=1 as it could be zero
    // then.
    if (GCMMessageService.getNotificationsCount() <= 1) {
      String noteId =
          getIntent().getStringExtra(NotificationsListFragment.NOTE_ID_EXTRA);
      if (!TextUtils.isEmpty(noteId)) {
        GCMMessageService.bumpPushNotificationsTappedAnalytics(noteId);
        // if voice reply is enabled in a wearable, it will come through the
        // remoteInput extra EXTRA_VOICE_OR_INLINE_REPLY
        String voiceReply = null;
        Bundle remoteInput = RemoteInput.getResultsFromIntent(getIntent());
        if (remoteInput != null) {
          CharSequence replyText = remoteInput.getCharSequence(
              GCMMessageService.EXTRA_VOICE_OR_INLINE_REPLY);
          if (replyText != null) {
        ...
        if (voiceReply != null) {
          NotificationsProcessingService.startServiceForReply(this, noteId,
                                                              voiceReply);
          finish();
          // we don't want this notification to be dismissed as we still have to
          // make sure we processed the voice reply, so we exit this function
          // immediately
          return;
        } else {
          boolean shouldShowKeyboard = getIntent().getBooleanExtra(
              NotificationsListFragment.NOTE_INSTANT_REPLY_EXTRA, false);
          NotificationsListFragment.openNoteForReply(
              this, noteId, shouldShowKeyboard, null,
              NotesAdapter.FILTERS.FILTER_ALL, true);
        }
      } else {
        AppLog.e(T.NOTIFS,
                 "app launched from a PN that doesn't have a note_id in it!!");
        return;
      }
    } else {
  ...
  /**
   * called from an internal pending draft notification, so the user can land in
   * the local draft and take action such as finish editing and publish, or
   * delete the post, etc.
   */
  private void launchWithPostId(int postId, boolean isPage) {
    ...
    AnalyticsTracker.track(
        AnalyticsTracker.Stat.NOTIFICATION_PENDING_DRAFTS_TAPPED);
  ...
  protected void onResume() {
    ...
    // ensure the deep linking activity is enabled. We might be returning from
    // the external-browser viewing of a post
    WPActivityUtils.enableComponent(this, ReaderPostPagerActivity.class);

    // We need to track the current item on the screen when this activity is
    // resumed. Ex: Notifications -> notifications detail -> back to
    // notifications
    ...
    if (currentPageType == PageType.NOTIFS) {
      // if we are presenting the notifications list, it's safe to clear any
  ...
  private void checkQuickStartNotificationStatus() {
    if (getSelectedSite() != null && NetworkUtils.isNetworkAvailable(this) &&
        ...
        !mQuickStartStore.getQuickStartNotificationReceived(
            getSelectedSite().getId())) {
      CompleteQuickStartPayload payload = new CompleteQuickStartPayload(
          getSelectedSite(), NEXT_STEPS.toString());
      mDispatcher.dispatch(
          SiteActionBuilder.newCompleteQuickStartAction(payload));
    }
  }

  private void announceTitleForAccessibility(PageType pageType) {
    getWindow().getDecorView().announceForAccessibility(
        mBottomNav.getContentDescriptionForPageType(pageType));
  }

  @Override
  public void onBackPressed() {
    // let the fragment handle the back button if it implements our
    // OnParentBackPressedListener
    Fragment fragment = mBottomNav.getActiveFragment();
    if (fragment instanceof OnActivityBackPressedListener) {
      boolean handled =
          ((OnActivityBackPressedListener)fragment).onActivityBackPressed();
      if (handled) {
        return;
      }
    }
  ...
  private void showBottomNav(boolean show) {
    ...
    findViewById(R.id.navbar_separator)
  ...
  public void onPageChanged(int position) {
    ...
    if (getMySiteFragment() != null) {
      ...
      if (pageType == PageType.READER &&
          getMySiteFragment().isQuickStartTaskActive(
  ...
  public void onNewPostButtonClicked() {
    if (!mSiteStore.hasSite()) {
      // No site yet - Move to My Sites fragment that shows the create new site
      // screen
      mBottomNav.setCurrentSelectedPage(PageType.MY_SITE);
      return;
    }
  ...
  private void updateTitle(PageType pageType) {
    if (pageType == PageType.MY_SITE && mSelectedSite != null) {
      ((MainToolbarFragment)mBottomNav.getActiveFragment())
          .setTitle(mSelectedSite.getName());
    } else {
      ((MainToolbarFragment)mBottomNav.getActiveFragment())
    ...
    case MY_SITE:
      ...
      if (trackAnalytics) {
        AnalyticsUtils.trackWithSiteDetails(
            AnalyticsTracker.Stat.MY_SITE_ACCESSED, getSelectedSite());
      }
      break;
    case READER:
  ...
  private void trackDefaultApp() {
    Intent wpcomIntent =
        new Intent(Intent.ACTION_VIEW,
                   Uri.parse(getString(R.string.wordpresscom_sample_post)));
    ResolveInfo resolveInfo = getPackageManager().resolveActivity(
        wpcomIntent, PackageManager.MATCH_DEFAULT_ONLY);
    if (resolveInfo != null &&
        !getPackageName().equals(resolveInfo.activityInfo.name)) {
      // not set as default handler so, track this to evaluate. Note, a
      // resolver/chooser might be the default.
      AnalyticsUtils.trackWithDefaultInterceptor(
          AnalyticsTracker.Stat.DEEP_LINK_NOT_DEFAULT_HANDLER,
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    case RequestCodes.EDIT_POST:
      ...
      final SiteModel site =
          (SiteModel)data.getSerializableExtra(WordPress.SITE);
      ...
      if (site != null && post != null) {
        UploadUtils.handleEditPostResultSnackbars(
            this, mDispatcher, findViewById(R.id.coordinator), data, post, site,
            mUploadActionUseCase.getUploadAction(post),
            new View.OnClickListener() {
              @Override
              public void onClick(View v) {
                UploadUtils.publishPost(WPMainActivity.this, post, site,
                                        mDispatcher);
              }
            });
      }
    ...
    case RequestCodes.CREATE_SITE:
      ...
      if (data != null) {
        int newSiteLocalID =
            data.getIntExtra(SitePickerActivity.KEY_LOCAL_ID, -1);
        SiteUtils.enableBlockEditorOnSiteCreation(mDispatcher, mSiteStore,
                                                  newSiteLocalID);
        ...
        if (newSiteModel != null) {
          AppPrefs.setShowGutenbergInfoPopupForTheNewPosts(
              newSiteModel.getUrl(), true);
        }
      }
    ...
    case RequestCodes.ADD_ACCOUNT:
      if (resultCode == RESULT_OK) {
        // Register for Cloud messaging
        startWithNewAccount();
      } else if (!FluxCUtils.isSignedInWPComOrHasWPOrgSite(mAccountStore,
                                                           mSiteStore)) {
        // can't do anything if user isn't signed in (either to wp.com or
        // self-hosted)
        finish();
      }
      break;
    ...
    case RequestCodes.SITE_PICKER:
      if (getMySiteFragment() != null) {
        ...
        boolean isSameSiteSelected =
            data != null && data.getIntExtra(SitePickerActivity.KEY_LOCAL_ID,
                                             -1) == AppPrefs.getSelectedSite();
        ...
        if (data != null &&
            data.getIntExtra(ARG_CREATE_SITE, 0) == RequestCodes.CREATE_SITE) {
          showQuickStartDialog();
        }
      }
      break;
    ...
    case RequestCodes.NOTE_DETAIL:
      if (getNotificationsListFragment() != null) {
        getNotificationsListFragment().onActivityResult(requestCode, resultCode,
                                                        data);
      }
      break;
    case RequestCodes.PHOTO_PICKER:
      Fragment fragment = mBottomNav.getActiveFragment();
      if (fragment instanceof MeFragment || fragment instanceof
                                                MySiteFragment) {
  ...
  private void showQuickStartDialog() {
    if (AppPrefs.isQuickStartDisabled() || getSelectedSite() == null ||
    ...
    promoDialog.initialize(
        tag, getString(R.string.quick_start_dialog_need_help_title),
  ...
  public void onAuthenticationChanged(OnAuthenticationChanged event) {
    if (event.isError()) {
      if (mSelectedSite != null &&
        ...
        AuthenticationDialogUtils.showAuthErrorView(this, mSiteStore,
                                                    mSelectedSite);
      }
  ...
  public void onQuickStartCompleted(OnQuickStartCompleted event) {
    if (getSelectedSite() != null && !event.isError()) {
      // as long as we get any response that is not an error mark quick start
      // notification as received
      mQuickStartStore.setQuickStartNotificationReceived(event.site.getId(),
                                                         true);
    }
  }
  ...
  public void onAccountChanged(OnAccountChanged event) {
    // Sign-out is handled in `handleSiteRemoved`, no need to show the signup
    // flow here
    if (mAccountStore.hasAccessToken()) {
  ...
  /**
   * Bumps stats related to a magic link sign up provided the account has been
   * updated with the username and email address needed to refresh analytics
   * meta data.
   */
  private void trackMagicLinkSignupIfNeeded() {
    ...
    if (!TextUtils.isEmpty(account.getUserName()) &&
      ...
      mLoginAnalyticsListener.trackCreatedAccount(account.getUserName(),
                                                  account.getEmail());
  ...
  @Subscribe(threadMode = ThreadMode.MAIN)
  public void
  onEventMainThread(NotificationEvents.NotificationsUnseenStatus event) {
    mBottomNav.showNoteBadge(event.hasUnseenNotes);
  }
  ...
  @Subscribe(threadMode = ThreadMode.MAIN)
  public void
  onEventMainThread(ConnectionChangeReceiver.ConnectionChangeEvent event) {
   ...
   */
  public @Nullable SiteModel getSelectedSite() { return mSelectedSite; }
  ...
  /**
   * This should not be moved to a SiteUtils.getSelectedSite() or similar static
   * method. We don't want this to be used globally like
   * WordPress.getCurrentBlog() was used. The state is maintained by this
   * Activity and the selected site parameter is passed along to other
  ...
  public void initSelectedSite() {
    ...
    if (siteLocalId != -1) {
      ...
      // If saved site exist, then return, else (site has been removed?) try to
      // select another site
      if (mSelectedSite != null) {
        updateTitle();
        return;
  ...
  public void onPostUploaded(OnPostUploaded event) {
    // WPMainActivity never stops listening for the Dispatcher events and as a
    // result it tries to show the SnackBar even when another activity is in the
    // foreground. However, this has a tricky side effect, as if the Activity in
    // the foreground is showing a Snackbar the SnackBar is dismissed as soon as
    // the WPMainActivity invokes show(). This condition makes sure, the
    // WPMainActivity invokes show() only when it's visible. For more info see
    // https://github.com/wordpress-mobile/WordPress-Android/issues/9604
    if (getLifecycle().getCurrentState().isAtLeast(STARTED)) {
      SiteModel site = getSelectedSite();
      if (site != null && event.post != null &&
          event.post.getLocalSiteId() == site.getId()) {
        UploadUtils.onPostUploadedSnackbarHandler(
            this, findViewById(R.id.coordinator), event.isError(), event.post,
            null, site, mDispatcher);
      }
    }
  }
  ...
  public void onSiteChanged(OnSiteChanged event) {
    ...
    // It would be better if the OnSiteChanged provided the list of changed
    // sites.
  ...
  public void onSiteEditorsChanged(OnSiteEditorsChanged event) {
    // When the site editor details are loaded from the remote backend, make
    // sure to set a default if empty
    if (event.isError()) {
      return;
    }
  ...
  @Subscribe(threadMode = ThreadMode.MAIN)
  public void
  onAllSitesMobileEditorChanged(OnAllSitesMobileEditorChanged event) {
    if (event.isError()) {
      return;
    }
    if (event.isNetworkResponse) {
      // We can remove the global app setting now, since we're sure the
      // migration ended with success.
  ...
  private void
  refreshCurrentSelectedSiteAfterEditorChanges(boolean alwaysRefreshUI,
                                               int localSiteID) {
  ...
  // because of the bottom nav implementation (we only get callback after active
  // fragment is changed) we need to manage SnackBar in Activity, instead of
  // Fragment
  ...
  // SnackBar sometimes do not appear when another SnackBar is still visible,
  // even in other activities (weird)
  @Override
