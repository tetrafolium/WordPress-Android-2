...
/**
 * Displayed after user selects multiple items from the WP media library to
 * insert into a post - provides a choice between inserting them individually or
 * as a gallery
 */
public class InsertMediaDialog extends AppCompatDialogFragment {
  public enum InsertType { INDIVIDUALLY, GALLERY }
  ...
  public static InsertMediaDialog
  newInstance(@NonNull InsertMediaCallback callback, @NonNull SiteModel site) {
    InsertMediaDialog dialog = new InsertMediaDialog();
    dialog.setStyle(AppCompatDialogFragment.STYLE_NORMAL,
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container,
    ...
    View view =
        localInflater.inflate(R.layout.insert_media_dialog, container, false);

    mInsertRadioGroup =
        (RadioGroup)view.findViewById(R.id.radio_group_insert_type);
    mGalleryRadioGroup =
        (RadioGroup)view.findViewById(R.id.radio_group_gallery_type);
    mNumColumnsContainer =
        (ViewGroup)view.findViewById(R.id.num_columns_container);
    mNumColumnsSeekBar =
        (SeekBar)mNumColumnsContainer.findViewById(R.id.seekbar_num_columns);

    mInsertRadioGroup.setOnCheckedChangeListener(
    ...
    if (enableGalleryType) {
      mGalleryRadioGroup.setOnCheckedChangeListener(
          new RadioGroup.OnCheckedChangeListener() {
            @Override
    ...
    mNumColumnsSeekBar.setOnSeekBarChangeListener(
  ...
  public void onActivityCreated(Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      ...
      int galleryTypeOrdinal =
          savedInstanceState.getInt(STATE_GALLERY_TYPE_ORD);
  ...
  public InsertType getInsertType() { return mInsertType; }
  ...
  private void setInsertType(@NonNull InsertType insertType) {
    ...
    @IdRes
    int radioId =
        insertType == InsertType.GALLERY ? R.id.radio_insert_as_gallery
                                         : R.id.radio_insert_individually;
    ...
    ViewGroup container =
        (ViewGroup)getView().findViewById(R.id.container_gallery_settings);
  ...
  private void setGalleryType(@NonNull GalleryType galleryType) {
    ...
    if (showNumColumns &&
        mNumColumnsContainer.getVisibility() != View.VISIBLE) {
      AniUtils.fadeIn(mNumColumnsContainer, AniUtils.Duration.SHORT);
    } else if (!showNumColumns &&
               mNumColumnsContainer.getVisibility() == View.VISIBLE) {
    ...
    // scale out the gallery type image, then set the new image and scale it
    // back in
    final ImageView imageView =
        (ImageView)getView().findViewById(R.id.image_gallery_type);
    AniUtils.scaleOut(imageView, View.VISIBLE, AniUtils.Duration.SHORT,
                      new AniUtils.AnimationEndListener() {
                        @Override
  ...
  private void setNumColumns(int numColumns, boolean fromSeekBar) {
    // seekbar is zero-based, so increment the column count if this was called
    // from it
    if (fromSeekBar) {
    ...
    TextView textValue =
        (TextView)getView().findViewById(R.id.text_num_columns_label);
    if (mNumColumns == 1) {
      textValue.setText(getString(R.string.media_gallery_column_count_single));
    } else {
      textValue.setText(String.format(
          getString(R.string.media_gallery_column_count_multi), mNumColumns));
    }
  }
}
