...
// A dialog preference that displays settings for a NotificationSettings Channel
// and Type
public class NotificationsSettingsDialogPreference extends DialogPreference {
  ...
  private OnNotificationsSettingsChangedListener
      mOnNotificationsSettingsChangedListener;

  public interface OnNotificationsSettingsChangedListener {
    void onSettingsChanged(Channel channel, Type type, long siteId,
                           JSONObject newValues);
  }

  public NotificationsSettingsDialogPreference(
      Context context, AttributeSet attrs, Channel channel, Type type,
      long blogId, NotificationsSettings settings,
  ...
  protected View onCreateDialogView() {
    ...
    int spacerHeight = getContext().getResources().getDimensionPixelSize(
        R.dimen.margin_medium);
    spacerView.setLayoutParams(new ViewGroup.LayoutParams(
        ActionBar.LayoutParams.MATCH_PARENT, spacerHeight));
  ...
  private View configureLayoutForView(LinearLayout view) {
    ...
    String[] settingsArray = new String[0], settingsValues = new String[0],
             summaryArray = new String[0];
    String typeString = mType.toString();

    switch (mChannel) {
    case BLOGS:
      settingsJson =
          JSONUtils.queryJSON(mSettings.getBlogSettings().get(mBlogId),
                              typeString, new JSONObject());
      settingsArray = getContext().getResources().getStringArray(
          R.array.notifications_blog_settings);
      settingsValues = getContext().getResources().getStringArray(
          R.array.notifications_blog_settings_values);
      break;
    case OTHER:
      settingsJson = JSONUtils.queryJSON(mSettings.getOtherSettings(),
                                         typeString, new JSONObject());
      settingsArray = getContext().getResources().getStringArray(
          R.array.notifications_other_settings);
      settingsValues = getContext().getResources().getStringArray(
    ...
    case WPCOM:
      ...
      settingsArray = getContext().getResources().getStringArray(
          R.array.notifications_wpcom_settings);
      settingsValues = getContext().getResources().getStringArray(
          R.array.notifications_wpcom_settings_values);
      summaryArray = getContext().getResources().getStringArray(
          R.array.notifications_wpcom_settings_summaries);
      break;
    }
    ...
    if (settingsJson != null && settingsArray.length == settingsValues.length) {
      for (int i = 0; i < settingsArray.length; i++) {
        ...
        View commentsSetting = View.inflate(
            getContext(), R.layout.notifications_settings_switch, null);
        TextView title =
            commentsSetting.findViewById(R.id.notifications_switch_title);
        ...
        if (mChannel == Channel.WPCOM && i < summaryArray.length) {
          ...
          TextView summary =
              commentsSetting.findViewById(R.id.notifications_switch_summary);
          summary.setVisibility(View.VISIBLE);
          summary.setText(summaryText);
        }

        final SwitchCompat toggleSwitch =
            commentsSetting.findViewById(R.id.notifications_switch);
        toggleSwitch.setChecked(
            JSONUtils.queryJSON(settingsJson, settingValue, true));
        ...
        public void onCheckedChanged(CompoundButton compoundButton,
          ...
          try {
            ...
            AppLog.e(AppLog.T.NOTIFS,
                     "Could not add notification setting change to JSONObject");
          }
        }
      };
  ...
  protected void onDialogClosed(boolean positiveResult) {
    if (positiveResult && mUpdatedJson.length() > 0 &&
        mOnNotificationsSettingsChangedListener != null) {
      mOnNotificationsSettingsChangedListener.onSettingsChanged(
          mChannel, mType, mBlogId, mUpdatedJson);
      ...
      while (keys.hasNext()) {
        ...
        mSettings.updateSettingForChannelAndType(
            mChannel, mType, settingName, mUpdatedJson.optBoolean(settingName),
            mBlogId);
      }
    }
  }
