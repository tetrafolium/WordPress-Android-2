...
public class SiteSettingsFormatDialog
	...
	public String[] getEntries(@NonNull Context context) {
		if (this == FormatType.DATE_FORMAT) {
			return context.getResources().getStringArray(
				R.array.date_format_entries);
		} else {
			return context.getResources().getStringArray(
				R.array.time_format_entries);
		}
	}
...
public static SiteSettingsFormatDialog
newInstance(@NonNull FormatType formatType, @NonNull String formatValue) {
...
public Dialog onCreateDialog(Bundle savedInstanceState) {
	View view =
		View.inflate(getActivity(), R.layout.site_settings_format_dialog, null);
	...
	@StringRes
	int titleRes = formatType == FormatType.DATE_FORMAT
	               ? R.string.site_settings_date_format_title
	               : R.string.site_settings_time_format_title;
	...
	txtHelp.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
			        ActivityLauncher.openUrlExternal(v.getContext(),
			                                         Constants.URL_DATETIME_FORMAT_HELP);
			}
		});
...
private void createRadioButtons() {
	...
	for (int i = 0; i < mEntries.length; i++) {
		...
		CompoundButtonCompat.setButtonTintList(
			radio, getResources().getColorStateList(
		...
		LinearLayout.LayoutParams params =
			(LinearLayout.LayoutParams)radio.getLayoutParams();
	...
	mRadioGroup.setOnCheckedChangeListener(
		new RadioGroup.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
			        mEditCustomFormat.setEnabled(
					isCustomFormatEntry(mEntries[checkedId]));
			}
		});
}
