...
public static boolean followBlogById(final long blogId,
	...
	if (isAskingToFollow) {
		AnalyticsUtils.trackWithSiteId(AnalyticsTracker.Stat.READER_BLOG_FOLLOWED,
		                               blogId);
	} else {
		AnalyticsUtils.trackWithSiteId(
			AnalyticsTracker.Stat.READER_BLOG_UNFOLLOWED, blogId);
	}

	final String actionName = (isAskingToFollow ? "follow" : "unfollow");
	final String path =
		"sites/" + blogId + "/follows/" +
		(isAskingToFollow ? "new?source=android" : "mine/delete");
	...
	RestRequest.ErrorListener errorListener = new RestRequest.ErrorListener() {
		...
		public void onErrorResponse(VolleyError volleyError) {
			...
			// check if we get a 403 when unfollowing - this will happen when we
			// attempt to unfollow a blog that no longer exists - the workaround is
			// to unfollow by url - note that the v1.2 endpoint will return a 404 in
			// this situation
...
private static void
internalUnfollowBlogByUrl(long blogId, final ActionListener actionListener) {
	...
	String path =
		"/read/following/mine/delete?url=" + UrlUtils.urlEncode(blogUrl);
	WordPress.getRestClientUtilsV1_1().post(path, listener, errorListener);
}
...
public static boolean followFeedById(final long feedId,
	...
	if (blogInfo != null) {
		return internalFollowFeed(blogInfo.feedId, blogInfo.getFeedUrl(),
	...
	updateFeedInfo(feedId, null, new UpdateBlogInfoListener() {
			...
			public void onResult(ReaderBlog blogInfo) {
			        if (blogInfo != null) {
			                internalFollowFeed(blogInfo.feedId, blogInfo.getFeedUrl(),
			                                   isAskingToFollow, actionListener);
				} else {
			                ReaderActions.callActionListener(actionListener, false);
				}
...
public static void followFeedByUrl(final String feedUrl,
	...
	ReaderBlog blogInfo =
		ReaderBlogTable.getFeedInfo(ReaderBlogTable.getFeedIdFromUrl(feedUrl));
	if (blogInfo != null) {
		internalFollowFeed(blogInfo.feedId, blogInfo.getFeedUrl(), true,
		                   actionListener);
		return;
	}
	...
	updateFeedInfo(0, feedUrl, new UpdateBlogInfoListener() {
			@Override
			public void onResult(ReaderBlog blogInfo) {
			        // note we attempt to follow even when the look up fails (blogInfo =
			        // null) because that endpoint doesn't perform feed discovery, whereas
			        // the endpoint to follow a feed does
			        long feedIdToFollow = blogInfo != null ? blogInfo.feedId : 0;
			        String feedUrlToFollow = (blogInfo != null && blogInfo.hasFeedUrl())
			             ? blogInfo.getFeedUrl()
			             : feedUrl;
			        internalFollowFeed(feedIdToFollow, feedUrlToFollow, true,
			                           actionListener);
			}
		});
}
...
internalFollowFeed(final long feedId, final String feedUrl,
                   final boolean isAskingToFollow,
                   final ActionListener actionListener) {
	// feedUrl is required
	if (TextUtils.isEmpty(feedUrl)) {
	...
	com.wordpress.rest.RestRequest.Listener listener =
		...
		public void onResponse(JSONObject jsonObject) {
			...
			if (success) {
				...
				AppLog.w(T.READER, "feed " + actionName + " failed - " +
				         jsonToString(jsonObject) + " - " + path);
 ...
 */
private static void localRevertFollowBlogId(long blogId,
                                            boolean isAskingToFollow) {
	ReaderBlogTable.setIsFollowedBlogId(blogId, !isAskingToFollow);
	ReaderPostTable.setFollowStatusForPostsInBlog(blogId, !isAskingToFollow);
}

private static void localRevertFollowFeedId(long feedId,
                                            boolean isAskingToFollow) {
	ReaderBlogTable.setIsFollowedFeedId(feedId, !isAskingToFollow);
	ReaderPostTable.setFollowStatusForPostsInFeed(feedId, !isAskingToFollow);
}
 ...
 */
private static boolean isFollowActionSuccessful(JSONObject json,
                                                boolean isAskingToFollow) {
	if (json == null) {
		return false;
	}
	...
	if (json.has("subscribed")) {
		...
		// site/$site/follows/
		isSubscribed =
			json.has("is_following") && json.optBoolean("is_following", false);
	}
 ...
 */
public static void updateBlogInfo(long blogId, final String blogUrl,
	...
	RestRequest.ErrorListener errorListener = new RestRequest.ErrorListener() {
		@Override
		public void onErrorResponse(VolleyError volleyError) {
			// authentication error may indicate that API access has been disabled
			// for this blog
			...
			// if we failed to get the blog info using the id and this isn't an
			// authentication error, try again using just the domain
			if (!isAuthErr && hasBlogId && hasBlogUrl) {
				AppLog.w(T.READER,
				         "failed to get blog info by id, retrying with url");
	...
	if (hasBlogId) {
		WordPress.getRestClientUtilsV1_1().get("read/sites/" + blogId, listener,
		                                       errorListener);
	} else {
...
public static void updateFeedInfo(long feedId, String feedUrl,
                                  final UpdateBlogInfoListener infoListener) {
	// must pass either a valid id or url
...
private static void
handleUpdateBlogInfoResponse(JSONObject jsonObject,
                             UpdateBlogInfoListener infoListener) {
	if (jsonObject == null) {
		if (infoListener != null) {
			infoListener.onResult(null);
		}
 ...
 * and does not account for 404 replacement pages used by ISPs such as Charter
 */
public static void
checkUrlReachable(final String blogUrl,
	...
	Response.ErrorListener errorListener = new Response.ErrorListener() {
		...
		public void onErrorResponse(VolleyError volleyError) {
			AppLog.e(T.READER, volleyError);
			int statusCode;
			// check specifically for auth failure class rather than relying on
			// status code since a redirect to an unauthorized url may return a 301
			// rather than a 401
	...
	// TODO: this should be a HEAD request, but even though Volley supposedly
	// supports HEAD using it results in "java.lang.IllegalStateException:
	// Unknown method type"
	StringRequest request =
		new StringRequest(Request.Method.GET, blogUrl, listener, errorListener);
	WordPress.sRequestQueue.add(request);
}

/*
 * block a blog - result includes the list of posts that were deleted by the
 * block so they can be restored if the user undoes the block
 */
public static BlockedBlogResult
blockBlogFromReader(final long blogId, final ActionListener actionListener) {
...
public static void
undoBlockBlogFromReader(final BlockedBlogResult blockResult) {
	if (blockResult == null) {
		return;
	}
	...
	com.wordpress.rest.RestRequest.Listener listener =
		...
		public void onResponse(JSONObject jsonObject) {
			boolean success =
				(jsonObject != null && jsonObject.optBoolean("success"));
			...
			if (success && blockResult.wasFollowing) {
				...
				AppLog.w(T.READER,
				         "failed to unblock blog " + blockResult.blogId);
			}
		}
	};
	RestRequest.ErrorListener errorListener = new RestRequest.ErrorListener() {
	...
	String path =
		"me/block/sites/" + Long.toString(blockResult.blogId) + "/delete";
	WordPress.getRestClientUtilsV1_1().post(path, listener, errorListener);
}
}
