...
public class PublicizeButtonPrefsFragment extends PublicizeBaseFragment
...
private final ArrayList<PublicizeButton> mPublicizeButtons =
	new ArrayList<>();
...
public static PublicizeButtonPrefsFragment
newInstance(@NonNull SiteModel site) {
...
public void onCreate(@Nullable Bundle savedInstanceState) {
	...
	mSiteSettings =
		SiteSettingsInterface.getInterface(getActivity(), mSite, this);
}
...
public View onCreateView(LayoutInflater inflater,
	...
	ViewGroup view = (ViewGroup)inflater.inflate(
		R.layout.publicize_button_prefs_fragment, container, false);

	mPrefButtonStyle = (WPPrefView)view.findViewById(R.id.pref_button_style);
	mPrefSharingButtons =
		(WPPrefView)view.findViewById(R.id.pref_sharing_buttons);
	...
	mPrefAllowCommentLikes =
		(WPPrefView)view.findViewById(R.id.pref_allow_comment_likes);
	...
	mSharingDisabledNotification =
		view.findViewById(R.id.sharing_disabled_notification);
 ...
 *
 * @param isSharingButtons true if called by mPrefSharingButtons, false if by
 *     mPrefMoreButtons
 */
private void saveSharingButtons(boolean isSharingButtons) {
	...
	// sharing buttons are visible and enabled, more buttons are invisible and
	// enabled, all others are invisible and disabled
	...
	WordPress.getRestClientUtilsV1_1().setSharingButtons(
		Long.toString(mSite.getSiteId()), jsonObject,
...
/*
 * show the twitter username pref only if there's a twitter sharing button
 * enabled
 */
...
private void configureSharingButtons() {
	WordPress.getRestClientUtilsV1_1().getSharingButtons(
		Long.toString(mSite.getSiteId()),
		new RestRequest.Listener() {
			@Override
			public void onResponse(JSONObject response) {
			        configureSharingButtonsFromResponse(response);
...
private void configureSharingButtonsFromResponse(JSONObject response) {
	...
	if (jsonArray == null) {
		AppLog.w(AppLog.T.SETTINGS,
		         "Publicize sharing buttons missing from response");
		return;
	}
...
private void getSiteSettings(boolean shouldFetchSettings) {
	if (mSiteSettings == null) {
		// mSiteSettings should not be null here, but we've had some cases where
		// it's null and the app crashed. See #6890
		if (mSite == null) {
			ToastUtils.showToast(getActivity(), R.string.blog_not_found,
			                     ToastUtils.Duration.SHORT);
			getActivity().finish();
		...
		// this creates a default site settings interface - the actual settings
		// will be retrieved when getSiteSettings() is called
		mSiteSettings =
			SiteSettingsInterface.getInterface(getActivity(), mSite, this);
	}
...
private void setPreferencesFromSiteSettings() {
	...
	try {
		...
		mSharingDisabledNotification.setVisibility(
			sharingModuleEnabled ? View.GONE : View.VISIBLE);
		mSharingSettingsWrapper.setVisibility(sharingModuleEnabled ? View.VISIBLE
		                                                 : View.GONE);

		mPrefLabel.setTextEntry(mSiteSettings.getSharingLabel());
		mPrefButtonStyle.setSummary(
		...
		String[] values =
			getResources().getStringArray(R.array.sharing_button_style_array);
