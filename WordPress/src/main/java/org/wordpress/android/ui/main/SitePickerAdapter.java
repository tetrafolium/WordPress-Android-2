...
public class SitePickerAdapter
  ...
  public interface HeaderHandler {
    RecyclerView.ViewHolder onCreateViewHolder(LayoutInflater layoutInflater,
                                               ViewGroup parent,
  ...
  public SitePickerAdapter(Context context, @LayoutRes int itemLayoutResourceId,
                           int currentLocalBlogId, String lastSearch,
                           boolean isInSearchMode,
                           OnDataLoadedListener dataLoadedListener) {
    this(context, itemLayoutResourceId, currentLocalBlogId, lastSearch,
         isInSearchMode, dataLoadedListener, null, null);
  }
  ...
  public SitePickerAdapter(Context context, @LayoutRes int itemLayoutResourceId,
    ...
    mBlavatarSz =
        context.getResources().getDimensionPixelSize(R.dimen.blavatar_sz);
    mTextColorNormal =
  ...
  private int getPositionOffset() { return (mHeaderHandler == null ? 0 : 1); }

  void
  setOnSelectedCountChangedListener(OnSelectedCountChangedListener listener) {
  ...
  @Override
  public void
  onBindViewHolder(@NonNull final RecyclerView.ViewHolder viewHolder,
                   int position) {
    ...
    if (holder.mIsSiteHidden == null ||
        holder.mIsSiteHidden != site.mIsHidden) {
      holder.mIsSiteHidden = site.mIsHidden;
      holder.mTxtTitle.setTextColor(site.mIsHidden ? mTextColorHidden
                                                   : mTextColorNormal);
      holder.mTxtTitle.setTypeface(holder.mTxtTitle.getTypeface(),
                                   site.mIsHidden ? Typeface.NORMAL
                                                  : Typeface.BOLD);
    ...
    if (holder.mDivider != null) {
      ...
      boolean showDivider = site.mIsRecentPick && !mIsInSearchMode &&
      ...
      holder.itemView.setOnClickListener(new View.OnClickListener() {
        ...
        public void onClick(View view) {
          ...
          if (isValidPosition(clickedPosition)) {
            ...
            AppLog.w(AppLog.T.MAIN, "site picker > invalid clicked position " +
                                        clickedPosition);
          }
        }
      });
        ...
        public boolean onLongClick(View view) {
          ...
          if (isValidPosition(clickedPosition)) {
            if (mIsMultiSelectEnabled) {
              ...
              return mSiteSelectedListener.onSiteLongClick(
  ...
  public String getLastSearch() { return mLastSearch; }

  public void setLastSearch(String lastSearch) { mLastSearch = lastSearch; }

  public boolean getIsInSearchMode() { return mIsInSearchMode; }
      ...
      for (SiteRecord site : sites) {
        ...
        if (index > -1) {
          ...
          if (siteRecord.mIsHidden == makeVisible) {
            ...
            if (!makeVisible && siteRecord.mLocalId != currentSiteId &&
  ...
  private SiteList filteredSitesByText(SiteList sites) {
    ...
    for (int i = 0; i < sites.size(); i++) {
      ...
      String siteNameLowerCase =
          record.mBlogName.toLowerCase(Locale.getDefault());
      String hostNameLowerCase = record.mHomeURL.toLowerCase(Locale.ROOT);

      if (siteNameLowerCase.contains(
              mLastSearch.toLowerCase(Locale.getDefault())) ||
          hostNameLowerCase.contains(mLastSearch.toLowerCase(Locale.ROOT))) {
    ...
    protected SiteList[] doInBackground(Void... params) {
      ...
      Collections.sort(sites, new Comparator<SiteRecord>() {
        public int compare(SiteRecord site1, SiteRecord site2) {
          ...
          return site1.getBlogNameOrHomeURL().compareToIgnoreCase(
              site2.getBlogNameOrHomeURL());
        }
      });

      // flag recently-picked sites and move them to the top if there are enough
      // sites and the user isn't searching
  ...
  public static class SiteRecord {
    ...
    public void setHidden(boolean hidden) { mIsHidden = hidden; }

    public String getHomeURL() { return mHomeURL; }

    public String getBlavatarUrl() { return mBlavatarUrl; }
  ...
  public static class SiteList extends ArrayList<SiteRecord> {
    ...
    boolean isSameList(SiteList sites) {
      ...
      for (SiteRecord site : sites) {
        i = indexOfSite(site);
        if (i == -1 || this.get(i).mIsHidden != site.mIsHidden ||
