...
public static void animateLikeButton(final View target,
	...
	animateButton(target, isAskingToLike ? ReaderButton.LIKE_ON
	                                 : ReaderButton.LIKE_OFF);
}
...
private static void animateButton(final View target, ReaderButton button) {
	...
	ObjectAnimator animX =
		ObjectAnimator.ofFloat(target, View.SCALE_X, 1f, 1.2f);
	animX.setRepeatMode(ValueAnimator.REVERSE);
	animX.setRepeatCount(1);

	ObjectAnimator animY =
		ObjectAnimator.ofFloat(target, View.SCALE_Y, 1f, 1.2f);
	...
	case LIKE_ON:
	case LIKE_OFF:
		// rotate like button +/- 72 degrees (72 = 360/5, 5 is the number of
		// points in the star)
		float endRotate = (button == ReaderButton.LIKE_ON ? 72f : -72f);
		ObjectAnimator animRotate =
			ObjectAnimator.ofFloat(target, View.ROTATION, 0f, endRotate);
