...
public class PublicizeServiceAdapter
	extends RecyclerView.Adapter<PublicizeServiceAdapter.SharingViewHolder> {
...
public PublicizeServiceAdapter(Context context, long siteId,
                               long currentUserId) {
	super();
	((WordPress)context.getApplicationContext()).component().inject(this);
	mSiteId = siteId;
	mBlavatarSz =
		context.getResources().getDimensionPixelSize(R.dimen.blavatar_sz_small);
...
public SharingViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
	View view =
		...
		.inflate(R.layout.publicize_listitem_service, parent, false);
...
public void onBindViewHolder(final SharingViewHolder holder, int position) {
	...
	final PublicizeConnectionList connections =
		mConnections.getServiceConnectionsForUser(mCurrentUserId,
		                                          service.getId());

	holder.mTxtService.setText(service.getLabel());
	String iconUrl = PhotonUtils.getPhotonImageUrl(service.getIconUrl(),
	                                               mBlavatarSz, mBlavatarSz);
...
private boolean isHiddenService(PublicizeService service) {
	boolean shouldHideGooglePlus =
		service.getId().equals(PublicizeConstants.GOOGLE_PLUS_ID) &&
		mShouldHideGPlus;
...
protected Boolean doInBackground(Void... params) {
	PublicizeConnectionList connections =
	...
	return !(mTmpServices.isSameAs(mServices) &&
	         mTmpConnections.isSameAs(mConnections));
}
...
private void sortConnections() {
	Collections.sort(mServices, new Comparator<PublicizeService>() {
				...
				public int compare(PublicizeService lhs, PublicizeService rhs) {
				        boolean isLhsConnected =
						mConnections.isServiceConnectedForUser(mCurrentUserId, lhs);
				        boolean isRhsConnected =
						mConnections.isServiceConnectedForUser(mCurrentUserId, rhs);
