...
public class EditTextPreferenceWithValidation
    extends SummaryEditTextPreference {
  ...
  public EditTextPreferenceWithValidation(Context context) { super(context); }
  ...
  protected void showDialog(Bundle state) {
    ...
    if (positiveButton != null) {
      ...
      positiveButton.setTextColor(ContextCompat.getColorStateList(
          getContext(), R.color.primary_40_neutral_40_selector));
        ...
        public void afterTextChanged(Editable s) {
          ...
          case PASSWORD_SELF_HOSTED:
            positiveButton.setEnabled(
                ValidationUtils.validatePasswordSelfHosted(s));
          }
        }
      });
    }
    ...
    // Use "hidden" input type for passwords so characters are replaced with
    // dots for added security.
    hideInputCharacters(mValidationType == ValidationType.PASSWORD ||
                        mValidationType == ValidationType.PASSWORD_SELF_HOSTED);
  }
  ...
  public void
  setStringToIgnoreForPrefilling(String stringToIgnoreForPrefilling) {
  ...
  private void hideInputCharacters(boolean hide) {
    ...
    getEditText().setTransformationMethod(
        hide ? PasswordTransformationMethod.getInstance() : null);
