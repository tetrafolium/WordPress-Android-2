...
/**
 * displays a row of image thumbnails from a reader post - only shows when two
 * or more images of a minimum size are found
 */
...
private void initView(Context context) {
	...
	mThumbnailHeight = context.getResources().getDimensionPixelSize(
		R.dimen.reader_thumbnail_strip_image_height);
...
public void loadThumbnails(long blogId, long postId, boolean isPrivate) {
	...
	final EnumSet<PhotoViewerOption> photoViewerOptions =
		EnumSet.of(PhotoViewerOption.IS_GALLERY_IMAGE);
	if (isPrivate) {
	...
	for (final String imageUrl : imageList) {
		View view =
			inflater.inflate(R.layout.reader_thumbnail_strip_image, mView, false);
		ImageView imageView = view.findViewById(R.id.thumbnail_strip_image);
		mView.addView(view);

		String photonUrl = PhotonUtils.getPhotonImageUrl(
			imageUrl, mThumbnailWidth, mThumbnailHeight);
		ImageManager.getInstance().load(imageView, ImageType.PHOTO, photonUrl,
		...
		imageView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View view) {
						...
						view.getContext(), imageUrl, content, view, photoViewerOptions, 0,
						0);
				}
			});
