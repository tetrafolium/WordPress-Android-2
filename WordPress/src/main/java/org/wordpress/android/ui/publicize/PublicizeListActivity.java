...
public class PublicizeListActivity extends AppCompatActivity
	implements PublicizeActions.OnPublicizeActionListener,
...
private void showDetailFragment(PublicizeService service) {
	...
	Fragment detailFragment =
		PublicizeDetailFragment.newInstance(mSite, service);
	getSupportFragmentManager()
	.beginTransaction()
...
private void showWebViewFragment(PublicizeService service,
	...
	Fragment webViewFragment = PublicizeWebViewFragment.newInstance(
		mSite, service, publicizeConnection);
	getSupportFragmentManager()
	.beginTransaction()
...
/*
 * user requested to reconnect a broken publicizeConnection from the detail
 * fragment
 */
@Override
public void onRequestReconnect(PublicizeService service,
                               PublicizeConnection publicizeConnection) {
...
/*
 * user requested to disconnect a service publicizeConnection from the detail
 * fragment
 */
@Override
...
private void
confirmDisconnect(final PublicizeConnection publicizeConnection) {
	AlertDialog.Builder builder = new AlertDialog.Builder(
		new ContextThemeWrapper(this, R.style.Calypso_Dialog_Alert));
	builder.setMessage(
		String.format(getString(R.string.dlg_confirm_publicize_disconnect),
		              publicizeConnection.getLabel()));
	builder.setTitle(R.string.share_btn_disconnect);
	builder.setCancelable(true);
	builder.setPositiveButton(
		R.string.share_btn_disconnect, new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int id) {
			        PublicizeActions.disconnect(publicizeConnection);
			        // if the user disconnected from G+, return to the list fragment
			        // since the detail fragment would give them the ability to
			        // reconnect
			        if (publicizeConnection.getService().equals(
					    PublicizeConstants.GOOGLE_PLUS_ID)) {
...
public void onEventMainThread(PublicizeEvents.ActionCompleted event) {
	...
	if (event.didSucceed()) {
		...
		if (event.getAction() == ConnectAction.CONNECT) {
			AnalyticsUtils.trackWithSiteDetails(Stat.PUBLICIZE_SERVICE_CONNECTED,
			                                    mSite, analyticsProperties);
		} else if (event.getAction() == ConnectAction.DISCONNECT) {
			AnalyticsUtils.trackWithSiteDetails(Stat.PUBLICIZE_SERVICE_DISCONNECTED,
			                                    mSite, analyticsProperties);
		}
	} else {
...
@Subscribe(threadMode = ThreadMode.MAIN)
public void
onEventMainThread(PublicizeEvents.ActionRequestChooseAccount event) {
	if (isFinishing()) {
		return;
	}
	...
	PublicizeAccountChooserDialogFragment dialogFragment =
		new PublicizeAccountChooserDialogFragment();
	Bundle args = new Bundle();
	args.putString(PublicizeConstants.ARG_CONNECTION_ARRAY_JSON,
	               event.getJSONObject().toString());
	args.putSerializable(WordPress.SITE, site);
	args.putString(PublicizeConstants.ARG_SERVICE_ID, event.getServiceId());
	dialogFragment.setArguments(args);
	dialogFragment.show(getSupportFragmentManager(),
	                    PublicizeAccountChooserDialogFragment.TAG);
}

@Override
public void onButtonPrefsClicked() {
	AnalyticsUtils.trackWithSiteDetails(Stat.OPENED_SHARING_BUTTON_MANAGEMENT,
	                                    mSite);
