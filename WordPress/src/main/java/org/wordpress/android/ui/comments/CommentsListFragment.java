...
public class CommentsListFragment extends Fragment {
  ...
  public enum CommentStatusCriteria implements FilterCriteria {
    ...
    public static CommentStatusCriteria
    fromCommentStatus(CommentStatus status) {
  ...
  private final CommentStatusCriteria[] mCommentStatuses = {
      CommentStatusCriteria.ALL, CommentStatusCriteria.UNAPPROVED,
      CommentStatusCriteria.APPROVED, CommentStatusCriteria.TRASH,
      CommentStatusCriteria.SPAM};

  private EmptyViewMessageType mEmptyViewMessageType =
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if (savedInstanceState == null) {
      SmartToast.show(getActivity(),
                      SmartToast.SmartToastType.COMMENTS_LONG_PRESS);
    }
  }
  ...
  private CommentAdapter getAdapter() {
    if (mAdapter == null) {
      ...
      CommentAdapter.OnDataLoadedListener dataLoadedListener =
          new CommentAdapter.OnDataLoadedListener() {
            @Override
            public void onDataLoaded(boolean isEmpty) {
              ...
              if (!isEmpty) {
                // Hide the empty view if there are already some displayed
                // comments
                mFilteredCommentsView.hideEmptyView();
                mFilteredCommentsView.setToolbarScrollFlags(
                    AppBarLayout.LayoutParams.SCROLL_FLAG_SCROLL |
                    AppBarLayout.LayoutParams.SCROLL_FLAG_ENTER_ALWAYS);
                mActionableEmptyView.setVisibility(View.GONE);
              } else if (!mIsUpdatingComments) {
                // Change LOADING to NO_CONTENT message
                mFilteredCommentsView.updateEmptyView(
                    EmptyViewMessageType.NO_CONTENT);
              }
            }
          };

      // adapter calls this to request more comments from server when it reaches
      // the end
      ...
      CommentAdapter.OnCommentPressedListener pressedListener =
          new CommentAdapter.OnCommentPressedListener() {
            @Override
  ...
  private boolean hasAdapter() { return (mAdapter != null); }
  ...
  public void onActivityCreated(Bundle savedInstanceState) {
    ...
    if (extras != null) {
      mHasAutoRefreshedComments =
          extras.getBoolean(CommentsActivity.KEY_AUTO_REFRESHED);
      mEmptyViewMessageType = EmptyViewMessageType.getEnumFromString(
          extras.getString(CommentsActivity.KEY_EMPTY_VIEW_MESSAGE));
    } else {
  ...
  public View onCreateView(@NonNull LayoutInflater inflater,
    ...
    View view =
        inflater.inflate(R.layout.comment_list_fragment, container, false);
    ...
    mFilteredCommentsView.setFilterListener(
        new FilteredRecyclerView.FilterListener() {
          @Override
          ...
          public String onShowEmptyViewMessage(
            ...
            if (emptyViewMsgType == EmptyViewMessageType.NO_CONTENT) {
              ...
              if (filter == null || filter == CommentStatusCriteria.ALL) {
                ...
                case APPROVED:
                  title =
                      getString(R.string.comments_empty_list_filtered_approved);
                  break;
                case UNAPPROVED:
                  title =
                      getString(R.string.comments_empty_list_filtered_pending);
                  break;
                case SPAM:
                ...
                case TRASH:
                  title =
                      getString(R.string.comments_empty_list_filtered_trashed);
                  break;
                default:
                  title = getString(R.string.comments_empty_list);
                }
          ...
          @Override
          public void onShowCustomEmptyView(
              EmptyViewMessageType emptyViewMsgType) {}
        });

    // the following will change the look and feel of the toolbar to match the
    // current design
    mFilteredCommentsView.setToolbarBackgroundColor(
        ContextCompat.getColor(getActivity(), R.color.primary_40));
    mFilteredCommentsView.setToolbarSpinnerTextColor(
        ContextCompat.getColor(getActivity(), android.R.color.white));
    mFilteredCommentsView.setToolbarSpinnerDrawable(
    ...
    if (mFilteredCommentsView.getAdapter() == null) {
      ...
      if (!NetworkUtils.isNetworkAvailable(getActivity())) {
        ToastUtils.showToast(
            getActivity(),
            getString(R.string.error_refresh_comments_showing_older));
      }
  ...
  public void setCommentStatusFilter(CommentStatus statusFilter) {
    mCommentStatusFilter =
        CommentStatusCriteria.fromCommentStatus(statusFilter);
  }
  ...
  private void confirmDeleteComments() {
    if (mCommentStatusFilter == CommentStatusCriteria.TRASH) {
      ...
      int resId = getAdapter().getSelectedCommentCount() > 1
      ...
      builder.setPositiveButton(
          R.string.trash_yes, new DialogInterface.OnClickListener() {
            @Override
      ...
      builder.setNegativeButton(
          R.string.trash_no, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int id) {
              dialog.cancel();
            }
  ...
  private void deleteSelectedComments(boolean deletePermanently) {
    ...
    final int dlgId = deletePermanently
                          ? CommentDialogs.ID_COMMENT_DLG_DELETING
  ...
  void loadComments() {
    // this is called from CommentsActivity when a comment was changed in the
    // detail view, and the change will already be in SQLite so simply reload
    // the comment adapter to show the change
    getAdapter().loadComments(mCommentStatusFilter.toCommentStatus());
  }

  void updateEmptyView() {
    // this is called from CommentsActivity in the case the last moment for a
    // given type has been changed from that status, leaving the list empty, so
    // we need to update the empty view. The method inside FilteredRecyclerView
  ...
  /*
   * get latest comments from server, or pass loadMore=true to get comments
  ...
  void updateComments(boolean loadMore) {
    ...
    // immediately load/refresh whatever we have in our local db as we wait for
    // the API call to get latest results
    if (!loadMore) {
      getAdapter().loadComments(mCommentStatusFilter.toCommentStatus());
    }
    ...
    mDispatcher.dispatch(CommentActionBuilder.newFetchCommentsAction(
        new FetchCommentsPayload(mSite, mCommentStatusFilter.toCommentStatus(),
                                 COMMENTS_PER_PAGE, offset)));
  }

  public String getEmptyViewMessage() { return mEmptyViewMessageType.name(); }
  ...
  private final class ActionModeCallback implements ActionMode.Callback {
    ...
    private void setItemEnabled(Menu menu, int menuId, boolean isEnabled,
      ...
      if (item == null ||
          (item.isEnabled() == isEnabled && item.isVisible() == isVisible)) {
        return;
      }
    ...
    public boolean onPrepareActionMode(ActionMode actionMode, Menu menu) {
      ...
      boolean hasApproved = hasSelection && selectedComments.hasAnyWithStatus(
                                                CommentStatus.APPROVED);
      boolean hasUnapproved = hasSelection && selectedComments.hasAnyWithStatus(
                                                  CommentStatus.UNAPPROVED);
      boolean hasSpam =
          hasSelection && selectedComments.hasAnyWithStatus(CommentStatus.SPAM);
      boolean hasAnyNonSpam =
          hasSelection &&
          selectedComments.hasAnyWithoutStatus(CommentStatus.SPAM);
      boolean hasTrash = hasSelection &&
                         selectedComments.hasAnyWithStatus(CommentStatus.TRASH);

      setItemEnabled(menu, R.id.menu_approve,
                     hasUnapproved || hasSpam || hasTrash, true);
      setItemEnabled(menu, R.id.menu_unapprove, hasApproved, true);
      setItemEnabled(menu, R.id.menu_spam, hasAnyNonSpam, hasAnyNonSpam);
      setItemEnabled(menu, R.id.menu_unspam, hasSpam && !hasAnyNonSpam,
                     hasSpam && !hasAnyNonSpam);
      setItemEnabled(menu, R.id.menu_trash, hasSelection, true);

      final MenuItem trashItem = menu.findItem(R.id.menu_trash);
      if (trashItem != null &&
          mCommentStatusFilter == CommentStatusCriteria.TRASH) {
  ...
  public void onCommentChanged(OnCommentChanged event) {
    ...
    if (event.isError()) {
      ...
      // Reload the comment list in case of an error, we want to revert the UI
