...
public class PublicizeWebViewFragment extends PublicizeBaseFragment {
  ...
  /*
   * returns a new webView fragment to connect to a publicize service - if
   * passed connection is non-null then we're reconnecting a broken connection,
   * otherwise we're creating a new connection to the service
   */
  public static PublicizeWebViewFragment
  newInstance(@NonNull SiteModel site, @NonNull PublicizeService service,
              PublicizeConnection connection) {
    Bundle args = new Bundle();
    args.putSerializable(WordPress.SITE, site);
    args.putString(PublicizeConstants.ARG_SERVICE_ID, service.getId());
    if (connection != null) {
      args.putInt(PublicizeConstants.ARG_CONNECTION_ID,
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      ...
      mServiceId =
          savedInstanceState.getString(PublicizeConstants.ARG_SERVICE_ID);
      mConnectionId =
          savedInstanceState.getInt(PublicizeConstants.ARG_CONNECTION_ID);
    }
  }
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container,
    ...
    ViewGroup rootView = (ViewGroup)inflater.inflate(
        R.layout.publicize_webview_fragment, container, false);
  ...
  /*
   * display the current connect URL for this service - this will ask the user
   * to authorize the connection via the external service
   */
  private void loadConnectUrl() {
    ...
    String postData = WPWebViewActivity.getAuthenticationPostData(
        WPWebViewActivity.WPCOM_LOGIN_URL, connectUrl,
      ...
      if (isAdded() && url != null) {
        ...
        if (uri.getHost().equals("public-api.wordpress.com") &&
          ...
          if (!TextUtils.isEmpty(denied)) {
            EventBus.getDefault().post(new PublicizeEvents.ActionCompleted(
                false, ConnectAction.CONNECT, mServiceId));
            return;
          }

          long currentUserId = mAccountStore.getAccount().getUserId();
          // call the endpoint to make the actual connection
          PublicizeActions.connect(mSite.getSiteId(), mServiceId,
                                   currentUserId);
  ...
  private class PublicizeWebChromeClient extends WebChromeClient {
    PublicizeWebChromeClient() { super(); }
