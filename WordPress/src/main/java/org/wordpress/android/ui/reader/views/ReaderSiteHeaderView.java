...
/**
 * topmost view in post adapter when showing blog preview - displays
 * description, follower count, and follow button
 */
public class ReaderSiteHeaderView extends LinearLayout {
  ...
  public ReaderSiteHeaderView(Context context) { this(context, null); }
  ...
  public void loadBlogInfo(long blogId, long feedId) {
    ...
    if (mBlogId != 0) {
      ...
      ToastUtils.showToast(getContext(),
                           R.string.reader_toast_err_get_blog_info);
      return;
    }
    ...
    if (localBlogInfo == null ||
        ReaderBlogTable.isTimeToUpdateBlogInfo(localBlogInfo)) {
      ReaderActions.UpdateBlogInfoListener listener =
          new ReaderActions.UpdateBlogInfoListener() {
            @Override
  ...
  private void showBlogInfo(ReaderBlog blogInfo) {
    ...
    TextView txtDescription =
        layoutInfo.findViewById(R.id.text_blog_description);
    TextView txtFollowCount =
        layoutInfo.findViewById(R.id.text_blog_follow_count);
    ...
    if (blogInfo.hasImageUrl()) {
      mImageManager.load(blavatarImg, ImageType.BLAVATAR,
                         PhotonUtils.getPhotonImageUrl(blogInfo.getImageUrl(),
  ...
  private void toggleFollowStatus(final View followButton) {
    ...
    if (mFollowListener != null) {
      if (isAskingToFollow) {
        mFollowListener.onFollowTapped(followButton, mBlogInfo.getName(),
                                       mBlogInfo.blogId);
      } else {
    ...
    if (mFeedId != 0) {
      result =
          ReaderBlogActions.followFeedById(mFeedId, isAskingToFollow, listener);
    } else {
      result =
          ReaderBlogActions.followBlogById(mBlogId, isAskingToFollow, listener);
    }
