...
public class ReaderImageScanner {
  ...
  /*
   * start scanning the content for images and notify the passed listener about
   * each one
   */
  ...
  /*
   * returns a list of image URLs in the content up to the max above a certain
   * width - pass zero to include all images regardless of size
   */
  ...
  /*
   * returns true if there at least `minImageCount` images in the post content
   * that are at least `minImageWidth` in size
   */
  ...
  /*
   * used when a post doesn't have a featured image assigned, searches post's
   * content for an image that may be large enough to be suitable as a featured
   * image
   */
    ...
    while (imgMatcher.find()) {
      ...
      if (width > currentMaxWidth) {
        ...
        currentMaxWidth = width;
                 ...
                 hasSuitableClassForFeaturedImage(imageTag)) {
        currentImageUrl = imageUrl;
      }
    }
  ...
  /*
   * returns true if the passed image tag has a "size-" class attribute which
   * would make it suitable for use as a featured image
   */
  private boolean hasSuitableClassForFeaturedImage(@NonNull String imageTag) {
    ...
    return (tagClass != null && (tagClass.contains("size-full") ||
                                 tagClass.contains("size-large") ||
                                 tagClass.contains("size-medium")));
  }

  /*
   * same as above, but doesn't enforce the max width - will return the first
   * image found if no images have their width set
   */
  public String getLargestImage() { return getLargestImage(-1); }
}
