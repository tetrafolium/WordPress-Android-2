...
public class ReaderFollowButton extends LinearLayout {
  ...
  private void initView(Context context, AttributeSet attrs) {
    ...
    if (attrs != null) {
      TypedArray array = context.getTheme().obtainStyledAttributes(
          attrs, R.styleable.ReaderFollowButton, 0, 0);
      if (array != null) {
        mShowCaption = array.getBoolean(
            R.styleable.ReaderFollowButton_wpShowFollowButtonCaption, true);
      }
    }
    ...
    if (!mShowCaption) {
      ...
      int iconSz = context.getResources().getDimensionPixelSize(
          R.dimen.reader_follow_icon_no_caption);
  ...
  private void updateFollowText() {
    if (mShowCaption) {
      mTextFollow.setText(mIsFollowed ? R.string.reader_btn_unfollow
                                      : R.string.reader_btn_follow);
    }
    mTextFollow.setSelected(mIsFollowed);

    // show green icon if site is followed, gray icon if not followed and
    // there's a caption, blue icon if not followed and there is no caption
    int drawableId;
    int colorId;
    if (mIsFollowed) {
    ...
    ColorUtils.INSTANCE.setImageResourceWithTint(mImageFollow, drawableId,
                                                 colorId);
  }
  ...
  private void setIsFollowed(boolean isFollowed, boolean animateChanges) {
    ...
    if (animateChanges) {
      ...
      long duration = getContext().getResources().getInteger(
