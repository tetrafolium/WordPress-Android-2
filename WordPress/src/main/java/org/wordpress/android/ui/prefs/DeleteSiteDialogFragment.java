...
public class DeleteSiteDialogFragment extends DialogFragment
    implements TextWatcher, DialogInterface.OnShowListener {
  ...
  public void onShow(DialogInterface dialog) {
    mDeleteButton =
        mDeleteSiteDialog.getButton(DialogInterface.BUTTON_POSITIVE);
    mDeleteButton.setEnabled(false);
  }
  ...
  private Spannable confirmationPromptString() {
    String deletePrompt = String.format(
        getString(R.string.confirm_delete_site_prompt), mSiteDomain);
    ...
    promptSpannable.setSpan(new StyleSpan(Typeface.BOLD), beginning, end,
  ...
  private void configureUrlConfirmation(AlertDialog.Builder builder) {
    ...
    View view = getActivity().getLayoutInflater().inflate(
        R.layout.delete_site_dialog, null);
  ...
  private void retrieveSiteDomain() {
    ...
    mSiteDomain =
        getString(R.string.wordpress_dot_com).toLowerCase(Locale.ROOT);
    if (args != null) {
  ...
  private boolean isUrlConfirmationTextValid() {
    String confirmationText =
        mUrlConfirmation.getText().toString().trim().toLowerCase(Locale.ROOT);
