...
public class JetpackConnectionResultActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (uri != null) {
      ...
      mSource = JetpackConnectionSource.fromString(
          uri.getQueryParameter(SOURCE_PARAM));
      if (mAccountStore.hasAccessToken()) {
        // if user is signed in wpcom show the stats or notifications right away
        trackResult();
        finishAndGoBackToSource();
      } else {
        // An edgecase when the user is logged out in the app but logged in in
        // webview
    ...
    if (!TextUtils.isEmpty(mReason)) {
      if (mReason.equals(ALREADY_CONNECTED)) {
        ...
        ToastUtils.showToast(
            this, getString(R.string.jetpack_already_connected_toast));
      } else {
        AppLog.e(AppLog.T.API,
                 "Could not connect to Jetpack, reason: " + mReason);
        JetpackConnectionUtils.trackFailureWithSource(mSource, mReason);
        ToastUtils.showToast(
            this,
            getString(R.string.jetpack_connection_failed_with_reason, mReason));
      }
    } else {
