...
public class MediaDeleteService extends Service {
  ...
  public class MediaDeleteBinder extends Binder {
    public MediaDeleteService getService() { return MediaDeleteService.this; }
  ...
  public void onDestroy() {
    ...
    // TODO: if event not dispatched for ongoing delete cancel it and dispatch
    // cancel event
    super.onDestroy();
  }
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    mDeleteQueue =
        (List<MediaModel>)intent.getSerializableExtra(MEDIA_LIST_KEY);

    // start deleting queued media
    deleteNextInQueue();

    // only run while app process is running, allows service to be stopped by
  ...
  public void onMediaChanged(OnMediaChanged event) {
    ...
    if (event.mediaList == null || event.mediaList.isEmpty() ||
        !matchesInProgressMedia(event.mediaList.get(0))) {
  ...
  private void handleOnMediaChangedError(@NonNull OnMediaChanged event) {
    ...
    case NULL_MEDIA_ARG:
      ...
      AppLog.d(T.MEDIA,
               "Null media argument supplied, skipping current delete.");
      completeCurrentDelete();
      break;
    case NOT_FOUND:
      if (media == null) {
      ...
      AppLog.d(T.MEDIA, "Could not find media (id=" + media.getMediaId() +
                            "). on remote");
      // remove media from local database
      mDispatcher.dispatch(
          MediaActionBuilder.newRemoveMediaAction(mCurrentDelete));
      break;
    case PARSE_ERROR:
      AppLog.d(T.MEDIA,
  ...
  private void deleteNextInQueue() {
    ...
    if (mCurrentDelete != null) {
      AppLog.i(
          T.MEDIA,
          "Ignoring request to deleteNextInQueue, only one media item can be deleted at a time.");
      return;
    }
    ...
    if (mSite == null) {
      AppLog.i(T.MEDIA,
               "Unexpected state, site is null. Stopping MediaDeleteService.");
      stopSelf();
      return;
    }
    ...
    if (mCurrentDelete == null) {
      AppLog.v(T.MEDIA,
               "No more media items to delete. Stopping MediaDeleteService.");
      stopSelf();
      return;
    }
  ...
  private void dispatchDeleteAction(@NonNull MediaModel media) {
    AppLog.v(T.MEDIA, "Deleting " + media.getTitle() +
                          " (id=" + media.getMediaId() + ")");
  ...
  /**
   * Compares site ID and media ID to determine if a given media item matches
   * the current media item being deleted.
   */
