...
public class ReaderPostAdapter
  ...
  private class ReaderRemovedPostViewHolder extends RecyclerView.ViewHolder {
    ...
    ReaderRemovedPostViewHolder(View itemView) {
      ...
      mRemovedPostContainer =
          itemView.findViewById(R.id.removed_item_container);
  ...
  private class ReaderPostViewHolder extends RecyclerView.ViewHolder {
    ...
    ReaderPostViewHolder(View itemView) {
      ...
      mTxtAuthorAndBlogName =
          itemView.findViewById(R.id.text_author_and_blog_name);
      ...
      mImgAvatarOrBlavatar =
          itemView.findViewById(R.id.image_avatar_or_blavatar);
      ...
      mImgDiscoverAvatar =
          mLayoutDiscover.findViewById(R.id.image_discover_avatar);
      ...
      // show author/blog link as disabled if we're previewing a blog, otherwise
      // show blog preview when the post header is clicked
      if (getPostListType() == ReaderTypes.ReaderPostListType.BLOG_PREVIEW) {
        int color = ContextExtensionsKt.getColorFromAttribute(
            itemView.getContext(), R.attr.wpColorText);
        ...
        postHeaderView.setOnClickListener(new View.OnClickListener() {
          ...
          public void onClick(View view) {
            ...
            ReaderPost post = getItem(position);
            if (post != null) {
              ReaderActivityLauncher.showReaderBlogPreview(view.getContext(),
                                                           post);
            }
          }
        });
      ...
      // play the featured video when the overlay image is tapped - note that
      // the overlay image only appears when there's a featured video
      mImgVideoOverlay.setOnClickListener(new View.OnClickListener() {
        ...
        public void onClick(View view) {
          ...
          if (post != null && post.hasFeaturedVideo()) {
            ReaderActivityLauncher.showReaderVideoViewer(
                view.getContext(), post.getFeaturedVideo());
          }
        }
      });
  ...
  @Override
  public @NonNull RecyclerView.ViewHolder
  onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    case VIEW_TYPE_SITE_HEADER:
      ReaderSiteHeaderView readerSiteHeaderView =
          new ReaderSiteHeaderView(context);
    ...
    case VIEW_TYPE_XPOST:
      postView = LayoutInflater.from(context).inflate(
          R.layout.reader_cardview_xpost, parent, false);
    ...
    default:
      postView = LayoutInflater.from(context).inflate(
          R.layout.reader_cardview_post, parent, false);
  ...
  public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder,
    ...
    if (holder instanceof ReaderPostViewHolder) {
      ...
      siteHolder.mSiteHeaderView.setOnBlogInfoLoadedListener(
          mBlogInfoLoadedListener);
      if (isDiscover()) {
        siteHolder.mSiteHeaderView.loadBlogInfo(
            ReaderConstants.DISCOVER_SITE_ID, 0);
      } else {
  ...
  private void renderXPost(int position, ReaderXPostViewHolder holder) {
    ...
    mImageManager.loadIntoCircle(
        holder.mImgAvatar, ImageType.AVATAR,
        GravatarUtils.fixGravatarUrl(post.getPostAvatar(), mAvatarSzSmall));

    mImageManager.load(
        holder.mImgBlavatar, ImageType.BLAVATAR,
  ...
  private void renderRemovedPost(final int position,
                                 final ReaderRemovedPostViewHolder holder) {
    final ReaderPost post = getItem(position);
    final Context context = holder.mRemovedPostContainer.getContext();
    holder.mTxtRemovedPostTitle.setText(
        createTextForRemovedPostContainer(post, context));
    Drawable drawable = ColorUtils.INSTANCE.applyTintToDrawable(
        context, R.drawable.ic_undo_white_24dp, R.color.primary_40);
    holder.mUndoRemoveAction.setCompoundDrawablesWithIntrinsicBounds(
  ...
  private void renderPost(final int position,
                          final ReaderPostViewHolder holder) {
    final ReaderPost post = getItem(position);
    ...
    holder.mTxtDateline.setText(DateTimeUtils.javaDateToTimeSpan(
        post.getDisplayDate(), WordPress.getContext()));

    // show avatar if it exists, otherwise show blavatar
    if (post.hasPostAvatar()) {
      String imageUrl =
          GravatarUtils.fixGravatarUrl(post.getPostAvatar(), mAvatarSzMedium);
      ...
      String imageUrl =
          GravatarUtils.fixGravatarUrl(post.getBlogImageUrl(), mAvatarSzMedium);
      mImageManager.load(holder.mImgAvatarOrBlavatar, ImageType.BLAVATAR,
                         imageUrl);
      holder.mImgAvatarOrBlavatar.setVisibility(View.VISIBLE);
    } else {
    ...
    // show author and blog name if both are available, otherwise show whichever
    // is available
    if (post.hasBlogName() && post.hasAuthorName() &&
        !post.getBlogName().equals(post.getAuthorName())) {
      holder.mTxtAuthorAndBlogName.setText(
          holder.mTxtAuthorAndBlogName.getResources().getString(
              R.string.author_name_blog_name, post.getAuthorName(),
              post.getBlogName()));
    ...
    if (post.getCardType() == ReaderCardType.PHOTO) {
      // posts with a suitable featured image that have very little text get the
      // "photo card" treatment - show the title overlaid on the featured image
      // without any text
      ...
      mImageManager.load(
          holder.mImgFeatured, ImageType.PHOTO,
      ...
      if (post.getCardType() == ReaderCardType.GALLERY) {
        // if this post is a gallery, scan it for images and show a thumbnail
        // strip of them - note that the thumbnail strip will take care of
        // making itself visible
        holder.mThumbnailStrip.loadThumbnails(post.blogId, post.postId,
                                              post.isPrivate);
        holder.mFramePhoto.setVisibility(View.GONE);
        titleMargin = mMarginLarge;
      } else if (post.getCardType() == ReaderCardType.VIDEO) {
        ReaderVideoUtils.retrieveVideoThumbnailUrl(
            post.getFeaturedVideo(), new VideoThumbnailUrlListener() {
              @Override
              public void showThumbnail(String thumbnailUrl) {
                mImageManager.load(holder.mImgFeatured, ImageType.PHOTO,
                                   thumbnailUrl, ScaleType.CENTER_CROP);
              }
              ...
              public void cacheThumbnailUrl(String thumbnailUrl) {
                ReaderThumbnailTable.addThumbnail(
                    post.postId, post.getFeaturedVideo(), thumbnailUrl);
              }
            });
        holder.mFramePhoto.setVisibility(View.VISIBLE);
        holder.mThumbnailStrip.setVisibility(View.GONE);
      ...
      // set the top margin of the title based on whether there's a featured
      // image
    ...
    holder.mImgVideoOverlay.setVisibility(
        post.getCardType() == ReaderCardType.VIDEO ? View.VISIBLE : View.GONE);
    ...
    // if we haven't already rendered this post and it has a "railcar" attached
    // to it, add it to the rendered list and record the TrainTracks render
    // event
  ...
  /*
   * follow button only shows for tags and "Posts I Like" - it doesn't show for
   * Followed Sites, Discover, lists, etc.
   */
  ...
  private void checkLoadMore(int position) {
    if (mCanRequestMorePosts && mDataRequestedListener != null &&
  ...
  private void showDiscoverData(final ReaderPostViewHolder postHolder,
    ...
    case EDITOR_PICK:
      mImageManager.loadIntoCircle(
          postHolder.mImgDiscoverAvatar, ImageType.AVATAR,
          GravatarUtils.fixGravatarUrl(discoverData.getAvatarUrl(),
                                       mAvatarSzSmall));
      // tapping an editor pick opens the source post, which is handled by the
      // existing post selection handler
    ...
    case SITE_PICK:
      ...
      mImageManager.load(postHolder.mImgDiscoverAvatar, ImageType.BLAVATAR,
                         GravatarUtils.fixGravatarUrl(
                             discoverData.getAvatarUrl(), mAvatarSzSmall));
      // site picks show "Visit [BlogName]" link - tapping opens the blog
      // preview if we have the blogId, if not show blog in internal webView
      postHolder.mLayoutDiscover.setOnClickListener(new View.OnClickListener() {
        @Override
        public void onClick(View v) {
          if (discoverData.getBlogId() != 0) {
            ReaderActivityLauncher.showReaderBlogPreview(
                v.getContext(), discoverData.getBlogId());
          } else if (discoverData.hasBlogUrl()) {
            ReaderActivityLauncher.openUrl(v.getContext(),
                                           discoverData.getBlogUrl());
          }
        }
      });
      break;
    ...
    case OTHER:
    default:
      mImageManager.cancelRequestAndClearImageView(
  ...
  public ReaderPostAdapter(Context context, ReaderPostListType postListType,
    ...
    mPhotonHeight = context.getResources().getDimensionPixelSize(
        R.dimen.reader_featured_image_height_cardview);

    setHasStableIds(true);
  }

  private boolean hasHeader() { return hasSiteHeader() || hasTagHeader(); }

  private boolean hasSiteHeader() {
    return isDiscover() ||
        getPostListType() == ReaderTypes.ReaderPostListType.BLOG_PREVIEW;
  }
  ...
  public void
  setOnPostSelectedListener(ReaderInterfaces.OnPostSelectedListener listener) {
    mPostSelectedListener = listener;
  }

  public void
  setOnDataLoadedListener(ReaderInterfaces.DataLoadedListener listener) {
    mDataLoadedListener = listener;
  }

  public void setOnPostBookmarkedListener(
      ReaderInterfaces.OnPostBookmarkedListener listener) {
    mOnPostBookmarkedListener = listener;
  }

  public void
  setOnDataRequestedListener(ReaderActions.DataRequestedListener listener) {
    mDataRequestedListener = listener;
  }

  public void setOnPostPopupListener(
      ReaderInterfaces.OnPostPopupListener onPostPopupListener) {
    mOnPostPopupListener = onPostPopupListener;
  }

  public void setOnBlogInfoLoadedListener(
      ReaderSiteHeaderView.OnBlogInfoLoadedListener listener) {
  ...
  public boolean isEmpty() { return (mPosts == null || mPosts.size() == 0); }
  ...
  private void showLikes(final ReaderPostViewHolder holder,
                         final ReaderPost post) {
    ...
    if (canShowLikes) {
      ...
      holder.mLikeCount.setContentDescription(ReaderUtils.getLongLikeLabelText(
          holder.mCardView.getContext(), post.numLikes,
  ...
  private void initBookmarkButton(final int position,
                                  final ReaderPostViewHolder holder,
                                  final ReaderPost post) {
    ...
    holder.mBtnBookmark.setOnClickListener(new OnClickListener() {
      ...
      public void onClick(View v) {
        ...
        if (isBookmarksList()) {
          ...
          // notifyItemChanged highlights the item for a bit, we need to
          // manually updateTheView to prevent it
  ...
  private void updateBookmarkView(final ReaderPostViewHolder holder,
                                  final ReaderPost post) {
  ...
  createTextForRemovedPostContainer(ReaderPost post, Context context) {
    ...
    str.setSpan(new StyleSpan(Typeface.BOLD), removedString.length(),
                removedPostTitle.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    return str;
  }
  ...
  private void showComments(final ReaderPostViewHolder holder,
    ...
    if (post.isDiscoverPost() || isBookmarksList()) {
      ...
      canShowComments =
          post.isWP() && (post.isCommentsOpen || post.numReplies > 0);
    }
    ...
    if (canShowComments) {
      ...
      holder.mCommentCount.setOnClickListener(new View.OnClickListener() {
        @Override
        public void onClick(View v) {
          ReaderActivityLauncher.showReaderComments(v.getContext(), post.blogId,
                                                    post.postId);
        }
      });
    } else {
   ...
   */
  private void toggleLike(Context context, ReaderPostViewHolder holder,
                          ReaderPost post) {
    ...
    ReaderAnim.animateLikeButton(holder.mLikeCount.getImageView(),
                                 isAskingToLike);

    if (!ReaderPostActions.performLikeAction(
            post, isAskingToLike, mAccountStore.getAccount().getUserId())) {
    ...
    if (isAskingToLike) {
      AnalyticsUtils.trackWithReaderPostDetails(
          AnalyticsTracker.Stat.READER_ARTICLE_LIKED, post);
      // Consider a like to be enough to push a page view - solves a
      // long-standing question from folks who ask 'why do I have more likes
      // than page views?'.
      ReaderPostActions.bumpPageViewForPost(mSiteStore, post);
    } else {
      AnalyticsUtils.trackWithReaderPostDetails(
          AnalyticsTracker.Stat.READER_ARTICLE_UNLIKED, post);
    }

    // update post in array and on screen
    int position = mPosts.indexOfPost(post);
    ReaderPost updatedPost =
        ReaderPostTable.getBlogPost(post.blogId, post.postId, true);
  ...
  private void toggleBookmark(final long blogId, final long postId) {
    ...
    if (post != null && position > -1) {
      ...
      if (mOnPostBookmarkedListener != null) {
        mOnPostBookmarkedListener.onBookmarkedStateChanged(
            post.isBookmarked, blogId, postId, !isBookmarksList());
      }
    }
  }
   ...
   */
  private void toggleFollow(final Context context, final View followButton,
                            final ReaderPost post) {
    if (post == null || !NetworkUtils.checkConnection(context)) {
      return;
    }
    ...
    if (mFollowListener != null) {
      if (isAskingToFollow) {
        mFollowListener.onFollowTapped(followButton, post.getBlogName(),
                                       post.blogId);
      } else {
  ...
  private boolean hasNewsCard() {
    // We don't want to display the card when we are displaying just a loading
    // screen. However, on Discover a header is shown, even when we are loading
    // data, so the card should be displayed. [moreover displaying the card only
    // after we fetch the data results in weird animation after configuration
    // change, since it plays insertion animation for all the data (including
    // the card) except of the header which hasn't changed].
  ...
  private class LoadPostsTask extends AsyncTask<Void, Void, Boolean> {
    ...
    protected Boolean doInBackground(Void... params) {
      ...
      case TAG_FOLLOWED:
      case SEARCH_RESULTS:
        mAllPosts = ReaderPostTable.getPostsWithTag(mCurrentTag, MAX_ROWS,
                                                    EXCLUDE_TEXT_COLUMN);
        numExisting = ReaderPostTable.getNumPostsWithTag(mCurrentTag);
        break;
      case BLOG_PREVIEW:
        if (mCurrentFeedId != 0) {
          mAllPosts = ReaderPostTable.getPostsInFeed(mCurrentFeedId, MAX_ROWS,
                                                     EXCLUDE_TEXT_COLUMN);
          numExisting = ReaderPostTable.getNumPostsInFeed(mCurrentFeedId);
        } else {
          mAllPosts = ReaderPostTable.getPostsInBlog(mCurrentBlogId, MAX_ROWS,
                                                     EXCLUDE_TEXT_COLUMN);
      ...
      // if we're not already displaying the max # posts, enable requesting more
      // when the user scrolls to the end of the list
      mCanRequestMorePostsTemp =
          (numExisting < ReaderConstants.READER_MAX_POSTS_TO_DISPLAY);
    ...
    private int getGapMarkerPosition() {
      ...
      ReaderBlogIdPostId gapMarkerIds =
          ReaderPostTable.getGapMarkerIdsForTag(mCurrentTag);
      ...
      if (gapMarkerPostPosition > -1) {
        ...
        if (gapMarkerPostPosition == mAllPosts.size() - 1) {
          ...
          // increment it if there are custom items at the top of the list
          // (header or newsCard)
          gapMarkerPosition += getItemPositionOffset();
          AppLog.d(AppLog.T.READER,
                   "gap marker at position " + gapMarkerPostPosition);
        }
      }
