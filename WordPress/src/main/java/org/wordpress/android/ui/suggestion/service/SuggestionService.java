...
public class SuggestionService extends Service {
  ...
  private final List<Long> mCurrentlyRequestingSuggestionsSiteIds =
      new ArrayList<>();
  ...
  // We're still using `siteId` here and not a SiteModel, because suggestions
  // the enpoint works on any site, not only on sites the user is a member of.
  // Username suggestions are used in the Reader to auto complete usernames when
  ...
  public void updateSuggestions(final long siteId) {
    ...
    AppLog.d(AppLog.T.SUGGESTION,
             "suggestion service > updating suggestions for siteId: " + siteId);
    String path = "/users/suggest"
                  + "?site_id=" + siteId;
    WordPress.getRestClientUtils().get(path, listener, errorListener);
  }

  private void handleSuggestionsUpdatedResponse(final long siteId,
    ...
    new Thread() {
      ...
      public void run() {
        ...
        List<Suggestion> suggestions =
            Suggestion.suggestionListFromJSON(jsonSuggestions, siteId);
        if (suggestions != null) {
          SuggestionTable.insertSuggestionsForSite(siteId, suggestions);
          EventBus.getDefault().post(
              new SuggestionEvents.SuggestionNameListUpdated(siteId));
        }
      }
    }.start();
  }
  ...
  private void
  removeSiteIdFromSuggestionRequestsAndStopServiceIfNecessary(long siteId) {
  ...
  public void updateTags(final long siteId) {
    ...
    AppLog.d(AppLog.T.SUGGESTION,
             "suggestion service > updating tags for siteId: " + siteId);
    String path = "/sites/" + siteId + "/tags";
    WordPress.getRestClientUtils().get(path, listener, errorListener);
  }

  private void handleTagsUpdatedResponse(final long siteId,
                                         final JSONObject jsonObject) {
    new Thread() {
      @Override
      public void run() {
        ...
        if (tags != null) {
          ...
          EventBus.getDefault().post(
              new SuggestionEvents.SuggestionTagListUpdated(siteId));
        }
      }
    }.start();
  }
  ...
  private void
  removeSiteIdFromTagRequestsAndStopServiceIfNecessary(long siteId) {
  ...
  public class SuggestionBinder extends Binder {
    public SuggestionService getService() { return SuggestionService.this; }
  }
}
