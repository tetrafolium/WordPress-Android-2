...
public class ReaderEmbedScanner {
  ...
  public ReaderEmbedScanner(String contentOfPost) {
    ...
    mKnownEmbeds.put(Pattern.compile("<blockquote[^<>]class=\"instagram-",
                                     Pattern.CASE_INSENSITIVE),
                     "https://platform.instagram.com/en_US/embeds.js");
    mKnownEmbeds.put(
        Pattern.compile("<fb:post", Pattern.CASE_INSENSITIVE),
  ...
  public void beginScan(ReaderHtmlUtils.HtmlScannerListener listener) {
    ...
    for (Pattern pattern : mKnownEmbeds.keySet()) {
      if (pattern.matcher(mContent).find()) {
        // Use the onTagFound callback to pass a URL. Not super clean, but avoid
        // clutter with more kind of listeners.
