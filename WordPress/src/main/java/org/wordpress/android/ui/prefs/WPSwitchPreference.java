...
public class WPSwitchPreference
	extends SwitchPreference implements PreferenceHint {
	...
	for (int i = 0; i < array.getIndexCount(); ++i) {
		...
		if (index == R.styleable.SummaryEditTextPreference_longClickHint) {
			...
			mTint = array.getResourceId(index, R.color.neutral);
		           ...
		           R.styleable.SummaryEditTextPreference_switchThumbTint) {
			mThumbTint = array.getColorStateList(index);
		} else if (index ==
		           R.styleable.SummaryEditTextPreference_switchTrackTint) {
			mTrackTint = array.getColorStateList(index);
		} else if (index ==
		           R.styleable.SummaryEditTextPreference_preferenceTextColor) {
			mTextColor = array.getResourceId(index, android.R.color.white);
		} else if (index ==
		           R.styleable.SummaryEditTextPreference_backgroundColorChecked) {
			mBackgroundCheckedColor =
				array.getResourceId(index, android.R.color.white);
		} else if (index ==
		           R.styleable
		           .SummaryEditTextPreference_backgroundColorUnchecked) {
			mBackgroundUncheckedColor =
				array.getResourceId(index, android.R.color.white);
		}
	}
...
protected void onBindView(@NonNull View view) {
	...
	if (icon != null && mTint != 0) {
		icon.setImageTintList(ColorStateList.valueOf(
					      ContextCompat.getColor(view.getContext(), mTint)));
	}
	...
	if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
		...
		if (switchControl != null) {
			if (mThumbTint == null) {
				switchControl.setThumbTintList(ContextCompat.getColorStateList(
								       this.getContext(), R.color.primary_40_gray_20_gray_40_selector));
			} else {
				switchControl.setThumbTintList(mThumbTint);
			}
			if (mTrackTint == null) {
				switchControl.setTrackTintList(ContextCompat.getColorStateList(
								       this.getContext(), R.color.primary_40_gray_90_gray_50_selector));
			} else {
...
private void setBackground(boolean checked) {
	if (mContainer != null && mBackgroundCheckedColor != 0 &&
	    mBackgroundUncheckedColor != 0) {
		if (checked) {
			mContainer.setBackgroundColor(
				ContextCompat.getColor(this.getContext(), mBackgroundCheckedColor));
		} else {
			mContainer.setBackgroundColor(ContextCompat.getColor(
							      this.getContext(), mBackgroundUncheckedColor));
		}
	}
}
