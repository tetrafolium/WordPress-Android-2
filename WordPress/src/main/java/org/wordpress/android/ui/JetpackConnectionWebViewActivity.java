...
/**
 * Activity that opens the Jetpack login flow and returns to StatsActivity when
 * finished. Use one of the static factory methods to start the flow.
 */
public class JetpackConnectionWebViewActivity extends WPWebViewActivity
  ...
  public static void startJetpackConnectionFlow(Context context,
    ...
    if (site.isJetpackInstalled()) {
      ...
      JetpackConnectionUtils.trackWithSource(
          AnalyticsTracker.Stat.INSTALL_JETPACK_SELECTED, source);
      ActivityLauncher.startJetpackInstall(context, source, site);
    }
  }

  static void startManualFlow(Context context, JetpackConnectionSource source,
                              SiteModel site, boolean authorized) {
  ...
  private static void
  startJetpackConnectionFlow(Context context, String url, SiteModel site,
                             boolean authorized,
                             JetpackConnectionSource source) {
    ...
    JetpackConnectionUtils.trackWithSource(
        AnalyticsTracker.Stat.CONNECT_JETPACK_SELECTED, source);
  }

  @Override
  public void onCreate(Bundle savedInstanceState) {
    mSite = (SiteModel)getIntent().getSerializableExtra(WordPress.SITE);
    mSource = (JetpackConnectionSource)getIntent().getSerializableExtra(
        TRACKING_SOURCE_KEY);
    // We need to get the site before calling super since it'll create the web
    // client
  ...
  protected WebViewClient createWebViewClient(List<String> allowedURL) {
    mWebViewClient =
        new JetpackConnectionWebViewClient(this, this, mSite.getUrl());
  ...
  protected void onActivityResult(int requestCode, int resultCode,
    ...
    if (requestCode == JETPACK_LOGIN && resultCode == RESULT_OK) {
      JetpackConnectionWebViewActivity.startJetpackConnectionFlow(
          this, mWebViewClient.getRedirectPage(), mSite,
  ...
  protected void cancel() {
    JetpackConnectionUtils.trackWithSource(
        AnalyticsTracker.Stat.INSTALL_JETPACK_CANCELLED,
        (JetpackConnectionSource)getIntent().getSerializableExtra(
            TRACKING_SOURCE_KEY));
  }
  ...
  protected void onSaveInstanceState(Bundle outState) {
    ...
    outState.putString(REDIRECT_PAGE_STATE_ITEM,
                       mWebViewClient.getRedirectPage());
  }

  @Override
  protected void onRestoreInstanceState(Bundle savedInstanceState) {
    super.onRestoreInstanceState(savedInstanceState);
    mWebViewClient.setRedirectPage(
        savedInstanceState.getString(REDIRECT_PAGE_STATE_ITEM));
  }
  ...
  public void onRequiresWPComLogin(WebView webView, String redirectPage) {
    ...
    String postData = WPWebViewActivity.getAuthenticationPostData(
        authenticationUrl, redirectPage, mSite.getUsername(),
        mSite.getPassword(), mAccountStore.getAccessToken());
  ...
  public void onJetpackSuccessfullyConnected(Uri uri) {
    JetpackConnectionUtils.trackWithSource(
        AnalyticsTracker.Stat.INSTALL_JETPACK_COMPLETED,
        (JetpackConnectionSource)getIntent().getSerializableExtra(
            TRACKING_SOURCE_KEY));
