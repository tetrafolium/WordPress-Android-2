...
public SmartLockHelper(@NonNull FragmentActivity activity) {
	if (activity instanceof OnConnectionFailedListener &&
	    activity instanceof ConnectionCallbacks) {
		mActivity = new WeakReference<>(activity);
	} else {
		throw new RuntimeException(
			      "SmartLockHelper constructor needs an activity that "
...
public void smartLockAutoFill(@NonNull final Callback callback) {
	...
	if (activity == null || mCredentialsClient == null ||
	    !mCredentialsClient.isConnected()) {
		return;
	}
	CredentialRequest credentialRequest =
		new CredentialRequest.Builder().setPasswordLoginSupported(true).build();
	Auth.CredentialsApi.request(mCredentialsClient, credentialRequest)
			                ...
			                if (status.getStatusCode() ==
			                    CommonStatusCodes.RESOLUTION_REQUIRED) {
			                        try {
			                                ...
			                                AppLog.d(
								T.NUX,
								"SmartLock: Failed to send resolution for credential request");
...
public void saveCredentialsInSmartLock(@NonNull final String username,
	...
	if (TextUtils.isEmpty(password) || TextUtils.isEmpty(username)) {
		AppLog.i(
			T.MAIN,
			String.format(
				"Cannot save Smart Lock credentials, username (%s) or password (%s) is empty",
				username, password));
		return;
	}
...
public void deleteCredentialsInSmartLock(@NonNull final String username,
                                         @NonNull final String password) {
	Activity activity = getActivityAndCheckAvailability();
	if (activity == null || mCredentialsClient == null ||
	    !mCredentialsClient.isConnected()) {
	...
	Credential credential =
		new Credential.Builder(username).setPassword(password).build();
	...
	.setResultCallback(new ResultCallback<Status>() {
			...
			public void onResult(@NonNull Status status) {
			        AppLog.i(T.NUX,
			                 status.isSuccess()
			 ? "SmartLock: credentials deleted for username: " +
			                 username
			 : "SmartLock: Credentials not deleted for username: " +
			                 username);
			}
		});
}
