...
/**
 * generates and displays the HTML for post detail content - main purpose is to
 * assign the height/width attributes on image tags to (1) avoid the webView
 * resizing as images are loaded, and (2) avoid requesting images at a size
 * larger than the display <p> important to note that displayed images rely on
 * dp rather than px sizes due to the fact that WebView "converts CSS pixel
 * values to density-independent pixel values"
...
public class ReaderPostRenderer {
  ...
  public ReaderPostRenderer(ReaderWebView webView, ReaderPost post) {
    if (webView == null) {
      throw new IllegalArgumentException(
          "ReaderPostRenderer requires a webView");
    }
    if (post == null) {
    ...
    // enable JavaScript in the webView, otherwise videos and other embedded
    // content won't work - note that the content is scrubbed on the backend so
  ...
  public void beginRender() {
    ...
    new Thread() {
      @Override
      public void run() {
        final boolean hasTiledGallery =
            hasTiledGallery(mRenderBuilder.toString());
        ...
        // Get the set of JS scripts to inject in our Webview to support some
        // specific Embeds.
        Set<String> jsToInject = injectJSForSpecificEmbedSupport();

        final String htmlContent = formatPostContentForWebView(
            mRenderBuilder.toString(), jsToInject, hasTiledGallery,
  ...
  /*
   * scan the content for images and make sure they're correctly sized for the
   * device
   */
  private void resizeImages() {
    ReaderHtmlUtils.HtmlScannerListener imageListener =
        new ReaderHtmlUtils.HtmlScannerListener() {
          @Override
    ...
    ReaderImageScanner scanner =
        new ReaderImageScanner(content, mPost.isPrivate);
    scanner.beginScan(imageListener);
  }

  /*
   * scan the content for iframes and make sure they're correctly sized for the
   * device
   */
  private void resizeIframes() {
    ReaderHtmlUtils.HtmlScannerListener iframeListener =
        new ReaderHtmlUtils.HtmlScannerListener() {
          @Override
  ...
  private Set<String> injectJSForSpecificEmbedSupport() {
    ...
    ReaderHtmlUtils.HtmlScannerListener embedListener =
        new ReaderHtmlUtils.HtmlScannerListener() {
          @Override
          public void onTagFound(String tag, String src) {
            jsToInject.add(src);
  ...
  private void renderHtmlContent(final String htmlContent) {
    ...
    // make sure webView is still valid (containing fragment may have been
    // detached)
  ...
  /*
   * called when image scanner finds an image, tries to replace the image tag
   * with one that has height & width attributes set correctly for the current
   * display, if that fails replaces it with one that has our 'size-none' class
   */
  private void replaceImageTag(final String imageTag, final String imageUrl) {
    ...
    if (isFullSize) {
      ...
      newImageTag = makeImageTag(imageUrl, origSize.width, origSize.height,
                                 "size-medium");
    } else if (hasWidth) {
      newImageTag =
  ...
  private String makeImageTag(final String imageUrl, int width, int height,
                              final String imageClass) {
    String newImageUrl = ReaderUtils.getResizedImageUrl(imageUrl, width, height,
                                                        mPost.isPrivate);
    if (height > 0) {
  ...
  private String makeFullSizeImageTag(final String imageUrl, int width,
                                      int height) {
    int newWidth;
    int newHeight;
    if (width > 0 && height > 0) {
  ...
  /*
   * returns true if the post has a featured image and there are no images in
   * the post's content - when this is the case, the featured image is inserted
   * at the top of the content
   */
  private boolean shouldAddFeaturedImage() {
    return mPost.hasFeaturedImage() && !mPost.getText().contains("<img") &&
  ...
  private String getPostContent() {
    String content =
        mPost.shouldShowExcerpt() ? mPost.getExcerpt() : mPost.getText();
    ...
    if (mPost.isDiscoverPost()) {
      ...
      if (discoverData != null && discoverData.getBlogId() != 0 &&
          discoverData.hasBlogName()) {
        String label = String.format(WordPress.getContext().getString(
   ...
   */
  String getRenderedHtml() { return mRenderedHtml; }

  /*
   * returns the HTML to use when inserting a featured image into the rendered
   * content
   */
  private String getFeaturedImageHtml() {
    String imageUrl = ReaderUtils.getResizedImageUrl(
        mPost.getFeaturedImage(), mResourceVars.mFullSizeImageWidthPx,
        mResourceVars.mFeaturedImageHeightPx, mPost.isPrivate);

    return "<img class='size-full' src='" + imageUrl + "'/>";
  }

  /*
   * replace the passed iframe tag with one that's correctly sized for the
   * device
   */
  private void replaceIframeTag(final String tag, final String src) {
    ...
    if (start == -1) {
      AppLog.w(AppLog.T.READER,
               "reader renderer > iframe not found in builder");
      return;
    }
  ...
  /*
   * returns the full content, including CSS, that will be shown in the WebView
   * for this post
   */
  private String formatPostContentForWebView(final String content,
                                             final Set<String> jsToInject,
                                             boolean hasTiledGallery,
    ...
    final String galleryOnlyClass =
        "gallery-only-class" + new Random().nextInt(1000);
    ...
    sbHtml
        .append("<title>Reader Post</title>")
        // https://developers.google.com/chrome/mobile/docs/webview/pixelperfect
        .append(
            "<meta name='viewport' content='width=device-width, initial-scale=1'>")
        .append("<style type='text/css'>")
        .append(
            " body { font-family: 'Noto Serif', serif; font-weight: 400; margin: 0px; padding: 0px;}")
        .append(
            " body, p, div { max-width: 100% !important; word-wrap: break-word; }")
        // set line-height, font-size but not for .tiled-gallery divs when
        // rendering as tiled gallery as those will be handled with the
        // .tiled-gallery rules bellow.
        .append(
            " p, div" +
            (renderAsTiledGallery ? ":not(." + galleryOnlyClass + ")" : "") +
            ", li { line-height: 1.6em; font-size: 100%; }")
        .append(" h1, h2 { line-height: 1.2em; }")
        // counteract pre-defined height/width styles, expect for the
        // tiled-gallery divs when rendering as tiled gallery as those will be
        // handled with the .tiled-gallery rules bellow.
        .append(
            " p, div" + (renderAsTiledGallery ? ":not(.tiled-gallery.*)" : "") +
        ...
        // use a consistent top/bottom margin for paragraphs, with no top margin
        // for the first one
        .append(" p { margin-top: ")
        .append(mResourceVars.mMarginMediumPx)
        .append("px;")
        .append(" margin-bottom: ")
        .append(mResourceVars.mMarginMediumPx)
        .append("px; }")
        .append(" p:first-child { margin-top: 0px; }")
        // add background color and padding to pre blocks, and add overflow
        // scrolling so user can scroll the block if it's wider than the display
        .append(" pre { overflow-x: scroll;")
        .append(" background-color: ")
        .append(mResourceVars.mGreyExtraLightStr)
        .append("; ")
        .append(" padding: ")
        .append(mResourceVars.mMarginMediumPx)
        .append("px; }")
        // add a left border to blockquotes
        .append(" blockquote { color: ")
        .append(mResourceVars.mGreyMediumDarkStr)
        .append("; ")
        ...
        .append(" a { text-decoration: none; color: ")
        .append(mResourceVars.mLinkColorStr)
        .append("; }")
        // make sure images aren't wider than the display, strictly enforced for
        // images without size
        .append(" img { max-width: 100%; width: auto; height: auto; }")
        .append(
            " img.size-none { max-width: 100% !important; height: auto !important; }")
        // center large/medium images, provide a small bottom margin, and add a
        // background color so the user sees something while they're loading
    ...
    sbHtml.append(" .wp-caption img { margin-top: 0px; margin-bottom: 0px; }")
        ...
        // make sure html5 videos fit the browser width and use 16:9 ratio
        // (YouTube standard)
        .append(" video {")
        .append(" width: ")
        .append(pxToDp(mResourceVars.mVideoWidthPx))
        .append("px !important;")
        .append(" height: ")
        .append(pxToDp(mResourceVars.mVideoHeightPx))
        .append("px !important; }")
        // hide forms, form-related elements, legacy RSS sharing links and other
        // ad-related content http://bit.ly/2FUTvsP
    ...
    // add a custom CSS class to (any) tiled gallery elements to make them
    // easier selectable for various rules
    final List<String> classAmendRegexes = Arrays.asList(
        "(tiled-gallery) ([\\s\"\'])", "(gallery-row) ([\\s\"'])",
        "(gallery-group) ([\\s\"'])", "(tiled-gallery-item) ([\\s\"'])");
    String contentCustomised = content;
    for (String classToAmend : classAmendRegexes) {
      contentCustomised = contentCustomised.replaceAll(
          classToAmend, "$1 " + galleryOnlyClass + "$2");
    }

    for (String jsUrl : jsToInject) {
      sbHtml.append("<script src=\"")
          .append(jsUrl)
  ...
  private ImageSize getImageSizeFromAttachments(final String imageUrl) {
    if (mAttachmentSizes == null) {
      mAttachmentSizes =
          new ImageSizeMap(mPost.getText(), mPost.getAttachmentsJson());
    }
  ...
  private ImageSize getImageSizeFromQueryParams(final String imageUrl) {
    if (imageUrl.contains("w=")) {
      ...
      return new ImageSize(StringUtils.stringToInt(uri.getQueryParameter("w")),
      ...
      if (param != null) {
        ...
        if (sizes.length == 2) {
          return new ImageSize(StringUtils.stringToInt(sizes[0]),
  ...
  private ImageSize getImageOriginalSizeFromAttributes(final String imageTag) {
    return new ImageSize(ReaderHtmlUtils.getOriginalWidthAttrValue(imageTag),
                         ReaderHtmlUtils.getOriginalHeightAttrValue(imageTag));
  }

  private ImageSize getImageSizeFromAttributes(final String imageTag) {
    return new ImageSize(ReaderHtmlUtils.getWidthAttrValue(imageTag),
