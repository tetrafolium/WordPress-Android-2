...
public class PeopleListFragment extends Fragment {
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container,
    ...
    mFilteredRecyclerView.addItemDecoration(new PeopleItemDecoration(
        getActivity(), R.drawable.people_list_divider));
    ...
    // current design
    mFilteredRecyclerView.setToolbarBackgroundColor(
        ContextCompat.getColor(getActivity(), R.color.primary_40));
    mFilteredRecyclerView.setToolbarSpinnerTextColor(
        ContextCompat.getColor(getActivity(), android.R.color.white));
    mFilteredRecyclerView.setToolbarSpinnerDrawable(
    ...
    mFilteredRecyclerView.setFilterListener(
        new FilteredRecyclerView.FilterListener() {
          ...
          public String onShowEmptyViewMessage(
            ...
            mFilteredRecyclerView.setToolbarScrollFlags(
            ...
            case NO_CONTENT:
              ...
              case FOLLOWERS:
                title =
                    getString(R.string.people_empty_list_filtered_followers);
                break;
              case EMAIL_FOLLOWERS:
                title = getString(
                    R.string.people_empty_list_filtered_email_followers);
                break;
              case VIEWERS:
  ...
  public void refreshPeopleList(boolean isFetching) {
    ...
    PeopleAdapter peopleAdapter =
        (PeopleAdapter)mFilteredRecyclerView.getAdapter();
  ...
  public void fetchingRequestFinished(PeopleListFilter filter,
                                      boolean isFirstPage,
    ...
    if (mPeopleListFilter == filter) {
      if (isFirstPage) {
        mFilteredRecyclerView.setRefreshing(false);
        if (!isSuccessful) {
          mFilteredRecyclerView.updateEmptyView(
              EmptyViewMessageType.GENERIC_ERROR);
        }
      } else {
  ...
  public class PeopleAdapter
    ...
    @NonNull
    @Override
    public PeopleViewHolder onCreateViewHolder(@NonNull ViewGroup parent,
                                               int viewType) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder,
      ...
      if (person != null) {
        String avatarUrl =
            GravatarUtils.fixGravatarUrl(person.getAvatarUrl(), mAvatarSz);
        mImageManager.loadIntoCircle(peopleViewHolder.mImgAvatar,
                                     ImageType.AVATAR_WITH_BACKGROUND,
                                     avatarUrl);
        peopleViewHolder.mTxtDisplayName.setText(
            StringEscapeUtils.unescapeHtml4(person.getDisplayName()));
        if (person.getRole() != null) {
          peopleViewHolder.mTxtRole.setVisibility(View.VISIBLE);
          peopleViewHolder.mTxtRole.setText(
              RoleUtils.getDisplayName(person.getRole(), mUserRoles));
        } else {
        ...
        if (!person.getUsername().isEmpty()) {
          ...
          peopleViewHolder.mTxtUsername.setText(
              String.format("@%s", person.getUsername()));
        } else {
        ...
        if (person.getPersonType() == Person.PersonType.USER ||
          ...
          String dateSubscribed = SimpleDateFormat.getDateInstance().format(
              person.getDateSubscribed());
          String dateText =
              getString(R.string.follower_subscribed_since, dateSubscribed);
