...
/**
 * Calypso-style Preference that has an icon and a widget in the correct place.
 * If there is a button with id R.id.button, an onPreferenceClick listener is
 * added.
 */
...
public WPStartOverPreference(Context context, AttributeSet attrs) {
	...
	TypedArray array = context.obtainStyledAttributes(
		attrs, R.styleable.WPStartOverPreference);
	...
	for (int i = 0; i < array.getIndexCount(); ++i) {
		...
		if (index == R.styleable.WPStartOverPreference_buttonText) {
			...
			mButtonTextColor = array.getColor(
				index, ContextCompat.getColor(context, android.R.color.black));
			...
			mPrefIcon = VectorDrawableCompat.create(
				context.getResources(), array.getResourceId(index, 0), null);
		}
	}
...
protected void onBindView(@NonNull View view) {
	...
	if (view.findViewById(R.id.button) != null) {
		...
		button.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
				        getOnPreferenceClickListener().onPreferenceClick(
						wpStartOverPreference);
				}
			});
	}
	...
	if (view.findViewById(R.id.domain) != null) {
		...
		// TODO: FluxC: We might want to get the selected site here and update the
		// view textView.setText(UrlUtils.getHost(blog.getHomeURL()));
	}
}
}
