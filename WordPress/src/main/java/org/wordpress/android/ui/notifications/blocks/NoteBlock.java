...
public class NoteBlock {
  ...
  public BlockType getBlockType() { return BlockType.BASIC; }

  FormattableContent getNoteData() { return mNoteData; }

  Spannable getNoteText() {
    return mNotificationsUtilsWrapper.getSpannableContentForRanges(
        mNoteData, null, mOnNoteBlockTextClickListener, false);
  }
  ...
  private boolean isPingBack() { return mIsPingback; }

  public void setIsPingback() { mIsPingback = true; }

  FormattableMedia getNoteMediaItem() {
    return FormattableContentUtilsKt.getMediaOrNull(mNoteData, 0);
  }

  public void setIsBadge() { mIsBadge = true; }

  public void setBackgroundColor(int backgroundColor) {
    mBackgroundColor = backgroundColor;
  }

  public int getLayoutResourceId() { return R.layout.note_block_basic; }
  ...
  boolean hasImageMediaItem() {
    return hasMediaArray() && getNoteMediaItem() != null &&
        !TextUtils.isEmpty(getNoteMediaItem().getType()) &&
        (getNoteMediaItem().getType().startsWith("image") ||
         getNoteMediaItem().getType().equals("badge")) &&
  ...
  public View configureView(final View view) {
    final BasicNoteBlockHolder noteBlockHolder =
        (BasicNoteBlockHolder)view.getTag();
    ...
    if (hasImageMediaItem()) {
      ...
      mImageManager.loadWithResultListener(
          noteBlockHolder.getImageView(), ImageType.IMAGE,
          StringUtils.notNullStr(getNoteMediaItem().getUrl()), ScaleType.CENTER,
          null, new ImageManager.RequestListener<Drawable>() {
            @Override
            ...
            public void onResourceReady(@Nullable Drawable resource) {
              if (!mHasAnimatedBadge && view.getContext() != null &&
                ...
                Animation pop =
                    AnimationUtils.loadAnimation(view.getContext(), R.anim.pop);
      ...
      if (mIsBadge) {
        noteBlockHolder.getImageView().setImportantForAccessibility(
    ...
    if (hasVideoMediaItem()) {
      noteBlockHolder.getVideoView().setVideoURI(
          Uri.parse(StringUtils.notNullStr(getNoteMediaItem().getUrl())));
    ...
    if (!TextUtils.isEmpty(noteText)) {
      if (isPingBack()) {
        ...
        noteBlockHolder.getButton().setOnClickListener(new OnClickListener() {
          ...
          public void onClick(View v) {
            if (getOnNoteBlockTextClickListener() != null) {
              getOnNoteBlockTextClickListener().showSitePreview(
                  0, getMetaSiteUrl());
            }
          }
        });
        ...
        if (mIsBadge) {
          ...
          noteBlockHolder.getTextView().setPadding(
              0, DisplayUtils.dpToPx(view.getContext(), 8), 0, 0);

          if (AccessibilityUtils.isAccessibilityEnabled(
                  noteBlockHolder.getTextView().getContext())) {
        ...
        NoteBlockClickableSpan[] spans = noteText.getSpans(
            0, noteText.length(), NoteBlockClickableSpan.class);
  ...
  static class BasicNoteBlockHolder {
    ...
    public WPTextView getTextView() { return mTextView; }

    public Button getButton() { return mButton; }

    public View getDivider() { return mDivider; }
    ...
    public ImageView getImageView() {
      if (mImageView == null) {
        ...
        int imagePadding =
            mRootLayout.getContext().getResources().getDimensionPixelSize(
    ...
    public VideoView getVideoView() {
      if (mVideoView == null) {
        ...
        final MediaController mediaController =
            new MediaController(mRootLayout.getContext());
