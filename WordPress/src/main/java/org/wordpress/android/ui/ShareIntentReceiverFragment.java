...
private static final String ARG_LAST_USED_BLOG_LOCAL_ID =
	"ARG_LAST_USED_BLOG_LOCAL_ID";
...
public static ShareIntentReceiverFragment
newInstance(boolean sharingMediaFile, int lastUsedBlogLocalId,
            boolean afterLogin) {
...
public void onAttach(Context context) {
	...
	if (context instanceof ShareIntentFragmentListener) {
		...
		throw new RuntimeException(
			      "The parent activity doesn't implement ShareIntentFragmentListener.");
	}
}
...
public View onCreateView(LayoutInflater inflater,
	...
	ViewGroup layout = (ViewGroup)inflater.inflate(
		R.layout.login_epilogue_screen, container, false);
...
private void loadSavedState(Bundle savedInstanceState) {
	if (savedInstanceState != null) {
		mLastUsedBlogLocalId =
			savedInstanceState.getInt(ARG_LAST_USED_BLOG_LOCAL_ID);
	}
}
...
public void onSaveInstanceState(Bundle outState) {
	...
	if (selectedItemLocalId != -1) {
		outState.putInt(ARG_LAST_USED_BLOG_LOCAL_ID,
		                mAdapter.getSelectedItemLocalId());
	}
}
...
private void addShareActionListener(final Button button,
                                    final ShareAction shareAction) {
	button.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
			        mShareIntentFragmentListener.share(shareAction,
			                                           mAdapter.getSelectedItemLocalId());
			}
		});
}
...
private Adapter createSiteAdapter() {
	mAdapter = new SitePickerAdapter(
		getActivity(), R.layout.share_intent_sites_listitem, 0, "",
		false, new SitePickerAdapter.OnDataLoadedListener() {
			@Override
			...
			public void onAfterLoad() {
					...
					public void run() {
					        ...
					        if (mRecyclerView.computeVerticalScrollRange() >
					            mRecyclerView.getHeight()) {
					                mBottomButtonsShadow.setVisibility(View.VISIBLE);
					                mBottomButtonsContainer.setBackgroundResource(
								android.R.color.white);
					                mShareMediaBtn.setTextColor(
								getResources().getColor(R.color.primary_50));
					                ViewUtils.setButtonBackgroundColor(
								getContext(), mShareMediaBtn,
								...
								ContextExtensionsKt.getColorFromAttribute(
					                ...
					                ViewUtils.setButtonBackgroundColor(
								getContext(), mShareMediaBtn, R.style.WordPress_Button,
...
private HeaderHandler createHeaderHandler() {
	return new HeaderHandler() {
		       ...
		       public ViewHolder onCreateViewHolder(LayoutInflater layoutInflater,
			       ...
			       return new LoginHeaderViewHolder(layoutInflater.inflate(
									R.layout.login_epilogue_header, parent, false));
		       }
...
private void refreshAccountDetails(LoginHeaderViewHolder holder,
	...
	holder.updateLoggedInAsHeading(getContext(), mImageManager, mAfterLogin,
	                               mAccountStore.getAccount());
	holder.showSitesHeading(getString(sites.size() == 1
	                                  ? R.string.share_intent_adding_to
	                                  : R.string.share_intent_pick_site));
}
