...
public class NotificationsUpdateLogic {
  ...
  public NotificationsUpdateLogic(String locale,
                                  ServiceCompletionListener listener) {
    mLocale = locale;
    mCompletionListener = listener;
  }

  public void performRefresh(String noteId,
    ...
    WordPress.getRestClientUtilsV1_1().getNotifications(params, listener,
                                                        listener);
  }

  private class RestListener
      implements RestRequest.Listener, RestRequest.ErrorListener {
    @Override
    public void onResponse(final JSONObject response) {
      List<Note> notes;
      if (response == null) {
        // Not sure this could ever happen, but make sure we're catching all
        // response types
        ...
        try {
          ...
          // if we have a note id, we were started from
          // NotificationsDetailActivity. That means we need to re-set the
          // *read* flag on this note.
  ...
  private static void logVolleyErrorDetails(final VolleyError volleyError) {
    if (volleyError == null) {
      AppLog.e(AppLog.T.NOTIFS,
               "Tried to log a VolleyError, but the error obj was null!");
      return;
    }
    if (volleyError.networkResponse != null) {
      ...
      if (networkResponse.data != null) {
        AppLog.e(AppLog.T.NOTIFS,
                 "Network data: " + new String(networkResponse.data));
      }
    }
    AppLog.e(AppLog.T.NOTIFS,
             "Volley Error Message: " + volleyError.getMessage(), volleyError);
  }
