...
/*
 * shows reader post detail fragments in a ViewPager - primarily used for easy
 * swiping between posts with a specific tag or in a specific blog, but can also
 * be used to show a single post detail.
 *
 ...
 *
 * Will also handle jumping to the comments section, liking a commend and liking
 * a post directly
 */
public class ReaderPostPagerActivity extends AppCompatActivity
   ...
   */
  private enum InterceptType { READER_BLOG, READER_FEED, WPCOM_POST_SLUG }
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      ...
      mIsSinglePostView =
          savedInstanceState.getBoolean(ReaderConstants.ARG_IS_SINGLE_POST);
      mIsRelatedPostView =
      ...
      if (savedInstanceState.containsKey(
        ...
        Serializable positions = savedInstanceState.getSerializable(
            ReaderConstants.KEY_TRACKED_POSITIONS);
      ...
      mDirectOperation = (DirectOperation)getIntent().getSerializableExtra(
          ReaderConstants.ARG_DIRECT_OPERATION);
      mCommentId = getIntent().getIntExtra(ReaderConstants.ARG_COMMENT_ID, 0);
      mIsSinglePostView = getIntent().getBooleanExtra(
          ReaderConstants.ARG_IS_SINGLE_POST, false);
      mIsRelatedPostView = getIntent().getBooleanExtra(
          ReaderConstants.ARG_IS_RELATED_POST, false);
      mInterceptedUri =
          getIntent().getStringExtra(ReaderConstants.ARG_INTERCEPTED_URI);
      if (getIntent().hasExtra(ReaderConstants.ARG_POST_LIST_TYPE)) {
        mPostListType = (ReaderPostListType)getIntent().getSerializableExtra(
            ReaderConstants.ARG_POST_LIST_TYPE);
      }
      if (getIntent().hasExtra(ReaderConstants.ARG_TAG)) {
        mCurrentTag = (ReaderTag)getIntent().getSerializableExtra(
            ReaderConstants.ARG_TAG);
      }
    }
    ...
    // using the back button will navigate through related posts
        ...
        new ViewPager.SimpleOnPageChangeListener() {
          ...
          public void onPageSelected(int position) {
            ...
            if (mLastSelectedPosition > -1 &&
              ...
              // pause the previous web view - important because otherwise
              // embedded content will continue to play
              ReaderPostDetailFragment lastFragment =
                  getDetailFragmentAtPosition(mLastSelectedPosition);
            ...
            ReaderPostDetailFragment thisFragment =
                getDetailFragmentAtPosition(position);
    ...
    mViewPager.setPageTransformer(
        false, new WPViewPagerTransformer(
                   WPViewPagerTransformer.TransformType.SLIDE_OVER));
  }
  ...
  private void updateTitle(int position) {
    ...
    if (ids != null) {
      String title =
          ReaderPostTable.getPostTitle(ids.getBlogId(), ids.getPostId());
      if (!title.isEmpty()) {
        setTitle(title);
        return;
      }
    ...
    setTitle(ActivityUtils.isDeepLinking(getIntent())
                 ? R.string.reader_title_post_detail_wpcom
                 : R.string.reader_title_post_detail);
  }

  /*
   * used by the detail fragment when a post was requested due to not existing
   * locally
   */
  ...
  private void handleDeepLinking() {
    ...
    AnalyticsUtils.trackWithDeepLinkData(AnalyticsTracker.Stat.DEEP_LINKED,
                                         action, host, uri);
    ...
    // Handled URLs look like this:
    // http[s]://wordpress.com/read/feeds/{feedId}/posts/{feedItemId} with the
    // first segment being 'read'.
    if (segments != null) {
  ...
  private void showPost(@NonNull InterceptType interceptType,
                        final String blogIdentifier,
                        final String postIdentifier) {
    if (!TextUtils.isEmpty(blogIdentifier) &&
        !TextUtils.isEmpty(postIdentifier)) {
      ...
      case READER_BLOG:
        if (parseIds(blogIdentifier, postIdentifier)) {
          AnalyticsUtils.trackWithBlogPostDetails(
              AnalyticsTracker.Stat.READER_BLOG_POST_INTERCEPTED, mBlogId,
              mPostId);
          // IDs have now been set so, let ReaderPostPagerActivity normally
          // display the post
        } else {
          ToastUtils.showToast(this, R.string.error_generic);
        }
      ...
      case READER_FEED:
        if (parseIds(blogIdentifier, postIdentifier)) {
          AnalyticsUtils.trackWithFeedPostDetails(
              AnalyticsTracker.Stat.READER_FEED_POST_INTERCEPTED, mBlogId,
              mPostId);
          // IDs have now been set so, let ReaderPostPagerActivity normally
          // display the post
        } else {
      ...
      case WPCOM_POST_SLUG:
        AnalyticsUtils.trackWithBlogPostDetails(
            ...
            blogIdentifier, postIdentifier, mCommentId);

        // try to get the post from the local db
        ReaderPost post =
            ReaderPostTable.getBlogPost(blogIdentifier, postIdentifier, true);
        if (post != null) {
          // set the IDs and let ReaderPostPagerActivity normally display the
          // post
          mBlogId = post.blogId;
          mPostId = post.postId;
        } else {
              ...
              new ReaderActions.OnRequestListener() {
                ...
                public void onSuccess() {
                  ...
                  ReaderPost post = ReaderPostTable.getBlogPost(
                      blogIdentifier, postIdentifier, true);
                  ReaderEvents.PostSlugsRequestCompleted slugsResolved =
                      (post != null)
                          ...
                          : new ReaderEvents.PostSlugsRequestCompleted(200, 0,
                                                                       0);
                ...
                public void onFailure(int statusCode) {
                  ...
                  EventBus.getDefault().post(
                      new ReaderEvents.PostSlugsRequestCompleted(statusCode, 0,
                                                                 0));
                }
              });
  ...
  /**
   * Parse the URL fragment and interpret it as an operation to perform. For
   * example, a "#comments" fragment is interpreted as a direct jump into the
  ...
  private void parseFragment(Uri uri) {
    ...
    final Pattern fragmentCommentsPattern =
        Pattern.compile("comments", Pattern.CASE_INSENSITIVE);
    final Pattern fragmentCommentIdPattern =
        Pattern.compile("comment-(\\d+)", Pattern.CASE_INSENSITIVE);
    final Pattern fragmentRespondPattern =
        Pattern.compile("respond", Pattern.CASE_INSENSITIVE);

    // check for the general "#comments" fragment to jump to the comments
    // section
      ...
      if (replyToCommentId != null) {
        try {
          ...
          AppLog.e(AppLog.T.UTILS,
                   "replytocom cannot be converted to int" + replyToCommentId,
                   e);
        }
      }
  ...
  /**
   * Parse the URL query parameters and detect attempt to like a post or a
   * comment
   *
  ...
  protected void onSaveInstanceState(@NonNull Bundle outState) {
    ...
    outState.putBoolean(ReaderConstants.ARG_IS_RELATED_POST,
                        mIsRelatedPostView);
    ...
    if (mTrackedPositions.size() > 0) {
      outState.putSerializable(ReaderConstants.KEY_TRACKED_POSITIONS,
                               mTrackedPositions);
    }
  ...
  public void onBackPressed() {
    ...
    if (fragment != null && fragment.isCustomViewShowing()) {
      // if full screen video is showing, hide the custom view rather than
      // navigate back
  ...
  /*
   * perform analytics tracking and bump the page view for the post at the
   * passed position if it hasn't already been done
   */
  ...
  /*
   * loads the blogId/postId pairs used to populate the pager adapter - passed
   * blogId/postId will be made active after loading unless gotoNext=true, in
   * which case the post after the passed one will be made active
   */
    ...
    new Thread() {
      ...
      public void run() {
        ...
        if (mIsSinglePostView) {
          ...
          case TAG_FOLLOWED:
          case TAG_PREVIEW:
            idList = ReaderPostTable.getBlogIdPostIdsWithTag(getCurrentTag(),
                                                             maxPosts);
            break;
          case BLOG_PREVIEW:
        ...
        runOnUiThread(new Runnable() {
          ...
          public void run() {
            ...
            if (adapter.getCount() > 1 &&
                !AppPrefs.isReaderSwipeToNavigateShown()) {
  ...
  private ReaderPostListType getPostListType() { return mPostListType; }
  ...
  /*
   * called when user scrolls towards the last posts - requests older posts with
   * the current tag or in the current blog
   */
  private void requestMorePosts() {
    ...
    case BLOG_PREVIEW:
      ReaderPostServiceStarter.startServiceForBlog(
          this, mBlogId, ReaderPostServiceStarter.UpdateAction.REQUEST_OLDER);
      break;
    }
  }
  ...
  public void onEventMainThread(ReaderEvents.DoSignIn event) {
    ...
    AnalyticsUtils.trackWithInterceptedUri(
        AnalyticsTracker.Stat.READER_SIGN_IN_INITIATED, mInterceptedUri);
  ...
  private class PostPagerAdapter extends FragmentStatePagerAdapter {
    ...
    private boolean canRequestMostPosts() {
      return !mAllPostsLoaded && !mIsSinglePostView &&
          (mIdList != null &&
    ...
    public Fragment getItem(int position) {
      ...
      return ReaderPostDetailFragment.newInstance(
          mIsFeed, mIdList.get(position).getBlogId(),
          mIdList.get(position).getPostId(), mDirectOperation, mCommentId,
          mIsRelatedPostView, mInterceptedUri, getPostListType(),
          mPostSlugsResolutionUnderway);
    }
