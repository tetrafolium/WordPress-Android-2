...
public class LoginEpilogueFragment
	extends LoginBaseFormFragment<LoginEpilogueListener> {
...
public static LoginEpilogueFragment
newInstance(boolean doLoginUpdate, boolean showAndReturn,
...
protected ViewGroup createMainView(LayoutInflater inflater,
	...
	return (ViewGroup)inflater.inflate(R.layout.login_epilogue_screen,
	                                   container, false);
}
		...
		new SitePickerAdapter.OnDataLoadedListener() {
			...
			public void onAfterLoad() {
					...
					public void run() {
					        ...
					        if (mSitesList.computeVerticalScrollRange() >
					            mSitesList.getHeight()) {
								...
								getContext(), mConnectMore, R.style.WordPress_Button_Grey,
...
public void onAttach(Context context) {
	...
	if (context instanceof LoginEpilogueListener) {
		...
		throw new RuntimeException(context.toString() +
		                           " must implement LoginEpilogueListener");
	}
}
...
public void onResume() {
	...
	if (mDoLoginUpdate) {
		// when from magiclink, we need to complete the login process here (update
...
private void refreshAccountDetails(LoginHeaderViewHolder holder,
	...
	if (isWpcom) {
		holder.updateLoggedInAsHeading(getContext(), mImageManager, true,
		...
		String avatarUrl =
			GravatarUtils.gravatarFromEmail(site.getEmail(), avatarSz);
		String username = site.getUsername();
		String displayName = site.getDisplayName();

		holder.updateLoggedInAsHeading(getContext(), mImageManager, true,
		                               avatarUrl, username, displayName);
	}
	...
	if (sites.size() == 0) {
		...
		holder.showSitesHeading(StringUtils.getQuantityString(
						getActivity(), R.string.days_quantity_one,
...
protected void onLoginFinished() {
	// we needed to complete the login process so, now just show an updated
	// screen to the user
