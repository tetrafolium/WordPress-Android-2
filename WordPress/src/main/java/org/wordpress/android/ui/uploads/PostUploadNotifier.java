...
class PostUploadNotifier {
  ...
  private enum PagesOrPostsType { POST, PAGE, POSTS, PAGES, PAGES_OR_POSTS }

  // used to hold notification data for everything (only one outstanding
  // foreground notification for the live UploadService instance
  ...
  PostUploadNotifier(Context context, UploadService service) {
    ...
    mNotificationManager = (NotificationManager)SystemServiceFactory.get(
        mContext, Context.NOTIFICATION_SERVICE);
  ...
  private void updateNotificationBuilder(@Nullable PostModel post) {
    ...
    // i.e. "1/3 Posts, 4/17 media items"
    if (sNotificationData.mTotalMediaItems > 0 &&
      ...
      if (sNotificationData.mTotalMediaItems == 1) {
        ...
        mNotificationBuilder.setContentText(
            buildNotificationSubtitleForMedia());
      } else {
        mNotificationBuilder.setContentTitle(
            buildNotificationTitleForMixedContent());
        mNotificationBuilder.setContentText(
            buildNotificationSubtitleForMedia());
      }
    } else if (sNotificationData.mTotalMediaItems == 0 &&
               sNotificationData.mTotalPostItems > 0) {
      // only Post / Pages are being uploaded
      // check if special case for ONE Post
      if (sNotificationData.mTotalPostItems == 1) {
        mNotificationBuilder.setContentTitle(
            buildNotificationTitleForPost(post));
        mNotificationBuilder.setContentText(
            buildNotificationSubtitleForPost(post));
      } else {
        mNotificationBuilder.setContentTitle(
            buildNotificationTitleForMixedContent());
        mNotificationBuilder.setContentText(
            buildNotificationSubtitleForPosts());
      }
    } else {
      ...
      mNotificationBuilder.setContentTitle(
          buildNotificationTitleForMixedContent());
      mNotificationBuilder.setContentText(
          buildNotificationSubtitleForMixedContent());
    }
  }

  private synchronized void
  startOrUpdateForegroundNotification(@Nullable PostModel post) {
    updateNotificationBuilder(post);
    if (sNotificationData.mNotificationId == 0) {
      sNotificationData.mNotificationId = (new Random()).nextInt();
      mService.startForeground(sNotificationData.mNotificationId,
  ...
  void removePostInfoFromForegroundNotificationData(
      @NonNull PostModel post, @Nullable List<MediaModel> media) {
  ...
  void addPostInfoToForegroundNotification(@NonNull PostModel post,
  ...
  void
  removePostInfoFromForegroundNotification(@NonNull PostModel post,
                                           @Nullable List<MediaModel> media) {
    removePostInfoFromForegroundNotificationData(post, media);
    startOrUpdateForegroundNotification(post);
  }

  void removeMediaInfoFromForegroundNotification(
  ...
  void
  addMediaInfoToForegroundNotification(@NonNull List<MediaModel> mediaList) {
    sNotificationData.mTotalMediaItems += mediaList.size();
  ...
  void incrementUploadedPostCountFromForegroundNotification(
      @NonNull PostModel post) {
    incrementUploadedPostCountFromForegroundNotification(post, false);
  }

  void
  incrementUploadedPostCountFromForegroundNotification(@NonNull PostModel post,
                                                       boolean force) {
    // first we need to check that we only count this post once as "ended"
    // (either successfully or with error) for every error we get. We'll then
    // try to increment the Post count as it's been cancelled/failed because the
    // related media was cancelled or has failed too (i.e. we can't upload a
    // Post with failed media, therefore it needs to be cancelled).
  ...
  private boolean removeNotificationAndStopForegroundServiceIfNoItemsInQueue() {
    if (sNotificationData.mCurrentPostItem ==
            sNotificationData.mTotalPostItems &&
        sNotificationData.mCurrentMediaItem ==
            sNotificationData.mTotalMediaItems) {
      mNotificationManager.cancel(sNotificationData.mNotificationId);
      // reset the notification id so a new one is generated next time the
      // service is started
  ...
  // cancels the error or success notification (only one of these exist per Post
  // at any given time
  static void cancelFinalNotification(Context context,
                                      @NonNull PostModel post) {
    if (context != null) {
      NotificationManager notificationManager =
          (NotificationManager)SystemServiceFactory.get(
              context, Context.NOTIFICATION_SERVICE);
      notificationManager.cancel((int)getNotificationIdForPost(post));
    }
  }

  static void cancelFinalNotificationForMedia(Context context,
  ...
  void updateNotificationSuccessForPost(@NonNull PostModel post,
                                        @NonNull SiteModel site,
                                        boolean isFirstTimePublish) {
    if (!WordPress.sAppIsInTheBackground) {
      // only produce success notifications for the user if the app is in the
      // background
      return;
    }
    ...
    String postTitle = TextUtils.isEmpty(post.getTitle())
                           ? mContext.getString(R.string.untitled)
                           : post.getTitle();
    ...
    case PUBLISHED:
      if (post.isPage()) {
        notificationTitle +=
            mContext.getString(isFirstTimePublish ? R.string.page_published
                                                  : R.string.page_updated);
      } else {
        notificationTitle +=
            mContext.getString(isFirstTimePublish ? R.string.post_published
                                                  : R.string.post_updated);
      }
      break;
    default:
      notificationTitle += mContext.getString(
          post.isPage() ? R.string.page_updated : R.string.post_updated);
      break;
    }

    notificationBuilder.setSmallIcon(R.drawable.ic_my_sites_white_24dp);
    notificationBuilder.setColor(
        mContext.getResources().getColor(R.color.primary_50));
    ...
    Intent notificationIntent =
        getNotificationIntent(post, site, notificationId);

    PendingIntent pendingIntentPost = PendingIntent.getActivity(
        mContext, (int)notificationId, notificationIntent,
        PendingIntent.FLAG_ONE_SHOT);
    notificationBuilder.setContentIntent(pendingIntentPost);

    // Share intent - started if the user tap the share link button - only if
    // the link exist
    if (shareableUrl != null &&
        PostStatus.fromPost(post) == PostStatus.PUBLISHED) {
      Intent shareIntent =
          new Intent(mContext, ShareAndDismissNotificationReceiver.class);
      shareIntent.putExtra(
          ShareAndDismissNotificationReceiver.NOTIFICATION_ID_KEY,
          notificationId);
      shareIntent.putExtra(Intent.EXTRA_TEXT, shareableUrl);
      shareIntent.putExtra(Intent.EXTRA_SUBJECT, post.getTitle());
      PendingIntent pendingIntent = PendingIntent.getBroadcast(
          mContext, 0, shareIntent, PendingIntent.FLAG_CANCEL_CURRENT);
      notificationBuilder.addAction(R.drawable.ic_share_white_24dp,
                                    mContext.getString(R.string.share_action),
                                    pendingIntent);
    }

    // add draft Publish action for drafts
    if (PostStatus.fromPost(post) == PostStatus.DRAFT ||
        PostStatus.fromPost(post) == PostStatus.PENDING) {
      Intent publishIntent = UploadService.getPublishPostServiceIntent(
          mContext, post, isFirstTimePublish);
      PendingIntent pendingIntent = PendingIntent.getService(
          mContext, 0, publishIntent, PendingIntent.FLAG_CANCEL_CURRENT);
      notificationBuilder.addAction(R.drawable.ic_posts_white_24dp,
                                    mContext.getString(R.string.button_publish),
                                    pendingIntent);
    }
  ...
  void updateNotificationSuccessForMedia(@NonNull List<MediaModel> mediaList,
                                         @NonNull SiteModel site) {
    // show the snackbar
    if (mediaList != null && !mediaList.isEmpty()) {
      String snackbarMessage =
          buildSnackbarSuccessMessageForMedia(mediaList.size());
      EventBus.getDefault().postSticky(
          new UploadService.UploadMediaSuccessEvent(mediaList,
                                                    snackbarMessage));
    }
    ...
    Intent notificationIntent =
        new Intent(mContext, MediaBrowserActivity.class);
    ...
    String notificationMessage = TextUtils.isEmpty(site.getName())
                                     ? mContext.getString(R.string.untitled)
                                     : site.getName();
    ...
    // NotificationCompat.BigTextStyle().bigText(newSuccessMessage));
    ...
    if (mediaList != null && !mediaList.isEmpty()) {
      ...
      writePostIntent.putExtra(EditPostActivity.EXTRA_INSERT_MEDIA,
                               mediaToIncludeInPost);
      writePostIntent.setAction(String.valueOf(notificationId));

      PendingIntent actionPendingIntent = PendingIntent.getActivity(
          mContext, RequestCodes.EDIT_POST, writePostIntent,
          PendingIntent.FLAG_CANCEL_CURRENT);
      notificationBuilder.addAction(
          0, mContext.getString(R.string.media_files_uploaded_write_post),
          actionPendingIntent);
    }
  ...
  public static long getNotificationIdForPost(PostModel post) {
    ...
    if (post.isLocalDraft()) {
      ...
      // Note: local drafts just don't have a remote post Id set yet, so they
      // are all 0. Because of this, we only use the local id for local drafts.
      // there could be the case where a local draft on the device, and a remote
      // post, can get the same ID, since the ID is returned from 2 different
      // sources. In theory this is true, but however the chances are too low so
      // it seems it should work in practically 100% of times.
    }
    // We can't use the local table post id here because it can change between
    // first post (local draft) to first edit (post pulled from the server) but,
    // if this is a local draft, we don't have a choice but to use the local id.
    // otherwise, remote ID is always 0 for any local draft, and this means we'd
    // be providing the same notificationId for 2 different local drafts for the
    // same site, with this ending in the latest notification "stepping" on the
    // first one (the user would always get to see the latest failed local
    // draft, but wouldn't get a notice about the previous ones).
  ...
  /*
   * This method will create an error notification with the description of the
   * *final state* of the queue for this Post (i.e. how many media items have
   * been uploaded successfully and how many failed, as well as the information
   * for the Post itself if we couldn't upload it).
   *
   * In order to give the user a description of the *current state* of failed
   * media items, you can pass a value other than zero (0) in
   * overrideMediaNotUploadedCount and this value will be shown instead.
   */
  void updateNotificationErrorForPost(@NonNull PostModel post,
                                      @NonNull SiteModel site,
                                      String errorMessage,
    ...
    String notificationTitle = String.format(
        mContext.getString(R.string.upload_failed_param), postTitle);

    String newErrorMessage =
        buildErrorMessageMixed(overrideMediaNotUploadedCount);
    String snackbarMessage =
        buildSnackbarErrorMessage(newErrorMessage, errorMessage);
    ...
    if (AppPrefs.isAztecEditorEnabled()) {
      Intent publishIntent = UploadService.getRetryUploadServiceIntent(
          mContext, post, PostUtils.isFirstTimePublish(post));
      PendingIntent actionPendingIntent = PendingIntent.getService(
          mContext, 0, publishIntent, PendingIntent.FLAG_CANCEL_CURRENT);
      notificationBuilder
          .addAction(0, mContext.getString(R.string.retry), actionPendingIntent)
          .setColor(mContext.getResources().getColor(R.color.accent));
    }

    EventBus.getDefault().postSticky(
        new UploadService.UploadErrorEvent(post, snackbarMessage));

    doNotify(notificationId, notificationBuilder.build());
  }

  @NonNull
  private Intent getNotificationIntent(@NonNull PostModel post,
                                       @NonNull SiteModel site,
                                       long notificationId) {
    ...
    if (post.isPage()) {
      ...
      notificationIntent.putExtra(EXTRA_PAGE_REMOTE_ID_KEY,
                                  post.getRemotePostId());
    } else {
      notificationIntent = new Intent(mContext, PostsListActivity.class);
      notificationIntent.putExtra(
          PostsListActivityKt.EXTRA_TARGET_POST_LOCAL_ID, post.getId());
    }
  ...
  void updateNotificationErrorForMedia(@NonNull List<MediaModel> mediaList,
                                       @NonNull SiteModel site,
                                       String errorMessage) {
    AppLog.d(AppLog.T.MEDIA,
             "updateNotificationErrorForMedia: " + errorMessage);
    ...
    String siteName = TextUtils.isEmpty(site.getName())
                          ? mContext.getString(R.string.untitled)
                          : site.getName();
    String notificationTitle = String.format(
        mContext.getString(R.string.upload_failed_param), siteName);
    ...
    if (mediaList != null && !mediaList.isEmpty()) {
      ...
      Intent publishIntent = UploadService.getUploadMediaServiceIntent(
          mContext, mediaListToRetry, true);
      PendingIntent actionPendingIntent = PendingIntent.getService(
          mContext, 1, publishIntent, PendingIntent.FLAG_CANCEL_CURRENT);
      notificationBuilder
          .addAction(0, mContext.getString(R.string.retry), actionPendingIntent)
          .setColor(mContext.getResources().getColor(R.color.accent));
    }
    ...
    EventBus.getDefault().postSticky(
        new UploadService.UploadErrorEvent(mediaList, snackbarMessage));
    doNotify(notificationId, notificationBuilder.build());
  }
  ...
  private String buildErrorMessageMixed(int overrideMediaNotUploadedCount) {
    ...
    int postItemsNotUploaded =
        sNotificationData.mTotalPostItems > 0
            ...
            : 0;
    int mediaItemsNotUploaded =
        overrideMediaNotUploadedCount > 0
            ? overrideMediaNotUploadedCount
    ...
    if (postItemsNotUploaded > 0 && mediaItemsNotUploaded > 0) {
      ...
      case POST:
        newErrorMessage =
            (mediaItemsNotUploaded == 1
                 ? mContext.getString(
                       R.string
                           .media_file_post_singular_mixed_not_uploaded_one_file)
                 : String.format(
                       mContext.getString(
                           R.string
                               .media_file_post_singular_mixed_not_uploaded_files_plural),
      ...
      case PAGE:
        newErrorMessage =
            (mediaItemsNotUploaded == 1
                 ? mContext.getString(
                       R.string
                           .media_file_page_singular_mixed_not_uploaded_one_file)
                 : String.format(
                       mContext.getString(
                           R.string
                               .media_file_page_singular_mixed_not_uploaded_files_plural),
                       mediaItemsNotUploaded));
        break;
      case PAGES:
        newErrorMessage =
            (mediaItemsNotUploaded == 1
                 ? String.format(
                       mContext.getString(
                           R.string
                               .media_file_pages_plural_mixed_not_uploaded_one_file),
                       postItemsNotUploaded)
                 : String.format(
                       mContext.getString(
                           R.string
                               .media_file_pages_plural_mixed_not_uploaded_files_plural),
        ...
        newErrorMessage =
            (mediaItemsNotUploaded == 1
                 ? String.format(
                       mContext.getString(
                           R.string
                               .media_file_pages_and_posts_mixed_not_uploaded_one_file),
                       postItemsNotUploaded)
                 : String.format(
                       mContext.getString(
                           R.string
                               .media_file_pages_and_posts_mixed_not_uploaded_files_plural),
                       postItemsNotUploaded, mediaItemsNotUploaded));
        break;
      case POSTS:
      default:
        newErrorMessage =
            (mediaItemsNotUploaded == 1
                 ? String.format(
                       mContext.getString(
                           R.string
                               .media_file_posts_plural_mixed_not_uploaded_one_file),
                       postItemsNotUploaded)
                 : String.format(
                       mContext.getString(
                           R.string
                               .media_file_posts_plural_mixed_not_uploaded_files_plural),
                       postItemsNotUploaded, mediaItemsNotUploaded));
        break;
      }
      ...
      switch (getPagesAndOrPostsType(postItemsNotUploaded)) {
      case POST:
        newErrorMessage = mContext.getString(
            R.string.media_file_post_singular_only_not_uploaded);
        break;
      case PAGE:
        newErrorMessage = mContext.getString(
            R.string.media_file_page_singular_only_not_uploaded);
        break;
      case PAGES:
        newErrorMessage = String.format(
            mContext.getString(
                R.string.media_file_pages_plural_only_not_uploaded),
            postItemsNotUploaded);
        break;
      case PAGES_OR_POSTS:
        newErrorMessage = String.format(
            mContext.getString(
                R.string.media_file_pages_and_posts_only_not_uploaded),
            postItemsNotUploaded);
        break;
      case POSTS:
      default:
        newErrorMessage = String.format(
            mContext.getString(
                R.string.media_file_posts_plural_only_not_uploaded),
            postItemsNotUploaded);
        break;
      }
      ...
      if (mediaItemsNotUploaded == 1) {
        ...
        newErrorMessage =
            String.format(mContext.getString(R.string.media_files_not_uploaded),
                          mediaItemsNotUploaded);
      }
    }

    if (mediaItemsNotUploaded > 0 && (getCurrentMediaItem()) > 0) {
      // some media items were uploaded successfully
      newErrorMessage += String.format(
          mContext.getString(R.string.media_files_uploaded_successfully),
          sNotificationData.mCurrentMediaItem);
    }
      ...
      if (mediaItemsNotUploaded <= sNotificationData.mCurrentMediaItem) {
        ...
        newErrorMessage +=
            " " + String.format(mContext.getString(
                                    R.string.media_files_uploaded_successfully),
                                sNotificationData.mCurrentMediaItem);
      }
    }
  ...
  private String buildSuccessMessageForMedia(int mediaItemsUploaded) {
    ...
    String successMessage =
        mediaItemsUploaded == 1
            ? mContext.getString(R.string.media_file_uploaded)
            : String.format(mContext.getString(
                                R.string.media_all_files_uploaded_successfully),
                            mediaItemsUploaded);
  ...
  private String buildSnackbarSuccessMessageForMedia(int mediaItemsUploaded) {
    ...
    if (mediaItemsUploaded > 0) {
      if (mediaItemsUploaded == 1) {
        ...
        successMessage +=
            String.format(mContext.getString(R.string.media_files_uploaded),
                          mediaItemsUploaded);
      }
    }
    return successMessage;
  ...
  private String buildSnackbarErrorMessage(String newErrorMessage,
  ...
  void updateNotificationProgressForMedia(MediaModel media, float progress) {
    ...
    // it could happen that a media item is already done uploading but we
    // receive an upload progress event from FluxC after that. We just need to
    // avoid re-adding the item to the map.
    Float currentProgress =
        sNotificationData.mediaItemToProgressMap.get(media.getId());
    // also, only set updates in increments of 5% per media item to avoid lots
  ...
  private void updateNotificationProgress() {
    ...
    mNotificationBuilder.setProgress(
        100, (int)Math.ceil(getCurrentOverallProgress() * 100), false);
    doNotify(sNotificationData.mNotificationId, mNotificationBuilder.build());
  }
  ...
  private float getCurrentOverallProgress() {
    int totalItemCount =
        sNotificationData.mTotalPostItems + sNotificationData.mTotalMediaItems;
  ...
  private synchronized void doNotify(long id, Notification notification) {
    try {
      ...
      CrashLoggingUtils.logException(runtimeException, AppLog.T.UTILS,
                                     "See issue #2858 / #3966");
      AppLog.d(AppLog.T.POSTS, "See issue #2858 / #3966; notify failed with:" +
                                   runtimeException);
    }
  }
  ...
  private String buildNotificationTitleForPost(PostModel post) {
    String postTitle = (post == null || TextUtils.isEmpty(post.getTitle()))
                           ? mContext.getString(R.string.untitled)
                           : post.getTitle();
    return String.format(mContext.getString(R.string.uploading_post),
                         postTitle);
  }
  ...
  private String buildNotificationSubtitleForPost(PostModel post) {
    String uploadingMessage =
        (post != null && post.isPage())
            ? mContext.getString(R.string.uploading_subtitle_pages_only_one)
  ...
  private String buildNotificationSubtitleForPosts() {
    ...
    case PAGES:
      strToUse =
          mContext.getString(R.string.uploading_subtitle_pages_only_plural);
      break;
    case PAGES_OR_POSTS:
    ...
    default:
      strToUse =
          mContext.getString(R.string.uploading_subtitle_posts_only_plural);
      break;
    }
  ...
  private PagesOrPostsType getPagesAndOrPostsType(int remaining) {
    ...
    if (sNotificationData.mTotalPageItemsIncludedInPostCount > 0 &&
        sNotificationData.mTotalPostItems > 0 &&
        sNotificationData.mTotalPostItems >
            sNotificationData.mTotalPageItemsIncludedInPostCount) {
  ...
  private String buildNotificationSubtitleForMedia() {
    ...
    if (sNotificationData.mTotalMediaItems == 1) {
      uploadingMessage =
          mContext.getString(R.string.uploading_subtitle_media_only_one);
    } else {
  ...
  private String buildNotificationSubtitleForMixedContent() {
    ...
    if (sNotificationData.mTotalMediaItems == 1) {
      ...
      case PAGES:
        uploadingMessage = String.format(
            mContext.getString(
                R.string.uploading_subtitle_mixed_pages_plural_media_one),
            remaining);
        break;
      case PAGE:
        uploadingMessage = mContext.getString(
            R.string.uploading_subtitle_mixed_page_singular_media_one);
        break;
      case POST:
        uploadingMessage = mContext.getString(
            R.string.uploading_subtitle_mixed_post_singular_media_one);
        break;
      case PAGES_OR_POSTS:
        uploadingMessage = String.format(
            mContext.getString(
                R.string
                    .uploading_subtitle_mixed_pages_and_posts_plural_media_one),
            remaining);
        break;
      case POSTS:
      default:
        uploadingMessage = String.format(
            mContext.getString(
                R.string.uploading_subtitle_mixed_posts_plural_media_one),
            remaining);
        break;
      }
    } else {
      ...
      case PAGES:
        uploadingMessage = String.format(
            mContext.getString(
                R.string.uploading_subtitle_mixed_pages_plural_media_plural),
            remaining,
            sNotificationData.mTotalMediaItems - getCurrentMediaItem(),
            sNotificationData.mTotalMediaItems);
        break;
      case PAGE:
        uploadingMessage = String.format(
            mContext.getString(
                R.string.uploading_subtitle_mixed_page_singular_media_plural),
            sNotificationData.mTotalMediaItems - getCurrentMediaItem(),
            sNotificationData.mTotalMediaItems);
        break;
      case POST:
        uploadingMessage = String.format(
            mContext.getString(
                R.string.uploading_subtitle_mixed_post_singular_media_plural),
            sNotificationData.mTotalMediaItems - getCurrentMediaItem(),
            sNotificationData.mTotalMediaItems);
        break;
      case PAGES_OR_POSTS:
        uploadingMessage = String.format(
            mContext.getString(
                R.string
                    .uploading_subtitle_mixed_pages_and_posts_plural_media_plural),
            remaining,
            sNotificationData.mTotalMediaItems - getCurrentMediaItem(),
            sNotificationData.mTotalMediaItems);
        break;
      ...
      default:
        uploadingMessage = String.format(
            mContext.getString(
                R.string.uploading_subtitle_mixed_posts_plural_media_plural),
            remaining,
            sNotificationData.mTotalMediaItems - getCurrentMediaItem(),
            sNotificationData.mTotalMediaItems);
        break;
  ...
  private int getCurrentPostItem() {
    int currentPostItem =
        sNotificationData.mCurrentPostItem >= sNotificationData.mTotalPostItems
            ? sNotificationData.mTotalPostItems - 1
            : sNotificationData.mCurrentPostItem;
  ...
  private int getCurrentMediaItem() {
    int currentMediaItem = sNotificationData.mCurrentMediaItem >=
                                   sNotificationData.mTotalMediaItems
