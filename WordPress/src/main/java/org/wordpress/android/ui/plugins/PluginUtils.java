...
private static final List<String> AUTO_MANAGED_PLUGINS = Arrays.asList(
	JETPACK_PLUGIN_NAME, "akismet/akismet", "vaultpress/vaultpress");
...
public static boolean isPluginFeatureAvailable(SiteModel site) {
	...
	return site.isWPCom() && SiteUtils.hasNonJetpackBusinessPlan(site) &&
	       site.getHasCapabilityManageOptions() // Automated Transfer require admin
	                                            // capabilities
	       && !site.isPrivate(); // Private sites are not eligible for Automated
	                             // Transfer
}

static boolean
isUpdateAvailable(@Nullable ImmutablePluginModel immutablePlugin) {
	...
	try {
		...
		String errorStr = String.format(
			...
			installedVersionStr, availableVersionStr);
		AppLog.e(AppLog.T.PLUGINS, errorStr, e);
		CrashLoggingUtils.logException(e, AppLog.T.PLUGINS, errorStr);
		// If the versions are not in the expected format, we can assume that an
		// update is available if the version values for the site plugin and wporg
...
public static boolean isAutoManaged(@NonNull SiteModel site,
                                    @NonNull ImmutablePluginModel plugin) {
	...
	for (String pluginName : AUTO_MANAGED_PLUGINS) {
		isAutoManaged =
			isAutoManaged || StringUtils.equals(plugin.getName(), pluginName);
	}
