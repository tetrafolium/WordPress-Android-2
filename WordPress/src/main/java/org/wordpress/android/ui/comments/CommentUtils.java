...
public class CommentUtils {
   ...
   */
  public static void displayHtmlComment(TextView textView, String content,
                                        int maxImageSize) {
    if (textView == null) {
      return;
    }
    ...
    if (maxImageSize > 0 && content.contains("<img")) {
      html = HtmlUtils.fromHtml(
          content, new WPCustomImageGetter(textView, maxImageSize));
    } else {
  ...
  public static void indentTextViewFirstLine(TextView textView,
                                             int textOffsetX) {
    ...
    text.setSpan(new TextWrappingLeadingMarginSpan(textOffsetX), 0,
                 text.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    textView.setText(text);
  }

  private static class TextWrappingLeadingMarginSpan
