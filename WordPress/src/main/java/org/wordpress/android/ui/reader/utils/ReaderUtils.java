...
public class ReaderUtils {
  public static String getResizedImageUrl(final String imageUrl, int width,
                                          int height, boolean isPrivate) {
    return getResizedImageUrl(imageUrl, width, height, isPrivate,
  ...
  public static String getResizedImageUrl(final String imageUrl, int width,
    ...
    if (isPrivate) {
      ...
      return PhotonUtils.getPhotonImageUrl(unescapedUrl, width, height,
                                           quality);
    }
  }

  /*
   * use this to request a reduced size image from a private post - images in
   * private posts can't use photon but these are usually wp images so they
   * support the h= and w= query params
   */
  private static String getPrivateImageForDisplay(final String imageUrl,
    ...
    // remove the existing query string, add the new one, and make sure the url
    // is https:
    return UrlUtils.removeQuery(UrlUtils.makeHttps(imageUrl)) + query;
  }

  /*
   * returns the passed string formatted for use with our API - see
   ...
   */
  public static String getLongLikeLabelText(Context context, int numLikes,
    ...
    if (isLikedByCurrentUser) {
      ...
      default:
        String youAndMultiLikes =
            context.getString(R.string.reader_likes_you_and_multi);
        return String.format(LocaleManager.getSafeLocale(context),
                             youAndMultiLikes, numLikes - 1);
      }
    } else {
      if (numLikes == 1) {
        ...
        return String.format(LocaleManager.getSafeLocale(context), likes,
                             numLikes);
      }
    }
  }
  ...
  public static String getShortLikeLabelText(Context context, int numLikes) {
    ...
    default:
      ...
      return String.format(
          context.getString(R.string.reader_short_like_count_multi), count);
    }
  }

  public static String getShortCommentLabelText(Context context,
                                                int numComments) {
    switch (numComments) {
    case 1:
    ...
    default:
      ...
      return String.format(
          context.getString(R.string.reader_short_comment_count_multi), count);
    }
  }
   ...
   */
  public static boolean postAndCommentExists(long blogId, long postId,
                                             long commentId) {
  ...
  /*
   * used by Discover site picks to add a "Visit [BlogName]" link which shows
   * the native blog preview for that blog
   */
  ...
  /*
   * returns the passed string prefixed with a "#" if it's non-empty and isn't
   * already prefixed with a "#"
   */
  ...
  /*
   * set the background of the passed view to the round ripple drawable - only
   * works on Lollipop or later, does nothing on earlier Android versions
   */
  ...
  /*
   * returns a tag object from the passed endpoint if tag is in database,
   * otherwise null
   */
  ...
  /*
   * returns a tag object from the passed tag name - first checks for it in the
   * tag db (so we can also get its title & endpoint), returns a new tag if that
   * fails
   */
  public static ReaderTag getTagFromTagName(String tagName,
  ...
  public static ReaderTag createTagFromTagName(String tagName,
                                               ReaderTagType tagType) {
    String tagSlug = sanitizeWithDashes(tagName).toLowerCase(Locale.ROOT);
    String tagDisplayName =
        tagType == ReaderTagType.DEFAULT ? tagName : tagSlug;
    return new ReaderTag(tagSlug, tagDisplayName, tagName, null, tagType);
  }

  /*
   * returns the default tag, which is the one selected by default in the reader
   * when the user hasn't already chosen one
   */
  public static ReaderTag getDefaultTag() {
    ReaderTag defaultTag = getTagFromEndpoint(ReaderTag.TAG_ENDPOINT_DEFAULT);
    if (defaultTag == null) {
      defaultTag =
          getTagFromTagName(ReaderTag.TAG_TITLE_DEFAULT, ReaderTagType.DEFAULT);
    }
    return defaultTag;
  }
  ...
  public static ReaderTag getTagForSearchQuery(@NonNull String query) {
    ...
    return new ReaderTag(slug, trimQuery, trimQuery, null,
