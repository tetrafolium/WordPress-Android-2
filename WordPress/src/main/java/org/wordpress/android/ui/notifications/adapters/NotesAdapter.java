...
public class NotesAdapter
  ...
  public interface DataLoadedListener { void onDataLoaded(int itemsCount); }

  public interface OnLoadMoreListener { void onLoadMore(long timestamp); }
  ...
  public NotesAdapter(Context context, DataLoadedListener dataLoadedListener,
    ...
    // this is on purpose - we don't show more than a hundred or so
    // notifications at a time so no need to set stable IDs. This helps prevent
    // crashes in case a note comes with no ID (we've code checking for that
    // elsewhere, but telling the RecyclerView.Adapter the notes have stable Ids
    // and then failing to provide them will make things go south as in
    // https://github.com/wordpress-mobile/WordPress-Android/issues/8741
    setHasStableIds(false);

    mAvatarSz = (int)context.getResources().getDimension(
        R.dimen.notifications_avatar_sz);
    mColorRead = context.getResources().getColor(android.R.color.white);
    mColorUnread =
        context.getResources().getColor(R.color.background_notification_unread);
    mTextIndentSize = context.getResources().getDimensionPixelSize(
        R.dimen.notifications_text_indent_sz);
  }

  public void setFilter(FILTERS newFilter) { mCurrentFilter = newFilter; }

  public FILTERS getCurrentFilter() { return mCurrentFilter; }
  ...
  // Instead of building the filtered notes list dynamically, create it once and
  // re-use it. Otherwise it's re-created so many times during layout.
  public static void buildFilteredNotesList(ArrayList<Note> filteredNotes,
  ...
  public void onBindViewHolder(NoteViewHolder noteViewHolder, int position) {
    ...
    Note.NoteTimeGroup timeGroup =
        Note.getTimeGroupForTimestamp(note.getTimestamp());

    Note.NoteTimeGroup previousTimeGroup = null;
    if (position > 0) {
      Note previousNote = getNoteAtPosition(position - 1);
      previousTimeGroup =
          Note.getTimeGroupForTimestamp(previousNote.getTimestamp());
    }
    ...
    Spanned noteSubjectSpanned =
        note.getFormattedSubject(mNotificationsUtilsWrapper);
    // Trim the '\n\n' added by Html.fromHtml()
    noteSubjectSpanned = (Spanned)noteSubjectSpanned.subSequence(
        0, TextUtils.getTrimmedLength(noteSubjectSpanned));

    NoteBlockClickableSpan[] spans = noteSubjectSpanned.getSpans(
        0, noteSubjectSpanned.length(), NoteBlockClickableSpan.class);
    ...
    if (!TextUtils.isEmpty(noteSubjectNoticon)) {
      ...
      if (parent instanceof ViewGroup) {
        int textDirection = BidiFormatter.getInstance().isRtl(
                                noteViewHolder.mTxtSubject.getText())
      ...
      CommentUtils.indentTextViewFirstLine(noteViewHolder.mTxtSubject,
                                           mTextIndentSize);
    ...
    String avatarUrl =
        GravatarUtils.fixGravatarUrl(note.getIconURL(), mAvatarSz);
    mImageManager.loadIntoCircle(noteViewHolder.mImgAvatar,
    ...
    if (commentStatus == CommentStatus.UNAPPROVED) {
      noteViewHolder.mNoteIcon.setBackgroundResource(
          R.drawable.bg_oval_warning_stroke_white);
    } else if (isUnread) {
      noteViewHolder.mNoteIcon.setBackgroundResource(
          R.drawable.bg_oval_primary_40_stroke_notification_unread);
    } else {
      noteViewHolder.mNoteIcon.setBackgroundResource(
          R.drawable.bg_oval_neutral_20_stroke_white);
    }
  ...
  public void reloadNotesFromDBAsync() {
    new ReloadNotesFromDBTask().executeOnExecutor(
        AsyncTask.THREAD_POOL_EXECUTOR);
  }

  private class ReloadNotesFromDBTask
      extends AsyncTask<Void, Void, ArrayList<Note>> {
    @Override
