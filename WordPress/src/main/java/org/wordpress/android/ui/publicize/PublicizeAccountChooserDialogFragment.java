...
public class PublicizeAccountChooserDialogFragment
    extends DialogFragment implements PublicizeAccountChooserListAdapter
                                          .OnPublicizeAccountChooserListener {
  ...
  public Dialog onCreateDialog(Bundle savedInstanceState) {
    ...
    View view =
        inflater.inflate(R.layout.publicize_account_chooser_dialog, null);
  ...
  public void onDismiss(DialogInterface dialog) {
    ...
    if (activity != null && activity instanceof
  ...
  private void configureRecyclerViews(View view) {
    PublicizeAccountChooserListAdapter notConnectedAdapter =
        new PublicizeAccountChooserListAdapter(
            getActivity(), mNotConnectedAccounts, this, false);
    notConnectedAdapter.setHasStableIds(true);
    mNotConnectedRecyclerView =
        (RecyclerView)view.findViewById(R.id.not_connected_recyclerview);
    mNotConnectedRecyclerView.setLayoutManager(
  ...
  private void hideConnectedView(View view) {
    LinearLayout connectedHeader =
        (LinearLayout)view.findViewById(R.id.connected_header);
    connectedHeader.setVisibility(View.GONE);
  }

  private void populateConnectedListView(View view) {
    RecyclerView listViewConnected =
        (RecyclerView)view.findViewById(R.id.connected_recyclerview);
    PublicizeAccountChooserListAdapter connectedAdapter =
        new PublicizeAccountChooserListAdapter(getActivity(),
                                               mConnectedAccounts, null, true);
  ...
  private void configureAlertDialog(View view, AlertDialog.Builder builder) {
    ...
    builder.setTitle(
        getString(R.string.connecting_social_network, mConnectionName));
    builder.setMessage(getString(R.string.connection_chooser_message));
    builder.setPositiveButton(
        R.string.share_btn_connect, new DialogInterface.OnClickListener() {
          @Override
          public void onClick(DialogInterface dialogInterface, int i) {
            dialogInterface.dismiss();
            int keychainId =
                mNotConnectedAccounts.get(mSelectedIndex).connectionId;
            String service =
                mNotConnectedAccounts.get(mSelectedIndex).getService();
            String externalUserId =
                mNotConnectedAccounts.get(mSelectedIndex).getExternalId();
            EventBus.getDefault().post(new PublicizeEvents.ActionAccountChosen(
                mSite.getSiteId(), keychainId, service, externalUserId));
          }
        });
        ...
        R.string.cancel, new DialogInterface.OnClickListener() {
          ...
          public void onClick(DialogInterface dialogInterface, int i) {
            ...
            ToastUtils.showToast(
                getActivity(),
                getActivity().getString(R.string.cannot_connect_account_error,
                                        mConnectionName));
          }
        });
  }
  ...
  private void retrieveCurrentSiteFromArgs() {
    ...
    if (args != null) {
      ...
      String jsonString =
          args.getString(PublicizeConstants.ARG_CONNECTION_ARRAY_JSON);
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.length(); i++) {
        ...
        PublicizeConnection connection =
            PublicizeConnection.fromJson(currentConnectionJson);
        if (connection.getService().equals(mServiceId)) {
          ...
          JSONArray externalJsonArray =
              currentConnectionJson.getJSONArray("additional_external_users");
          for (int j = 0; j < externalJsonArray.length(); j++) {
            JSONObject currentExternalConnectionJson =
                externalJsonArray.getJSONObject(j);
            PublicizeConnection.updateConnectionfromExternalJson(
                connection, currentExternalConnectionJson);
            if (connection.isInSite(mSite.getSiteId())) {
              mConnectedAccounts.add(connection);
            } else {
              mNotConnectedAccounts.add(connection);
