...
public class CommentDetailFragment
    extends Fragment implements NotificationFragment {
  ...
  private SuggestionServiceConnectionManager
  ...
  /*
   * these determine which actions (moderation, replying, marking as spam) to
   * enable for this comment - all actions are enabled when opened from the
   * comment list, only changed when opened from a notification
   */
  private EnumSet<EnabledActions> mEnabledActions =
      EnumSet.allOf(EnabledActions.class);

  /*
   * used when called from comment list
   */
  static CommentDetailFragment newInstance(SiteModel site,
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    case FROM_BLOG_COMMENT:
      setComment(getArguments().getLong(KEY_COMMENT_ID),
                 getArguments().getInt(KEY_SITE_LOCAL_ID));
      break;
    case FROM_NOTE:
  ...
  // touching the file resulted in the MethodLength, it's suppressed until we
  // get time to refactor this method
  @SuppressWarnings("checkstyle:MethodLength")
  @Override
  public View onCreateView(LayoutInflater inflater, ViewGroup container,
                           Bundle savedInstanceState) {
    final View view =
        inflater.inflate(R.layout.comment_detail_fragment, container, false);

    mTxtStatus = view.findViewById(R.id.text_status);
    mTxtContent = view.findViewById(R.id.text_content);

    // noinspection InflateParams
    mLayoutButtons = (ViewGroup)inflater.inflate(R.layout.comment_action_footer,
                                                 null, false);
    ...
    // As we are using CommentDetailFragment in a ViewPager, and we also use
    // nested fragments within CommentDetailFragment itself: It is important to
    // have a live reference to the Comment Container layout at the moment this
    // layout is inflated (onCreateView), so we can make sure we set its ID
    // correctly once we have an actual Comment object to populate it with.
    // Otherwise, we could be searching and finding the container for _another
    // fragment/page in the viewpager_, which would cause strange results
    // (changing the views for a different fragment than we intended to).
    ...
    mSubmitReplyBtn.setOnLongClickListener(new OnLongClickListener() {
      ...
      public boolean onLongClick(View view) {
        ...
        Toast.makeText(view.getContext(), R.string.send, Toast.LENGTH_SHORT)
            .show();
        return true;
      }
    });
    ...
    buttonExpand.setOnClickListener(new OnClickListener() {
      ...
      public void onClick(View v) {
        Bundle bundle = Companion.newBundle(mEditReply.getText().toString(),
    ...
    buttonExpand.setOnLongClickListener(new OnLongClickListener() {
      ...
      public boolean onLongClick(View view) {
        ...
        Toast
            .makeText(view.getContext(), R.string.description_expand,
                      Toast.LENGTH_SHORT)
            .show();
        return true;
      }
    ...
    // hide comment like button until we know it can be enabled in
    // showCommentAsNotification()
    mBtnLikeComment.setVisibility(View.GONE);

    // hide moderation buttons until updateModerationButtons() is called
    mLayoutButtons.setVisibility(View.GONE);

    // this is necessary in order for anchor tags in the comment text to be
    // clickable
    ...
    mBtnSpamComment.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        ...
        if (CommentStatus.fromString(mComment.getStatus()) ==
            CommentStatus.SPAM) {
    ...
    mBtnTrashComment.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        ...
        // If the comment status is trash or spam, next deletion is a permanent
        // deletion.
        if (status == CommentStatus.TRASH || status == CommentStatus.SPAM) {
          ...
          dialogBuilder.setMessage(
              getResources().getText(R.string.dlg_sure_to_delete_comment));
  ...
  private void setupSuggestionServiceAndAdapter() {
    if (!isAdded() || mSite == null ||
        !SiteUtils.isAccessedViaWPComRest(mSite)) {
      return;
    }
    mSuggestionServiceConnectionManager =
        new SuggestionServiceConnectionManager(getActivity(),
                                               mSite.getSiteId());
    mSuggestionAdapter = SuggestionUtils.setupSuggestions(
        mSite, getActivity(), mSuggestionServiceConnectionManager);
  ...
  private void setReplyUniqueId() {
    if (mEditReply != null && isAdded()) {
      ...
      if (mSite != null && mComment != null) {
        sId = String.format(Locale.US, "%d-%d", mSite.getSiteId(),
                            mComment.getRemoteCommentId());
      } else if (mNote != null) {
        sId = String.format(Locale.US, "%d-%d", mNote.getSiteId(),
                            mNote.getCommentId());
      }
      if (sId != null) {
  ...
  private void setComment(final long commentRemoteId, final int siteLocalId) {
    ...
    setComment(mCommentStore.getCommentBySiteAndRemoteId(site, commentRemoteId),
               site);
  }

  private void setComment(@Nullable final CommentModel comment,
                          @Nullable final SiteModel site) {
    mComment = comment;
    mSite = site;

    // is this comment on one of the user's blogs? it won't be if this was
    // displayed from a notification about a reply to a comment this user posted
  ...
  public void enableShouldFocusReplyField() { mShouldFocusReplyField = true; }
  ...
  public void setNote(Note note) {
    ...
    if (mSite == null) {
      // This should not exist, we should clean that screen so a note without a
      // site/comment can be displayed
  ...
  public void
  onEventMainThread(SuggestionEvents.SuggestionNameListUpdated event) {
    // check if the updated suggestions are for the current blog and update the
    // suggestions
    if (event.mRemoteBlogId != 0 && mSite != null &&
        event.mRemoteBlogId == mSite.getSiteId() &&
        mSuggestionAdapter != null) {
      List<Suggestion> suggestions =
          SuggestionTable.getSuggestionsForSite(event.mRemoteBlogId);
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (requestCode == INTENT_COMMENT_EDITOR &&
  ...
  private void reloadComment() {
    ...
    CommentModel updatedComment =
        mCommentStore.getCommentByLocalId(mComment.getId());
  ...
  private void editComment() {
    ...
    // IMPORTANT: don't use getActivity().startActivityForResult() or else
    // onActivityResult() won't be called in this fragment
  ...
  private void showComment() {
    ...
    // hide container views when comment is null (will happen when opened from a
    // notification)
    if (mComment == null) {
      ...
      if (mNote != null) {
        ...
        if (site == null) {
          // This should not exist, we should clean that screen so a note
          // without a site/comment can be displayed
          site = createDummyWordPressComSite(mNote.getSiteId());
        }

        // Check if the comment is already in our store
        CommentModel comment = mCommentStore.getCommentBySiteAndRemoteId(
            site, mNote.getCommentId());
        if (comment != null) {
          ...
          RemoteCommentPayload payload =
              new RemoteCommentPayload(site, mNote.getCommentId());
          mDispatcher.dispatch(
              CommentActionBuilder.newFetchCommentAction(payload));
          setProgressVisible(true);

          // Show a "temporary" comment built from the note data, the view will
          // be refreshed once the comment has been fetched.
    ...
    txtName.setText(mComment.getAuthorName() == null
                        ? getString(R.string.anonymous)
                        : mComment.getAuthorName());
    txtDate.setText(DateTimeUtils.javaDateToTimeSpan(
        DateTimeUtils.dateFromIso8601(mComment.getDatePublished()),
        WordPress.getContext()));

    int maxImageSz = getResources().getDimensionPixelSize(
        R.dimen.reader_comment_max_image_size);
    CommentUtils.displayHtmlComment(mTxtContent, mComment.getContent(),
                                    maxImageSz);
    ...
    if (mComment.getAuthorProfileImageUrl() != null) {
      ...
      avatarUrl =
          GravatarUtils.gravatarFromEmail(mComment.getAuthorEmail(), avatarSz);
    }
    mImageManager.loadIntoCircle(imgAvatar, ImageType.AVATAR_WITH_BACKGROUND,
                                 avatarUrl);
      ...
      View.OnClickListener authorListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
          ReaderActivityLauncher.openUrl(getActivity(),
                                         mComment.getAuthorUrl());
        }
      };
      imgAvatar.setOnClickListener(authorListener);
      txtName.setOnClickListener(authorListener);
      txtName.setTextColor(
          ContextCompat.getColor(getActivity(), R.color.link_reader));
    } else {
      txtName.setTextColor(
          ContextCompat.getColor(getActivity(), R.color.neutral_60));
    }
  ...
  /*
   * displays the passed post title for the current comment, updates stored
   * title if one doesn't exist
   */
  private void setPostTitle(TextView txtTitle, String postTitle,
                            boolean isHyperlink) {
    ...
    // if comment doesn't have a post title, set it to the passed one and save
    // to comment table
    if (mComment != null && mComment.getPostTitle() == null) {
      mComment.setPostTitle(postTitle);
      mDispatcher.dispatch(
          CommentActionBuilder.newUpdateCommentAction(mComment));
    }

    // display "on [Post Title]..."
    if (isHyperlink) {
      String html =
          getString(R.string.on) + " <font color=" +
  ...
  /*
   * ensure the post associated with this comment is available to the reader and
   * show its title above the comment
   */
  private void showPostTitle(final SiteModel site, final long postId) {
    ...
    // the post this comment is on can only be requested if this is a .com blog
    // or a jetpack-enabled self-hosted blog, and we have valid .com credentials
    boolean canRequestPost = SiteUtils.isAccessedViaWPComRest(site) &&
    ...
    // if this is a .com or jetpack blog, tapping the title shows the associated
    // post in the reader
    if (canRequestPost) {
      // first make sure this post is available to the reader, and once it's
      // retrieved set the title if it wasn't set above
      if (!postExists) {
        AppLog.d(T.COMMENTS, "comment detail > retrieving post");
        ReaderPostActions.requestBlogPost(
            site.getSiteId(), postId, new ReaderActions.OnRequestListener() {
              @Override
              public void onSuccess() {
                ...
                if (!hasTitle) {
                  String postTitle =
                      ReaderPostTable.getPostTitle(site.getSiteId(), postId);
                  if (!TextUtils.isEmpty(postTitle)) {
        ...
        public void onClick(View v) {
          if (mOnPostClickListener != null) {
            ...
            ReaderActivityLauncher.showReaderPostDetail(
                getActivity(), site.getSiteId(), mComment.getRemotePostId());
          }
        }
      });
    }
  ...
  private void moderateComment(CommentStatus newStatus) {
    ...
    if (mNote != null && mOnNoteCommentActionListener != null) {
      ...
      // Sad, but onModerateComment does the moderation itself (due to the undo
      // bar), this should be refactored, That's why we don't call
      // dispatchModerationAction() here.
    }
  ...
  private void submitReply() {
    ...
    final ProgressBar progress =
        getView().findViewById(R.id.progress_submit_comment);
    ...
    mDispatcher.dispatch(CommentActionBuilder.newCreateNewCommentAction(
        new RemoteCreateCommentPayload(mSite, mComment, reply)));
  }
  ...
  private void updateStatusViews() {
    ...
    // comment status is only shown if this comment is from one of this user's
    // blogs and the comment hasn't been CommentStatus.APPROVED
    if (mIsUsersBlog && commentStatus != CommentStatus.APPROVED) {
      mTxtStatus.setText(
          getString(statusTextResId).toUpperCase(Locale.getDefault()));
    ...
    if (canTrash()) {
      ...
      if (commentStatus == CommentStatus.TRASH) {
        ColorUtils.INSTANCE.setImageResourceWithTint(
            mBtnModerateIcon, R.drawable.ic_undo_white_24dp, R.color.neutral);
  ...
  private void setModerateButtonForStatus(CommentStatus status) {
    ...
    ColorUtils.INSTANCE.setImageResourceWithTint(
        mBtnModerateIcon, R.drawable.ic_checkmark_white_24dp, color);
    mBtnModerateTextView.setTextColor(
  ...
  private boolean canModerate() {
    return mEnabledActions != null &&
        (mEnabledActions.contains(EnabledActions.ACTION_APPROVE) ||
         mEnabledActions.contains(EnabledActions.ACTION_UNAPPROVE));
  }

  private boolean canMarkAsSpam() {
    return (mEnabledActions != null &&
            mEnabledActions.contains(EnabledActions.ACTION_SPAM));
  }

  private boolean canReply() {
    return (mEnabledActions != null &&
            mEnabledActions.contains(EnabledActions.ACTION_REPLY));
  }

  private boolean canTrash() { return canModerate(); }

  private boolean canEdit() { return (mSite != null && canModerate()); }

  private boolean canLike() {
    return (mEnabledActions != null &&
            mEnabledActions.contains(EnabledActions.ACTION_LIKE) &&
   ...
   */
  private void showCommentAsNotification(Note note, @NonNull SiteModel site,
    ...
    /*
     * determine which actions to enable for this comment - if the comment is
     * from this user's blog then all actions will be enabled, but they won't be
     * if it's a reply to a comment this user made on someone else's blog
     */
    ...
    if (!TextUtils.isEmpty(mNote.getCommentAuthorName()) &&
      ...
      mEditReply.setHint(
          String.format(getString(R.string.comment_reply_to_user),
                        mNote.getCommentAuthorName()));
    }
  ...
  private void addDetailFragment(String noteId) {
    ...
    mNotificationsDetailListFragment =
        NotificationsDetailListFragment.newInstance(noteId);
    mNotificationsDetailListFragment.setFooterView(mLayoutButtons);
    fragmentTransaction.replace(mCommentContentLayout.getId(),
                                mNotificationsDetailListFragment);
  ...
  private void likeComment(boolean forceLike) {
    ...
    // Bump analytics
    AnalyticsTracker.track(mBtnLikeComment.isActivated()
    ...
    mDispatcher.dispatch(
        CommentActionBuilder.newLikeCommentAction(new RemoteLikeCommentPayload(
            mSite, mComment, mBtnLikeComment.isActivated())));
    mBtnLikeComment.announceForAccessibility(getText(
        mBtnLikeComment.isActivated() ? R.string.comment_liked_talkback
  ...
  private void onCommentModerated(OnCommentChanged event) {
    ...
    if (mNote != null) {
      EventBus.getDefault().postSticky(
          new NotificationEvents.NoteLikeOrModerationStatusChanged(
              mNote.getId()));
    }
  ...
  private void onCommentCreated(OnCommentChanged event) {
    ...
    getView()
        .findViewById(R.id.progress_submit_comment)
        .setVisibility(View.GONE);
    updateStatusViews();

    if (event.isError()) {
      if (isAdded()) {
        String strUnEscapeHTML =
            StringEscapeUtils.unescapeHtml4(event.error.message);
        ToastUtils.showToast(getActivity(), strUnEscapeHTML,
    ...
    if (isAdded()) {
      ToastUtils.showToast(getActivity(),
                           getString(R.string.note_reply_successful));
      mEditReply.setText(null);
      mEditReply.getAutoSaveTextHelper().clearSavedText(mEditReply);
    }

    // approve the comment
    if (mComment != null && !(CommentStatus.fromString(mComment.getStatus()) ==
                              CommentStatus.APPROVED)) {
      moderateComment(CommentStatus.APPROVED);
    }
  }
  ...
  private void
  announceCommentStatusChangeForAccessibility(CommentStatus newStatus) {
    int resId = -1;
    switch (newStatus) {
    case APPROVED:
