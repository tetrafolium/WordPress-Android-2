...
public void showTargetScreen(String action, Activity activity,
                             SiteModel currentSite) {
	Shortcut shortcut = Shortcut.fromActionString(action);
	if (shortcut == null) {
		AppLog.e(AppLog.T.MAIN,
		         String.format("Unknown Android Shortcut action[%s]", action));
		return;
	}
	...
	case OPEN_NOTIFICATIONS:
		AnalyticsTracker.track(
			AnalyticsTracker.Stat.SHORTCUT_NOTIFICATIONS_CLICKED);
		ActivityLauncher.viewNotifications(activity);
		break;
	default:
		AppLog.e(AppLog.T.MAIN,
		         String.format("Unknown Android Shortcut[%s]", shortcut));
	}
}
}
