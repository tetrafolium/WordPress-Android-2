...
private static final String RELATED_POSTS_ALLOWED_KEY =
	"jetpack_relatedposts_allowed";
private static final String RELATED_POSTS_ENABLED_KEY =
	"jetpack_relatedposts_enabled";
private static final String RELATED_POSTS_HEADER_KEY =
	"jetpack_relatedposts_show_headline";
private static final String RELATED_POSTS_IMAGES_KEY =
...
private static final String CLOSE_OLD_COMMENTS_KEY =
	"close_comments_for_old_posts";
private static final String CLOSE_OLD_COMMENTS_DAYS_KEY =
	"close_comments_days_old";
private static final String THREAD_COMMENTS_KEY = "thread_comments";
private static final String THREAD_COMMENTS_DEPTH_KEY =
...
private static final String JETPACK_SEARCH_ENABLED_KEY =
	"jetpack_search_enabled";
private static final String JETPACK_SEARCH_SUPPORTED_KEY =
...
// used to track network fetches to prevent multiple errors from generating
// multiple toasts
 ...
 */
WPComSiteSettings(Context host, SiteModel site,
                  SiteSettingsListener listener) {
...
protected void fetchRemoteData() {
	if (mFetchRequestCount > 0) {
		AppLog.v(AppLog.T.SETTINGS,
		         "Network fetch prevented, there's already a fetch in progress.");
		return;
	}
...
static boolean supportsJetpackSiteAcceleratorSettings(SiteModel site) {
	return SiteUtils.checkMinimalJetpackVersion(
		site, SPEED_UP_SETTINGS_JETPACK_VERSION);
}
		...
		new RestRequest.Listener() {
			...
			public void onResponse(JSONObject response) {
			        AppLog.d(AppLog.T.API,
			                 "Received response to Settings REST request.");
			        ...
			        if (!mRemoteSettings.equals(mSettings)) {
			                // postFormats setting is not returned by this api call so copy it
			                // over
			                final Map<String, String> currentPostFormats =
						mSettings.postFormats;
		...
		new RestRequest.ErrorListener() {
			...
			public void onErrorResponse(VolleyError error) {
			        AppLog.w(AppLog.T.API,
			                 "Error response to Settings REST request: " + error);
...
private void fetchCategories() {
	++mFetchRequestCount;
	// TODO: Replace with FluxC (GET_CATEGORIES +
	// TaxonomyStore.getCategoriesForSite())
	WordPress.getRestClientUtilsV1_1().getCategories(
		mSite.getSiteId(),
		new RestRequest.Listener() {
			@Override
			public void onResponse(JSONObject response) {
...
private void fetchJetpackProtectAndSsoSettings() {
	++mFetchRequestCount;
	WordPress.getRestClientUtilsV1_1().getJetpackSettings(
		mSite.getSiteId(),
		new RestRequest.Listener() {
			@Override
			public void onResponse(JSONObject response) {
			        ...
			        if (data == null) {
			                AppLog.w(
						AppLog.T.API,
						"Unexpected state: Received empty Jetpack settings response");
			                onFetchResponseReceived(null);
			                return;
				}
					...
					data.optBoolean("protect", false);
			        mRemoteJpSettings.ssoActive = data.optBoolean("sso", false);
			        mRemoteJpSettings.ssoMatchEmail =
					data.optBoolean("jetpack_sso_match_by_email", false);
			        mRemoteJpSettings.ssoRequireTwoFactor =
					data.optBoolean("jetpack_sso_require_two_step", false);
			        mRemoteJpSettings.commentLikes =
					data.optBoolean(COMMENT_LIKES, false);

			        JSONObject jetpackProtectWhitelist =
					data.optJSONObject("jetpack_protect_global_whitelist");
			        if (jetpackProtectWhitelist != null) {
			                // clear existing whitelist entries before adding items from
			                // response
			                mRemoteJpSettings.jetpackProtectWhitelist.clear();

			                JSONArray whitelistItems =
						jetpackProtectWhitelist.optJSONArray("local");
			                if (whitelistItems != null) {
			                        for (int i = 0; i < whitelistItems.length(); ++i) {
			                                String item = whitelistItems.optString(i, "");
			                                if (!item.isEmpty() &&
			                                    !mRemoteJpSettings.jetpackProtectWhitelist.contains(
								    item)) {
			        ...
			        mJpSettings.jetpackProtectWhitelist.addAll(
			        ...
			        mJpSettings.ssoRequireTwoFactor =
					mRemoteJpSettings.ssoRequireTwoFactor;
	...
	WordPress.getRestClientUtilsV1_1().getJetpackMonitorSettings(
		...
		new RestRequest.Listener() {
			...
			public void onResponse(JSONObject response) {
			        ...
			        mJpSettings.emailNotifications =
					mRemoteJpSettings.emailNotifications;
		...
		new RestRequest.ErrorListener() {
			...
			public void onErrorResponse(VolleyError error) {
			        AppLog.w(AppLog.T.API,
			                 "Error fetching Jetpack Monitor module options: " + error);
			        onFetchResponseReceived(error);
			}
		});
}
...
private void fetchJetpackModuleSettings() {
	...
	WordPress.getRestClientUtilsV1_1().getJetpackModuleSettings(
		...
		new RestRequest.Listener() {
			...
			public void onResponse(JSONObject response) {
			        if (response == null) {
			                AppLog.w(
						AppLog.T.API,
						"Unexpected state: Received empty Jetpack modules response");
			                onFetchResponseReceived(null);
			                return;
				}
			        ...
			        if (array != null) {
						...
						case AD_FREE_VIDEO_HOSTING_MODULE:
			                ...
			                mJpSettings.serveImagesFromOurServers =
						mRemoteJpSettings.serveImagesFromOurServers;
						...
						mRemoteJpSettings.serveStaticFilesFromOurServers;
			                ...
			                mJpSettings.adFreeVideoHosting =
						mRemoteJpSettings.adFreeVideoHosting;
				}
			        onFetchResponseReceived(null);
			}
		},
		new RestRequest.ErrorListener() {
			...
			public void onErrorResponse(VolleyError error) {
			        AppLog.w(AppLog.T.API,
			                 "Error fetching Jetpack module settings: " + error);
			        onFetchResponseReceived(error);
			}
		});
}
...
private void pushWpSettings() {
	...
	try {
		...
		AppLog.w(AppLog.T.API,
		         "Error serializing settings changes: " + exception);
	...
	WordPress.getRestClientUtilsV1_1().setGeneralSiteSettings(
		...
		new RestRequest.Listener() {
			...
			public void onResponse(JSONObject response) {
			        ...
			        if (response != null) {
						...
						AnalyticsTracker.Stat.SITE_SETTINGS_SAVED_REMOTELY, mSite,
						properties);
				}
		...
		new RestRequest.ErrorListener() {
			...
			public void onErrorResponse(VolleyError error) {
			        AppLog.w(AppLog.T.API,
			                 "Error POSTing site settings changes: " + error);
...
private void pushJetpackProtectAndSsoSettings() {
	...
	if (params.isEmpty()) {
		AppLog.v(
			AppLog.T.API,
			"No Jetpack settings changes detected. Skipping network POST call.");
		return;
	}

	// The response object doesn't contain any relevant info so we have to
	// create a copy of values being sent over the network in case mJpSettings
	// is modified while awaiting response
	final JetpackSettingsModel sentJpData =
		new JetpackSettingsModel(mJpSettings);
	++mSaveRequestCount;
	WordPress.getRestClientUtilsV1_1().setJetpackSettings(
		mSite.getSiteId(), params,
		new RestRequest.Listener() {
			@Override
			public void onResponse(JSONObject response) {
			        ...
			        mRemoteJpSettings.jetpackProtectWhitelist.clear();
			        mRemoteJpSettings.jetpackProtectWhitelist.addAll(
					sentJpData.jetpackProtectWhitelist);
...
private void pushJetpackMonitorSettings() {
	...
	WordPress.getRestClientUtilsV1_1().setJetpackMonitorSettings(
		mSite.getSiteId(), serializeJetpackMonitorParams(),
		new RestRequest.Listener() {
			@Override
			public void onResponse(JSONObject response) {
			        AppLog.d(AppLog.T.API, "Jetpack Monitor module updated");
			        mRemoteJpSettings.emailNotifications =
					sentJpData.emailNotifications;
		...
		new RestRequest.ErrorListener() {
			@Override
			public void onErrorResponse(VolleyError error) {
			        AppLog.w(AppLog.T.API, "Error updating Jetpack Monitor module: " +
			                 error.getMessage());
			        onSaveResponseReceived(error);
			}
		});
...
private void pushServeImagesFromOurServersModuleSettings() {
	++mSaveRequestCount;
	// The API returns 400 if we try to sync the same value twice so we need to
	// keep it locally.
	if (mJpSettings.serveImagesFromOurServers !=
	    mRemoteJpSettings.serveImagesFromOurServers) {
		...
		WordPress.getRestClientUtilsV1_1().setJetpackModuleSettings(
			mSite.getSiteId(), SERVE_IMAGES_FROM_OUR_SERVERS,
			mJpSettings.serveImagesFromOurServers,
			new RestRequest.Listener() {
				@Override
				public void onResponse(JSONObject response) {
				        AppLog.d(
						AppLog.T.API,
						"Jetpack module updated - Serve images from our servers");
				        onSaveResponseReceived(null);
				}
			},
			new RestRequest.ErrorListener() {
				...
				public void onErrorResponse(VolleyError error) {
				        ...
				        AppLog.w(
						AppLog.T.API,
						"Error updating Jetpack module - Serve images from our servers: " +
						error);
				        onSaveResponseReceived(error);
				}
			});
...
private void pushServeStaticFilesFromOurServersModuleSettings() {
	...
	// The API returns 400 if we try to sync the same value twice so we need to
	// keep it locally.
	if (mJpSettings.serveStaticFilesFromOurServers !=
	    mRemoteJpSettings.serveStaticFilesFromOurServers) {
		...
		mRemoteJpSettings.serveStaticFilesFromOurServers =
			mJpSettings.serveStaticFilesFromOurServers;
		WordPress.getRestClientUtilsV1_1().setJetpackModuleSettings(
			mSite.getSiteId(), SERVE_STATIC_FILES_FROM_OUR_SERVERS,
			mJpSettings.serveStaticFilesFromOurServers,
			new RestRequest.Listener() {
				@Override
				public void onResponse(JSONObject response) {
				        AppLog.d(
						AppLog.T.API,
						"Jetpack module updated - Serve static files from our servers");
				        onSaveResponseReceived(null);
				}
			},
			new RestRequest.ErrorListener() {
				...
				public void onErrorResponse(VolleyError error) {
				        ...
				        AppLog.w(
						AppLog.T.API,
						"Error updating Jetpack module - Serve static files from our servers: " +
						error);
				        onSaveResponseReceived(error);
				}
			});
...
private void pushLazyLoadModule() {
	...
	if (mJpSettings.lazyLoadImages != mRemoteJpSettings.lazyLoadImages) {
		...
		WordPress.getRestClientUtilsV1_1().setJetpackModuleSettings(
			mSite.getSiteId(), LAZY_LOAD_IMAGES, mJpSettings.lazyLoadImages,
			new RestRequest.Listener() {
				@Override
				public void onResponse(JSONObject response) {
				        AppLog.d(AppLog.T.API,
				                 "Jetpack module updated - Lazy load images");
				        onSaveResponseReceived(null);
				}
			},
			new RestRequest.ErrorListener() {
				...
				public void onErrorResponse(VolleyError error) {
				        ...
				        error.printStackTrace();
				        AppLog.w(AppLog.T.API,
				                 "Error updating Jetpack module - Lazy load images: " +
				                 error);
				        onSaveResponseReceived(error);
				}
			});
...
private void pushImprovedSearchModule() {
	...
	if (mJpSettings.improvedSearch != mRemoteJpSettings.improvedSearch) {
		...
		WordPress.getRestClientUtilsV1_1().setJetpackModuleSettings(
			mSite.getSiteId(), SEARCH_MODULE, mJpSettings.improvedSearch,
			new RestRequest.Listener() {
				@Override
				public void onResponse(JSONObject response) {
				        AppLog.d(AppLog.T.API,
				                 "Jetpack module updated - Improved search");
				        onSaveResponseReceived(null);
				}
			},
			new RestRequest.ErrorListener() {
				...
				public void onErrorResponse(VolleyError error) {
				        ...
				        error.printStackTrace();
				        AppLog.w(AppLog.T.API,
				                 "Error updating Jetpack module - Improved search: " +
				                 error);
				        onSaveResponseReceived(error);
				}
			});
...
private void pushAdFreeVideoHostingModule() {
	...
	// The API returns 400 if we try to sync the same value twice so we need to
	// keep it locally.
	if (mJpSettings.adFreeVideoHosting !=
	    mRemoteJpSettings.adFreeVideoHosting) {
		...
		WordPress.getRestClientUtilsV1_1().setJetpackModuleSettings(
			mSite.getSiteId(), AD_FREE_VIDEO_HOSTING_MODULE,
			mJpSettings.adFreeVideoHosting,
			new RestRequest.Listener() {
				@Override
				public void onResponse(JSONObject response) {
			...
			new RestRequest.ErrorListener() {
				...
				public void onErrorResponse(VolleyError error) {
				        ...
				        error.printStackTrace();
				        AppLog.w(AppLog.T.API,
				                 "Error updating Jetpack module - Videopress: " + error);
				        onSaveResponseReceived(error);
				}
			});
	}
...
private void onFetchResponseReceived(Exception error) {
	if (error != null) {
		...
		// we received successful response to GET request, notify listener to
		// update UI
		notifyUpdatedOnUiThread();
	}
	if (--mFetchRequestCount <= 0 && mWasFetchError) {
		// all of our GET requests are completed and at least one had an error so
		// we need to notify
...
private void onSaveResponseReceived(Exception error) {
	if (error != null) {
		...
		// we received successful response to POST request, notify listener to
		// update UI
		notifySavedOnUiThread();
	}
	if (--mSaveRequestCount <= 0 && mWasSaveError) {
		// all of our POST requests are completed and at least one had an error so
		// we need to notify
 ...
 */
private void deserializeWpComRestResponse(SiteModel site,
                                          JSONObject response) {
	...
	mRemoteSettings.defaultCategory =
		settingsObject.optInt(DEF_CATEGORY_KEY, 1);
	mRemoteSettings.defaultPostFormat =
		settingsObject.optString(DEF_POST_FORMAT_KEY, STANDARD_POST_FORMAT_KEY);
	mRemoteSettings.language =
		languageIdToLanguageCode(Integer.toString(mRemoteSettings.languageId));
	mRemoteSettings.allowComments =
		settingsObject.optBoolean(ALLOW_COMMENTS_KEY, true);
	mRemoteSettings.sendPingbacks =
		settingsObject.optBoolean(SEND_PINGBACKS_KEY, false);
	mRemoteSettings.receivePingbacks =
		settingsObject.optBoolean(RECEIVE_PINGBACKS_KEY, true);
	mRemoteSettings.shouldCloseAfter =
		settingsObject.optBoolean(CLOSE_OLD_COMMENTS_KEY, false);
	mRemoteSettings.closeCommentAfter =
		settingsObject.optInt(CLOSE_OLD_COMMENTS_DAYS_KEY, 0);
	mRemoteSettings.shouldThreadComments =
		settingsObject.optBoolean(THREAD_COMMENTS_KEY, false);
	mRemoteSettings.threadingLevels =
		settingsObject.optInt(THREAD_COMMENTS_DEPTH_KEY, 0);
	mRemoteSettings.shouldPageComments =
		settingsObject.optBoolean(PAGE_COMMENTS_KEY, false);
	mRemoteSettings.commentsPerPage =
		settingsObject.optInt(PAGE_COMMENT_COUNT_KEY, 0);
	mRemoteSettings.commentApprovalRequired =
		settingsObject.optBoolean(COMMENT_MODERATION_KEY, false);
	mRemoteSettings.commentsRequireIdentity =
		settingsObject.optBoolean(REQUIRE_IDENTITY_KEY, false);
	mRemoteSettings.commentsRequireUserAccount =
		settingsObject.optBoolean(REQUIRE_USER_ACCOUNT_KEY, true);
	mRemoteSettings.commentAutoApprovalKnownUsers =
		settingsObject.optBoolean(WHITELIST_KNOWN_USERS_KEY, false);
	...
	mRemoteSettings.sharingLabel =
		settingsObject.optString(SHARING_LABEL_KEY, "");
	mRemoteSettings.sharingButtonStyle = settingsObject.optString(
		SHARING_BUTTON_STYLE_KEY, DEFAULT_SHARING_BUTTON_STYLE);
	mRemoteSettings.allowCommentLikes =
		settingsObject.optBoolean(SHARING_COMMENT_LIKES_KEY, false);
	mRemoteSettings.twitterUsername =
		settingsObject.optString(TWITTER_USERNAME_KEY, "");
	mRemoteSettings.startOfWeek =
		settingsObject.optString(START_OF_WEEK_KEY, "");
	...
	mRemoteSettings.ampSupported =
		settingsObject.optBoolean(AMP_SUPPORTED_KEY, false);
	mRemoteSettings.ampEnabled =
		settingsObject.optBoolean(AMP_ENABLED_KEY, false);
	mRemoteSettings.jetpackSearchSupported =
		settingsObject.optBoolean(JETPACK_SEARCH_SUPPORTED_KEY, false);
	mRemoteSettings.jetpackSearchEnabled =
		settingsObject.optBoolean(JETPACK_SEARCH_ENABLED_KEY, false);

	boolean reblogsDisabled =
		settingsObject.optBoolean(SHARING_REBLOGS_DISABLED_KEY, false);
	boolean likesDisabled =
		settingsObject.optBoolean(SHARING_LIKES_DISABLED_KEY, false);
	...
	JSONObject jetpackProtectWhitelist =
		settingsObject.optJSONObject(JP_PROTECT_WHITELIST_KEY);
	if (jetpackProtectWhitelist != null) {
		...
		if (whitelistItems != null) {
			for (int i = 0; i < whitelistItems.length(); ++i) {
				String item = whitelistItems.optString(i, "");
				if (!item.isEmpty() &&
				    !mRemoteJpSettings.jetpackProtectWhitelist.contains(item)) {
					mRemoteJpSettings.jetpackProtectWhitelist.add(item);
				}
			}
		}
	...
	if (settingsObject.optBoolean(RELATED_POSTS_ALLOWED_KEY, false)) {
		mRemoteSettings.showRelatedPosts =
			settingsObject.optBoolean(RELATED_POSTS_ENABLED_KEY, false);
		mRemoteSettings.showRelatedPostHeader =
			settingsObject.optBoolean(RELATED_POSTS_HEADER_KEY, false);
		mRemoteSettings.showRelatedPostImages =
			settingsObject.optBoolean(RELATED_POSTS_IMAGES_KEY, false);
	}
}

/**
 * Need to use JSONObject's instead of HashMap<String, String> to serialize
 * array values (Jetpack Whitelist)
 */
private JSONObject serializeWpComParamsToJSONObject() throws JSONException {
	JSONObject params = new JSONObject();

	if (mSettings.title != null &&
	    !mSettings.title.equals(mRemoteSettings.title)) {
		params.put(SET_TITLE_KEY, mSettings.title);
	}
	if (mSettings.tagline != null &&
	    !mSettings.tagline.equals(mRemoteSettings.tagline)) {
	...
	if (mSettings.defaultPostFormat != null &&
	    !mSettings.defaultPostFormat.equals(
		    mRemoteSettings.defaultPostFormat)) {
		params.put(DEF_POST_FORMAT_KEY, mSettings.defaultPostFormat);
	}
	if (mSettings.showRelatedPosts != mRemoteSettings.showRelatedPosts ||
	    mSettings.showRelatedPostHeader !=
	    mRemoteSettings.showRelatedPostHeader ||
	    mSettings.showRelatedPostImages !=
	    mRemoteSettings.showRelatedPostImages) {
		params.put(RELATED_POSTS_ENABLED_KEY,
		           String.valueOf(mSettings.showRelatedPosts));
		params.put(RELATED_POSTS_HEADER_KEY,
		           String.valueOf(mSettings.showRelatedPostHeader));
		params.put(RELATED_POSTS_IMAGES_KEY,
		           String.valueOf(mSettings.showRelatedPostImages));
	}
	...
	if (mSettings.receivePingbacks != mRemoteSettings.receivePingbacks) {
		params.put(RECEIVE_PINGBACKS_KEY,
		           String.valueOf(mSettings.receivePingbacks));
	}
	if (mSettings.commentApprovalRequired !=
	    mRemoteSettings.commentApprovalRequired) {
		params.put(COMMENT_MODERATION_KEY,
		           String.valueOf(mSettings.commentApprovalRequired));
	}
	if (mSettings.closeCommentAfter != mRemoteSettings.closeCommentAfter ||
		...
		params.put(CLOSE_OLD_COMMENTS_KEY,
		           String.valueOf(mSettings.shouldCloseAfter));
		params.put(CLOSE_OLD_COMMENTS_DAYS_KEY,
	...
	if (mSettings.threadingLevels != mRemoteSettings.threadingLevels ||
	    mSettings.shouldThreadComments !=
	    mRemoteSettings.shouldThreadComments) {
		params.put(THREAD_COMMENTS_KEY,
		           String.valueOf(mSettings.shouldThreadComments));
		params.put(THREAD_COMMENTS_DEPTH_KEY,
		           String.valueOf(mSettings.threadingLevels));
	}
	if (mSettings.commentsPerPage != mRemoteSettings.commentsPerPage ||
		...
		params.put(PAGE_COMMENTS_KEY,
		           String.valueOf(mSettings.shouldPageComments));
		params.put(PAGE_COMMENT_COUNT_KEY,
		           String.valueOf(mSettings.commentsPerPage));
	}
	if (mSettings.commentsRequireIdentity !=
	    mRemoteSettings.commentsRequireIdentity) {
		params.put(REQUIRE_IDENTITY_KEY,
		           String.valueOf(mSettings.commentsRequireIdentity));
	}
	if (mSettings.commentsRequireUserAccount !=
	    mRemoteSettings.commentsRequireUserAccount) {
		params.put(REQUIRE_USER_ACCOUNT_KEY,
		           String.valueOf(mSettings.commentsRequireUserAccount));
	}
	if (mSettings.commentAutoApprovalKnownUsers !=
	    mRemoteSettings.commentAutoApprovalKnownUsers) {
		params.put(WHITELIST_KNOWN_USERS_KEY,
		           String.valueOf(mSettings.commentAutoApprovalKnownUsers));
	}
	...
	if (mSettings.holdForModeration != null &&
	    !mSettings.holdForModeration.equals(
		    mRemoteSettings.holdForModeration)) {
	...
	if (mSettings.blacklist != null &&
	    !mSettings.blacklist.equals(mRemoteSettings.blacklist)) {
		StringBuilder builder = new StringBuilder();
	...
	if (mSettings.sharingLabel != null &&
	    !mSettings.sharingLabel.equals(mRemoteSettings.sharingLabel)) {
		params.put(SHARING_LABEL_KEY, String.valueOf(mSettings.sharingLabel));
	}
	if (mSettings.sharingButtonStyle != null &&
	    !mSettings.sharingButtonStyle.equals(
	...
	if (mSettings.allowReblogButton != mRemoteSettings.allowReblogButton) {
		params.put(SHARING_REBLOGS_DISABLED_KEY,
		           String.valueOf(!mSettings.allowReblogButton));
	}
	if (mSettings.allowLikeButton != mRemoteSettings.allowLikeButton) {
		params.put(SHARING_LIKES_DISABLED_KEY,
		           String.valueOf(!mSettings.allowLikeButton));
	}
	if (mSettings.allowCommentLikes != mRemoteSettings.allowCommentLikes) {
		params.put(SHARING_COMMENT_LIKES_KEY,
		           String.valueOf(mSettings.allowCommentLikes));
	}
	if (mSettings.twitterUsername != null &&
	    !mSettings.twitterUsername.equals(mRemoteSettings.twitterUsername)) {
		params.put(TWITTER_USERNAME_KEY, mSettings.twitterUsername);
	}
	if (mSettings.startOfWeek != null &&
	    !mSettings.startOfWeek.equals(mRemoteSettings.startOfWeek)) {
		params.put(START_OF_WEEK_KEY, mSettings.startOfWeek);
	}
	if (mSettings.dateFormat != null &&
	    !mSettings.dateFormat.equals(mRemoteSettings.dateFormat)) {
		params.put(DATE_FORMAT_KEY, mSettings.dateFormat);
	}
	if (mSettings.timeFormat != null &&
	    !mSettings.timeFormat.equals(mRemoteSettings.timeFormat)) {
	...
	if (mSettings.jetpackSearchSupported !=
	    mRemoteSettings.jetpackSearchSupported) {
		params.put(JETPACK_SEARCH_SUPPORTED_KEY,
		           String.valueOf(mSettings.jetpackSearchSupported));
	}
	if (mSettings.jetpackSearchEnabled !=
	    mRemoteSettings.jetpackSearchEnabled) {
		params.put(JETPACK_SEARCH_ENABLED_KEY,
		           String.valueOf(mSettings.jetpackSearchEnabled));
	}

	return params;
}

private void deserializeJetpackRestResponse(SiteModel site,
                                            JSONObject response) {
	if (site == null || response == null)
		return;
	JSONObject settingsObject = response.optJSONObject("settings");
	mRemoteJpSettings.emailNotifications =
		settingsObject.optBoolean(JP_MONITOR_EMAIL_NOTES_KEY, false);
	mRemoteJpSettings.wpNotifications =
		settingsObject.optBoolean(JP_MONITOR_WP_NOTES_KEY, false);
}

private @NonNull Map<String, String> serializeJetpackMonitorParams() {
	Map<String, String> params = new HashMap<>();
	params.put(JP_MONITOR_EMAIL_NOTES_KEY,
	           String.valueOf(mJpSettings.emailNotifications));
	params.put(JP_MONITOR_WP_NOTES_KEY,
	           String.valueOf(mJpSettings.wpNotifications));
	return params;
}
...
private Map<String, Object> serializeJetpackProtectAndSsoParams() {
	...
	if (mJpSettings.jetpackProtectEnabled !=
	    mRemoteJpSettings.jetpackProtectEnabled) {
		params.put("protect", mJpSettings.jetpackProtectEnabled);
	}
	if (!mJpSettings.whitelistMatches(
		    mRemoteJpSettings.jetpackProtectWhitelist)) {
		String whitelistArray =
			TextUtils.join(",", mJpSettings.jetpackProtectWhitelist);
	...
	if (mJpSettings.ssoRequireTwoFactor !=
	    mRemoteJpSettings.ssoRequireTwoFactor) {
		params.put("jetpack_sso_require_two_step",
		           mJpSettings.ssoRequireTwoFactor);
	}
...
private CategoryModel deserializeCategoryFromJson(JSONObject category)
...
private CategoryModel[] deserializeCategoryRestResponse(JSONObject response) {
	try {
		...
		AppLog.d(AppLog.T.API,
		         "Error parsing WP.com categories response:" + response);
		return null;
	}
}
}
