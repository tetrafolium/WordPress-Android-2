...
/**
 * service which requests the user's available sharing services and publicize
 * connections
 */
 ...
 */
public static void updateConnectionsForSite(Context context,
...
public static void enqueueWork(Context context, Intent work) {
	enqueueWork(context, PublicizeUpdateService.class,
	            JOB_PUBLICIZE_UPDATE_SERVICE_ID, work);
}
...
protected void onHandleWork(@NonNull Intent intent) {
	...
	// update list of services if we haven't done so yet - only done once per
	// session since it rarely changes
...
public boolean onStopCurrentWork() {
	// this Service was failing silently if it couldn't get to update its data,
	// so that hints us that we shouldn't really care about rescheduling this
	// job in the case something failed.
	return false;
}
...
private void updateServices() {
	RestRequest.Listener listener = new RestRequest.Listener() {
		...
		public void onResponse(JSONObject jsonObject) {
			PublicizeServiceList serverList =
				PublicizeServiceList.fromJson(jsonObject);
...
private void updateConnections(final long siteId) {
	RestRequest.Listener listener = new RestRequest.Listener() {
		@Override
		public void onResponse(JSONObject jsonObject) {
			PublicizeConnectionList serverList =
				PublicizeConnectionList.fromJson(jsonObject);
			PublicizeConnectionList localList =
	...
	String path =
		String.format(Locale.ROOT, "sites/%d/publicize-connections", siteId);
	WordPress.getRestClientUtilsV1_1().get(path, null, null, listener,
	                                       errorListener);
}
}
