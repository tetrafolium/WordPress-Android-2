...
/*
 * add the passed comment text to the passed post - caller must pass a unique
 * "fake" comment id to give the comment that's generated locally
 */
public static ReaderComment
submitPostComment(final ReaderPost post, final long fakeCommentId,
                  final String commentText, final long replyToCommentId,
	...
	if (replyToCommentId != 0) {
		pageNumber = ReaderCommentTable.getPageNumberForComment(
			post.blogId, post.postId, replyToCommentId);
	} else {
		pageNumber =
			ReaderCommentTable.getLastPageNumberForPost(post.blogId, post.postId);
	}

	// create a "fake" comment that's added to the db so it can be shown right
	// away - will be replaced with actual comment if it succeeds to be posted,
	// or deleted if comment fails to be posted
	...
	// different endpoint depending on whether the new comment is a reply to
	// another comment
	final String path;
	if (replyToCommentId == 0) {
		path = "sites/" + post.blogId + "/posts/" + post.postId + "/replies/new";
	} else {
		path = "sites/" + post.blogId + "/comments/" +
		       Long.toString(replyToCommentId) + "/replies/new";
	}
	...
	RestRequest.Listener listener = new RestRequest.Listener() {
		...
		public void onResponse(JSONObject jsonObject) {
			...
			ReaderComment newComment =
				ReaderComment.fromJson(jsonObject, post.blogId);
			newComment.pageNumber = pageNumber;
			ReaderCommentTable.addOrUpdateComment(newComment);
...
public static boolean performLikeAction(final ReaderComment comment,
	...
	boolean isCurrentlyLiked =
		ReaderCommentTable.isCommentLikedByCurrentUser(comment);
	...
	int newNumLikes =
		(isAskingToLike ? comment.numLikes + 1 : comment.numLikes - 1);
	...
	String path = "sites/" + comment.blogId + "/comments/" + comment.commentId +
	              "/likes/";
	if (isAskingToLike) {
		path += "new";
	} else {
	...
	RestRequest.Listener listener = new RestRequest.Listener() {
		...
		public void onResponse(JSONObject jsonObject) {
			boolean success =
				(jsonObject != null && JSONUtils.getBool(jsonObject, "success"));
			if (success) {
				...
				ReaderCommentTable.setLikesForComment(comment, comment.numLikes,
				                                      comment.isLikedByCurrentUser);
				ReaderLikeTable.setCurrentUserLikesComment(
					comment, comment.isLikedByCurrentUser, wpComUserId);
			}
		}
	};
	...
	RestRequest.ErrorListener errorListener = new RestRequest.ErrorListener() {
		...
		public void onErrorResponse(VolleyError volleyError) {
			...
			if (TextUtils.isEmpty(error)) {
				...
				AppLog.w(T.READER,
				         String.format("comment %s failed (%s)", actionName, error));
			}
			AppLog.e(T.READER, volleyError);
			ReaderCommentTable.setLikesForComment(comment, comment.numLikes,
			                                      comment.isLikedByCurrentUser);
