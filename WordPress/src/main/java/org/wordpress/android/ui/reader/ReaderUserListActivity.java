...
public class ReaderUserListActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    long commentId =
        getIntent().getLongExtra(ReaderConstants.ARG_COMMENT_ID, 0);
  ...
  private void loadUsers(final long blogId, final long postId,
    ...
    new Thread() {
      ...
      public void run() {
        ...
        if (commentId == 0) {
          // commentId is empty (not passed), so we're showing users who like a
          // post
          users = ReaderUserTable.getUsersWhoLikePost(
              blogId, postId, ReaderConstants.READER_MAX_USERS_TO_DISPLAY);
        } else {
          // commentId is non-empty, so we're showing users who like a comment
          users = ReaderUserTable.getUsersWhoLikeComment(
              blogId, commentId, ReaderConstants.READER_MAX_USERS_TO_DISPLAY);
        }
  ...
  private String getTitleString(final long blogId, final long postId,
    ...
    if (commentId == 0) {
      ...
      isLikedByCurrentUser =
          ReaderPostTable.isPostLikedByCurrentUser(blogId, postId);
    } else {
      numLikes =
          ReaderCommentTable.getNumLikesForComment(blogId, postId, commentId);
      isLikedByCurrentUser = ReaderCommentTable.isCommentLikedByCurrentUser(
          blogId, postId, commentId);
    }
    return ReaderUtils.getLongLikeLabelText(this, numLikes,
                                            isLikedByCurrentUser);
  }
}
