 ...
 * Caching happens on UI thread, so any configuration change will restart it
 * from scratch.
 */
public class ReaderPostWebViewCachingFragment extends Fragment {
  ...
  public static ReaderPostWebViewCachingFragment newInstance(long blogId,
                                                             long postId) {
    ReaderPostWebViewCachingFragment fragment =
        new ReaderPostWebViewCachingFragment();
  ...
  public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
    ...
    // check network again to detect disconnects during loading + configuration
    // change
    if (NetworkUtils.isNetworkAvailable(view.getContext())) {
      ...
      ((ReaderWebView)view)
          .setBlogSchemeIsHttps(UrlUtils.isHttps(post.getBlogUrl()));
      ((ReaderWebView)view)
          .setPageFinishedListener(
              new ReaderWebView.ReaderWebViewPageFinishedListener() {
                @Override
      ...
      ReaderPostRenderer rendered =
          new ReaderPostRenderer((ReaderWebView)view, post);
      rendered.beginRender(); // rendering will cache post content using native
