...
/**
 * used by the detail view to display related posts, which can be either local
 * (related posts from the same site as the source post) or global (related
 * posts from across wp.com)
 */
...
public void
showPosts(ReaderSimplePostList posts, String siteName, boolean isGlobal,
	...
	if (isGlobal) {
		label.setText(
			getContext().getString(R.string.reader_label_global_related_posts));
	} else {
		label.setText(String.format(
				      getContext().getString(R.string.reader_label_local_related_posts),
				      siteName));
	}
}
...
/*
 * called by reader detail when scrolled into view, tracks railcar events for
 * each post
 */
