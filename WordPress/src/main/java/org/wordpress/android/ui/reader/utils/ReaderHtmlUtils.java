...
public class ReaderHtmlUtils {
  ...
  private static final Pattern ORIGINAL_WIDTH_ATTR_PATTERN =
      Pattern.compile("data-orig-size\\s*=\\s*(?:'|\") (.*?),.*?(?:'|\")",
  ...
  /*
   * returns the integer value from the data-orig-size attribute in the passed
   * html tag
   */
  ...
  public static int getWidthAttrValue(final String tag) {
    ...
    if (matcher.find()) {
      ...
      return StringUtils.stringToInt(
          tag.substring(matcher.start() + 7, matcher.end() - 1), 0);
    } else {
      return 0;
    }
  }
  ...
  public static int getHeightAttrValue(final String tag) {
    ...
    if (matcher.find()) {
      return StringUtils.stringToInt(
          tag.substring(matcher.start() + 8, matcher.end() - 1), 0);
    } else {
      return 0;
    }
  }
  ...
  /*
   * returns the integer value of the passed query param in the passed url -
   * returns zero if the url is invalid, or the param doesn't exist, or the
   * param value could not be converted to an int
   */
  public static int getIntQueryParam(final String url,
                                     @SuppressWarnings("SameParameterValue")
                                     final String param) {
    if (url == null || param == null || !url.startsWith("http") ||
        !url.contains(param + "=")) {
      return 0;
    }
