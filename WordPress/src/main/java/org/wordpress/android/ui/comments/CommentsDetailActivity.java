...
public static final String COMMENT_STATUS_FILTER_EXTRA =
	"commentStatusFilter";
...
public void onCreate(Bundle savedInstanceState) {
	...
	if (savedInstanceState == null) {
		...
		mStatusFilter = (CommentStatus)getIntent().getSerializableExtra(
			COMMENT_STATUS_FILTER_EXTRA);
	} else {
		mCommentId = savedInstanceState.getLong(COMMENT_ID_EXTRA);
		mSite = (SiteModel)savedInstanceState.getSerializable(WordPress.SITE);
		mStatusFilter = (CommentStatus)savedInstanceState.getSerializable(
			COMMENT_STATUS_FILTER_EXTRA);
	}
...
private void updateComments() {
	if (mIsUpdatingComments) {
		...
		ToastUtils.showToast(
			this, getString(R.string.error_refresh_comments_showing_older));
		return;
	...
	mDispatcher.dispatch(
		CommentActionBuilder.newFetchCommentsAction(new FetchCommentsPayload(
								    mSite, mStatusFilter, COMMENTS_PER_PAGE, offset)));
...
private void loadDataInViewPager() {
	if (mIsLoadingComments) {
		...
		new LoadCommentsTask(mCommentStore, mStatusFilter, mSite,
		                     new LoadCommentsTask.LoadingCallback() {
				@Override
...
private void showCommentList(CommentList commentList) {
	...
	if (mAdapter != null && mAdapter.isAddingNewComments(commentList)) {
		...
		mAdapter = new CommentDetailFragmentAdapter(getSupportFragmentManager(),
		                                            commentList, mSite,
...
public void onModerateComment(final SiteModel site,
	...
	resultIntent.putExtra(CommentsActivity.COMMENT_MODERATE_ID_EXTRA,
	                      comment.getRemoteCommentId());
	resultIntent.putExtra(CommentsActivity.COMMENT_MODERATE_STATUS_EXTRA,
	                      newStatus.toString());
