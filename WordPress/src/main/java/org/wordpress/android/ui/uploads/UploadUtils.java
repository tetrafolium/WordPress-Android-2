...
public class UploadUtils {
   ...
   */
  static @NonNull String getErrorMessage(Context context, PostModel post,
                                         String errorMessage,
    ...
    if (post.isPage()) {
      if (isMediaError) {
        baseErrorString =
            context.getString(R.string.error_upload_page_media_param);
      } else {
      ...
      if (isMediaError) {
        baseErrorString =
            context.getString(R.string.error_upload_post_media_param);
      } else {
  ...
  UiString getErrorMessageResIdFromPostError(PostStatus postStatus,
                                             boolean isPage, PostError error,
    ...
    case UNAUTHORIZED:
      return isPage
          ? new UiStringRes(R.string.error_refresh_unauthorized_pages)
          : new UiStringRes(R.string.error_refresh_unauthorized_posts);
    case UNSUPPORTED_ACTION:
    case INVALID_RESPONSE:
    case GENERIC_ERROR:
    default:
      AppLog.w(T.MAIN, "Error message: " + error.message +
                           " ,Error Type: " + error.type);
      if (eligibleForAutoUpload) {
        switch (postStatus) {
        case PRIVATE:
          return new UiStringRes(
              R.string.error_post_not_published_retrying_private);
        case PUBLISHED:
   ...
   */
  public static @NonNull
  String getErrorMessageFromMediaError(Context context, MediaModel media,
                                       MediaError error) {
    String errorMessage = WPMediaUtils.getErrorMessage(context, media, error);

    if (errorMessage == null) {
      // In case of a generic or uncaught error, return the message from the API
      // response or the error type
      errorMessage = TextUtils.isEmpty(error.message) ? error.type.toString()
                                                      : error.message;
    }
  ...
  public static void handleEditPostResultSnackbars(
      @NonNull final Activity activity, @NonNull final Dispatcher dispatcher,
      @NonNull View snackbarAttachView, @NonNull Intent data,
      @NonNull final PostModel post, @NonNull final SiteModel site,
    ...
    boolean savedLocally = data.getBooleanExtra(
        EditPostActivity.EXTRA_SAVED_AS_LOCAL_DRAFT, false);
    if (savedLocally && !NetworkUtils.isNetworkAvailable(activity)) {
      // The network is not available, we can enqueue a request to upload local
      // changes later
      UploadWorkerKt.enqueueUploadWorkRequestForSite(site);
      // And tell the user about it
      showSnackbar(snackbarAttachView,
                   getDeviceOfflinePostNotUploadedMessage(post, uploadAction),
                   R.string.cancel, v -> {
    ...
    boolean hasFailedMedia =
        data.getBooleanExtra(EditPostActivity.EXTRA_HAS_FAILED_MEDIA, false);
    if (hasFailedMedia) {
      showSnackbar(
          snackbarAttachView, R.string.editor_post_saved_locally_failed_media,
          R.string.button_edit, new View.OnClickListener() {
            @Override
    ...
    if (isScheduledPost) {
      // if it's a scheduled post, we only want to show a "Sync" button if it's
      // locally saved
      if (savedLocally) {
        showSnackbar(snackbarAttachView, R.string.editor_post_saved_locally,
                     R.string.button_sync, publishPostListener);
      }
      return;
    }
    ...
    if (isPublished) {
      // if it's a published post, we only want to show a "Sync" button if it's
      // locally saved
      if (savedLocally) {
        showSnackbar(snackbarAttachView, R.string.editor_post_saved_locally,
                     R.string.button_sync, publishPostListener);
    ...
    if (isDraft) {
      if (PostUtils.isPublishable(post)) {
        ...
        if (savedLocally) {
          showSnackbarSuccessAction(
              snackbarAttachView, R.string.editor_draft_saved_locally,
          ...
          if (UploadService.hasPendingOrInProgressMediaUploadsForPost(post) ||
            ...
            showSnackbarSuccessAction(
                snackbarAttachView, R.string.editor_draft_saved_online,
                R.string.button_publish, publishPostListener);
          }
        }
      } else {
      ...
      if (savedLocally) {
        ...
        if (UploadService.hasPendingOrInProgressMediaUploadsForPost(post) ||
          ...
          showSnackbarSuccessAction(
              snackbarAttachView, R.string.editor_post_saved_online,
              R.string.button_publish, publishPostListener);
        }
      }
    }
  ...
  public static void
  showSnackbarSuccessAction(View view, int messageRes, int buttonTitleRes,
                            View.OnClickListener onClickListener) {
    WPSnackbar.make(view, messageRes, K_SNACKBAR_WAIT_TIME_MS)
        .setAction(buttonTitleRes, onClickListener)
        .show();
  ...
  private static void
  showSnackbarSuccessAction(View view, String message, int buttonTitleRes,
                            View.OnClickListener onClickListener) {
    WPSnackbar.make(view, message, K_SNACKBAR_WAIT_TIME_MS)
        .setAction(buttonTitleRes, onClickListener)
        .show();
  ...
  public static void
  showSnackbarSuccessActionOrange(View view, int messageRes, int buttonTitleRes,
                                  View.OnClickListener onClickListener) {
  ...
  public static void publishPost(Activity activity, final PostModel post,
                                 SiteModel site, Dispatcher dispatcher) {
    // If the post is empty, don't publish
    if (!PostUtils.isPublishable(post)) {
      String message =
          activity.getString(post.isPage() ? R.string.error_publish_empty_page
  ...
  /*
   * returns true if the user has permission to publish the post - assumed to be
   * true for dot.org sites because we can't retrieve their capabilities
   */
  public static boolean userCanPublish(SiteModel site) {
    return !SiteUtils.isAccessedViaWPComRest(site) ||
        site.getHasCapabilityPublishPosts();
  }

  public static void onPostUploadedSnackbarHandler(
      final Activity activity, View snackbarAttachView, boolean isError,
      final PostModel post, final String errorMessage, final SiteModel site,
    ...
    if (isError) {
      if (errorMessage != null) {
        ...
        if (AppPrefs.isAztecEditorEnabled()) {
          UploadUtils.showSnackbarError(
              snackbarAttachView, errorMessage, R.string.retry,
        ...
        UploadUtils.showSnackbar(snackbarAttachView,
      ...
      if (post != null) {
        ...
        if (snackbarButtonRes > 0) {
          UploadUtils.showSnackbarSuccessAction(
              snackbarAttachView, snackbarMessageRes, snackbarButtonRes,
              publishPostListener);
        } else {
  ...
  public static void onMediaUploadedSnackbarHandler(
      final Activity activity, View snackbarAttachView, boolean isError,
    ...
    if (isError) {
      if (messageForUser != null) {
        ...
        if (mediaList != null && !mediaList.isEmpty()) {
          UploadUtils.showSnackbarError(
              snackbarAttachView, messageForUser, R.string.retry,
              new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                  ...
                  Intent retryIntent =
                      UploadService.getUploadMediaServiceIntent(
                          activity, mediaListToRetry, true);
      ...
      // show success snackbar for media only items and offer the WRITE POST
      // functionality)
      UploadUtils.showSnackbarSuccessAction(
          snackbarAttachView, messageForUser,
          R.string.media_files_uploaded_write_post, new View.OnClickListener() {
            ...
            public void onClick(View view) {
              ...
              Intent writePostIntent =
                  new Intent(activity, EditPostActivity.class);
  ...
  @StringRes
  private static int getDeviceOfflinePostNotUploadedMessage(
      @NonNull final PostModel post, @NonNull final UploadAction uploadAction) {
    if (uploadAction != UploadAction.UPLOAD) {
      ...
      case TRASHED:
        throw new IllegalArgumentException(
            "Trashing posts should be handled in a different code path.");
      }
    }
    throw new RuntimeException(
        "This code should be unreachable. Missing case in switch statement.");
  }
  ...
  public static int cancelPendingAutoUpload(PostModel post,
                                            Dispatcher dispatcher) {
    /*
     * `changesConfirmedContentHashcode` field holds a hashcode of the post
     * content at the time when user pressed updated/publish/sync/submit/..
     * buttons. Clearing the hashcode will prevent the PostUploadHandler to
     * auto-upload the changes - it'll only remote-auto-save them -> which is
     * exactly what the cancel action is supposed to do.
     */
