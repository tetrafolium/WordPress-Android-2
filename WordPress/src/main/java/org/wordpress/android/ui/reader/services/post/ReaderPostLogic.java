...
public class ReaderPostLogic {
  ...
  public void performTask(Object companion, UpdateAction action, ReaderTag tag,
                          long blogId, long feedId) {
  ...
  private void updatePostsWithTag(final ReaderTag tag,
                                  final UpdateAction action) {
    requestPostsWithTag(tag, action, new ReaderActions.UpdateResultListener() {
      @Override
      public void onUpdateResult(ReaderActions.UpdateResult result) {
        EventBus.getDefault().post(
            new ReaderEvents.UpdatePostsEnded(tag, result, action));
  ...
  private void updatePostsInBlog(long blogId, final UpdateAction action) {
    ReaderActions.UpdateResultListener listener =
        new ReaderActions.UpdateResultListener() {
          @Override
          public void onUpdateResult(ReaderActions.UpdateResult result) {
            EventBus.getDefault().post(
                new ReaderEvents.UpdatePostsEnded(result, action));
            mCompletionListener.onCompleted(mListenerCompanion);
          }
        };
  ...
  private static void
  requestPostsWithTag(final ReaderTag tag, final UpdateAction updateAction,
    ...
    // return newest posts first (this is the default, but make it explicit
    ...
    com.wordpress.rest.RestRequest.Listener listener =
        new RestRequest.Listener() {
          ...
          public void onResponse(JSONObject jsonObject) {
            ...
            handleUpdatePostsResponse(tag, jsonObject, updateAction,
                                      resultListener);
          }
        };
    ...
    WordPress.getRestClientUtilsV1_2().get(sb.toString(), null, null, listener,
                                           errorListener);
  }

  private static void
  requestPostsForBlog(final long blogId, final UpdateAction updateAction,
                      final ReaderActions.UpdateResultListener resultListener) {
    String path = "read/sites/" + blogId + "/posts/?meta=site,likes";

    // append the date of the oldest cached post in this blog when requesting
    // older posts
    ...
    com.wordpress.rest.RestRequest.Listener listener =
        new RestRequest.Listener() {
          @Override
          public void onResponse(JSONObject jsonObject) {
            handleUpdatePostsResponse(null, jsonObject, updateAction,
                                      resultListener);
          }
        };
    RestRequest.ErrorListener errorListener = new RestRequest.ErrorListener() {
  ...
  private static void
  requestPostsForFeed(final long feedId, final UpdateAction updateAction,
                      final ReaderActions.UpdateResultListener resultListener) {
  ...
  /*
   * called after requesting posts with a specific tag or in a specific
   * blog/feed
   */
  private static void handleUpdatePostsResponse(
      final ReaderTag tag, final JSONObject jsonObject,
    ...
    new Thread() {
      ...
      public void run() {
        ...
        ReaderActions.UpdateResult updateResult =
            ReaderPostTable.comparePosts(serverPosts);
        if (updateResult.isNewOrChanged()) {
          ...
          if (tag != null) {
            ...
            case REQUEST_NEWER:
              ...
              if (numServerPosts >= 2 &&
                ...
                ReaderBlogIdPostId gapMarker =
                    ReaderPostTable.getGapMarkerIdsForTag(tag);
                if (gapMarker != null) {
                  // We mustn't have two gapMarkers at the same time. Therefor
                  // we need to delete all posts before the current gapMarker
                  // and clear the gapMarker flag.
              ...
              break;
            case REQUEST_OLDER_THAN_GAP:
              // if service was started as a request to fill a gap, delete
              // existing posts before the one with the gap marker, then remove
              // the existing gap marker
              ReaderPostTable.deletePostsBeforeGapMarkerForTag(tag);
              ReaderPostTable.removeGapMarkerForTag(tag);
              break;
            case REQUEST_OLDER:
          ...
          if (postWithGap != null) {
            ReaderPostTable.setGapMarkerForTag(postWithGap.blogId,
                                               postWithGap.postId, tag);
            AppLog.d(AppLog.T.READER,
                     "added gap marker to tag " + tag.getTagNameForLog());
          }
        } else if (updateResult == ReaderActions.UpdateResult.UNCHANGED &&
                   updateAction == UpdateAction.REQUEST_OLDER_THAN_GAP) {
          // edge case - request to fill gap returned nothing new, so remove the
          // gap marker
          ReaderPostTable.removeGapMarkerForTag(tag);
          AppLog.w(AppLog.T.READER, "attempt to fill gap returned nothing new");
        }
        AppLog.d(AppLog.T.READER,
                 "requested posts response = " + updateResult.toString());
  ...
  private static String getRelativeEndpointForTag(ReaderTag tag) {
    ...
    // never hand craft the endpoint for default tags, since these MUST be
    // updated using their stored endpoints
    if (tag.tagType == ReaderTagType.DEFAULT) {
      return null;
    }

    return String.format("read/tags/%s/posts",
                         ReaderUtils.sanitizeWithDashes(tag.getTagSlug()));
  }
