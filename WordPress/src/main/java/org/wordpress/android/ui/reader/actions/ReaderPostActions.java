...
public class ReaderPostActions {
  ...
  private ReaderPostActions() { throw new AssertionError(); }
  ...
  public static boolean performLikeAction(final ReaderPost post,
    ...
    int numCurrentLikes =
        ReaderPostTable.getNumLikesForPost(post.blogId, post.postId);
    int newNumLikes =
        (isAskingToLike ? numCurrentLikes + 1 : numCurrentLikes - 1);
    if (newNumLikes < 0) {
      newNumLikes = 0;
    }
    ...
    RestRequest.ErrorListener errorListener = new RestRequest.ErrorListener() {
      ...
      public void onErrorResponse(VolleyError volleyError) {
        ...
        if (TextUtils.isEmpty(error)) {
          ...
          AppLog.w(T.READER,
                   String.format("post %s failed (%s)", actionName, error));
        }
        AppLog.e(T.READER, volleyError);
        ReaderPostTable.setLikesForPost(post, post.numLikes,
  ...
  /*
   * get the latest version of this post - note that the post is only considered
   * changed if the like/comment count has changed, or if the current user's
   * like/follow status has changed
   */
  public static void updatePost(final ReaderPost localPost,
                                final UpdateResultListener resultListener) {
    String path = "read/sites/" + localPost.blogId + "/posts/" +
                  localPost.postId + "/?meta=site,likes";
    ...
    WordPress.getRestClientUtilsV1_2().get(path, null, null, listener,
                                           errorListener);
  }

  private static void
  handleUpdatePostResponse(final ReaderPost localPost,
    ...
    new Thread() {
      ...
      public void run() {
        ...
        // TODO: this temporary fix was added 25-Apr-2016 as a workaround for
        // the fact that the read/sites/{blogId}/posts/{postId} endpoint doesn't
        // contain the feedId or feedItemId of the post. because of this, we
        // need to copy them from the local post before calling isSamePost
        // (since the difference in those IDs causes it to return false)
        ...
        if (hasChanges) {
          ...
          // copy changes over to the local post - this is done instead of
          // simply overwriting the local post with the server post because the
          // server post was retrieved using the
          // read/sites/$siteId/posts/$postId endpoint which is missing some
          // information
          ...
          localPost.isFollowedByCurrentUser =
              serverPost.isFollowedByCurrentUser;
        ...
        // always update liking users regardless of whether changes were
        // detected - this ensures that the liking avatars are immediately
        // available to post detail
        ...
        if (resultListener != null) {
          final UpdateResult result =
              (hasChanges ? UpdateResult.CHANGED : UpdateResult.UNCHANGED);
          handler.post(new Runnable() {
            public void run() { resultListener.onUpdateResult(result); }
          });
        }
      }
    }.start();
  ...
  /*
   * updates local liking users based on the "likes" meta section of the post's
   * json - requires using the /sites/ endpoint with ?meta=likes - returns true
   * if likes have changed
   */
  private static boolean handlePostLikes(final ReaderPost post,
                                         JSONObject jsonPost) {
   ...
   **/
  public static void
  requestBlogPost(final long blogId, final long postId,
                  final ReaderActions.OnRequestListener requestListener) {
    String path =
        "read/sites/" + blogId + "/posts/" + postId + "/?meta=site,likes";
   ...
   **/
  public static void
  requestFeedPost(final long feedId, final long feedItemId,
                  final ReaderActions.OnRequestListener requestListener) {
    String path =
        "read/feed/" + feedId + "/posts/" + feedItemId + "/?meta=site,likes";
   ...
   **/
  public static void
  requestBlogPost(final String blogSlug, final String postSlug,
                  final ReaderActions.OnRequestListener requestListener) {
    String path =
        "sites/" + blogSlug + "/posts/slug:" + postSlug + "/?meta=site,likes";
    requestPost(WordPress.getRestClientUtilsV1_1(), path, requestListener);
  }

  private static void
  requestPost(RestClientUtils restClientUtils, String path,
              final ReaderActions.OnRequestListener requestListener) {
    com.wordpress.rest.RestRequest.Listener listener =
        new RestRequest.Listener() {
          @Override
    ...
    RestRequest.ErrorListener errorListener = new RestRequest.ErrorListener() {
      ...
      public void onErrorResponse(VolleyError volleyError) {
        ...
        if (requestListener != null) {
          ...
          // "body":{"error":"unauthorized","message":"User cannot access this
          // private blog."}}
  ...
  private static String getTrackingPixelForPost(@NonNull ReaderPost post) {
    return "https://pixel.wp.com/g.gif?v=wpcom&reader=1"
        + "&blog=" + post.blogId + "&post=" + post.postId +
        "&host=" + UrlUtils.urlEncode(UrlUtils.getHost(post.getBlogUrl())) +
        "&ref=" + UrlUtils.urlEncode(TRACKING_REFERRER) +
        "&t=" + RANDOM.nextInt();
  }

  public static void bumpPageViewForPost(SiteStore siteStore, long blogId,
                                         long postId) {
    bumpPageViewForPost(siteStore,
  ...
  public static void bumpPageViewForPost(SiteStore siteStore, ReaderPost post) {
    ...
    // don't bump stats for posts in sites the current user is an admin of,
    // unless this is a private post since we count views for private posts from
    // owner or member
    SiteModel site = siteStore.getSiteBySiteId(post.blogId);
    // site will be null here if the user is not the owner or a member of the
    // site
    ...
    Request request =
        new StringRequest(Request.Method.GET, getTrackingPixelForPost(post),
  ...
  /*
   * request posts related to the passed one, endpoint returns a combined list
   * of related posts posts from across wp.com and related posts from the same
  ...
  public static void requestRelatedPosts(final ReaderPost sourcePost) {
    ...
    String path = "/read/site/" + sourcePost.blogId + "/post/" +
                  sourcePost.postId + "/related"
  ...
  private static void handleRelatedPostsResponse(final ReaderPost sourcePost,
    ...
    new Thread() {
      ...
      public void run() {
        ...
        if (jsonPosts != null) {
          ReaderSimplePostList allRelatedPosts =
              ReaderSimplePostList.fromJsonPosts(jsonPosts);
          // split into posts from the passed site (local) and from across
          // wp.com (global)
          ...
          EventBus.getDefault().post(new ReaderEvents.RelatedPostsUpdated(
              sourcePost, localRelatedPosts, globalRelatedPosts));
        }
      }
    }.start();
  }
  ...
  public static void removeFromBookmarked(@NonNull final ReaderPost post) {
    if (post.isBookmarked) {
      ...
      String msg =
          "removeFromBookmarked called on a post which wasn't bookmarked.";
      AppLog.w(T.READER, msg);
      if (BuildConfig.DEBUG) {
        throw new RuntimeException(msg);
      }
