...
public class NotificationsDetailListFragment
    extends ListFragment implements NotificationFragment {
  ...
  private CommentUserNoteBlock
      .OnCommentStatusChangeListener mOnCommentStatusChangeListener;
  ...
  public static NotificationsDetailListFragment
  newInstance(final String noteId) {
    NotificationsDetailListFragment fragment =
        new NotificationsDetailListFragment();
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null &&
        savedInstanceState.containsKey(KEY_NOTE_ID)) {
  ...
  public View onCreateView(@NonNull LayoutInflater inflater,
    ...
    View view = inflater.inflate(R.layout.notifications_fragment_detail_list,
                                 container, false);
  ...
  public void onSaveInstanceState(Bundle outState) {
    if (mNote != null) {
      ...
      outState.putInt(KEY_LIST_POSITION,
  ...
  public void setFooterView(ViewGroup footerView) { mFooterView = footerView; }

  private final NoteBlock
      .OnNoteBlockTextClickListener mOnNoteBlockTextClickListener =
      new NoteBlock.OnNoteBlockTextClickListener() {
        @Override
        public void onNoteBlockTextClicked(NoteBlockClickableSpan clickedSpan) {
          if (!isAdded() ||
              !(getActivity() instanceof NotificationsDetailActivity)) {
            return;
          }

          handleNoteBlockSpanClick((NotificationsDetailActivity)getActivity(),
                                   clickedSpan);
        }
        ...
        public void showDetailForNoteIds() {
          ...
          NotificationsDetailActivity detailActivity =
              (NotificationsDetailActivity)getActivity();
          if (mNote.isCommentReplyType() ||
              (!mNote.isCommentType() && mNote.getCommentId() > 0)) {
            long commentId = mNote.isCommentReplyType()
                                 ? mNote.getParentCommentId()
                                 : mNote.getCommentId();

            // show comments list if it exists in the reader
            if (ReaderUtils.postAndCommentExists(
                    mNote.getSiteId(), mNote.getPostId(), commentId)) {
              detailActivity.showReaderCommentsList(
                  mNote.getSiteId(), mNote.getPostId(), commentId);
            } else {
            ...
            detailActivity.showPostActivity(mNote.getSiteId(),
                                            mNote.getPostId());
          }
        }
        ...
        public void showReaderPostComments() {
          ...
          ReaderActivityLauncher.showReaderComments(
              getActivity(), mNote.getSiteId(), mNote.getPostId(),
              mNote.getCommentId());
        }
        ...
        public void handleNoteBlockSpanClick(
          ...
          case POST:
            // Show post detail
            activity.showPostActivity(clickedSpan.getSiteId(),
                                      clickedSpan.getId());
            break;
          case COMMENT:
            // Load the comment in the reader list if it exists, otherwise show
            // a webview
            if (ReaderUtils.postAndCommentExists(clickedSpan.getSiteId(),
                                                 clickedSpan.getPostId(),
                                                 clickedSpan.getId())) {
              activity.showReaderCommentsList(clickedSpan.getSiteId(),
                                              clickedSpan.getPostId(),
                                              clickedSpan.getId());
            } else {
          ...
          case FOLLOW:
            ...
            activity.showStatsActivityForSite(clickedSpan.getSiteId(),
                                              clickedSpan.getRangeType());
            break;
          case LIKE:
            if (ReaderPostTable.postExists(clickedSpan.getSiteId(),
                                           clickedSpan.getId())) {
              activity.showReaderPostLikeUsers(clickedSpan.getSiteId(),
                                               clickedSpan.getId());
            } else {
              activity.showPostActivity(clickedSpan.getSiteId(),
                                        clickedSpan.getId());
            ...
            break;
          default:
            // We don't know what type of id this is, let's see if it has a URL
            // and push a webview
  ...
  private final UserNoteBlock
      .OnGravatarClickedListener mOnGravatarClickedListener =
  ...
  private boolean hasNoteBlockAdapter() { return mNoteBlockAdapter != null; }

  // Loop through the 'body' items in this note, and create blocks for each.
  private class LoadNoteBlocksTask
      extends AsyncTask<Void, Boolean, List<NoteBlock>> {
    ...
    protected List<NoteBlock> doInBackground(Void... params) {
      ...
      if (mNote.getHeader() != null) {
        ImageType imageType = mNote.isFollowType()
                                  ? ImageType.BLAVATAR
                                  : ImageType.AVATAR_WITH_BACKGROUND;
        HeaderNoteBlock headerNoteBlock = new HeaderNoteBlock(
            getActivity(), transformToFormattableContentList(mNote.getHeader()),
            imageType, mOnNoteBlockTextClickListener,
            mOnGravatarClickedListener, mImageManager,
        ...
        for (int i = 0; i < bodyArray.length(); i++) {
          try {
            ...
            if (BlockType.fromString(noteObject.getType()) == BlockType.USER) {
              if (mNote.isCommentType()) {
                ...
                if (i + 1 < bodyArray.length()) {
                  commentTextBlock =
                      mNotificationsUtilsWrapper.mapJsonToFormattableContent(
                          bodyArray.getJSONObject(i + 1));
                  i++;
                }

                noteBlock = new CommentUserNoteBlock(
                    getActivity(), noteObject, commentTextBlock,
                    mNote.getTimestamp(), mOnNoteBlockTextClickListener,
                    mOnGravatarClickedListener, mImageManager,
                    mNotificationsUtilsWrapper);
                pingbackUrl = noteBlock.getMetaSiteUrl();

                // Set listener for comment status changes, so we can update bg
                // and text colors
                CommentUserNoteBlock commentUserNoteBlock =
                    (CommentUserNoteBlock)noteBlock;
                mOnCommentStatusChangeListener =
                    commentUserNoteBlock.getOnCommentChangeListener();
                ...
                noteBlock = new UserNoteBlock(
                    getActivity(), noteObject, mOnNoteBlockTextClickListener,
                    mOnGravatarClickedListener, mImageManager,
                    mNotificationsUtilsWrapper);
              }
            } else if (isFooterBlock(noteObject)) {
              noteBlock = new FooterNoteBlock(noteObject, mImageManager,
                                              mNotificationsUtilsWrapper,
                                              mOnNoteBlockTextClickListener);
              if (noteObject.getRanges() != null &&
                  noteObject.getRanges().size() > 0) {
                FormattableRange range = noteObject.getRanges().get(
                    noteObject.getRanges().size() - 1);
                ((FooterNoteBlock)noteBlock)
                    .setClickableSpan(range, mNote.getType());
              }
            } else {
              noteBlock = new NoteBlock(noteObject, mImageManager,
                                        mNotificationsUtilsWrapper,
                                        mOnNoteBlockTextClickListener);
            }
            ...
            if (isAdded() && noteBlock.containsBadgeMediaType()) {
              mIsBadgeView = true;
              mBackgroundColor = getActivity().getResources().getColor(
      ...
      if (isPingback) {
        // Remove this when we start receiving "Read the source post block" from
        // the backend
        NoteBlock generatedBlock = buildGeneratedLinkBlock(
    ...
    @NonNull
    private List<FormattableContent>
    transformToFormattableContentList(JSONArray headerArray) {
    ...
    private NoteBlock buildGeneratedLinkBlock(
        OnNoteBlockTextClickListener onNoteBlockTextClickListener,
        String pingbackUrl, String message) {
      return new GeneratedNoteBlock(message, mImageManager,
                                    mNotificationsUtilsWrapper,
    ...
    protected void onPostExecute(List<NoteBlock> noteList) {
      ...
      if (!hasNoteBlockAdapter()) {
        mNoteBlockAdapter =
            new NoteBlockAdapter(getActivity(), noteList, mBackgroundColor);
  ...
  private boolean isFooterBlock(FormattableContent blockObject) {
    ...
    if (mNote.isCommentType()) {
      Long commentReplyId =
          FormattableContentUtilsKt.getRangeIdOrZero(blockObject, 1);
      // Check if this is a comment notification that has been replied to
      // The block will not have a type, and its id will match the comment reply
      // id in the Note.
      return (blockObject.getType() == null &&
              mNote.getCommentReplyId() == commentReplyId);
    } else if (mNote.isFollowType() || mNote.isLikeType() ||
               mNote.isReblogType()) {
      // User list notifications have a footer if they have 10 or more users in
      // the body The last block will not have a type, so we can use that to
      // determine if it is the footer
      return blockObject.getType() == null;
    }
  ...
  public void refreshBlocksForCommentStatus(CommentStatus newStatus) {
    if (mOnCommentStatusChangeListener != null) {
      ...
      ListView listView = getListView();
      if (listView == null ||
          mCommentListPosition == ListView.INVALID_POSITION) {
        return;
      }

      // Redraw the comment row if it is visible so that the background and text
      // colors update See:
      // http://stackoverflow.com/questions/4075975/redraw-a-single-row-in-a-listview/9987616#9987616
  ...
  private void requestReaderContentForNote() {
    ...
    if (mNote.isUserList() &&
        !ReaderPostTable.postExists(mNote.getSiteId(), mNote.getPostId())) {
      ReaderPostActions.requestBlogPost(mNote.getSiteId(), mNote.getPostId(),
                                        null);
    }

    // Request reader comments until we retrieve the comment for this note
    if ((mNote.isCommentLikeType() || mNote.isCommentReplyType() ||
         mNote.isCommentWithUserReply()) &&
        !ReaderCommentTable.commentExists(mNote.getSiteId(), mNote.getPostId(),
                                          mNote.getCommentId())) {
