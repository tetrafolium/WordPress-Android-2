...
public class ReaderPostDetailFragment extends Fragment
  ...
  private static final String BOOKMARKS_SAVED_LOCALLY_DIALOG =
  ...
  public static ReaderPostDetailFragment newInstance(
      boolean isFeed, long blogId, long postId, DirectOperation directOperation,
      int commentId, boolean isRelatedPost, String interceptedUri,
      ReaderPostListType postListType, boolean postSlugsResolutionUnderway) {
    ...
    args.putBoolean(ReaderConstants.KEY_POST_SLUGS_RESOLUTION_UNDERWAY,
  ...
  public void setArguments(Bundle args) {
    ...
    if (args != null) {
      ...
      mDirectOperation = (DirectOperation)args.getSerializable(
      ...
      mPostSlugsResolutionUnderway =
          args.getBoolean(ReaderConstants.KEY_POST_SLUGS_RESOLUTION_UNDERWAY);
    }
  }
  ...
  public void onAttach(Activity activity) {
    ...
    mToolbarHeight =
        activity.getResources().getDimensionPixelSize(R.dimen.toolbar_height);
  }

  @Override
  public View onCreateView(LayoutInflater inflater, ViewGroup container,
                           Bundle savedInstanceState) {
    final View view = inflater.inflate(R.layout.reader_fragment_post_detail,
                                       container, false);

    CustomSwipeRefreshLayout swipeRefreshLayout =
        view.findViewById(R.id.swipe_to_refresh);

    // this fragment hides/shows toolbar with scrolling, which messes up ptr
    // animation position so we have to set it manually
    int swipeToRefreshOffset =
        getResources().getDimensionPixelSize(R.dimen.toolbar_content_offset);
    swipeRefreshLayout.setProgressViewOffset(false, 0, swipeToRefreshOffset);

    mSwipeToRefreshHelper = buildSwipeToRefreshHelper(
        swipeRefreshLayout, new SwipeToRefreshHelper.RefreshListener() {
          @Override
    ...
    View relatedPostsContainer =
        view.findViewById(R.id.container_related_posts);
    mGlobalRelatedPostsView =
        relatedPostsContainer.findViewById(R.id.related_posts_view_global);
    mGlobalRelatedPostsView.setOnFollowListener(this);
    mLocalRelatedPostsView =
        relatedPostsContainer.findViewById(R.id.related_posts_view_local);
  ...
  private boolean hasPost() { return (mPost != null); }
  ...
  public void onPrepareOptionsMenu(Menu menu) {
    ...
    // browse & share require the post to have a URL (some feed-based posts
    // don't have one)
    ...
    if (i == R.id.menu_browse) {
      if (hasPost()) {
        ...
        AnalyticsUtils.trackWithInterceptedUri(
            AnalyticsTracker.Stat.DEEP_LINKED_FALLBACK, mInterceptedUri);
        ReaderActivityLauncher.openUrl(getActivity(), mInterceptedUri,
                                       OpenUrlType.EXTERNAL);
        getActivity().finish();
      }
      return true;
  ...
  public void onSaveInstanceState(Bundle outState) {
    ...
    outState.putSerializable(ReaderConstants.ARG_DIRECT_OPERATION,
                             mDirectOperation);
    ...
    outState.putBoolean(ReaderConstants.KEY_POST_SLUGS_RESOLUTION_UNDERWAY,
                        mPostSlugsResolutionUnderway);
    outState.putBoolean(ReaderConstants.KEY_ALREADY_UPDATED,
                        mHasAlreadyUpdatedPost);
    outState.putBoolean(ReaderConstants.KEY_ALREADY_REQUESTED,
                        mHasAlreadyRequestedPost);

    outState.putBoolean(
        ReaderConstants.KEY_ALREADY_TRACKED_GLOBAL_RELATED_POSTS,
        mHasTrackedGlobalRelatedPosts);
    outState.putBoolean(ReaderConstants.KEY_ALREADY_TRACKED_LOCAL_RELATED_POSTS,
  ...
  private void restoreState(Bundle savedInstanceState) {
    if (savedInstanceState != null) {
      ...
      mDirectOperation = (DirectOperation)savedInstanceState.getSerializable(
          ReaderConstants.ARG_DIRECT_OPERATION);
      mCommentId = savedInstanceState.getInt(ReaderConstants.ARG_COMMENT_ID);
      mIsRelatedPost =
          savedInstanceState.getBoolean(ReaderConstants.ARG_IS_RELATED_POST);
      mInterceptedUri =
          savedInstanceState.getString(ReaderConstants.ARG_INTERCEPTED_URI);
      mPostSlugsResolutionUnderway = savedInstanceState.getBoolean(
      ...
      mHasTrackedGlobalRelatedPosts = savedInstanceState.getBoolean(
          ReaderConstants.KEY_ALREADY_TRACKED_GLOBAL_RELATED_POSTS);
      mHasTrackedLocalRelatedPosts = savedInstanceState.getBoolean(
      ...
      if (savedInstanceState.containsKey(ReaderConstants.KEY_ERROR_MESSAGE)) {
        mErrorMessage =
            savedInstanceState.getString(ReaderConstants.KEY_ERROR_MESSAGE);
      }
    }
  }
  ...
  public void onFollowTapped(View view, String blogName, final long blogId) {
    ...
    if (isAdded()) {
      ...
      WPSnackbar
          .make(view,
                Html.fromHtml(
                    getString(R.string.reader_followed_blog_notifications,
                              "<b>", blog, "</b>")),
              ...
              new View.OnClickListener() {
                ...
                public void onClick(View view) {
                  ...
                  AddOrDeleteSubscriptionPayload payload =
                      new AddOrDeleteSubscriptionPayload(
  ...
  public void onSubscriptionUpdated(OnSubscriptionUpdated event) {
    if (event.isError()) {
      AppLog.e(T.API, ReaderPostDetailFragment.class.getSimpleName() +
  ...
  private void showBookmarksSavedLocallyDialog() {
    ...
    basicFragmentDialog.show(getFragmentManager(),
                             BOOKMARKS_SAVED_LOCALLY_DIALOG);
  }
  ...
  private void setPostLike(boolean isAskingToLike) {
    ...
    if (isAskingToLike != ReaderPostTable.isPostLikedByCurrentUser(mPost)) {
      ...
      boolean success = ReaderPostActions.performLikeAction(
          mPost, isAskingToLike, mAccountStore.getAccount().getUserId());
      if (!success) {
        likeCount.setSelected(!isAskingToLike);
        return;
      }
    ...
    if (isAskingToLike) {
      AnalyticsUtils.trackWithReaderPostDetails(
          AnalyticsTracker.Stat.READER_ARTICLE_DETAIL_LIKED, mPost);
    } else {
      AnalyticsUtils.trackWithReaderPostDetails(
          AnalyticsTracker.Stat.READER_ARTICLE_DETAIL_UNLIKED, mPost);
    }
  }
  ...
  private void replacePost(long blogId, long postId,
    ...
    // hide views that would show info for the previous post - these will be
    // re-displayed with the correct info once the new post loads
    ...
    // clear the webView - otherwise it will remain scrolled to where the user
    // scrolled to
  ...
  public void onEventMainThread(ReaderEvents.RelatedPostsUpdated event) {
    ...
    if (event.getSourcePostId() == mPost.postId &&
        event.getSourceSiteId() == mPost.blogId) {
  ...
  /*
   * show the passed list of related posts - can be either global (related posts
   * from across wp.com) or local (related posts from the same site as the
   * current post)
   */
  private void showRelatedPosts(@NonNull ReaderSimplePostList relatedPosts,
                                final boolean isGlobal) {
    // tapping a related post should open the related post detail
    ReaderSimplePostView.OnSimplePostClickListener listener =
        new ReaderSimplePostView.OnSimplePostClickListener() {
          @Override
    ...
    ReaderSimplePostContainerView relatedPostsView =
        isGlobal ? mGlobalRelatedPostsView : mLocalRelatedPostsView;
    relatedPostsView.showPosts(relatedPosts, mPost.getBlogName(), isGlobal,
                               listener);
  ...
  /*
   * track that related posts have loaded and are scrolled into view if we
   * haven't already tracked it
   */
  private void trackRelatedPostsIfShowing() {
    if (!mHasTrackedGlobalRelatedPosts &&
        isVisibleAndScrolledIntoView(mGlobalRelatedPostsView)) {
    ...
    if (!mHasTrackedLocalRelatedPosts &&
        isVisibleAndScrolledIntoView(mLocalRelatedPostsView)) {
  ...
  /*
   * returns True if the passed view is visible and has been scrolled into view
   * - assumes that the view is a child of mScrollView
   */
  ...
  /*
   * user clicked a single related post - if we're already viewing a related
   * post, add it to the history stack so the user can back-button through the
   * history - otherwise start a new detail activity for this related post
   */
  private void showRelatedPostDetail(long blogId, long postId,
                                     boolean isGlobal) {
    AnalyticsTracker.Stat stat =
        isGlobal ? AnalyticsTracker.Stat.READER_GLOBAL_RELATED_POST_CLICKED
    ...
    if (mIsRelatedPost) {
      ...
      ReaderActivityLauncher.showReaderPostDetail(getActivity(), false, blogId,
                                                  postId, null, 0, true, null);
    }
  }

  /*
   * if the fragment is maintaining a backstack of posts, navigate to the
   * previous one
   */
    ...
    ReaderActions.UpdateResultListener resultListener =
        new ReaderActions.UpdateResultListener() {
          ...
          public void onUpdateResult(ReaderActions.UpdateResult result) {
            ...
            // if the post has changed, reload it from the db and update the
            // like/comment counts
            if (result.isNewOrChanged()) {
              mPost = ReaderPostTable.getBlogPost(mPost.blogId, mPost.postId,
                                                  false);
              refreshIconCounts();
            }
            // refresh likes if necessary - done regardless of whether the post
            // has changed since it's possible likes weren't stored until the
            // post was updated
  ...
  private void refreshIconCounts() {
    ...
    final ReaderIconCountView countLikes =
        getView().findViewById(R.id.count_likes);
    final ReaderIconCountView countComments =
        getView().findViewById(R.id.count_comments);
    ...
    if (canShowCommentCount()) {
      ...
      countComments.setOnClickListener(new View.OnClickListener() {
        @Override
        public void onClick(View v) {
          ReaderActivityLauncher.showReaderComments(getActivity(), mPost.blogId,
                                                    mPost.postId);
        }
      });
    } else {
    ...
    if (canShowLikeCount()) {
      ...
      countLikes.setContentDescription(ReaderUtils.getLongLikeLabelText(
          getActivity(), mPost.numLikes, mPost.isLikedByCurrentUser));
      ...
      // if we know refreshLikes() is going to show the liking users, force
      // liking user views to take up space right now
  ...
  private void doLikePost() {
    ...
    if (!mPost.canLikePost()) {
      ToastUtils.showToast(getActivity(),
                           R.string.reader_toast_err_cannot_like_post);
      return;
    }
    ...
    mLikingUsersView.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View view) {
        ReaderActivityLauncher.showReaderLikingUsers(
            getActivity(), mPost.blogId, mPost.postId);
      }
    });
  ...
  private boolean showPhotoViewer(String imageUrl, View sourceView, int startX,
                                  int startY) {
    ...
    EnumSet<PhotoViewerOption> options =
        EnumSet.noneOf(PhotoViewerOption.class);
    if (isPrivatePost) {
  ...
  private void requestPost() {
    ...
    ReaderActions.OnRequestListener listener =
          ...
          public void onSuccess() {
            ...
            if (isAdded()) {
              ...
              EventBus.getDefault().post(
                  new ReaderEvents.SinglePostDownloaded());
            }
          }
  ...
  private void onRequestFailure(int statusCode) {
    ...
    if (!NetworkUtils.isNetworkAvailable(getActivity())) {
      ...
      case 401:
      case 403:
        final boolean offerSignIn =
            WPUrlUtils.isWordPressCom(mInterceptedUri) &&
            !mAccountStore.hasAccessToken();

        if (!offerSignIn) {
          errMsgResId =
              (mInterceptedUri == null)
          ...
          AnalyticsUtils.trackWithReaderPostDetails(
              AnalyticsTracker.Stat.READER_WPCOM_SIGN_IN_NEEDED, mPost);
        }
        AnalyticsUtils.trackWithReaderPostDetails(
            AnalyticsTracker.Stat.READER_USER_UNAUTHORIZED, mPost);
        break;
      case 404:
  ...
  /*
   * shows an error message in the middle of the screen - used when requesting
   * post fails
   */
  ...
  private class ShowPostTask extends AsyncTask<Void, Void, Boolean> {
    ...
    protected Boolean doInBackground(Void... params) {
      ...
      if (mPost.isDiscoverPost()) {
        ...
        if (discoverData != null &&
            ...
            discoverData.getBlogId() != 0 && discoverData.getPostId() != 0) {
          mIsFeed = false;
    ...
    protected void onPostExecute(Boolean result) {
      ...
      if (!result) {
        // post couldn't be loaded which means it doesn't exist in db, so
        // request it from the server if it hasn't already been requested
        if (!mHasAlreadyRequestedPost) {
          AppLog.i(T.READER,
                   "reader post detail > post not found, requesting it");
          requestPost();
        } else if (!TextUtils.isEmpty(mErrorMessage)) {
          // post has already been requested and failed, so restore previous
          // error message
      ...
      if (mDirectOperation != null) {
        ...
        case COMMENT_LIKE:
          ReaderActivityLauncher.showReaderComments(
              getActivity(), mPost.blogId, mPost.postId, mDirectOperation,
              mCommentId, mInterceptedUri);
          getActivity().finish();
          getActivity().overridePendingTransition(0, 0);
          return;
        case POST_LIKE:
          // Liking needs to be handled "later" after the post has been updated
          // from the server so, nothing special to do here
          break;
        }
      }

      AnalyticsUtils.trackWithReaderPostDetails(
          AnalyticsTracker.Stat.READER_ARTICLE_RENDERED, mPost);
      ...
      ReaderPostDetailHeaderView headerView =
          getView().findViewById(R.id.header_view);
      headerView.setOnFollowListener(ReaderPostDetailFragment.this);
      if (!canShowFooter()) {
        mLayoutFooter.setVisibility(View.GONE);
      }

      // add padding to the scrollView to make room for the top and bottom
      // toolbars - this also ensures the scrollbar matches the content so it
      // doesn't disappear behind the toolbars
      ...
      // scrollView was hidden in onCreateView, show it now that we have the
      // post
      ...
      // if we're showing just the excerpt, also show a footer which links to
      // the full post
      if (mPost.shouldShowExcerpt()) {
        ...
        String blogName =
            "<font color='" +
            HtmlUtils.colorResToHtmlColor(getActivity(), R.color.link_reader) +
            "'>" + mPost.getBlogName() + "</font>";
        String linkText = String.format(
            WordPress.getContext().getString(R.string.reader_excerpt_link),
            blogName);

        TextView txtExcerptFooter =
            excerptFooter.findViewById(R.id.text_excerpt_footer);
      ...
      txtTitle.setText(mPost.hasTitle()
                           ? mPost.getTitle()
                           : getString(R.string.reader_untitled_post));

      String timestamp = DateTimeUtils.javaDateToTimeSpan(
          mPost.getDisplayDate(), WordPress.getContext());
  ...
  /*
   * called by the web view when the content finishes loading - likes aren't
   * displayed until this is triggered, to avoid having them appear before the
   * webView content
   */
  @Override
  ...
  public ViewGroup onRequestCustomView() {
    if (isAdded()) {
      return (ViewGroup)getView().findViewById(
          R.id.layout_custom_view_container);
    } else {
      return null;
    }
  }
  ...
  /*
   * return the container view that should be hidden when full screen video is
   * shown
   */
  @Override
  public ViewGroup onRequestContentView() {
    if (isAdded()) {
      return (ViewGroup)getView().findViewById(
          R.id.layout_post_detail_container);
    } else {
      return null;
    }
  }
  ...
  public boolean onUrlClick(String url) {
    // if this is a "wordpress://blogpreview?" link, show blog preview for the
    // blog - this is used for Discover posts that highlight a blog
    ...
    OpenUrlType openUrlType =
        shouldOpenExternal(url) ? OpenUrlType.EXTERNAL : OpenUrlType.INTERNAL;
  ...
  private boolean shouldOpenExternal(String url) {
    ...
    // if the mime type starts with "application" open it externally - this will
    // either open it in the associated app or the default browser (which will
    // enable the user to download it)
  ...
  public boolean onFileDownloadClick(String fileUrl) {
    ...
    if (activity != null && fileUrl != null &&
        PermissionUtils.checkAndRequestStoragePermission(
            this, READER_FILE_DOWNLOAD_PERMISSION_REQUEST_CODE)) {
  ...
  private void downloadFile(String fileUrl, FragmentActivity activity) {
    ...
    DownloadManager dm =
        (DownloadManager)activity.getSystemService(DOWNLOAD_SERVICE);
    dm.enqueue(r);
  }
  ...
  public void onScrollUp(float distanceY) {
    if (!mIsToolbarShowing && -distanceY >= MIN_SCROLL_DISTANCE_Y) {
      showToolbar(true);
      showFooter(true);
    }
  }
  ...
  private boolean canShowCommentCount() {
    ...
    return mPost.isWP() && !mPost.isDiscoverPost() &&
