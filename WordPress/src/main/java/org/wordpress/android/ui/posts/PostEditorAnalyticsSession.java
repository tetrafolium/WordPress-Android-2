...
public class PostEditorAnalyticsSession implements Serializable {
  ...
  private static final String KEY_HAS_UNSUPPORTED_BLOCKS =
  ...
  enum Editor { GUTENBERG, CLASSIC, HTML }

  enum Outcome {
    CANCEL,
    DISCARD, // not used in WPAndroid, but kept for parity with iOS
    // see
    // https://github.com/wordpress-mobile/gutenberg-mobile/issues/556#issuecomment-462678807
    SAVE,
    PUBLISH
  }
  ...
  public void start(ArrayList<Object> unsupportedBlocksList) {
    if (!mStarted) {
      mHasUnsupportedBlocks =
          unsupportedBlocksList != null && unsupportedBlocksList.size() > 0;
      Map<String, Object> properties = getCommonProperties();
      properties.put(KEY_UNSUPPORTED_BLOCKS, unsupportedBlocksList != null
      ...
      AppLog.w(
          T.EDITOR,
          "An editor session cannot be attempted to be started more than once, "
  ...
  public void setOutcome(Outcome newOutcome) {
    // on WPiOS we're using `forceOutcome` and some exception logic to allow /
    // not allow forcing an outcome, but in WPAndroid we slightly changed the
    // interface as we're not really `forcing` but just `setting` the outcome on
    // specific places where we know what happened or is going to happen.
    // Session ending will only properly end the session with the outcome
    // already being set.
    mOutcome = newOutcome;
  }

  public void end() {
    // don't try to send an "end" event if the session wasn't started in the
    // first place
    if (mStarted) {
      if (mOutcome == null) {
        // outcome should have already been set with setOutcome at specific user
        // actions if outcome is still unknown, chances are Activity was killed
        // / user cancelled so, set to CANCEL.
      ...
      AppLog.e(T.EDITOR,
               "A non-started editor session cannot be attempted to be ended");
    }
  }

  private Map<String, Object> getCommonProperties() {
    Map<String, Object> properties = new HashMap<>();
    properties.put(KEY_EDITOR,
                   mCurrentEditor.toString().toLowerCase(Locale.ROOT));
    ...
    properties.put(KEY_HAS_UNSUPPORTED_BLOCKS,
                   mHasUnsupportedBlocks ? "1" : "0");
    return properties;
  }
}
