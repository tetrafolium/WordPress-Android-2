...
QuickStartAdapter(Context context, List<QuickStartTask> tasksUncompleted,
	...
	if (!tasksCompleted.isEmpty()) {
		mTasks.add(null); // adding null where the complete tasks header
		                  // simplifies a lot of logic for us
	}
...
public ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup,
	...
	case VIEW_TYPE_COMPLETED_TASKS_HEADER:
		return new CompletedHeaderViewHolder(inflater.inflate(
							     R.layout.quick_start_completed_tasks_list_header, viewGroup, false));
	default:
...
public void onBindViewHolder(@NonNull ViewHolder viewHolder, int position) {
	...
	if (viewType == VIEW_TYPE_COMPLETED_TASKS_HEADER) {
		CompletedHeaderViewHolder headerViewHolder =
			(CompletedHeaderViewHolder)viewHolder;
		headerViewHolder.mTitle.setText(mContext.getString(
							R.string.quick_start_complete_tasks_header, mTaskCompleted.size()));

		if (mIsCompletedTaskListExpanded) {
			headerViewHolder.itemView.setBackgroundResource(
				R.drawable.bg_rectangle_white_radius_top_card);
			headerViewHolder.mChevron.setRotation(EXPANDED_CHEVRON_ROTATION);
			headerViewHolder.mChevron.setContentDescription(mContext.getString(
										R.string.quick_start_completed_tasks_header_chevron_collapse_desc));
		} else {
			headerViewHolder.itemView.setBackgroundResource(
				R.drawable.bg_rectangle_white_radius_card);
			headerViewHolder.mChevron.setRotation(COLLAPSED_CHEVRON_ROTATION);
			headerViewHolder.mChevron.setContentDescription(mContext.getString(
										R.string.quick_start_completed_tasks_header_chevron_expand_desc));
		}
	...
	if (position == 0) {
		...
		if (getItemViewType(position + 1) == VIEW_TYPE_COMPLETED_TASKS_HEADER) {
			taskViewHolder.itemView.setBackgroundResource(
				R.drawable.bg_rectangle_white_radius_card);
			// Use top rounded background when next item is not header (i.e. middle
			// or bottom).
		} else {
			taskViewHolder.itemView.setBackgroundResource(
				R.drawable.bg_rectangle_white_radius_top_card);
		}
		// When last list item or next item is header, use bottom rounded
		// background.
	} else if (position == mTasks.size() - 1 ||
	           getItemViewType(position + 1) ==
	           VIEW_TYPE_COMPLETED_TASKS_HEADER) {
		taskViewHolder.itemView.setBackgroundResource(
			R.drawable.bg_rectangle_white_radius_bottom_card);
	...
	if (!isEnabled) {
		taskViewHolder.mTitle.setPaintFlags(
			taskViewHolder.mTitle.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);
	}
	...
	QuickStartTaskDetails quickStartTaskDetails =
		QuickStartTaskDetails.getDetailsForTask(task);

	if (quickStartTaskDetails == null) {
		throw new IllegalStateException(task.toString() +
		                                " task is not recognized in adapter.");
	}
...
void updateContent(List<QuickStartTask> tasksUncompleted,
	...
	DiffUtil.DiffResult diffResult = DiffUtil.calculateDiff(
		new QuickStartTasksDiffCallback(mTasks, newList));
	...
	// Notify adapter of each task change individually.  Using
	// notifyDataSetChanged() kills list changing animation.
			...
			popup.setOnMenuItemClickListener(new OnMenuItemClickListener() {
						...
						public boolean onMenuItemClick(MenuItem item) {
						        if (item.getItemId() == R.id.quick_start_task_menu_skip) {
						                if (mListener != null) {
						                        mListener.onSkipTaskTapped(
										mTasks.get(getAdapterPosition()));
								}
						                return true;
							}
						        return false;
...
private void toggleCompletedTasksList() {
	ViewPropertyAnimator viewPropertyAnimator =
		mChevron.animate()
		.rotation(mIsCompletedTaskListExpanded
		            ? COLLAPSED_CHEVRON_ROTATION
