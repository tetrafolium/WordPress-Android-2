...
public class ReaderLikingUsersView extends LinearLayout {
  ...
  public ReaderLikingUsersView(Context context) { this(context, null); }
  ...
  public void showLikingUsers(final ReaderPost post, final long currentUserId) {
    ...
    mLoadAvatarsTask = new LoadAvatarsTask(this, currentUserId, mLikeAvatarSz,
                                           getMaxAvatars());
  ...
  private int getMaxAvatars() {
    final int marginAvatar =
        getResources().getDimensionPixelSize(R.dimen.margin_extra_small);
    final int marginReader =
        getResources().getDimensionPixelSize(R.dimen.reader_detail_margin);
  ...
  private void showLikingAvatars(final ArrayList<String> avatarUrls) {
    ...
    for (String url : avatarUrls) {
      ...
      if (index < numExistingViews) {
        ...
        imgAvatar = (ImageView)inflater.inflate(R.layout.reader_like_avatar,
                                                this, false);
        addView(imgAvatar);
      }
      mImageManager.loadIntoCircle(imgAvatar, ImageType.AVATAR,
                                   StringUtils.notNullStr(url));
      index++;
    }
  }

  private static class LoadAvatarsTask
      extends AsyncTask<ReaderPost, Void, ArrayList<String>> {
    ...
    LoadAvatarsTask(ReaderLikingUsersView view, long currentUserId,
                    int likeAvatarSz, int maxAvatars) {
    ...
    protected ArrayList<String> doInBackground(ReaderPost... posts) {
      ...
      return ReaderUserTable.getAvatarUrls(avatarIds, mMaxAvatars,
                                           mLikeAvatarSize, mCurrentUserId);
    }
