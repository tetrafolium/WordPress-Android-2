...
/**
 * hash map of sizes of attachments in a reader post - created from the json
 * "attachments" section of the post endpoints
 */
  ...
  public ImageSizeMap(@NonNull String postContent, String jsonString) {
    ...
    try {
      ...
      while (it.hasNext()) {
        ...
        if (jsonAttach != null &&
            JSONUtils.getString(jsonAttach, "mime_type").startsWith("image")) {
          String normUrl = UrlUtils.normalizeUrl(
              UrlUtils.removeQuery(JSONUtils.getString(jsonAttach, "URL")));

          // make sure this image actually appears in the post content - it's
          // possible for an image to be in the attachments but not in the post
          // itself
