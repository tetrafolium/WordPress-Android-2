...
public void onBackPressed() {
	CollapseFullScreenDialogFragment fragment =
		(CollapseFullScreenDialogFragment)getSupportFragmentManager()
		.findFragmentByTag(CollapseFullScreenDialogFragment.TAG);
...
public void onCreate(Bundle savedInstanceState) {
	...
	if (savedInstanceState == null) {
		...
		mIsTappedOnNotification =
			getIntent().getBooleanExtra(IS_TAPPED_ON_NOTIFICATION, false);
	} else {
		if (savedInstanceState.containsKey(ARG_TITLE) &&
		    getSupportActionBar() != null) {
			getSupportActionBar().setTitle(
				StringUtils.notNullStr(savedInstanceState.getString(ARG_TITLE)));
		}
		mNoteId =
			savedInstanceState.getString(NotificationsListFragment.NOTE_ID_EXTRA);
		mIsTappedOnNotification =
			savedInstanceState.getBoolean(IS_TAPPED_ON_NOTIFICATION);
	}
	...
	// if this is coming from a tapped push notification, let's try refreshing
	// it as its contents may have been updated since the notification was first
	// received and created on the system's dashboard
	updateUIAndNote((note == null) || mIsTappedOnNotification);

	// Hide the keyboard, unless we arrived here from the 'Reply' action in a
	// push notification
...
private void updateUIAndNote(boolean doRefresh) {
	...
	// here compare the current Note - is it any different from the Note the
	// user is currently viewing? if it is, then replace the dataset with the
	// new one. If not, just let it be.
	if (mAdapter != null) {
	...
	if (getIntent().hasExtra(
		    NotificationsListFragment.NOTE_CURRENT_LIST_FILTER_EXTRA)) {
		filter = (NotesAdapter.FILTERS)getIntent().getSerializableExtra(
			NotificationsListFragment.NOTE_CURRENT_LIST_FILTER_EXTRA);
	}
	...
	// If `note.getTimestamp()` is not the most recent seen note, the server
	...
	AnalyticsTracker.track(
		AnalyticsTracker.Stat.NOTIFICATIONS_OPENED_NOTIFICATION_DETAILS,
		properties);
	...
	if (mOnPageChangeListener != null) {
		...
		mOnPageChangeListener = new ViewPager.OnPageChangeListener() {
			...
			public void onPageSelected(int position) {
				AnalyticsTracker.track(
					AnalyticsTracker.Stat.NOTIFICATION_SWIPE_PAGE_CHANGED);
...
protected void onStart() {
	...
	// If the user hasn't used swipe yet and if the adapter is initialised and
	// have at least 2 notifications, show a hint to promote swipe usage on the
	// ViewPager
	if (!AppPrefs.isNotificationsSwipeToNavigateShown() && mAdapter != null &&
	    mAdapter.getCount() > 1) {
...
private void markNoteAsRead(Note note) {
	GCMMessageService.removeNotificationWithNoteIdFromSystemBar(this,
	                                                            note.getId());
...
private NotificationDetailFragmentAdapter
buildNoteListAdapterAndSetPosition(Note note, NotesAdapter.FILTERS filter) {
	NotificationDetailFragmentAdapter adapter;
	ArrayList<Note> notes = NotificationsTable.getLatestNotes();
	ArrayList<Note> filteredNotes = new ArrayList<>();

	// apply filter to the list so we show the same items that the list show
	...
	mViewPager.setCurrentItem(
		NotificationsUtils.findNoteInNoteArray(filteredNotes, note.getId()));
...
private Fragment getDetailFragmentForNote(Note note) {
	...
	if (note.isCommentType()) {
		...
		boolean isInstantReply = getIntent().getBooleanExtra(
			NotificationsListFragment.NOTE_INSTANT_REPLY_EXTRA, false);
		...
		// show reader post detail for automattchers about posts - note that
		// comment automattchers are handled by note.isCommentType() above
		boolean isPost = (note.getSiteId() != 0 && note.getPostId() != 0 &&
		                  note.getCommentId() == 0);
		if (isPost) {
			fragment = ReaderPostDetailFragment.newInstance(note.getSiteId(),
			                                                note.getPostId());
		} else {
...
public void showStatsActivityForSite(long siteId,
                                     FormattableRangeType rangeType) {
	SiteModel site = mSiteStore.getSiteBySiteId(siteId);
	if (site == null) {
		// One way the site can be null: new site created, receive a notification
		// from this site, but the site list is not yet updated in the app.
...
private void showStatsActivityForSite(@NonNull SiteModel site,
                                      FormattableRangeType rangeType) {
	if (isFinishing()) {
		return;
	}
	...
	if (rangeType == FormattableRangeType.FOLLOW) {
		ActivityLauncher.viewAllTabbedInsightsStats(this, StatsViewType.FOLLOWERS,
		                                            0, site.getId());
	} else {
...
public void onModerateCommentForNote(Note note, CommentStatus newStatus) {
	...
	resultIntent.putExtra(NotificationsListFragment.NOTE_MODERATE_ID_EXTRA,
	                      note.getId());
	resultIntent.putExtra(NotificationsListFragment.NOTE_MODERATE_STATUS_EXTRA,
	                      newStatus.toString());
...
public void onEventMainThread(
	final NotificationEvents.NotificationsRefreshCompleted event) {
	setProgressVisible(false);
...
@Subscribe(threadMode = ThreadMode.MAIN)
public void
onEventMainThread(NotificationEvents.NotificationsRefreshError error) {
	setProgressVisible(false);
	if (mNoteId == null) {
		showErrorToastAndFinish();
		return;
...
public void onPositiveClicked(@NotNull String instanceTag) {
	...
	if (fragment instanceof
	    BasicFragmentDialog.BasicDialogPositiveClickInterface) {
		((BasicDialogPositiveClickInterface)fragment)
		.onPositiveClicked(instanceTag);
	}
}

private class NotificationDetailFragmentAdapter
	extends FragmentStatePagerAdapter {
private final ArrayList<Note> mNoteList;

NotificationDetailFragmentAdapter(FragmentManager fm,
...
public Parcelable saveState() {
	...
	// https://github.com/wordpress-mobile/WordPress-Android/issues/5456 See
	// https://issuetracker.google.com/issues/37103380#comment77 for more
	// details
