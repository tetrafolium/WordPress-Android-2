...
void onRequestReconnect(PublicizeService service,
                        PublicizeConnection connection);
}

private static class PublicizeConnectionValidationException
	extends Exception {
...
public static void disconnect(@NonNull final PublicizeConnection connection) {
	String path =
		String.format(Locale.ROOT, "sites/%d/publicize-connections/%d/delete",
		              connection.siteId, connection.connectionId);

	RestRequest.Listener listener = new RestRequest.Listener() {
		@Override
		public void onResponse(JSONObject jsonObject) {
			AppLog.d(AppLog.T.SHARING, "disconnect succeeded");
			EventBus.getDefault().post(new ActionCompleted(
							   true, ConnectAction.DISCONNECT, connection.getService()));
		}
	};
	RestRequest.ErrorListener errorListener = new RestRequest.ErrorListener() {
		...
		public void onErrorResponse(VolleyError volleyError) {
			...
			EventBus.getDefault().post(new ActionCompleted(
							   false, ConnectAction.DISCONNECT, connection.getService()));
		}
	};
...
public static void reconnect(@NonNull final PublicizeConnection connection) {
	...
	RestRequest.Listener listener = new RestRequest.Listener() {
		...
		public void onResponse(JSONObject jsonObject) {
			AppLog.d(AppLog.T.SHARING, "disconnect succeeded");
			EventBus.getDefault().post(new ActionCompleted(
							   true, ConnectAction.RECONNECT, connection.getService()));
		}
	};
	RestRequest.ErrorListener errorListener = new RestRequest.ErrorListener() {
		@Override
		public void onErrorResponse(VolleyError volleyError) {
			...
			PublicizeTable.addOrUpdateConnection(connection);
			EventBus.getDefault().post(new ActionCompleted(
							   false, ConnectAction.RECONNECT, connection.getService()));
		}
	};
 ...
 */
public static void connect(long siteId, String serviceId,
                           long currentUserId) {
	if (TextUtils.isEmpty(serviceId)) {
		AppLog.w(AppLog.T.SHARING, "cannot connect without service");
		EventBus.getDefault().post(
			new ActionCompleted(false, ConnectAction.CONNECT, serviceId));
		return;
	}
...
/*
 * step one in creating a publicize connection: request the list of keyring
 * connections and find the one for the passed service
 */
private static void connectStepOne(final long siteId, final String serviceId,
	...
	RestRequest.Listener listener = new RestRequest.Listener() {
		...
		public void onResponse(JSONObject jsonObject) {
			...
			try {
				showChooserDialog =
					shouldShowChooserDialog(siteId, serviceId, jsonObject);
			} catch (PublicizeConnectionValidationException e) {
				final ActionCompleted event = new ActionCompleted(
					false, ConnectAction.CONNECT, serviceId, e.mReasonResId);
			...
			if (showChooserDialog) {
				...
				EventBus.getDefault().post(
					new PublicizeEvents.ActionRequestChooseAccount(siteId, serviceId,
					                                               jsonObject));
			} else {
				long keyringConnectionId =
					parseServiceKeyringId(serviceId, currentUserId, jsonObject);
...
/*
 * step two in creating a publicize connection: now that we have the keyring
 * connection id, create the actual connection
 */
public static void connectStepTwo(final long siteId, long keyringConnectionId,
	...
	RestRequest.Listener listener = new RestRequest.Listener() {
		...
		public void onResponse(JSONObject jsonObject) {
			...
			PublicizeConnection connection =
				PublicizeConnection.fromJson(jsonObject);
			PublicizeTable.addOrUpdateConnection(connection);
			EventBus.getDefault().post(
				new ActionCompleted(true, ConnectAction.CONNECT, serviceId));
		}
	};
	RestRequest.ErrorListener errorListener = new RestRequest.ErrorListener() {
		@Override
	...
	// Sending the external id for Twitter and LinkedIn connections result in an
	// error
	if (!TextUtils.isEmpty(externalUserId) &&
	    PublicizeConstants.FACEBOOK_ID.equals(serviceId)) {
		params.put("external_user_ID", externalUserId);
	}
	String path = String.format(Locale.ROOT,
	                            "/sites/%d/publicize-connections/new", siteId);
	WordPress.getRestClientUtilsV1_1().post(path, params, null, listener,
	                                        errorListener);
}

private static boolean shouldShowChooserDialog(long siteId, String serviceId,
	...
	try {
		for (int i = 0; i < jsonConnectionList.length(); i++) {
			...
			PublicizeConnection publicizeConnection =
				PublicizeConnection.fromJson(connectionObject);
			if (publicizeConnection.getService().equals(serviceId) &&
			    !publicizeConnection.isInSite(siteId)) {
				totalAccounts++;
				JSONArray externalJsonArray =
					connectionObject.getJSONArray("additional_external_users");
		...
		if (PublicizeTable.onlyExternalConnections(serviceId)) {
			if (!hasExternalAccounts &&
			    serviceId.equals(PublicizeService.FACEBOOK_SERVICE_ID)) {
				AppLog.i(
					T.SHARING,
					"The Facebook account cannot be linked because either there was no Page selected or the "
					+ "Page is set as not published.");
				throw new PublicizeConnectionValidationException(
					      R.string.sharing_facebook_account_must_have_pages);
			} else {
 ...
 */
private static long
parseServiceKeyringId(String serviceId, long currentUserId, JSONObject json) {
