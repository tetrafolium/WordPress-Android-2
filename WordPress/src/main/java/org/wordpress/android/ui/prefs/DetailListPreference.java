...
public class DetailListPreference
	extends ListPreference implements PreferenceHint {
...
protected void onBindView(@NonNull View view) {
	...
	setupView((TextView)view.findViewById(android.R.id.summary),
	          ...
	          ContextExtensionsKt.getColorResIdFromAttribute(
			  getContext(), R.attr.wpColorTextSubtle),
	          R.color.neutral_20);
}
...
protected void showDialog(Bundle state) {
	...
	mListAdapter = new DetailListAdapter(
		getContext(), R.layout.detail_list_preference, mDetails);
	mStartingValue = getValue();
	mSelectedIndex = findIndexOfValue(mStartingValue);

	builder.setSingleChoiceItems(
		mListAdapter, mSelectedIndex, new DialogInterface.OnClickListener() {
...
protected void onDialogClosed(boolean positiveResult) {
	int index =
		positiveResult ? mSelectedIndex : findIndexOfValue(mStartingValue);
 ...
 */
private void setupView(TextView view, int sizeRes, int enabledColorRes,
                       int disabledColorRes) {
	if (view != null) {
		Resources res = getContext().getResources();
		view.setTextSize(TypedValue.COMPLEX_UNIT_PX,
		                 res.getDimensionPixelSize(sizeRes));
		// noinspection deprecation
		view.setTextColor(
			res.getColor(isEnabled() ? enabledColorRes : disabledColorRes));
	}
}
...
public View getView(final int position, View convertView,
	...
	if (convertView == null) {
		convertView =
			View.inflate(getContext(), R.layout.detail_list_preference, null);
	}
	...
	TextView detailText =
		(TextView)convertView.findViewById(R.id.detail_text);

	if (mainText != null && getEntries() != null &&
	    position < getEntries().length) {
		mainText.setText(getEntries()[position]);
	}

	if (detailText != null) {
		if (mDetails != null && position < mDetails.length &&
		    !TextUtils.isEmpty(mDetails[position])) {
