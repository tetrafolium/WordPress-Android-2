...
public class ReaderUpdateLogic {
  /***
   * This class holds the business logic for Reader Updates, serving both
   * ReaderUpdateService (<API26) and ReaderUpdateJobService (API26+). Updates
   * followed/recommended tags and blogs for the Reader, relies on EventBus to
   * notify of changes
   */

  public enum UpdateTask { TAGS, FOLLOWED_BLOGS, RECOMMENDED_BLOGS }
  ...
  public ReaderUpdateLogic(Context context, WordPress app,
  ...
  public void performTasks(EnumSet<UpdateTask> tasks, Object companion) {
    ...
    // perform in priority order - we want to update tags first since without
    // them the Reader can't show anything
  ...
  /***
   * update the tags the user is followed - also handles recommended (popular)
   * tags since they're included in the response
   */
  private void updateTags() {
    ...
    WordPress.getRestClientUtilsV1_2().get("read/menu", params, null, listener,
                                           errorListener);
  }
  ...
  private void handleUpdateTagsResponse(final JSONObject jsonObject) {
    new Thread() {
      @Override
      public void run() {
        // get server topics, both default & followed - but use "recommended"
        // for logged-out reader since user won't have any followed tags
        ReaderTagList serverTopics = new ReaderTagList();
        serverTopics.addAll(
            parseTags(jsonObject, "default", ReaderTagType.DEFAULT));
        if (!mAccountStore.hasAccessToken()) {
          serverTopics.addAll(
              parseTags(jsonObject, "recommended", ReaderTagType.FOLLOWED));
        } else {
          serverTopics.addAll(
              parseTags(jsonObject, "subscribed", ReaderTagType.FOLLOWED));
        }

        // manually insert Bookmark tag, as server doesn't support bookmarking
        // yet
        serverTopics.add(new ReaderTag(
            "", "", mContext.getString(R.string.reader_save_for_later_title),
            "", ReaderTagType.BOOKMARKED));

        // parse topics from the response, detect whether they're different from
        // local
        ...
        if (!localTopics.isSameList(serverTopics)) {
          ...
          // if any local topics have been removed from the server, make sure to
          // delete them locally (including their posts)
        ...
        if (mAccountStore.hasAccessToken()) {
          ReaderTagList serverRecommended =
              parseTags(jsonObject, "recommended", ReaderTagType.RECOMMENDED);
          ReaderTagList localRecommended =
              ReaderTagTable.getRecommendedTags(false);
          if (!serverRecommended.isSameList(localRecommended)) {
            AppLog.d(AppLog.T.READER,
                     "reader service > recommended topics changed");
            ReaderTagTable.setRecommendedTags(serverRecommended);
            EventBus.getDefault().post(
                new ReaderEvents.RecommendedTagsChanged());
          }
        }
   ...
   */
  private static ReaderTagList parseTags(JSONObject jsonObject, String name,
    ...
    while (it.hasNext()) {
      ...
      if (jsonTopic != null) {
        String tagTitle = JSONUtils.getStringDecoded(
            jsonTopic, ReaderConstants.JSON_TAG_TITLE);
        String tagDisplayName = JSONUtils.getStringDecoded(
            jsonTopic, ReaderConstants.JSON_TAG_DISPLAY_NAME);
        String tagSlug = JSONUtils.getStringDecoded(
            jsonTopic, ReaderConstants.JSON_TAG_SLUG);
        String endpoint =
            JSONUtils.getString(jsonTopic, ReaderConstants.JSON_TAG_URL);

        // if the endpoint contains `read/list` then this is a custom list -
        // these are included in the response as default tags
        if (tagType == ReaderTagType.DEFAULT &&
            endpoint.contains("/read/list/")) {
          topics.add(new ReaderTag(tagSlug, tagDisplayName, tagTitle, endpoint,
  ...
  private void updateFollowedBlogs() {
    ...
    WordPress.getRestClientUtilsV1_1().get(
        "read/following/mine?meta=site%2Cfeed", listener, errorListener);
  }
    ...
    new Thread() {
      ...
      public void run() {
        ...
        if (!localBlogs.isSameList(serverBlogs)) {
          // always update the list of followed blogs if there are *any* changes
          // between server and local (including subscription count,
          // description, etc.)
          ReaderBlogTable.setFollowedBlogs(serverBlogs);
          // ...but only update the follow status and alert that followed blogs
          // have changed if the server list doesn't have the same blogs as the
          // local list (ie: a blog has been followed/unfollowed since local was
          // last updated)
          if (!localBlogs.hasSameBlogs(serverBlogs)) {
            ReaderPostTable.updateFollowedStatus();
            AppLog.i(AppLog.T.READER,
                     "reader blogs service > followed blogs changed");
  ...
  private void updateRecommendedBlogs() {
    ...
    String path =
        ...
        + "?source=mobile"
        + "&number=" +
        Integer.toString(ReaderConstants.READER_MAX_RECOMMENDED_TO_REQUEST);
  ...
  private void handleRecommendedBlogsResponse(final JSONObject jsonObject) {
    new Thread() {
      @Override
      public void run() {
        ReaderRecommendBlogList serverBlogs =
            ReaderRecommendBlogList.fromJson(jsonObject);
        ReaderRecommendBlogList localBlogs =
            ReaderBlogTable.getRecommendedBlogs();

        if (!localBlogs.isSameList(serverBlogs)) {
          ReaderBlogTable.setRecommendedBlogs(serverBlogs);
          EventBus.getDefault().post(
              new ReaderEvents.RecommendedBlogsChanged());
        }
