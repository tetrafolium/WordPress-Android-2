...
private final List<PreferenceCategory> mTypePreferenceCategories =
	new ArrayList<>();
...
public void onCreate(Bundle savedInstanceState) {
	...
	if (savedInstanceState == null) {
		AnalyticsTracker.track(
			AnalyticsTracker.Stat.NOTIFICATION_SETTINGS_LIST_OPENED);
	}
}
...
private void removeSightAndSoundsForAPI26() {
	...
	if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
		PreferenceScreen preferenceScreen = (PreferenceScreen)findPreference(
			getActivity().getString(R.string.wp_pref_notifications_root));
...
public void onActivityCreated(Bundle savedInstanceState) {
	...
	mDeviceId =
		settings.getString(NotificationsUtils.WPCOM_PUSH_DEVICE_SERVER_ID, "");
...
public void onResume() {
	...
	mNotificationsEnabled =
		NotificationsUtils.isNotificationsEnabled(getActivity());
	refreshSettings();
}
...
public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
	...
	mSearchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
			...
			public boolean onQueryTextChange(String newText) {
			        // we need to perform this check because when the search menu item is
			        // collapsed a new queryTExtChange event is triggered with an empty
			        // value "", and we only would want to take care of it when the user
			        // actively opened/cleared the search term
			        configureBlogsSettings(mBlogsCategory, !mSearchMenuItemCollapsed);
			        configureFollowedBlogsSettings(mFollowedBlogsCategory,
			                                       !mSearchMenuItemCollapsed);
			        return true;
			}
		});
...
public void onActivityResult(int requestCode, int resultCode, Intent data) {
	...
	if (data != null && requestCode == NOTIFICATION_SETTINGS) {
		boolean notifyPosts = data.getBooleanExtra(
			NotificationSettingsFollowedDialog.KEY_NOTIFICATION_POSTS, false);
		boolean emailPosts = data.getBooleanExtra(
			NotificationSettingsFollowedDialog.KEY_EMAIL_POSTS, false);
		String emailPostsFrequency = data.getStringExtra(
			NotificationSettingsFollowedDialog.KEY_EMAIL_POSTS_FREQUENCY);
		boolean emailComments = data.getBooleanExtra(
			NotificationSettingsFollowedDialog.KEY_EMAIL_COMMENTS, false);

		if (notifyPosts != mPreviousNotifyPosts) {
			ReaderBlogTable.setNotificationsEnabledByBlogId(
				Long.parseLong(mNotificationUpdatedSite), notifyPosts);
			AddOrDeleteSubscriptionPayload payload;

			if (notifyPosts) {
				AnalyticsTracker.track(Stat.FOLLOWED_BLOG_NOTIFICATIONS_SETTINGS_ON);
				payload = new AddOrDeleteSubscriptionPayload(mNotificationUpdatedSite,
				                                             SubscriptionAction.NEW);
			} else {
				AnalyticsTracker.track(Stat.FOLLOWED_BLOG_NOTIFICATIONS_SETTINGS_OFF);
				payload = new AddOrDeleteSubscriptionPayload(
					mNotificationUpdatedSite, SubscriptionAction.DELETE);
			}
		...
		if (emailPostsFrequency != null &&
		    !emailPostsFrequency.equalsIgnoreCase(mPreviousEmailPostsFrequency)) {
			SubscriptionFrequency subscriptionFrequency =
				getSubscriptionFrequencyFromString(emailPostsFrequency);
			mUpdateSubscriptionFrequencyPayload = new UpdateSubscriptionPayload(
				mNotificationUpdatedSite, subscriptionFrequency);
			/*
			 * The email post frequency update will be overridden by the email post
			 * update if the email post frequency callback returns first.  Thus, the
			 * updates must be dispatched sequentially when the email post update is
...
public void onSubscriptionsChanged(OnSubscriptionsChanged event) {
	if (event.isError()) {
		AppLog.e(T.API, "NotificationsSettingsFragment.onSubscriptionsChanged: " +
		         event.error.message);
	} else {
		configureFollowedBlogsSettings(mFollowedBlogsCategory,
		                               !mSearchMenuItemCollapsed);
...
public void onSubscriptionUpdated(OnSubscriptionUpdated event) {
	if (event.isError()) {
		AppLog.e(T.API, "NotificationsSettingsFragment.onSubscriptionUpdated: " +
		         event.error.message);
	} else if (event.type == SubscriptionType.EMAIL_POST &&
	           mUpdateEmailPostsFirst) {
...
private SubscriptionFrequency getSubscriptionFrequencyFromString(String s) {
	if (s.equalsIgnoreCase(SubscriptionFrequency.DAILY.toString())) {
		AnalyticsTracker.track(
			Stat.FOLLOWED_BLOG_NOTIFICATIONS_SETTINGS_EMAIL_DAILY);
		return SubscriptionFrequency.DAILY;
	} else if (s.equalsIgnoreCase(SubscriptionFrequency.WEEKLY.toString())) {
		AnalyticsTracker.track(
			Stat.FOLLOWED_BLOG_NOTIFICATIONS_SETTINGS_EMAIL_WEEKLY);
		return SubscriptionFrequency.WEEKLY;
	} else {
		AnalyticsTracker.track(
			Stat.FOLLOWED_BLOG_NOTIFICATIONS_SETTINGS_EMAIL_INSTANTLY);
...
private void refreshSettings() {
	if (!hasNotificationsSettings()) {
		EventBus.getDefault().post(
			new NotificationEvents.NotificationsSettingsStatusChanged(
	...
	NotificationsUtils.getPushNotificationSettings(
		getActivity(),
...
private void loadNotificationsAndUpdateUI(boolean shouldUpdateUI) {
	...
	try {
		SharedPreferences sharedPreferences =
			PreferenceManager.getDefaultSharedPreferences(getActivity());
		settingsJson = new JSONObject(sharedPreferences.getString(
						      NotificationsUtils.WPCOM_PUSH_DEVICE_NOTIFICATION_SETTINGS, ""));
...
// Updates the UI for preference screens based on if notifications are enabled
// or not
private void updateUIForNotificationsEnabledState() {
	if (mTypePreferenceCategories == null ||
	    mTypePreferenceCategories.size() == 0) {
		return;
	}
	...
	for (final PreferenceCategory category : mTypePreferenceCategories) {
		...
		if (category.getPreferenceCount() >= TYPE_COUNT &&
			...
			category.getPreference(TYPE_COUNT - 1)
			.setEnabled(mNotificationsEnabled);
		}
	}
}

private void configureBlogsSettings(PreferenceCategory blogsCategory,
                                    boolean showAll) {
	if (!isAdded()) {
		return;
	}
	...
	if (mSearchView != null && !TextUtils.isEmpty(mSearchView.getQuery())) {
		...
		sites = mSiteStore.getSitesAccessedViaWPComRestByNameOrUrlMatching(
			trimmedQuery);
	} else {
	...
	if (mSiteCount > 0) {
		Collections.sort(sites, new Comparator<SiteModel>() {
				...
				public int compare(SiteModel o1, SiteModel o2) {
				        return SiteUtils.getSiteNameOrHomeURL(o1).compareToIgnoreCase(
						SiteUtils.getSiteNameOrHomeURL(o2));
				}
			});
	}
	...
	int maxSitesToShow =
		showAll ? NO_MAXIMUM : MAX_SITES_TO_SHOW_ON_FIRST_SCREEN;
	int count = 0;
	for (SiteModel site : sites) {
		...
		addPreferencesForPreferenceScreen(prefScreen, Channel.BLOGS,
		                                  site.getSiteId());
	...
	if (mSiteCount == 0 && !TextUtils.isEmpty(trimmedQuery)) {
		...
		searchResultsPref.setSummary(String.format(
						     getString(R.string.notifications_no_search_results), trimmedQuery));
	...
	if (mSiteCount > maxSitesToShow && !showAll) {
		...
		appendViewAllSitesOption(
			context, getString(R.string.pref_notification_blogs), false);
	}

	updateSearchMenuVisibility();
}

private void configureFollowedBlogsSettings(PreferenceCategory blogsCategory,
	...
	if (subscriptions.size() > 0) {
		Collections.sort(subscriptions, new Comparator<SubscriptionModel>() {
				...
				public int compare(SubscriptionModel o1, SubscriptionModel o2) {
				        return getSiteNameOrHostFromSubscription(o1).compareToIgnoreCase(
						getSiteNameOrHostFromSubscription(o2));
				}
			});
	}
	...
	for (final SubscriptionModel subscription : subscriptions) {
		...
		// Subscriptions with a "false" blogId are for feeds and don't have
		...
		prefScreen.setOnPreferenceClickListener(
			new Preference.OnPreferenceClickListener() {
				@Override
				public boolean onPreferenceClick(Preference preference) {
						...
						subscription.getEmailPostsFrequency();
				        mPreviousEmailComments = subscription.getShouldEmailComments();
				        NotificationSettingsFollowedDialog dialog =
						new NotificationSettingsFollowedDialog();
				        ...
				        dialog.setTargetFragment(NotificationsSettingsFragment.this,
				                                 NOTIFICATION_SETTINGS);
				        dialog.show(getFragmentManager(),
				                    NotificationSettingsFollowedDialog.TAG);
				        return true;
				}
			});
	...
	if (mSubscriptionCount == 0 && !TextUtils.isEmpty(query)) {
		Preference searchResultsPref = new Preference(context);
		searchResultsPref.setSummary(String.format(
						     getString(R.string.notifications_no_search_results), query));
		blogsCategory.addPreference(searchResultsPref);
	}
	...
	if (!showAll && mSubscriptionCount > maxSitesToShow) {
		appendViewAllSitesOption(
			context, getString(R.string.pref_notification_blogs_followed), true);
	}
...
private String
getSiteNameOrHostFromSubscription(SubscriptionModel subscription) {
...
private void appendViewAllSitesOption(Context context, String preference,
                                      boolean isFollowed) {
	PreferenceCategory blogsCategory =
		(PreferenceCategory)findPreference(preference);
...
private void configureWPComSettings() {
	PreferenceCategory otherPreferenceCategory =
		(PreferenceCategory)findPreference(
			getString(R.string.pref_notification_other_category));
	NotificationsSettingsDialogPreference devicePreference =
		new NotificationsSettingsDialogPreference(
			getActivity(), null, Channel.WPCOM,
			NotificationsSettings.Type.DEVICE, 0, mNotificationsSettings,
			mOnSettingsChangedListener);
	devicePreference.setTitle(
		R.string.notification_settings_item_other_account_emails);
	devicePreference.setDialogTitle(
		R.string.notification_settings_item_other_account_emails);
	devicePreference.setSummary(
		R.string.notification_settings_item_other_account_emails_summary);
	otherPreferenceCategory.addPreference(devicePreference);
}

private void
addPreferencesForPreferenceScreen(PreferenceScreen preferenceScreen,
                                  Channel channel, long blogId) {
	Context context = getActivity();
	if (context == null) {
		return;
	}
	...
	NotificationsSettingsDialogPreference timelinePreference =
		new NotificationsSettingsDialogPreference(
			context, null, channel, NotificationsSettings.Type.TIMELINE, blogId,
			mNotificationsSettings, mOnSettingsChangedListener);

	timelinePreference.setIcon(ColorUtils.INSTANCE.applyTintToDrawable(
					   context, R.drawable.ic_bell_white_24dp, R.color.neutral_60));
	...
	NotificationsSettingsDialogPreference emailPreference =
		new NotificationsSettingsDialogPreference(
			context, null, channel, NotificationsSettings.Type.EMAIL, blogId,
			mNotificationsSettings, mOnSettingsChangedListener);

	emailPreference.setIcon(ColorUtils.INSTANCE.applyTintToDrawable(
					context, R.drawable.ic_mail_white_24dp, R.color.neutral_60));
	...
	String deviceID = settings.getString(
		NotificationsUtils.WPCOM_PUSH_DEVICE_SERVER_ID, null);
	if (!TextUtils.isEmpty(deviceID)) {
		...
		devicePreference.setIcon(ColorUtils.INSTANCE.applyTintToDrawable(
						 context, R.drawable.ic_phone_white_24dp, R.color.neutral_60));
...
private void addSitesForViewAllSitesScreen(PreferenceScreen preferenceScreen,
	...
	rootCategory.setTitle(
		isFollowed ? R.string.notification_settings_category_followed_sites
...
private final NotificationsSettingsDialogPreference
.OnNotificationsSettingsChangedListener mOnSettingsChangedListener =
	new NotificationsSettingsDialogPreference
	.OnNotificationsSettingsChangedListener() {
	@SuppressWarnings("unchecked")
	@Override
	public void onSettingsChanged(Channel channel,
	                              NotificationsSettings.Type type,
	                              long blogId, JSONObject newValues) {
		if (!isAdded()) {
			return;
		}
		...
		case BLOGS:
			try {
				...
				if (type == Type.DEVICE) {
					newValues.put(NotificationsSettings.KEY_DEVICE_ID,
					              Long.parseLong(mDeviceId));
					JSONArray devicesArray = new JSONArray();
					devicesArray.put(newValues);
					blogObject.put(NotificationsSettings.KEY_DEVICES,
					               devicesArray);
				...
				settingsObject.put(NotificationsSettings.KEY_BLOGS,
				                   blogsArray);
			} catch (JSONException e) {
				AppLog.e(T.NOTIFS,
				         "Could not build notification settings object");
			}
			break;
		case OTHER:
			try {
				...
				if (type == Type.DEVICE) {
					...
					JSONArray devicesArray = new JSONArray();
					devicesArray.put(newValues);
					otherObject.put(NotificationsSettings.KEY_DEVICES,
					                devicesArray);
				} else {
					otherObject.put(type.toString(), newValues);
				}

				settingsObject.put(NotificationsSettings.KEY_OTHER,
				                   otherObject);
			} catch (JSONException e) {
				AppLog.e(T.NOTIFS,
				         "Could not build notification settings object");
		...
		case WPCOM:
			try {
				settingsObject.put(NotificationsSettings.KEY_WPCOM,
				                   newValues);
			} catch (JSONException e) {
				AppLog.e(T.NOTIFS,
				         "Could not build notification settings object");
		...
		if (settingsObject.length() > 0) {
			WordPress.getRestClientUtilsV1_1().post(
				"/me/notifications/settings", settingsObject, null, null,
				null);
		}
	}
};
...
public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
	...
	if (preference instanceof PreferenceScreen) {
		...
		AnalyticsTracker.track(
			AnalyticsTracker.Stat.NOTIFICATION_SETTINGS_STREAMS_OPENED);
	} else {
		AnalyticsTracker.track(
			AnalyticsTracker.Stat.NOTIFICATION_SETTINGS_DETAILS_OPENED);
	}
...
public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
	...
	if (key.equals(getString(R.string.pref_key_notification_pending_drafts))) {
		if (getActivity() != null) {
			SharedPreferences prefs =
				androidx.preference.PreferenceManager.getDefaultSharedPreferences(
					getActivity());
			boolean shouldNotifyOfPendingDrafts =
		...
		final String defaultPath = getString(
			R.string
		...
		if (value.trim().toLowerCase(Locale.ROOT).startsWith("file://")) {
			// sound path begins with 'file://` which will lead to
			// FileUriExposedException when used. Revert to
			//  default and let the user know.
			AppLog.w(T.NOTIFS,
			         "Notification sound starts with unacceptable scheme: " +
			         value);
