...
public static Intent createIntent(Context context, @NonNull String targetUrl,
	...
	intent.putExtra(ReleaseNotesActivity.KEY_ORIGIN,
	                origin != null ? origin : Origin.RELEASE_NOTES);
...
public boolean onOptionsItemSelected(final MenuItem item) {
	...
	case R.id.menu_share:
		...
		return true;
	case R.id.menu_bug:
		HelpActivity.Origin origin =
			(HelpActivity.Origin)getIntent().getSerializableExtra(KEY_ORIGIN);
		SiteModel selectedSite =
			(SiteModel)getIntent().getSerializableExtra(WordPress.SITE);
		mZendeskHelper.createNewTicket(ReleaseNotesActivity.this, origin,
		                               selectedSite);
		return true;
	}
