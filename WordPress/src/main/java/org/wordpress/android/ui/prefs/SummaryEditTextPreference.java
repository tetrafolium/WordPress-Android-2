 ...
 * <p>
 * Created for and used by {@link SiteSettingsFragment} to style some
 * Preferences. <p> When declaring this class in a layout file you can use the
 * following attributes:
 * - app:summaryLines : sets the number of lines to display in the Summary field
 * (see {@link TextView#setLines(int)} for details)
 * - app:maxSummaryLines : sets the maximum number of lines the Summary field
 * can display (see {@link TextView#setMaxLines(int)} for details)
 * - app:longClickHint : sets the string to be shown in a Toast when preference
 * is long clicked
 * - app:dialogSummary : sets the summary text in the dialog
 */

public class SummaryEditTextPreference
    extends EditTextPreference implements PreferenceHint {
  private int mLines;
  ...
  public SummaryEditTextPreference(Context context) { super(context); }
  ...
  public SummaryEditTextPreference(Context context, AttributeSet attrs) {
    ...
    TypedArray array = context.obtainStyledAttributes(
        attrs, R.styleable.SummaryEditTextPreference);
    ...
    for (int i = 0; i < array.getIndexCount(); ++i) {
      ...
      if (index == R.styleable.SummaryEditTextPreference_summaryLines) {
        mLines = array.getInt(index, -1);
                 ...
                 R.styleable.SummaryEditTextPreference_maxSummaryLines) {
  ...
  protected void showDialog(Bundle state) {
    ...
    AlertDialog.Builder builder =
        new AlertDialog.Builder(context, R.style.Calypso_Dialog_Alert);
    View titleView =
        View.inflate(getContext(), R.layout.detail_list_preference_title, null);
    mWhichButtonClicked = DialogInterface.BUTTON_NEGATIVE;

    builder.setPositiveButton(android.R.string.ok, this);
    builder.setNegativeButton(
        res.getString(android.R.string.cancel).toUpperCase(Locale.getDefault()),
        this);
    if (titleView != null) {
  ...
  protected void onBindDialogView(final View view) {
    ...
    if (oldParent != view) {
      if (oldParent != null && oldParent instanceof ViewGroup) {
        ...
        ViewCompat.setPaddingRelative(groupParent,
                                      ViewCompat.getPaddingStart(groupParent),
                                      0, ViewCompat.getPaddingEnd(groupParent),
    ...
    ViewGroup.MarginLayoutParams layoutParams =
        (ViewGroup.MarginLayoutParams)message.getLayoutParams();
    int leftMargin = 0;
    int bottomMargin =
        view.getResources().getDimensionPixelSize(R.dimen.margin_small);
    // Different versions handle the message view's margin differently
    // This is a small hack to try to make it align with the input for earlier
    // versions
