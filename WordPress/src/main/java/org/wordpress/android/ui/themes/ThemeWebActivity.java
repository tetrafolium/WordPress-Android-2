...
public class ThemeWebActivity extends WPWebViewActivity {
  ...
  private static final String THEME_URL_PREVIEW =
      "https://wordpress.com/customize/%s?theme=%s/%s&hide_close=true";
  private static final String THEME_URL_SUPPORT =
      "https://wordpress.com/theme/%s/support/?preview=true&iframe=true";
  private static final String THEME_URL_DETAILS =
      "https://wordpress.com/theme/%s/?preview=true&iframe=true";
  private static final String THEME_URL_DEMO_PARAMETER =
      "demo=true&iframe=true&theme_preview=true";

  enum ThemeWebActivityType { PREVIEW, DEMO, DETAILS, SUPPORT }
  ...
  public static void openTheme(Activity activity, @NonNull SiteModel site,
    ...
    if (type == ThemeWebActivityType.PREVIEW || !theme.isWpComTheme()) {
      ...
      // Customizer may need to access local files (mostly pictures) on the
      // device storage, and our internal webview doesn't handle this feature
      // yet. Ref:
      // https://github.com/wordpress-mobile/WordPress-Android/issues/4934
  ...
  private static void openWPCOMURL(Activity activity, String url,
                                   ThemeModel theme, SiteModel site) {
    if (activity == null) {
      ...
      ToastUtils.showToast(activity, R.string.invalid_site_url_message,
    ...
    activity.startActivityForResult(intent,
                                    ThemeBrowserActivity.ACTIVATE_THEME);
  }

  public static String getIdentifierForCustomizer(@NonNull SiteModel site,
                                                  @NonNull ThemeModel theme) {
    if (site.isJetpackConnected()) {
  ...
  public static String getUrl(@NonNull SiteModel site,
                              @NonNull ThemeModel theme,
                              @NonNull ThemeWebActivityType type,
                              boolean isPremium) {
    if (theme.isWpComTheme()) {
      ...
      case PREVIEW:
        ...
        return String.format(THEME_URL_PREVIEW, UrlUtils.getHost(site.getUrl()),
                             domain, theme.getThemeId());
      case DEMO:
      ...
      switch (type) {
      case PREVIEW:
        return site.getAdminUrl() +
            "customize.php?theme=" + getIdentifierForCustomizer(site, theme);
      case DEMO:
  ...
  /**
   * Show Activate in the Action Bar menu if the theme is free and not the
   * current theme.
   */
  private boolean shouldShowActivateMenuItem() {
    boolean isPremiumTheme =
        getIntent().getBooleanExtra(IS_PREMIUM_THEME, false);
    boolean isCurrentTheme =
        getIntent().getBooleanExtra(IS_CURRENT_THEME, false);
