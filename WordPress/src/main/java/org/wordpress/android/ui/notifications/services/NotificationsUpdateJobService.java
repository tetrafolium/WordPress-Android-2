...
public boolean onStartJob(JobParameters params) {
	...
	if (params.getExtras() != null &&
	    params.getExtras().containsKey(
		    NotificationsListFragment.NOTE_ID_EXTRA)) {
		noteId =
			params.getExtras().getString(NotificationsListFragment.NOTE_ID_EXTRA);
		isStartedByTappingOnNotification =
			params.getExtras().getBoolean(IS_TAPPED_ON_NOTIFICATION, false);
	}
	mNotificationsUpdateLogic.performRefresh(
		noteId, isStartedByTappingOnNotification, params);
	return true;
}
...
public void onCompleted(Object companion) {
	AppLog.i(AppLog.T.NOTIFS,
	         "notifications update job service > all tasks completed");
