...
// The server will discard the value if we've already seen a most recent note
// elsewhere or on this device.
...
// If `note.getTimestamp()` is not the most recent seen note, the server will
// discard the value.
	...
	WordPress.getRestClientUtilsV1_1().markNotificationsSeen(
		...
		new RestRequest.Listener() {
			...
			public void onResponse(JSONObject response) {
			        // Assuming that we've marked the most recent notification as seen.
			        // (Beware, seen != read).
		...
		new RestRequest.ErrorListener() {
			...
			public void onErrorResponse(VolleyError error) {
			        AppLog.e(AppLog.T.NOTIFS,
			                 "Could not mark notifications/seen' value via API.",
			                 error);
			}
		});
}
...
public static void markNoteAsRead(final Note note) {
	...
	if (note.isUnread()) {
		WordPress.getRestClientUtilsV1_1().decrementUnreadCount(
			note.getId(), "9999",
			new RestRequest.Listener() {
				@Override
				public void onResponse(JSONObject response) {
...
public static void
downloadNoteAndUpdateDB(final String noteID,
	...
	WordPress.getRestClientUtilsV1_1().getNotification(
		...
		new RestRequest.Listener() {
			...
			public void onResponse(JSONObject response) {
			        if (response == null) {
			                // Not sure this could ever happen, but make sure we're catching
			                // all response types
			                AppLog.w(AppLog.T.NOTIFS,
			                         "Success, but did not receive any notes");
				}
			        try {
			                ...
			                if (notes.size() > 0) {
			                        ...
			                        EventBus.getDefault().post(
							new NotificationEvents.NotificationsChanged(
								notes.get(0).isUnread()));
					} else {
			                ...
			                AppLog.e(
						AppLog.T.NOTIFS,
						"Success, but can't parse the response for the note_id " +
						noteID,
						e);
				}
		...
		new RestRequest.ErrorListener() {
			@Override
			public void onErrorResponse(VolleyError error) {
			        AppLog.e(AppLog.T.NOTIFS, "Error retrieving note with ID " + noteID,
			                 error);
