...
/**
 * Created so that the base suggestions functionality can become shareable as
 * similar functionality is being used in the the Account settings & sign-up
 * flow to change the username.
 */
public abstract class BaseUsernameChangerFullScreenDialogFragment
    extends DaggerFragment
    implements FullScreenDialogContent, OnUsernameSelectedListener {
  ...
  private boolean mShouldWatchText; // Flag handling text watcher to avoid
                                    // network call on device rotation.
  private int mUsernameSelectedIndex;

  public static final String EXTRA_DISPLAY_NAME = "EXTRA_DISPLAY_NAME";
  public static final String EXTRA_USERNAME = "EXTRA_USERNAME";
  public static final String KEY_IS_SHOWING_DISMISS_DIALOG =
      "KEY_IS_SHOWING_DISMISS_DIALOG";
  ...
  public static final String KEY_USERNAME_SELECTED_INDEX =
      "KEY_USERNAME_SELECTED_INDEX";
  public static final String KEY_USERNAME_SUGGESTIONS =
      "KEY_USERNAME_SUGGESTIONS";
  ...
  /**
   * Fragments that extend this class are required to provide the event that
   * should be tracked in case fetching of the username suggestions fail.
   *
   * @return {@link Stat}
   */
  ...
  /**
   * Specifies if the header text should be updated when a new username is
   * selected or if the the initial username should remain.
   *
  ...
  public View onCreateView(LayoutInflater inflater,
    ...
    ViewGroup rootView = (ViewGroup)inflater.inflate(
        R.layout.username_changer_dialog_fragment, container, false);
    ...
    mUsernameSuggestions.setLayoutManager(
        new LinearLayoutManager(getActivity()));
    // Stop list from blinking when data set is updated.
    ((SimpleItemAnimator)mUsernameSuggestions.getItemAnimator())
  ...
  public void onActivityCreated(@Nullable final Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      mIsShowingDismissDialog =
          savedInstanceState.getBoolean(KEY_IS_SHOWING_DISMISS_DIALOG);
      ...
      mUsernameSelectedIndex =
          savedInstanceState.getInt(KEY_USERNAME_SELECTED_INDEX);
      setUsernameSuggestions(
          savedInstanceState.getStringArrayList(KEY_USERNAME_SUGGESTIONS));
      ...
      public void afterTextChanged(Editable s) {
        if (s.toString().trim().isEmpty()) {
          if (canHeaderTextLiveUpdate()) {
            mHeaderView.setText(
                getHeaderText(getUsernameOrSelected(), mDisplayName));
          }
          mGetSuggestionsHandler.removeCallbacks(mGetSuggestionsRunnable);
        } else if (mShouldWatchText) {
          mGetSuggestionsHandler.removeCallbacks(mGetSuggestionsRunnable);
          mGetSuggestionsHandler.postDelayed(mGetSuggestionsRunnable,
                                             GET_SUGGESTIONS_INTERVAL_MS);
        } else {
          mShouldWatchText = true;
        }
      }
  ...
  onUsernameConfirmed(FullScreenDialogController controller,
  ...
  public void onSaveInstanceState(Bundle outState) {
    ...
    outState.putStringArrayList(KEY_USERNAME_SUGGESTIONS,
                                new ArrayList<>(mUsernamesAdapter.mItems));
  }
  ...
  public String getUsernameSelected() { return mUsernameSelected; }

  private void getUsernameSuggestions(String usernameQuery) {
    showProgress(true);

    FetchUsernameSuggestionsPayload payload =
        new FetchUsernameSuggestionsPayload(usernameQuery);
    mDispatcher.dispatch(
        AccountActionBuilder.newFetchUsernameSuggestionsAction(payload));
  }
  ...
  private void setUsernameSuggestions(List<String> suggestions) {
    mUsernamesAdapter =
        new UsernameChangerRecyclerViewAdapter(getActivity(), suggestions);
    mUsernamesAdapter.setOnUsernameSelectedListener(
        BaseUsernameChangerFullScreenDialogFragment.this);
  ...
  private void showDismissDialog() {
    ...
    new AlertDialog
        .Builder(new ContextThemeWrapper(getContext(), R.style.LoginTheme))
  ...
  public void onUsernameSuggestionsFetched(OnUsernameSuggestionsFetched event) {
    ...
    if (event.isError()) {
      ...
      AppLog.e(T.API, "onUsernameSuggestionsFetched: " + event.error.type +
                          " - " + event.error.message);
      showErrorDialog(new SpannedString(
          getString(R.string.username_changer_error_generic)));
    } else if (event.suggestions.size() == 0) {
      showErrorDialog(Html.fromHtml(
          String.format(getString(R.string.username_changer_error_none), "<b>",
                        mUsernameSuggestionInput, "</b>")));
    } else {
