...
public class MediaGridAdapter
    extends RecyclerView.Adapter<MediaGridAdapter.GridViewHolder> {
  ...
  public MediaGridAdapter(@NonNull Context context, @NonNull SiteModel site,
  ...
  /*
   * returns the most optimal url to use when retrieving a media image for
   * display here
   */
  private String getBestImageUrl(@NonNull MediaModel media) {
    // return photon-ized url if the site allows it since this gives us the
    // image at the exact size we need here
    if (SiteUtils.isPhotonCapable(mSite)) {
      return PhotonUtils.getPhotonImageUrl(media.getUrl(), mThumbWidth,
                                           mThumbHeight);
    }

    // can't use photon, so try the various image sizes - note we favor
    // medium-large and medium because they're more bandwidth-friendly than
    // large
    ...
    // next stop is to return the thumbnail, which will look pixelated in the
    // grid but it's better than eating bandwidth showing the full-sized image
  ...
  public void onBindViewHolder(GridViewHolder holder, int position) {
    ...
    boolean isLocalFile = MediaUtils.isLocalFile(strState) &&
                          !TextUtils.isEmpty(media.getFilePath());
    boolean isSelected = isItemSelected(media.getId());
    boolean canSelect = canSelectPosition(position);
    boolean isImage =
        media.getMimeType() != null && media.getMimeType().startsWith("image/");

    if (!mLoadThumbnails) {
      holder.mFileContainer.setVisibility(View.GONE);
      mImageManager.load(holder.mImageView, ImageType.PHOTO, "",
                         ScaleType.CENTER_CROP);
    } else if (isImage) {
      holder.mFileContainer.setVisibility(View.GONE);
      if (isLocalFile) {
        mImageManager.load(holder.mImageView, ImageType.PHOTO,
                           media.getFilePath(), ScaleType.CENTER_CROP);
      } else {
        mImageManager.load(holder.mImageView, ImageType.PHOTO,
                           getBestImageUrl(media), ScaleType.CENTER_CROP);
      }
      ...
      String fileExtension =
          MediaUtils.getExtensionForMimeType(media.getMimeType());
      ...
      ColorUtils.INSTANCE.setImageResourceWithTint(
          holder.mFileTypeImageView, placeholderResId, R.color.neutral_30);
      mImageManager.cancelRequestAndClearImageView(holder.mImageView);
    }
    holder.mImageView.setContentDescription(
        mContext.getString(R.string.media_grid_item_image_desc,
    ...
    if (mBrowserType.canMultiselect() && canSelect) {
      ...
      if (isSelected) {
        ...
        holder.mSelectionCountTextView.setText(
    ...
    if (state != MediaUploadState.UPLOADED) {
      ...
      boolean showProgress = state == MediaUploadState.UPLOADING ||
                             state == MediaUploadState.DELETING;
      holder.mProgressUpload.setVisibility(showProgress ? View.VISIBLE
                                                        : View.GONE);
      ...
      if (state == MediaUploadState.FAILED ||
        ...
        holder.mImgRetry.setVisibility(
            state == MediaUploadState.FAILED ? View.VISIBLE : View.GONE);
      } else {
      ...
      holder.mVideoOverlayContainer.setVisibility(media.isVideo() ? View.VISIBLE
                                                                  : View.GONE);
    }
  ...
  public ArrayList<Integer> getSelectedItems() { return mSelectedItems; }

  public int getSelectedItemCount() { return mSelectedItems.size(); }
  ...
  class GridViewHolder extends RecyclerView.ViewHolder {
    ...
    GridViewHolder(View view) {
      ...
      mStateContainer =
          view.findViewById(R.id.media_grid_item_upload_state_container);
      mStateTextView =
          mStateContainer.findViewById(R.id.media_grid_item_upload_state);
      mProgressUpload =
          mStateContainer.findViewById(R.id.media_grid_item_upload_progress);
      ...
      mFileTypeView =
          mFileContainer.findViewById(R.id.media_grid_item_filetype);
      mFileTypeImageView =
          mFileContainer.findViewById(R.id.media_grid_item_filetype_image);
      ...
      mProgressUpload.getIndeterminateDrawable().setColorFilter(
    ...
    private void doAdapterItemClicked(int position, boolean isLongClick) {
      ...
      if (isInMultiSelect() && !isLongClick) {
        ...
        if (mBrowserType.canMultiselect() && canSelectPosition(position) &&
            !isLongClick) {
          setInMultiSelect(true);
          toggleItemSelected(GridViewHolder.this, position);
        }
  ...
  public boolean isInMultiSelect() { return mInMultiSelect; }
  ...
  /*
   * determines whether the media item at the passed position can be selected -
   * not allowed for local files or deleted items when used as a picker
   */
  private boolean canSelectPosition(int position) {
    ...
    if (mBrowserType.isPicker()) {
      ...
      MediaUploadState state =
          MediaUploadState.fromString(media.getUploadState());
      return state != MediaUploadState.DELETING &&
          state != MediaUploadState.DELETED;
    } else {
      return true;
    }
  }
  ...
  /*
   * loads the thumbnail for the passed video media item - works with both local
   * and network videos
   */
  private void loadVideoThumbnail(final int position,
                                  final @NonNull MediaModel media,
                                  @NonNull final ImageView imageView) {
    // if we have a thumbnail url, use it and be done
    if (!TextUtils.isEmpty(media.getThumbnailUrl()) &&
        !MediaUtils.isVideo(media.getThumbnailUrl())) {
      mImageManager.load(imageView, ImageType.VIDEO, media.getThumbnailUrl(),
                         ScaleType.CENTER_CROP);
      return;
    }

    // thumbnail url is empty, so either this is a local (still uploading) video
    // or the server simply hasn't supplied the thumbnail url
    final String filePath;
    if (!TextUtils.isEmpty(media.getFilePath()) &&
        new File(media.getFilePath()).exists()) {
    ...
    new Thread() {
      ...
      public void run() {
        final Bitmap thumb =
            ImageUtils.getVideoFrameFromVideo(filePath, mThumbWidth);
        if (thumb != null) {
          mHandler.post(new Runnable() {
            @Override
            public void run() {
              WordPress.getBitmapCache().put(filePath, thumb);
              if (imageView.getTag(R.id.media_grid_file_path_id) instanceof
                      String &&
                  (imageView.getTag(R.id.media_grid_file_path_id))
                      .equals(filePath)) {
  ...
  public boolean isEmpty() { return mMediaList.isEmpty(); }
  ...
  public void setHasRetrievedAll(boolean b) { mHasRetrievedAll = b; }
  ...
  void setLoadThumbnails(boolean loadThumbnails) {
    if (loadThumbnails != mLoadThumbnails) {
      ...
      AppLog.d(AppLog.T.MEDIA,
               "MediaGridAdapter > loadThumbnails = " + loadThumbnails);
      if (mLoadThumbnails) {
        notifyDataSetChanged();
      }
    }
  ...
  private void setItemSelectedByPosition(GridViewHolder holder, int position,
                                         boolean selected) {
    if (!isValidPosition(position)) {
      return;
    }
    ...
    if (selected) {
      holder.mSelectionCountTextView.setText(String.format(
          Locale.getDefault(), "%d", mSelectedItems.indexOf(localMediaId) + 1));
    } else {
      holder.mSelectionCountTextView.setText(null);
    }
    AniUtils.startAnimation(holder.mSelectionCountContainer, R.anim.pop);
    holder.mSelectionCountTextView.setVisibility(selected ? View.VISIBLE
                                                          : View.GONE);

    // scale the thumbnail
    if (selected) {
      AniUtils.scale(holder.mImageView, SCALE_NORMAL, SCALE_SELECTED,
                     AniUtils.Duration.SHORT);
    } else {
      AniUtils.scale(holder.mImageView, SCALE_SELECTED, SCALE_NORMAL,
                     AniUtils.Duration.SHORT);
    }
