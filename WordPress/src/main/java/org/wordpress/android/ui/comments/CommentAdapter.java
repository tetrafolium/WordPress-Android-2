...
public class CommentAdapter
  ...
  CommentAdapter(Context context, SiteModel site) {
    ...
    mStatusTextSpam =
        context.getResources().getString(R.string.comment_status_spam);
    mStatusTextUnapproved =
        context.getResources().getString(R.string.comment_status_unapproved);
  ...
  void
  setOnSelectedItemsChangeListener(OnSelectedItemsChangeListener listener) {
  ...
  private String getFormattedTitle(CommentModel comment) {
    ...
    if (!TextUtils.isEmpty(comment.getPostTitle())) {
      formattedTitle = context.getString(R.string.comment_title, author,
  ...
  private String getAvatarForDisplay(CommentModel comment, int avatarSize) {
    ...
    if (!TextUtils.isEmpty(comment.getAuthorProfileImageUrl())) {
      avatarForDisplay = GravatarUtils.fixGravatarUrl(
          comment.getAuthorProfileImageUrl(), avatarSize);
    } else if (!TextUtils.isEmpty(comment.getAuthorEmail())) {
      avatarForDisplay =
          GravatarUtils.gravatarFromEmail(comment.getAuthorEmail(), avatarSize);
    }
  ...
  private String getFormattedDate(CommentModel comment, Context context) {
    if (comment.getDatePublished() != null) {
      return DateTimeUtils.javaDateToTimeSpan(
          DateTimeUtils.dateFromIso8601(comment.getDatePublished()), context);
    }
    return "";
  }
  ...
  public void onBindViewHolder(RecyclerView.ViewHolder viewHolder,
    ...
    // Note: following operation can take some time, we could maybe cache the
    // calculated objects (title, spanned content) to make the list scroll
    // smoother.
    ...
    // comment text needs to be to the left of date/status when the title is a
    // single line and the status is displayed or else the status may overlap
    // the comment text - note that getLineCount() will return 0 if the view
    // hasn't been rendered yet, which is why we check getLineCount() <= 1
    boolean adjustComment =
        (showStatus && holder.mTxtTitle.getLineCount() <= 1);
    RelativeLayout.LayoutParams params =
        (RelativeLayout.LayoutParams)holder.mTxtComment.getLayoutParams();
    if (adjustComment) {
  ...
  int getSelectedCommentCount() { return mSelectedPositions.size(); }
  ...
  void setItemSelected(int position, boolean isSelected, View view) {
    ...
    if (view != null && view.getTag() instanceof CommentHolder) {
      ...
      AniUtils.startAnimation(holder.mImgCheckmark, isSelected
                                                        ? R.anim.cab_select
                                                        : R.anim.cab_deselect);
  ...
  void loadComments(CommentStatus statusFilter) {
    if (mIsLoadTaskRunning) {
      ...
      new LoadCommentsTask(statusFilter)
  ...
  private class LoadCommentsTask extends AsyncTask<Void, Void, Boolean> {
    ...
    protected Boolean doInBackground(Void... params) {
      ...
      if (mStatusFilter == null || mStatusFilter == CommentStatus.ALL) {
        // The "all" filter actually means "approved" + "unapproved" (but not
        // "spam", "trash" or "deleted")
        comments = mCommentStore.getCommentsForSite(
            mSite, false, CommentStatus.APPROVED, CommentStatus.UNAPPROVED);
      } else {
        comments =
            mCommentStore.getCommentsForSite(mSite, false, mStatusFilter);
      }
      ...
      if (result) {
        ...
        Collections.sort(mComments, new Comparator<CommentModel>() {
          ...
          public int compare(CommentModel commentModel, CommentModel t1) {
            Date d0 =
                DateTimeUtils.dateFromIso8601(commentModel.getDatePublished());
