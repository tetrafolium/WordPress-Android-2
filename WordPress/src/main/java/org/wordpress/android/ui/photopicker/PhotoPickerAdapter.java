...
public class PhotoPickerAdapter
    extends RecyclerView.Adapter<PhotoPickerAdapter.ThumbnailViewHolder> {
  ...
  PhotoPickerAdapter(Context context, MediaBrowserType browserType,
  ...
  void refresh(boolean forceReload) {
    if (mIsListTaskRunning) {
      AppLog.w(AppLog.T.MEDIA,
               "photo picker > build list task already running");
      return;
    }
    ...
    boolean sizeChanged =
        thumbWidth != mThumbWidth || thumbHeight != mThumbHeight;

    // if thumb sizes have changed (due to device rotation, or never being set),
    // we must reload from scratch - otherwise we can do a refresh so the
    // adapter is only loaded if there are changes
    boolean mustReload;
    if (sizeChanged) {
    ...
    new BuildDeviceMediaListTask(mustReload)
  ...
  private boolean isEmpty() { return mMediaList.size() == 0; }
  ...
  @Override
  public ThumbnailViewHolder onCreateViewHolder(ViewGroup parent,
                                                int viewType) {
    View view =
        mInflater.inflate(R.layout.photo_picker_thumbnail, parent, false);
    return new ThumbnailViewHolder(view);
  }

  private void
  updateSelectionCountForPosition(int position, boolean isSelected,
                                  @NonNull TextView txtSelectionCount) {
    if (canMultiselect() && isSelected) {
      int count = mSelectedPositions.indexOf(position) + 1;
      txtSelectionCount.setText(
  ...
  public void onBindViewHolder(ThumbnailViewHolder holder, int position) {
    ...
    holder.mTxtSelectionCount.setVisibility(
        isSelected || canMultiselect() ? View.VISIBLE : View.GONE);
    updateSelectionCountForPosition(position, isSelected,
                                    holder.mTxtSelectionCount);
    ...
    holder.mVideoOverlay.setVisibility(item.mIsVideo ? View.VISIBLE
                                                     : View.GONE);

    if (mLoadThumbnails) {
      mImageManager.load(holder.mImgThumbnail, ImageType.PHOTO,
                         item.mUri.toString(), ScaleType.FIT_CENTER);
    } else {
  ...
  private PhotoPickerItem getItemAtPosition(int position) {
    if (!isValidPosition(position)) {
      AppLog.w(AppLog.T.POSTS,
               "photo picker > invalid position in getItemAtPosition");
      return null;
    }
  ...
  private void setItemSelected(int position, boolean isSelected,
    ...
    // if an item is already selected and multiselect isn't allowed, deselect
    ...
    if (holder != null) {
      ...
      if (isSelected) {
        AniUtils.scale(holder.mImgThumbnail, SCALE_NORMAL, SCALE_SELECTED,
                       ANI_DURATION);
      } else {
        AniUtils.scale(holder.mImgThumbnail, SCALE_SELECTED, SCALE_NORMAL,
                       ANI_DURATION);
      }
  ...
  private ThumbnailViewHolder getViewHolderAtPosition(int position) {
    ...
    return (ThumbnailViewHolder)mRecycler.findViewHolderForAdapterPosition(
        position);
  }
  ...
  ArrayList<Integer> getSelectedPositions() { return mSelectedPositions; }
  ...
  private boolean canMultiselect() { return mBrowserType.canMultiselect(); }

  int getNumSelected() { return mSelectedPositions.size(); }
  ...
  class ThumbnailViewHolder extends RecyclerView.ViewHolder {
    ...
    ThumbnailViewHolder(View view) {
      ...
      if (!canMultiselect()) {
        mTxtSelectionCount.setBackgroundResource(
            R.drawable.photo_picker_circle_pressed);
      }
  ...
  private void showPreview(int position) {
    ...
    if (item != null) {
      ...
      MediaPreviewActivity.showPreview(mContext, null, item.mUri.toString());
    }
  }
    ...
    new Thread(new Runnable() {
      public void run() {
        Map<String, Object> properties = AnalyticsUtils.getMediaProperties(
            mContext, item.mIsVideo, item.mUri, null);
        properties.put("is_video", item.mIsVideo);
        AnalyticsTracker.track(
            AnalyticsTracker.Stat.MEDIA_PICKER_PREVIEW_OPENED, properties);
      }
    }).start();
  }
   ...
   */
  private class BuildDeviceMediaListTask
    ...
    protected Boolean doInBackground(Void... params) {
      ...
      // if we're reloading then return true so the adapter is updated,
      // otherwise only return true if changes are detected
    ...
    private void addMedia(Uri baseUri, boolean isVideo) {
      ...
      Cursor cursor = null;
      try {
        cursor = mContext.getContentResolver().query(baseUri, projection, null,
                                                     null, null);
