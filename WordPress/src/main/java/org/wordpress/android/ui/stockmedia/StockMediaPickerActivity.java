...
public class StockMediaPickerActivity
  ...
  private static final String KEY_IS_SHOWING_EMPTY_VIEW =
      "is_showing_empty_view";
  ...
  @SuppressWarnings("unused") @Inject StockMediaStore mStockMediaStore;
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    mRetainedFragment =
        (StockMediaRetainedFragment)fm.findFragmentByTag(TAG_RETAINED_FRAGMENT);
    ...
    if (savedInstanceState == null) {
      ...
      mIsShowingEmptyView =
          savedInstanceState.getBoolean(KEY_IS_SHOWING_EMPTY_VIEW);
  ...
  protected void onSaveInstanceState(Bundle outState) {
    ...
    String query =
        mSearchView != null ? mSearchView.getQuery().toString() : null;
    outState.putString(KEY_SEARCH_QUERY, query);

    StockMediaRetainedData data = new StockMediaRetainedData(
        mAdapter.mItems, mAdapter.mSelectedItems, mCanLoadMore, mNextPage);
  ...
  private void showEmptyView(boolean show) {
    if (!isFinishing()) {
      ...
      ActionableEmptyView actionableEmptyView =
          findViewById(R.id.actionable_empty_view);
      actionableEmptyView.setVisibility(show ? View.VISIBLE : View.GONE);
      actionableEmptyView.updateLayoutForSearch(true, 0);

      if (show) {
        boolean isEmpty = mSearchQuery == null ||
                          mSearchQuery.length() < MIN_SEARCH_QUERY_SIZE;

        if (isEmpty) {
          actionableEmptyView.title.setText(
              R.string.stock_media_picker_initial_empty_text);
          String link = "<a href='https://pexels.com/'>Pexels</a>";
          Spanned html = Html.fromHtml(getString(
              R.string.stock_media_picker_initial_empty_subtext, link));
          actionableEmptyView.subtitle.setText(html);
          actionableEmptyView.getSubtitle().setMovementMethod(
              WPLinkMovementMethod.getInstance());
  ...
  private void fetchStockMedia(@Nullable String searchQuery, int page) {
    // We should always fetch the first page, but We should only load more pages
    // if we are not already fetching anything
    ...
    FetchStockMediaListPayload payload =
        new FetchStockMediaListPayload(searchQuery, page);
    mDispatcher.dispatch(
        StockMediaActionBuilder.newFetchStockMediaAction(payload));
  }
  ...
  public void onStockMediaListFetched(OnStockMediaListFetched event) {
    ...
    // set the results to the event's mediaList if this is the first page,
    // otherwise add to the existing results
    if (event.nextPage == 2) {
  ...
  public void onStockMediaUploaded(OnStockMediaUploaded event) {
    ...
    if (event.isError()) {
      ...
      if (enableMultiselect()) {
        ...
        intent.putExtra(PhotoPickerActivity.EXTRA_MEDIA_ID,
                        event.mediaList.get(0).getMediaId());
      }
  ...
  private void
  trackUploadedStockMediaEvent(@NonNull List<MediaModel> mediaList) {
    if (mediaList.size() == 0) {
      AppLog.e(AppLog.T.MEDIA,
               "Cannot track uploaded stock media event if mediaList is empty");
      return;
    }
    ...
    AnalyticsTracker.track(AnalyticsTracker.Stat.STOCK_MEDIA_UPLOADED,
                           properties);
  }
  ...
  private void showSelectionBar(final boolean show) {
    ...
    // when the animation completes, adjust the relative layout params of the
    // recycler to make sure the bar doesn't overlap the bottom row when showing
      ...
      public void run() {
        if (!isFinishing()) {
          RelativeLayout.LayoutParams params =
              (RelativeLayout.LayoutParams)mRecycler.getLayoutParams();
          if (show) {
  ...
  private void notifySelectionCountChanged() {
    ...
    if (numSelected > 0) {
      if (enableMultiselect()) {
        String labelAdd =
            String.format(getString(R.string.add_count), numSelected);
        mTextAdd.setText(labelAdd);

        String labelPreview =
            String.format(getString(R.string.preview_count), numSelected);
  ...
  private void previewSelection() {
    ...
    MediaPreviewActivity.showPreview(this, null, imageUrlList,
                                     imageUrlList.get(0));
  }
  ...
  class StockMediaAdapter extends RecyclerView.Adapter<StockViewHolder> {
    ...
    StockMediaAdapter() { setHasStableIds(true); }
    ...
    boolean isEmpty() { return getItemCount() == 0; }

    @NonNull
    @Override
    public StockViewHolder onCreateViewHolder(@NonNull ViewGroup parent,
                                              int viewType) {
      View view = getLayoutInflater().inflate(R.layout.media_picker_thumbnail,
                                              parent, false);
    ...
    public void onBindViewHolder(@NonNull StockViewHolder holder,
      ...
      String imageUrl = PhotonUtils.getPhotonImageUrl(
          media.getThumbnail(), mThumbWidth, mThumbHeight);
      mImageManager.load(holder.mImageView, ImageType.PHOTO, imageUrl,
    ...
    void setItemSelected(StockViewHolder holder, int position,
                         boolean selected) {
      ...
      if (selected && !enableMultiselect() && !mSelectedItems.isEmpty()) {
        ...
        StockViewHolder prevHolder =
            (StockViewHolder)mRecycler.findViewHolderForAdapterPosition(
                prevPosition);
        if (prevHolder != null) {
      ...
      if (enableMultiselect()) {
        ...
        if (selected) {
          AniUtils.scaleIn(holder.mSelectionCountTextView,
                           AniUtils.Duration.MEDIUM);
        } else {
          AniUtils.scaleOut(holder.mSelectionCountTextView,
                            AniUtils.Duration.MEDIUM);
        }
      }
    ...
    int getSelectionCount() { return mSelectedItems.size(); }
  }
        ...
        public boolean onLongClick(View v) {
          ...
          if (mAdapter.isValidPosition(position)) {
            MediaPreviewActivity.showPreview(
                v.getContext(), mSite, mAdapter.mItems.get(position).getUrl());
          }
          return true;
        }
      });
