...
public class CommentsActivity extends AppCompatActivity
  ...
  private static final String SAVED_COMMENTS_STATUS_TYPE =
      "saved_comments_status_type";
  public static final String COMMENT_MODERATE_ID_EXTRA = "commentModerateId";
  public static final String COMMENT_MODERATE_STATUS_EXTRA =
      "commentModerateStatus";
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if (getIntent() != null &&
        getIntent().hasExtra(SAVED_COMMENTS_STATUS_TYPE)) {
      mCurrentCommentStatusType =
          (CommentStatus)getIntent().getSerializableExtra(
              SAVED_COMMENTS_STATUS_TYPE);
    } else {
    ...
    if (savedInstanceState == null) {
      ...
      getIntent().putExtra(KEY_AUTO_REFRESHED,
                           savedInstanceState.getBoolean(KEY_AUTO_REFRESHED));
      getIntent().putExtra(KEY_EMPTY_VIEW_MESSAGE, savedInstanceState.getString(
  ...
  private boolean hasListFragment() { return (getListFragment() != null); }
  ...
  private void showReaderFragment(long remoteBlogId, long postId) {
    ...
    Fragment fragment =
        ReaderPostDetailFragment.newInstance(remoteBlogId, postId);
  ...
  public void onCommentSelected(long commentId, CommentStatus statusFilter) {
    ...
    detailIntent.putExtra(CommentsDetailActivity.COMMENT_STATUS_FILTER_EXTRA,
                          statusFilter);
  ...
  /*
   * called from comment detail when user taps a link to a post - show the post
   * in a reader detail fragment
   */
  @Override
  ...
  public void onSaveInstanceState(@NonNull Bundle outState) {
    ...
    if (hasListFragment()) {
      outState.putBoolean(KEY_AUTO_REFRESHED,
                          getListFragment().mHasAutoRefreshedComments);
      outState.putString(KEY_EMPTY_VIEW_MESSAGE,
                         getListFragment().getEmptyViewMessage());
    }
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    if (resultCode == RESULT_OK) {
      ...
      if (commentId >= 0 && !TextUtils.isEmpty(newStatus)) {
        onModerateComment(
            mCommentStore.getCommentBySiteAndRemoteId(mSite, commentId),
  ...
  public void onModerateComment(final CommentModel comment,
    ...
    if (newStatus == CommentStatus.APPROVED ||
      ...
      String message =
          (newStatus == CommentStatus.TRASH
               ? getString(R.string.comment_trashed)
               : newStatus == CommentStatus.SPAM
                     ? getString(R.string.comment_spammed)
      ...
      WPSnackbar snackbar =
          WPSnackbar
              .make(getListFragment().getView(), message, Snackbar.LENGTH_LONG)
      ...
      snackbar.setCallback(new Snackbar.Callback() {
        ...
        public void onDismissed(Snackbar snackbar, int event) {
          ...
          // comment will no longer exist in moderating list if action was
          // undone
  ...
  public void onPositiveClicked(@NotNull String instanceTag) {
    Fragment fragmentById = getSupportFragmentManager().findFragmentById(
        R.id.layout_fragment_container);
    if (fragmentById instanceof
        BasicFragmentDialog.BasicDialogPositiveClickInterface) {
      ((BasicDialogPositiveClickInterface)fragmentById)
