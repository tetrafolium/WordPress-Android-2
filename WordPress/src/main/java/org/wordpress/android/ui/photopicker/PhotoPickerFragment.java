...
public class PhotoPickerFragment extends Fragment {
  ...
  public static PhotoPickerFragment
  newInstance(@NonNull PhotoPickerListener listener,
              @NonNull MediaBrowserType browserType, @Nullable SiteModel site) {
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    mBrowserType =
        (MediaBrowserType)getArguments().getSerializable(ARG_BROWSER_TYPE);
    mSite = (SiteModel)getArguments().getSerializable(WordPress.SITE);

    if (savedInstanceState != null) {
      String savedLastTappedIconName =
          savedInstanceState.getString(KEY_LAST_TAPPED_ICON);
      mLastTappedIcon = savedLastTappedIconName == null
                            ? null
                            : PhotoPickerIcon.valueOf(savedLastTappedIconName);
      if (savedInstanceState.containsKey(KEY_SELECTED_POSITIONS)) {
        mSelectedPositions =
            savedInstanceState.getIntegerArrayList(KEY_SELECTED_POSITIONS);
      }
    }
  }
  ...
  public View onCreateView(@NonNull LayoutInflater inflater,
    ...
    View view =
        inflater.inflate(R.layout.photo_picker_fragment, container, false);
    ...
    final int minDistance =
        WPMediaUtils.getFlingDistanceToDisableThumbLoading(getActivity());
    ...
    if (!canShowBottomBar()) {
      ...
      mBottomBar.findViewById(R.id.icon_camera)
      ...
      mBottomBar.findViewById(R.id.icon_picker)
          .setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
  ...
  private boolean canShowBottomBar() {
    if (mBrowserType == MediaBrowserType.AZTEC_EDITOR_PICKER &&
        DisplayUtils.isLandscape(getActivity())) {
      return true;
  ...
  public void onSaveInstanceState(@NonNull Bundle outState) {
    ...
    outState.putString(KEY_LAST_TAPPED_ICON,
                       mLastTappedIcon == null ? null : mLastTappedIcon.name());
  ...
  public void doIconClicked(@NonNull PhotoPickerIcon icon) {
    ...
    case ANDROID_CAPTURE_PHOTO:
      trackSelectedOtherSourceEvents(
          AnalyticsTracker.Stat.MEDIA_PICKER_OPEN_CAPTURE_MEDIA, false);
      break;
    case ANDROID_CAPTURE_VIDEO:
      trackSelectedOtherSourceEvents(
          AnalyticsTracker.Stat.MEDIA_PICKER_OPEN_CAPTURE_MEDIA, true);
      break;
    case ANDROID_CHOOSE_PHOTO:
      trackSelectedOtherSourceEvents(
          AnalyticsTracker.Stat.MEDIA_PICKER_OPEN_DEVICE_LIBRARY, false);
      break;
    case ANDROID_CHOOSE_VIDEO:
      trackSelectedOtherSourceEvents(
          AnalyticsTracker.Stat.MEDIA_PICKER_OPEN_DEVICE_LIBRARY, true);
      break;
    case WP_MEDIA:
  ...
  public void showPickerPopupMenu(@NonNull View view) {
    ...
    MenuItem itemPhoto =
        popup.getMenu().add(R.string.photo_picker_choose_photo);
    ...
    if (!mBrowserType.isSingleImagePicker()) {
      MenuItem itemVideo =
          popup.getMenu().add(R.string.photo_picker_choose_video);
    ...
    if (mSite != null) {
      MenuItem itemStock =
          popup.getMenu().add(R.string.photo_picker_stock_media);
      ...
      itemGiphy.setOnMenuItemClickListener(
          new MenuItem.OnMenuItemClickListener() {
            @Override
            public boolean onMenuItemClick(MenuItem item) {
  ...
  public void showCameraPopupMenu(@NonNull View view) {
    ...
    MenuItem itemPhoto =
        popup.getMenu().add(R.string.photo_picker_capture_photo);
    itemPhoto.setOnMenuItemClickListener(
        new MenuItem.OnMenuItemClickListener() {
          @Override
          public boolean onMenuItemClick(MenuItem item) {
    ...
    MenuItem itemVideo =
        popup.getMenu().add(R.string.photo_picker_capture_video);
    itemVideo.setOnMenuItemClickListener(
        new MenuItem.OnMenuItemClickListener() {
          @Override
          public boolean onMenuItemClick(MenuItem item) {
  ...
  private final PhotoPickerAdapterListener mAdapterListener =
      new PhotoPickerAdapterListener() {
        @Override
  ...
  private PhotoPickerAdapter getAdapter() {
    if (mAdapter == null) {
      mAdapter =
          new PhotoPickerAdapter(getActivity(), mBrowserType, mAdapterListener);
    }
    return mAdapter;
  }
  ...
  /*
   * load the photos if we have the necessary permission, otherwise show the
   * "soft ask" view which asks the user to allow the permission
   */
  ...
  private void requestStoragePermission() {
    ...
    requestPermissions(
        permissions,
        WPPermissionUtils.PHOTO_PICKER_STORAGE_PERMISSION_REQUEST_CODE);
  }

  private void requestCameraPermission() {
    // in addition to CAMERA permission we also need a storage permission, to
    // store media from the camera
    String[] permissions =
        new String[] {permission.CAMERA, permission.WRITE_EXTERNAL_STORAGE};
    requestPermissions(
        permissions,
        WPPermissionUtils.PHOTO_PICKER_CAMERA_PERMISSION_REQUEST_CODE);
  }
  ...
  /*
   * shows the "soft ask" view which should appear when storage permission
   * hasn't been granted
   */
    ...
    if (show) {
      ...
      if (isAlwaysDenied) {
        String permissionName =
            ...
            "</strong>";
        label = String.format(
            getString(R.string.photo_picker_soft_ask_permissions_denied),
            appName, permissionName);
      } else {
        label = String.format(getString(R.string.photo_picker_soft_ask_label),
                              appName);
      }

      mSoftAskView.title.setText(Html.fromHtml(label));

      // when the user taps Allow, request the required permissions unless the
      // user already denied them permanently, in which case take them to the
      // device settings for this app so the user can change permissions there
      int allowId = isAlwaysDenied ? R.string.button_edit_permissions
  ...
  private void trackAddRecentMediaEvent(List<Uri> uriList) {
    ...
    for (Uri mediaUri : uriList) {
      if (mediaUri != null) {
        ...
        Map<String, Object> properties = AnalyticsUtils.getMediaProperties(
            getActivity(), isVideo, mediaUri, null);
        properties.put("is_part_of_multiselection", isMultiselection);
        if (isMultiselection) {
          properties.put("number_of_media_selected", uriList.size());
        }
        AnalyticsTracker.track(
            AnalyticsTracker.Stat.MEDIA_PICKER_RECENT_MEDIA_SELECTED,
            properties);
      }
    }
  }

  private void trackSelectedOtherSourceEvents(AnalyticsTracker.Stat stat,
                                              boolean isVideo) {
