...
public class NotificationsListFragmentPage extends Fragment
	implements OnScrollToTopListener, NotesAdapter.DataLoadedListener {
...
public static Fragment newInstance(int position) {
	NotificationsListFragmentPage fragment =
		new NotificationsListFragmentPage();
...
public interface OnNoteClickListener { void onClickNote(String noteId); }
...
public void onActivityCreated(Bundle savedInstanceState) {
	...
	if (savedInstanceState != null) {
		mTabPosition =
			savedInstanceState.getInt(KEY_TAB_POSITION, TAB_POSITION_ALL);
	}
...
public View onCreateView(@NonNull LayoutInflater inflater,
	...
	View view = inflater.inflate(R.layout.notifications_list_fragment_page,
	                             container, false);
...
private final OnNoteClickListener mOnNoteClickListener =
	...
	public void onClickNote(String noteId) {
		...
		// Open the latest version of this note in case it has changed, which
		// can happen if the note was tapped from the list after it was
		// updated by another fragment (such as
...
private static Intent getOpenNoteIntent(Activity activity, String noteId) {
	Intent detailIntent =
		new Intent(activity, NotificationsDetailActivity.class);
...
private void hideNewNotificationsBar() {
	if (!isAdded() || !isNewNotificationsBarShowing() ||
	    mIsAnimatingOutNewNotificationsBar) {
		return;
	}
	...
	AniUtils.startAnimation(mNewNotificationsBar,
	                        R.anim.notifications_bottom_bar_out, listener);
}

private boolean isNewNotificationsBarShowing() {
	return (mNewNotificationsBar != null &&
	        mNewNotificationsBar.getVisibility() == View.VISIBLE);
}

public static void openNoteForReply(Activity activity, String noteId,
	...
	detailIntent.putExtra(IS_TAPPED_ON_NOTIFICATION,
	                      isTappedFromPushNotification);
	openNoteForReplyWithParams(detailIntent, activity);
}

private static void openNoteForReplyWithParams(Intent detailIntent,
                                               Activity activity) {
...
private void performActionForActiveFilter() {
	...
	if (mTabPosition == TAB_POSITION_UNREAD) {
		ActivityLauncher.addNewPostForResult(getActivity(), getSelectedSite(),
		                                     false);
...
private void showEmptyViewForCurrentFilter() {
	...
	switch (mTabPosition) {
	case TAB_POSITION_ALL:
		showEmptyView(R.string.notifications_empty_all,
		              R.string.notifications_empty_action_all,
		              R.string.notifications_empty_view_reader);
		break;
	case TAB_POSITION_COMMENT:
		showEmptyView(R.string.notifications_empty_comments,
		              R.string.notifications_empty_action_comments,
		              R.string.notifications_empty_view_reader);
		break;
	case TAB_POSITION_FOLLOW:
		showEmptyView(R.string.notifications_empty_followers,
		              R.string.notifications_empty_action_followers_likes,
		              R.string.notifications_empty_view_reader);
		break;
	case TAB_POSITION_LIKE:
		showEmptyView(R.string.notifications_empty_likes,
		              R.string.notifications_empty_action_followers_likes,
		              R.string.notifications_empty_view_reader);
		break;
	case TAB_POSITION_UNREAD:
	...
	mActionableEmptyView.image.setVisibility(
		DisplayUtils.isLandscape(getContext()) ? View.GONE : View.VISIBLE);
}
...
private void showNewNotificationsBar() {
	...
	AniUtils.startAnimation(mNewNotificationsBar,
	                        R.anim.notifications_bottom_bar_in);
...
private void showNewUnseenNotificationsUI() {
	...
	if (first != null &&
	    mRecyclerView.getLayoutManager().getPosition(first) > 0) {
	...
	NotificationsActions.downloadNoteAndUpdateDB(
		...
		new RestRequest.Listener() {
			...
			public void onResponse(JSONObject response) {
					...
					NoteLikeOrModerationStatusChanged.class);
