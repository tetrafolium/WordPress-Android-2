...
/*
 * utility class which accepts a list of comments and then creates a "level
 * list" from it which places child comments below their parents with
 * indentation levels applied
 */
...
public class ReaderCommentLeveler {
  ...
  public ReaderCommentList createLevelList() {
    ...
    // check for orphans (child comments whose parents weren't found above) and
    // give them a non-zero level so they're indented by ReaderCommentAdapter
  ...
  /*
   * walk comments in the passed list that have the passed level and add their
   * children beneath them
   */
  private boolean walkCommentsAtLevel(@NonNull ReaderCommentList comments,
                                      int level) {
    boolean hasChanges = false;
    for (int index = 0; index < comments.size(); index++) {
      ReaderComment parent = comments.get(index);
      if (parent.level == level && hasChildren(parent.commentId)) {
        // get children for this comment, set their level, then add them below
        // the parent
