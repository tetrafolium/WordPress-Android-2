...
public class FooterNoteBlock extends NoteBlock {
  ...
  public FooterNoteBlock(
      FormattableContent noteObject, ImageManager imageManager,
  ...
  public View configureView(final View view) {
    final FooterNoteBlockHolder noteBlockHolder =
        (FooterNoteBlockHolder)view.getTag();

    // Note text
    if (!TextUtils.isEmpty(getNoteText())) {
      Spannable spannable = getNoteText();
      NoteBlockClickableSpan[] spans = spannable.getSpans(
          0, spannable.length(), NoteBlockClickableSpan.class);
  ...
  Spannable getNoteText() {
    return mNotificationsUtilsWrapper.getSpannableContentForRanges(
        getNoteData(), null, getOnNoteBlockTextClickListener(), true);
  }
  ...
  class FooterNoteBlockHolder {
    ...
    public TextView getNoticonView() { return mNoticonView; }
  }
