...
public class AddCategoryFragment extends AppCompatDialogFragment {
  ...
  private boolean addCategory() {
    ...
    CategoryNode selectedCategory =
        (CategoryNode)mParentSpinner.getSelectedItem();
    long parentId =
        (selectedCategory != null) ? selectedCategory.getCategoryId() : 0;
  ...
  private void loadCategories() {
    CategoryNode rootCategory = CategoryNode.createCategoryTreeFromList(
    ...
    categoryLevels.add(
        0, new CategoryNode(0, 0, getString(R.string.top_level_category_name)));
    if (categoryLevels.size() > 0) {
      ParentCategorySpinnerAdapter categoryAdapter =
          new ParentCategorySpinnerAdapter(
              getActivity(), R.layout.categories_row_parent, categoryLevels);
