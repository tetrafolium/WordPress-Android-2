...
public class MediaPreviewActivity extends AppCompatActivity
    implements MediaPreviewFragment.OnMediaTappedListener {
  ...
  // will contain a list of media IDs when PreviewType = MULTI_MEDIA_IDS or a
  // list of image URLs when PreviewType = MULTI_IMAGE_URLS
  ...
  static class MediaPreviewSwiped {
    ...
    MediaPreviewSwiped(int mediaId) { this.mediaId = mediaId; }
  }
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      ...
      mContentUri = savedInstanceState.getString(
          MediaPreviewFragment.ARG_MEDIA_CONTENT_URI);
      mPreviewType =
          (PreviewType)savedInstanceState.getSerializable(ARG_PREVIEW_TYPE);
      if (savedInstanceState.containsKey(ARG_ID_OR_URL_LIST)) {
        mMediaIdOrUrlList =
            savedInstanceState.getStringArrayList(ARG_ID_OR_URL_LIST);
      }
    } else {
      mSite = (SiteModel)getIntent().getSerializableExtra(WordPress.SITE);
      mMediaId = getIntent().getIntExtra(MediaPreviewFragment.ARG_MEDIA_ID, 0);
      mContentUri = getIntent().getStringExtra(
          MediaPreviewFragment.ARG_MEDIA_CONTENT_URI);
      mPreviewType =
          (PreviewType)getIntent().getSerializableExtra(ARG_PREVIEW_TYPE);
      if (getIntent().hasExtra(ARG_ID_OR_URL_LIST)) {
        mMediaIdOrUrlList =
            getIntent().getStringArrayListExtra(ARG_ID_OR_URL_LIST);
      }
    }
    ...
    int toolbarColor =
        ContextCompat.getColor(this, R.color.black_translucent_40);
    ...
    // use a ViewPager if we're passed a list of media, otherwise show a single
    // fragment
  ...
  /*
   * shows a single preview fragment within this activity - called when we can't
   * use a ViewPager to swipe between media (ie: we're previewing a local file)
   */
    ...
    public void run() {
      if (!isFinishing() && mToolbar.getVisibility() == View.VISIBLE) {
        AniUtils.startAnimation(
            mToolbar, R.anim.toolbar_fade_out_and_up,
  ...
  private void showToolbar() {
    if (!isFinishing()) {
      if (getPreviewType().isMulti()) {
        ...
        if (count > 1) {
          String title = String.format(getString(R.string.media_preview_title),
                                       position + 1, count);
      ...
      if (mToolbar.getVisibility() != View.VISIBLE) {
        AniUtils.startAnimation(
            mToolbar, R.anim.toolbar_fade_in_and_down,
            new Animation.AnimationListener() {
              @Override
              public void onAnimationStart(Animation animation) {
  ...
  private void setupViewPager() {
    ...
    mViewPager.setPageTransformer(
        false, new WPViewPagerTransformer(TransformType.SLIDE_OVER));

    // determine the position of the original media item so we can page to it
    // immediately
    int initialPos = 0;
    String compareTo = getPreviewType() == PreviewType.MULTI_MEDIA_IDS
        ...
        new ViewPager.SimpleOnPageChangeListener() {
          ...
          public void onPageSelected(int position) {
            ...
            case MULTI_MEDIA_IDS:
              // pause the outgoing fragment and unpause the incoming one - this
              // prevents audio/video from playing in inactive fragments
              ...
              // fire event so settings activity shows the same media as this
              // activity (user may have swiped)
  ...
  class MediaPagerAdapter extends FragmentStatePagerAdapter {
    ...
    MediaPagerAdapter(FragmentManager fm) { super(fm); }
    ...
    public Fragment getItem(int position) {
      ...
      case MULTI_MEDIA_IDS:
        ...
        // make sure we autoplay the initial item (relevant only for
        // audio/video)
        boolean autoPlay;
