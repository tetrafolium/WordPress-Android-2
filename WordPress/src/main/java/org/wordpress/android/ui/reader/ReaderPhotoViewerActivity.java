...
/**
 * Full-screen photo viewer - uses a ViewPager to enable scrolling between
 * images in a blog post, but also supports viewing a single image
 */
public class ReaderPhotoViewerActivity
    extends AppCompatActivity implements PhotoViewListener {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      mInitialImageUrl =
          savedInstanceState.getString(ReaderConstants.ARG_IMAGE_URL);
      mIsPrivate =
          savedInstanceState.getBoolean(ReaderConstants.ARG_IS_PRIVATE);
      mIsGallery =
          savedInstanceState.getBoolean(ReaderConstants.ARG_IS_GALLERY);
      mContent = savedInstanceState.getString(ReaderConstants.ARG_CONTENT);
    } else if (getIntent() != null) {
      mInitialImageUrl =
          getIntent().getStringExtra(ReaderConstants.ARG_IMAGE_URL);
      mIsPrivate =
          getIntent().getBooleanExtra(ReaderConstants.ARG_IS_PRIVATE, false);
      mIsGallery =
          getIntent().getBooleanExtra(ReaderConstants.ARG_IS_GALLERY, false);
    ...
    mViewPager.setPageTransformer(
        false, new WPViewPagerTransformer(TransformType.FLOW));
  ...
  private void loadImageList() {
    // content will be empty when viewing a single image, otherwise content is
    // HTML so parse images from it
    final ReaderImageList imageList;
    if (TextUtils.isEmpty(mContent)) {
      imageList = new ReaderImageList(mIsPrivate);
    } else {
      int minImageWidth =
          mIsGallery ? ReaderConstants.MIN_GALLERY_IMAGE_WIDTH : 0;
  ...
  private void updateTitle(int position) {
    ...
    String title =
        String.format(titlePhotoViewer, position + 1, getImageCount());
   ...
   */
  private boolean canShowTitle() { return (getImageCount() > 1); }
  ...
  private class PhotoPagerAdapter extends FragmentStatePagerAdapter {
    ...
    PhotoPagerAdapter(FragmentManager fm) { super(fm); }
    ...
    public Fragment getItem(int position) {
      return ReaderPhotoViewerFragment.newInstance(mImageList.get(position),
    ...
    private boolean isValidPosition(int position) {
      return (mImageList != null && position >= 0 && position < getCount());
    }
