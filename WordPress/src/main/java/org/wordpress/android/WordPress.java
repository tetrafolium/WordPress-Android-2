...
public class WordPress extends MultiDexApplication
	implements HasServiceInjector, HasSupportFragmentInjector,
	                       LifecycleObserver {
...
private static final int SECONDS_BETWEEN_BLOGLIST_UPDATE =
	15 * 60; // 15 minutes
...
/**
 * Update site list in a background task. (WPCOM site list, and eventually
 * self hosted multisites)
 */
public RateLimitedTask mUpdateSiteList =
	new RateLimitedTask(SECONDS_BETWEEN_BLOGLIST_UPDATE) {
	protected boolean run() {
		if (mAccountStore.hasAccessToken()) {
			mDispatcher.dispatch(SiteActionBuilder.newFetchSitesAction());
			mDispatcher.dispatch(
				AccountActionBuilder.newFetchSubscriptionsAction());
		}
		return true;
	}
};
 ...
 */
public RateLimitedTask mUpdateSelectedSite =
	new RateLimitedTask(SECONDS_BETWEEN_SITE_UPDATE) {
	protected boolean run() {
		int siteLocalId = AppPrefs.getSelectedSite();
		SiteModel selectedSite = mSiteStore.getSiteByLocalId(siteLocalId);
		if (selectedSite != null) {
			mDispatcher.dispatch(
				SiteActionBuilder.newFetchSiteAction(selectedSite));
			// Reload editor details from the remote backend
			if (!AppPrefs.isDefaultAppWideEditorPreferenceSet()) {
				// Check if the migration from app-wide to per-site setting has
				// already happened - v12.9->13.0
				mDispatcher.dispatch(
					SiteActionBuilder.newFetchSiteEditorsAction(selectedSite));
			}
		}
		return true;
	}
...
public static BitmapLruCache getBitmapCache() {
	if (mBitmapCache == null) {
		...
		int cacheSize =
			maxMemory /
			4; // Use 1/4th of the available memory for this memory cache.
...
public void onCreate() {
	...
	// Init static fields from dagger injected singletons, for legacy Actions
	// and Utilities
	...
	mZendeskHelper.setupZendesk(this, BuildConfig.ZENDESK_DOMAIN,
	                            BuildConfig.ZENDESK_APP_ID,
	                            BuildConfig.ZENDESK_OAUTH_CLIENT_ID);

	MemoryAndConfigChangeMonitor memoryAndConfigChangeMonitor =
		new MemoryAndConfigChangeMonitor();
	registerComponentCallbacks(memoryAndConfigChangeMonitor);

	// initialize our ApplicationLifecycleMonitor, which is the App's
	// LifecycleObserver implementation
	mApplicationLifecycleMonitor = new ApplicationLifecycleMonitor();
	ProcessLifecycleOwner.get().getLifecycle().addObserver(this);

	// Make the UploadStarter observe the app process so it can auto-start
	// uploads
	mUploadStarter.activateAutoUploading(
	...
	// Allows vector drawable from resources (in selectors for instance) on
	// Android < 21 (can cause issues with memory usage and the use of
	// Configuration). More information: http://bit.ly/2H1KTQo Note: if removed,
	// this will cause crashes on Android < 21
	...
	mDispatcher.dispatch(ListActionBuilder.newRemoveExpiredListsAction(
				     new RemoveExpiredListsPayload()));
	...
	// Enqueue our periodic upload work request. The UploadWorkRequest will be
	// called even if the app is closed. It will upload local draft or published
	// posts with local changes to the server.
	UploadWorkerKt.enqueuePeriodicUploadWorkRequestForAllSites();
}

protected void initWorkManager() {
	UploadWorker.Factory factory =
		new UploadWorker.Factory(mUploadStarter, mSiteStore);
...
private void sanitizeMediaUploadStateForSite() {
	...
	if (selectedSite != null) {
		new Thread(new Runnable() {
				@Override
				public void run() {
				        UploadService.sanitizeMediaUploadStateForSite(
						mMediaStore, mDispatcher, selectedSite);
				}
			}).start();
	}
}
...
private void createNotificationChannelsOnSdk26() {
	...
	if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
		...
		NotificationChannel importantChannel = new NotificationChannel(
			...
			getString(R.string.notification_channel_important_title),
			NotificationManager.IMPORTANCE_HIGH);
		// Register the channel with the system; you can't change the importance
		// or other notification behaviors after this
		notificationManager.createNotificationChannel(importantChannel);

		// Create the REMINDER channel (used for various reminders, like Quick
		// Start, etc.)
		NotificationChannel reminderChannel = new NotificationChannel(
			getString(R.string.notification_channel_reminder_id),
			getString(R.string.notification_channel_reminder_title),
			NotificationManager.IMPORTANCE_LOW);
		// Register the channel with the system; you can't change the importance
		// or other notification behaviors after this
		notificationManager.createNotificationChannel(reminderChannel);

		// Create the TRANSIENT channel (used for short-lived notifications such
		// as processing a Like/Approve, or media upload)
		NotificationChannel transientChannel = new NotificationChannel(
			getString(R.string.notification_channel_transient_id),
			getString(R.string.notification_channel_transient_title),
			NotificationManager.IMPORTANCE_DEFAULT);
 ...
 * it can be called while the app is in background by a sticky service or a
 * receiver, so we don't want Application.onCreate to make network request or
 * other heavy tasks. <p> This deferredInit method is called when a user
 * starts an activity for the first time, ie. when he sees a screen for the
 * first time. This allows us to have heavy calls on first activity startup
...
public static RestClientUtils getRestClientUtils() {
	if (sRestClientUtils == null) {
		sRestClientUtils = new RestClientUtils(mContext, sRequestQueue,
		                                       sOAuthAuthenticator, null);
	}
...
public static RestClientUtils getRestClientUtilsV1_1() {
	if (sRestClientUtilsVersion1p1 == null) {
		sRestClientUtilsVersion1p1 =
			new RestClientUtils(mContext, sRequestQueue, sOAuthAuthenticator,
...
public void wordPressComSignOut() {
	// Keep the analytics tracking at the beginning, before the account data is
	// actual removed.
	...
	// Once fully logged out refresh the metadata so the user information
	// doesn't persist for logged out events
	AnalyticsUtils.refreshMetadata(mAccountStore, mSiteStore);
}
...
public void onAccountChanged(OnAccountChanged event) {
	...
	if (!event.isError() && mAccountStore.hasAccessToken()) {
		// previously we reset the reader database on logout but this meant losing
		// saved posts so now we only reset it when the user id changes
		if (event.causeOfChange == AccountAction.FETCH_ACCOUNT) {
			...
			boolean hasUserOptedOut =
				!prefs.getBoolean(getString(R.string.pref_key_send_usage), true);
			...
			if (hasUserOptedOut != mAccountStore.getAccount().getTracksOptOut()) {
				AnalyticsUtils.updateAnalyticsPreference(getContext(), mDispatcher,
				                                         mAccountStore, true);
			}
		}
	}
...
public void onAuthenticationChanged(OnAuthenticationChanged event) {
	if (mAccountStore.hasAccessToken()) {
		// Make sure the Push Notification token is sent to our servers after a
		// successful login
...
public void onUnexpectedError(OnUnexpectedError event) {
	AppLog.d(T.API, "Receiving OnUnexpectedError event, message: " +
	         event.exception.getMessage());
}

public void removeWpComUserRelatedData(Context context) {
	// cancel all Volley requests - do this before unregistering push since that
	// uses a Volley request
	...
	mDispatcher.dispatch(
		SiteActionBuilder.newRemoveWpcomAndJetpackSitesAction());
	// remove all lists
 ...
 * E.g.:
 * "Mozilla/5.0 (Linux; Android 6.0; Android SDK built for x86_64
 * Build/MASTER; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0
...
public static String getDefaultUserAgent() {
	if (mDefaultUserAgent == null) {
		...
		} catch (AndroidRuntimeException | NullPointerException |
		         IllegalArgumentException e) {
			// Catch AndroidRuntimeException that could be raised by the WebView()
			// constructor. See
			// https://github.com/wordpress-mobile/WordPress-Android/issues/3594
			// Catch NullPointerException that could be raised by
			// WebSettings.getDefaultUserAgent() See
			// https://github.com/wordpress-mobile/WordPress-Android/issues/3838
			// Catch IllegalArgumentException that could be raised by
			// WebSettings.getDefaultUserAgent() See
			// https://github.com/wordpress-mobile/WordPress-Android/issues/9015
...
/**
 * User-Agent string when making HTTP connections, for both API traffic and
 * WebViews. Appends "wp-android/version" to WebView's default User-Agent
 * string for the webservers to get the full feature list of the browser and
 * serve content accordingly, e.g.: "Mozilla/5.0 (Linux; Android 6.0; Android
 * SDK built for x86_64 Build/MASTER; wv) AppleWebKit/537.36 (KHTML, like
 * Gecko) Version/4.0 Chrome/44.0.2403.119 Mobile Safari/537.36 wp-android/4.7"
 * Note that app versions prior to 2.7 simply used "wp-android" as the user
 * agent
 **/
...
/**
 * Gets a field from the project's BuildConfig using reflection. This is
 * useful when flavors are used at the project level to set custom fields.
 ...
 */
public static Object getBuildConfigValue(Application application,
                                         String fieldName) {
	try {
 ...
 * @return The string value of the field, or empty string if the field is not
 *     found.
 */
public static String getBuildConfigString(Activity activity,
                                          String configValueName) {
	if (!BuildConfig.DEBUG) {
		return "";
	}

	String value = (String)WordPress.getBuildConfigValue(
		activity.getApplication(), configValueName);
	if (!TextUtils.isEmpty(value)) {
		AppLog.d(AppLog.T.NUX,
		         "Auto-filled from build config: " + configValueName);
		return value;
	}
...
public void onAppGoesToBackground() {
	...
	ActivityId lastActivity =
		ActivityId.getActivityIdFromName(lastActivityString);
	Map<String, Object> properties = new HashMap<String, Object>();
	properties.put("last_visible_screen", lastActivity.toString());
	if (mApplicationOpenedDate != null) {
		Date now = new Date();
		properties.put("time_in_app", DateTimeUtils.secondsBetween(
				       now, mApplicationOpenedDate));
		mApplicationOpenedDate = null;
	}
	AnalyticsTracker.track(AnalyticsTracker.Stat.APPLICATION_CLOSED,
	                       properties);
	AnalyticsTracker.endSession(false);
	// Methods onAppComesFromBackground and onAppGoesToBackground are only
	// workarounds to track when the app goes to or comes from background. The
	// workarounds are not 100% reliable, so avoid unregistering the receiver
	// twice.
	if (mConnectionReceiverRegistered) {
		...
		try {
			...
			AppLog.d(T.MAIN,
			         "ConnectionChangeReceiver successfully unregistered");
 ...
 * 1. the app starts (but it's not opened by a service or a broadcast
 * receiver, i.e. an activity is resumed)
...
public void onAppComesFromBackground() {
	...
	// this broadcast if the broadcast receiver is declared in their manifest.
	// Apps will still receive broadcasts if BroadcastReceiver is registered
	...
	// Before making ANY modification to this stat please refer to:
	// p4qSXL-35X-p2
	...
	// Let's migrate the old editor preference if available in AppPrefs to the
	// remote backend
	SiteUtils.migrateAppWideMobileEditorPreferenceToRemote(mContext,
	                                                       mDispatcher);
...
/**
 * Uses ComponentCallbacks2 is used for memory-related event handling and
