...
/**
 * service which makes it possible to process Notifications quick actions in the
 * background, such as:
 * - like
 * - reply-to-comment
 * - approve
...
public class NotificationsProcessingService extends Service {
  ...
  public static final String ARG_ACTION_DRAFT_PENDING_DISMISS =
      "action_draft_pending_dismiss";
  public static final String ARG_ACTION_DRAFT_PENDING_IGNORE =
      "action_draft_pending_ignore";
   ...
   * */
  public static void startServiceForReply(Context context, String noteId,
  ...
  getPendingIntentForNotificationDismiss(Context context, int pushId) {
    Intent intent = new Intent(context, NotificationsProcessingService.class);
    ...
    return PendingIntent.getService(context, pushId, intent,
  ...
  public void onCreate() {
    ...
    AppLog.i(AppLog.T.NOTIFS,
             "notifications action processing service > created");
  }

  @Override
  public void onDestroy() {
    AppLog.i(AppLog.T.NOTIFS,
             "notifications action processing service > destroyed");
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    new Thread(new Runnable() {
      public void run() { mQuickActionProcessor.process(); }
    }).start();
  ...
  private class QuickActionProcessor {
    ...
    public void process() {
      ...
      if (mActionType != null) {
        ...
        if (mActionType.equals(ARG_ACTION_AUTH_IGNORE)) {
          ...
          GCMMessageService.removeNotification(
        ...
        if (mActionType.equals(ARG_ACTION_NOTIFICATION_DISMISS)) {
          ...
          if (notificationId == GCMMessageService.GROUP_NOTIFICATION_ID) {
            ...
            // Dismiss the grouped notification if a user dismisses all
            // notifications from a wear device
            if (!GCMMessageService.hasNotifications()) {
              NotificationManagerCompat notificationManager =
                  NotificationManagerCompat.from(mContext);
              notificationManager.cancel(
                  GCMMessageService.GROUP_NOTIFICATION_ID);
            }
          }
          return;
        }
        ...
        if (mActionType.equals(ARG_ACTION_DRAFT_PENDING_IGNORE)) {
          // dismiss notif
          int postId = mIntent.getIntExtra(
              NotificationsPendingDraftsReceiver.POST_ID_EXTRA, 0);
          if (postId != 0) {
            NativeNotificationsUtils.dismissNotification(
                PendingDraftsNotificationsUtils.makePendingDraftNotificationId(
                    postId),
                mContext);
          }
          AnalyticsTracker.track(
              AnalyticsTracker.Stat.NOTIFICATION_PENDING_DRAFTS_IGNORED);
          return;
        }
        ...
        if (mActionType.equals(ARG_ACTION_DRAFT_PENDING_DISMISS)) {
          AnalyticsTracker.track(
              AnalyticsTracker.Stat.NOTIFICATION_PENDING_DRAFTS_DISMISSED);
          return;
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N &&
            mActionType.equals(ARG_ACTION_REPLY)) {
          // we don't need showing the infinite progress bar in case of REPLY on
          // Android N, because we've got inline-reply there with its own
          // spinner to show progress no op
        } else {
    ...
    private void getDataFromIntent() {
      ...
      // default value for push notification ID is likely GROUP_NOTIFICATION_ID
      // for the only notif in active notifs map (there is only one notif if
      ...
      if (TextUtils.isEmpty(mReplyText)) {
        ...
        // remoteInput extra EXTRA_VOICE_OR_INLINE_REPLY same thing with
        // direct-reply in Android 7
      ...
      // we probably have the note in the PN payload and such it's passed in the
      // intent extras bundle. If we have it, no need to go fetch it through
      // REST API.
    ...
    private void obtainReplyTextFromRemoteInputBundle(Bundle bundle) {
      CharSequence replyText =
          bundle.getCharSequence(GCMMessageService.EXTRA_VOICE_OR_INLINE_REPLY);
      if (replyText != null) {
    ...
    private void requestCompleted(String actionType) {
      ...
      NativeNotificationsUtils.showFinalMessageToUser(
          successMessage, GCMMessageService.ACTIONS_PROGRESS_NOTIFICATION_ID,
          mContext);
      // remove the original notification from the system bar
      GCMMessageService.removeNotificationWithNoteIdFromSystemBar(mContext,
                                                                  mNoteId);
    ...
    private void requestFailed(String actionType) {
      ...
      NativeNotificationsUtils.dismissNotification(
          GCMMessageService.ACTIONS_PROGRESS_NOTIFICATION_ID, mContext);
      NativeNotificationsUtils.showFinalMessageToUser(
          errorMessage, GCMMessageService.ACTIONS_RESULT_NOTIFICATION_ID,
          mContext);
    ...
    private void getNoteFromNoteId(String noteId, RestRequest.Listener listener,
      ...
      WordPress.getRestClientUtils().getNotification(params, noteId, listener,
                                                     errorListener);
    }
    ...
    private void likeComment() {
      ...
      AnalyticsUtils.trackWithBlogPostDetails(
          AnalyticsTracker.Stat.NOTIFICATION_QUICK_ACTIONS_LIKED,
          mNote.getSiteId(), mNote.getPostId());
      AnalyticsUtils.trackQuickActionTouched(
          QuickActionTrackPropertyValue.LIKE,
          mSiteStore.getSiteBySiteId(mNote.getSiteId()), mNote.buildComment());
      ...
      if (site != null) {
        ...
        requestFailed(ARG_ACTION_LIKE);
        AppLog.e(T.NOTIFS, "Site with id: " + mNote.getSiteId() +
                               " doesn't exist in the Site store");
      }
    }
    ...
    private void approveComment() {
      ...
      // Bump analytics
      AnalyticsUtils.trackWithBlogPostDetails(
          AnalyticsTracker.Stat.NOTIFICATION_QUICK_ACTIONS_APPROVED,
          mNote.getSiteId(), mNote.getPostId());
      AnalyticsUtils.trackQuickActionTouched(
      ...
      if (site == null) {
        AppLog.e(
            T.NOTIFS,
            "Impossible to approve a comment on a site that is not in the App. SiteId: " +
                mNote.getSiteId());
        requestFailed(ARG_ACTION_APPROVE);
        return;
      }

      // keep the CommentId, we'll use it later to know whether to trigger the
      // end processing notification or not
      ...
      mDispatcher.dispatch(CommentActionBuilder.newPushCommentAction(
          new RemoteCommentPayload(site, comment)));
    }
    ...
    private void replyToComment() {
      ...
      if (!TextUtils.isEmpty(mReplyText)) {
        ...
        if (site == null) {
          AppLog.e(
              T.NOTIFS,
              "Impossible to reply to a comment on a site that is not in the App. SiteId: " +
                  mNote.getSiteId());
          requestFailed(ARG_ACTION_APPROVE);
          return;
        }
        ...
        // Push the reply
        RemoteCreateCommentPayload payload =
            new RemoteCreateCommentPayload(site, comment, reply);
        mDispatcher.dispatch(
            CommentActionBuilder.newCreateNewCommentAction(payload));

        // Bump analytics
        AnalyticsUtils.trackCommentReplyWithDetails(true, site, comment);
        AnalyticsUtils.trackQuickActionTouched(
            QuickActionTrackPropertyValue.REPLY_TO, site, comment);
      } else {
    ...
    private void resetOriginalNotification() {
      GCMMessageService.rebuildAndUpdateNotificationsOnSystemBarForThisNote(
          mContext, mNoteId);
    }
  }
  ...
  public void onCommentChanged(OnCommentChanged event) {
    ...
    // LIKE_COMMENT events do not hold event.changedCommentsLocalIds info so,
    // just process them also CREATE_NEW_COMMENT, which are received for REPLY
    // quick actions won't have a matching id as FluxC only notifies us of _new_
    // comments (the actual reply) so, no way to connect one another in this
    // place. Therefore, we only care and match for `PUSH_COMMENT` to make sure
    // there's a corresponding initial APPROVE quick action that corresponds to
    // the resulting FluxC event.
    if (event.causeOfChange == CommentAction.PUSH_COMMENT) {
      ...
      if (mActionedCommentsRemoteIds.size() > 0) {
        ...
        for (Integer commentLocalId : event.changedCommentsLocalIds) {
          CommentModel localComment =
              mCommentStore.getCommentByLocalId(commentLocalId);
          if (localComment != null) {
            eventChangedCommentsRemoteIds.add(
                localComment.getRemoteCommentId());
          }
        }

        // here we need to check ids: is an event corresponding to an action
