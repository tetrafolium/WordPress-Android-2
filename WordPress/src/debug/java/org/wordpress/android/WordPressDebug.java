...
protected void initWorkManager() {
	Configuration config = (new Configuration.Builder())
	                       ...
	                       .setWorkerFactory(new UploadWorker.Factory(
							 mUploadStarter, mSiteStore))
	                       .build();
...
protected void initDaggerComponent() {
	mAppComponent = DaggerAppComponentDebug.builder().application(this).build();
}
...
private void enableStrictMode() {
	...
	if (!BuildConfig.DEBUG) {
		AppLog.e(T.UTILS,
		         "You should not call enableStrictMode() on a non debug build");
		return;
	}
