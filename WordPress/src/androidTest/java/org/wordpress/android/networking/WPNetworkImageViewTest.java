...
public void testVolleyImageLoaderGetNullHost() throws InterruptedException {
	Handler mainLooperHandler =
		new Handler(WordPress.getContext().getMainLooper());
	...
	Runnable getImage = new Runnable() {
		...
		public void run() {
			try {
				...
				WordPress.sImageLoader.get(
					"http;///hello/null/host", new ImageListener() {
						@Override
						public void onResponse(ImageContainer imageContainer,
						                       boolean b) {
						}
