...
public static RestClient.REST_CLIENT_VERSIONS sVersion =
	REST_CLIENT_VERSIONS.V1;
public static Context sContext;
// keep a reference to each instances so we can update contexts and prefixes
// after instantiation
public static Set<RestClientCustomizableMock> sInstances =
	new HashSet<RestClientCustomizableMock>();
...
public static void setContextAllInstances(Context context) {
	...
	if (sMode != Mode.CUSTOMIZABLE) {
		AppLog.e(
			T.TESTS,
			"You try to change context on a non-customizable RestClient mock");
	}
...
public static void setPrefixAllInstances(String prefix) {
	...
	if (sMode != Mode.CUSTOMIZABLE) {
		AppLog.e(
			T.TESTS,
			"You try to change prefix on a non-customizable RestClient mock");
	}
	for (RestClientCustomizableMock client : sInstances) {
...
public RestClient make(RequestQueue queue) {
	...
	case CUSTOMIZABLE:
		...
		if (sContext != null) {
			...
			AppLog.e(
				T.TESTS,
				"You have to set RestClientFactoryTest.sContext field before running tests");
